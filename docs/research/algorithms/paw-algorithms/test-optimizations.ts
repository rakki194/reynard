...
async function testMemoryPoolOptimization(): Promise<void> {
  ...
  console.log(
    `   Hit Rate: ${((poolStats.poolHits / (poolStats.poolHits + poolStats.poolMisses)) * 100).toFixed(2)}%`,
  );
  console.log(
    `   Spatial Hash Pool: ${poolInfo.spatialHashPool.inUse}/${poolInfo.spatialHashPool.total} in use`,
  );
  console.log(
    `   Union-Find Pool: ${poolInfo.unionFindPool.inUse}/${poolInfo.unionFindPool.total} in use`,
  );
  console.log(
    `   Collision Array Pool: ${poolInfo.collisionArrayPool.inUse}/${poolInfo.collisionArrayPool.total} in use`,
  );
}
...
async function testScalabilityImprovements(): Promise<void> {
  ...
  for (const objectCount of objectCounts) {
    ...
    console.log(
      `🦦> ${objectCount.toString().padStart(11)} | ${originalMean.toFixed(4).padStart(13)} | ${optimizedMean.toFixed(4).padStart(14)} | ${improvement.toFixed(2).padStart(10)}%`,
    );
  }
}
...
async function testCacheEffectiveness(): Promise<void> {
  ...
  console.log(
    `   Cache Hit Rate: ${((cacheStats.cacheHits / (cacheStats.cacheHits + cacheStats.cacheMisses)) * 100).toFixed(2)}%`,
  );
}
...
async function runOptimizationTests(): Promise<void> {
  ...
  console.log(
    "🦦> This will test the proposed optimizations and measure their effectiveness.\n",
  );
  ...
  try {
    ...
    console.log(
      `   Performance Comparisons: ${comparison.performanceComparison.length}`,
    );
    console.log(`   Memory Comparisons: ${comparison.memoryComparison.length}`);
    console.log(
      `   Micro-benchmarks: ${comparison.microBenchmarkResults.length}`,
    );

    // Show average improvements
    const avgTimeImprovement =
      comparison.performanceComparison.reduce(
        (sum, comp) => sum + comp.timeImprovement,
        0,
      ) / comparison.performanceComparison.length;
    const avgMemoryImprovement =
      comparison.performanceComparison.reduce(
        (sum, comp) => sum + comp.memoryImprovement,
        0,
      ) / comparison.performanceComparison.length;
    ...
    console.log(
      `   Average Memory Improvement: ${avgMemoryImprovement.toFixed(2)}%`,
    );
...
async function main(): Promise<void> {
  ...
  console.log(
    "🦊> The results demonstrate the effectiveness of the proposed optimizations.",
  );
  console.log(
    "🦦> Memory pooling shows significant improvements in allocation overhead.",
  );
  console.log("🐺> Cache optimization provides additional performance gains.");
  console.log(
    "🦊> The enhanced benchmark suite provides comprehensive performance analysis.",
  );
}
