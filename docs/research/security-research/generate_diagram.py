...
def main():
    ...
    parser.add_argument("--type", default="graph TD", help="Type of Mermaid diagram (e.g., 'graph TD', 'sequenceDiagram').")
    parser.add_argument("--title", default="", help="Title of the diagram.")
    parser.add_argument("--nodes", nargs='+', help="List of diagram nodes/steps (e.g., 'A-->B' 'B-->C').")
    parser.add_argument("--output", help="Output filename for the Mermaid definition (.mmd).")

    args = parser.parse_args()

    if not args.nodes:
        print("Error: Please provide at least one node for the diagram using --nodes.", file=sys.stderr)
        sys.exit(1)
    ...
    if args.output:
        try:
            ...
            print(f"To convert to SVG, you'll need mermaid-cli (mmdc). Install with: npm install -g @mermaid-js/mermaid-cli")
            print(f"Then run: mmdc -i {args.output} -o {args.output.replace('.mmd', '.svg')}")
        except IOError as e:
