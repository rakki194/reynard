...
def test_login_endpoint():
    ...
    try:
        ...
        if response.status_code == 401:
            print("    [OK] Login failed as expected for invalid credentials (Status 401 Unauthorized).")
        else:
            print("    [WARNING] Unexpected status code for invalid credentials. Expected 401.")
    ...
    if response.status_code == 400:
        print("    [OK] Login failed as expected for empty credentials (Status 400 Bad Request).")
    else:
        print("    [WARNING] Unexpected status code for empty credentials. Expected 400.")
    ...
    if response.status_code in [401, 400]:
        ...
        print("    [WARNING] SQL Injection attempt in username might have succeeded or returned unexpected status.")
    ...
    if response.status_code in [401, 400]:
        ...
        print("    [WARNING] SQL Injection attempt in password might have succeeded or returned unexpected status.")
    ...
    print(f"    Response: {response.json()}")
    if response.status_code in [401, 400]:
        print("    [OK] SQL Injection bypass attempt in username with empty password failed as expected.")
    else:
        print("    [WARNING] SQL Injection bypass attempt in username with empty password might have succeeded or returned unexpected status.")
    ...
    try:
        ...
        if response.status_code == 401 or response.status_code == 403:
            ...
            print(f"    [WARNING] Unexpected status code {response.status_code} for /api/config (expected 401/403).")
    except requests.exceptions.ConnectionError as e:
        print(f"    [ERROR] Could not connect to the backend. Please ensure it's running on {BASE_URL}. Error: {e}")
