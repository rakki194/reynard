...
async def fuzz_change_user_password(headers):
    ...
    for _ in range(10):
        ...
        password_data = {
            ...
            "new_password": str(new_password_fuzz),
        }
        try:
            response = await httpx.AsyncClient().put(
...
async def fuzz_admin_get_user_engagement_data(admin_headers):
    ...
    for username in test_usernames + [target_username]:
        try:
            response = await httpx.AsyncClient().get(
                f"{BASE_URL}/api/engagement/users/{username}",
...
async def fuzz_admin_delete_user_engagement_data(admin_headers):
    ...
    if not admin_headers:
        ...
        return
    test_usernames = [
        "test_user_to_delete_engagement",
        "non_existent_user_delete",
        generate_random_string(20),
        "",
        "<script>alert(1)</script>",
    ]
...
async def create_test_memory(
    ...
    title_prefix="test_memory_",
    content_prefix="test_content_",
):
    if not headers:
...
async def fuzz_get_memory_by_id_endpoint(
    ...
    if test_user_memory_id:
        test_memory_ids.append(test_user_memory_id)
    if admin_user_memory_id:
        test_memory_ids.append(admin_user_memory_id)

    for memory_id in test_memory_ids:
        # Test with regular user headers
        if headers:
            await _test_memory_by_id_with_headers(memory_id, headers, "Regular")

        # Test with admin headers
        if admin_headers:
            await _test_memory_by_id_with_headers(memory_id, admin_headers, "Admin")
...
async def fuzz_search_memories_endpoint(headers, admin_headers):
    ...
    if not headers and not admin_headers:
        ...
        return
    search_queries = [
        generate_random_string(random.randint(1, 20)),
        "",
        "<script>alert(1)</script>",
        "test",
        "content",
...
async def fuzz_update_memory_endpoint(
    ...
    for memory_id in test_memory_ids:
        for _ in range(5):  # Multiple updates per ID
            ...
            if not update_data:
                continue  # Ensure at least one field is being updated
...
async def fuzz_delete_memory_endpoint(
    ...
    all_test_memory_ids = [
        mid
        for mid in memories_to_delete_regular + memories_to_delete_admin
        if mid is not None
    ]
    ...
    if test_user_memory_id:
        fuzzed_ids.append(test_user_memory_id)
    if admin_user_memory_id:
        fuzzed_ids.append(admin_user_memory_id)
    ...
    for model_name in model_names:
        ...
        try:
            ...
            async with httpx.AsyncClient().stream(
                "POST",
                f"{BASE_URL}/api/ollama/models/pull",
                headers=headers,
                json=payload,
                timeout=30,
            ) as response:
...
async def fuzz_ollama_ensure_model(headers):
    ...
    for model_name in model_names:
        ...
        try:
            ...
            async with httpx.AsyncClient().stream(
                "POST",
                f"{BASE_URL}/api/ollama/assistant/ensure-model",
                headers=headers,
                json=payload,
                timeout=30,
            ) as response:
...
async def fuzz_ollama_assistant_context(headers):
    ...
    for path in paths:
        try:
            ...
            response = await httpx.AsyncClient().get(
                f"{BASE_URL}/api/ollama/assistant/context/{encoded_path}",
                headers=headers,
                timeout=5,
            )
            print(
...
def fuzz_login_endpoint():
    ...
    common_fuzz_values = [
        ...
        "undefined",
        "Undefined",
        "UNDEFINED",  # Undefined-like strings
        "0",
        "1",
        "-1",
        "99999999999999999999999999999999999999",  # Numbers
        "true",
        "false",  # Booleans
        "[]",
        "{}",  # Empty array/object
        ...
        "admin' #",  # SQL Injection
        "' or '1'='1",  # SQL Injection
        r"\\",  # Backslash for path traversal or escaping
        "/../",  # Path traversal
