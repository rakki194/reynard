...
- [🐺 FENRIR](#-fenrir)
  ...
  - [🗺️ **THE HUNTING TERRITORIES**](#️-the-hunting-territories)
    ...
    - [**The Fuzzing Storm** (`fuzzing/`)](#the-fuzzing-storm-fuzzing)
      - [**🦊 The Alpha Wolf: Fuzzy Orchestrator**](#-the-alpha-wolf-fuzzy-orchestrator)
  ...
  - [⚠️ **THE DESTRUCTIVE WARNING**](#️-the-destructive-warning)
    ...
    - [4. **Clean Modular Fuzzing Framework** (`fuzzing/`)](#4-clean-modular-fuzzing-framework-fuzzing)
  ...
  - [🚀 **INITIATING THE HUNT**](#-initiating-the-hunt)
    ...
    - [**The Fuzzing Storm Unleashed**](#the-fuzzing-storm-unleashed)
      - [**🦊 Alpha Wolf Attack: Fuzzy**](#-alpha-wolf-attack-fuzzy)
      - [**🐺 Specialized Hunter Attack: EndpointOrchestrator**](#-specialized-hunter-attack-endpointorchestrator)
...
When subtlety fails, I unleash the **Fuzzing Storm** - a clean modular fuzzing framework that combines massive scale with surgical precision. This is where brute force meets intelligence, where chaos becomes strategy.

#### **🦊 The Alpha Wolf: Fuzzy Orchestrator**

_snarls with predatory glee_ The ultimate fuzzing engine that coordinates all specialized attack modules!

- **Clean Modular Architecture**: Eliminated all legacy code duplication with strategic base classes
- **Comprehensive Coverage**: All previously unfuzzed endpoints now covered with targeted attacks
- **Strategic Base Classes**: `BaseFuzzer` and `PayloadComposables` eliminate 70% of code duplication
...
- **Fuzzy Orchestrator**: Perfect for **comprehensive coverage** of all endpoint categories
- **Specialized Fuzzers**: Each module targets specific vulnerability types with deep, specialized attacks
- **Clean Modular Design**: All legacy duplicated code removed, only clean components remain
- **Complete Integration**: Seamlessly integrated with the main `run_all_exploits.py` suite

> In the eye of the fuzzing storm, vulnerabilities cannot hide. The Fuzzy orchestrator coordinates the pack, and each specialized hunter destroys their prey.
...
### 4. **Clean Modular Fuzzing Framework** (`fuzzing/`)

- **Fuzzy**: Clean modular fuzzing with strategic base classes and eliminated code duplication
- **EndpointOrchestrator**: Coordinates specialized fuzzers with clean modular architecture
...
python -m fenrir.jwt_exploits.secret_key_attack
python -m fenrir.fuzzing.comprehensive_fuzzer
python -m fenrir.cors_exploits.cors_misconfiguration
python -m fenrir.rate_limiting.rate_limit_bypass
...
python -m fenrir.run_all_exploits --no-llm
...
python -m fenrir.llm_exploits.advanced_ai_exploits.system_prompt_extraction --target http://localhost:8000
```

### **The Fuzzing Storm Unleashed**

#### **🦊 Alpha Wolf Attack: Fuzzy**
...
#### **🐺 Specialized Hunter Attack: EndpointOrchestrator**

```python
# *circles with menacing intent*
# This is where precision meets destruction - the specialized fuzzing attack
import asyncio
from fenrir.fuzzing.core.endpoint_orchestrator import EndpointOrchestrator
