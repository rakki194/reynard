    ...
    def generate_fuzz_report(self) -> None:
        ...
        *alpha wolf dominance radiates* Creates detailed report
        ...
        table.add_row(
            ...
            f"{(vulnerabilities_found / total_requests) * 100:.1f}%",
        )
        ...
        if self.specialized_endpoint_results:
            ...
            for category, results in self.specialized_endpoint_results.items():
                if results:
                    ...
                    console.print(
                        f"  {category}: {len(vulnerabilities)} vulnerabilities found in {len(results)} requests",
                    )
...
async def main():
    ...
    async with Fuzzy() as fuzzer:
        ...
        await fuzzer.fuzz_websocket_endpoint(
            "/api/embedding-visualization/progress",
            ["message_injection", "frame_manipulation"],
        )

        # 4. ML model fuzzing
        console.print("\n[bold blue]Phase 4: ML Model Fuzzing[/bold blue]")
        await fuzzer.fuzz_ml_endpoint(
            "/api/embedding-visualization/reduce",
            "POST",
            ["parameter_manipulation", "adversarial_inputs"],
        )
        await fuzzer.fuzz_ml_endpoint(
            "/api/diffusion/generate",
            "POST",
            ["parameter_manipulation", "resource_exhaustion"],
        )
        ...
        for category, results in specialized_results.items():
            if results:
                vulnerabilities = [r for r in results if r.vulnerability_detected]
                console.print(
                    f"[yellow]{category}: {len(vulnerabilities)} vulnerabilities found in {len(results)} requests[/yellow]",
                )
