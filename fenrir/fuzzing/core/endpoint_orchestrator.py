...
class EndpointOrchestrator:
    ...
    *red fur gleams with intelligence* This orchestrator coordinates all
    ...
    async def fuzz_endpoint_category(self, category: str, **kwargs) -> List[FuzzResult]:
        ...
        *fox pack hunting* Coordinates fuzzing of all endpoints in
        ...
        if category not in self.fuzzer_registry:
            console.print(f"[red]❌ No fuzzer registered for category: {category}[/red]")
            return []
        ...
        async with fuzzer_class(self.base_url, self.max_concurrent) as fuzzer:
            ...
            if not fuzz_method:
                console.print(f"[red]❌ No fuzz_{category}_endpoints method found in {fuzzer_class.__name__}[/red]")
                return []
    ...
    async def fuzz_all_categories(self, categories: Optional[List[str]] = None, **kwargs) -> Dict[str, List[FuzzResult]]:
        """
        ...
        *fox pack coordination* Coordinates fuzzing of all registered
        ...
        for category in categories:
            if category in self.fuzzer_registry:
                ...
                console.print(f"   ✅ {category}: {len(vulnerabilities)} vulnerabilities found")
            else:
                console.print(f"[yellow]⚠️  No fuzzer registered for category: {category}[/yellow]")
    ...
    def get_results_summary(self) -> Dict[str, Any]:
        ...
        *fox analytical mind* Provides comprehensive analysis of
        ...
        """
        if not self.results:
            return {"total_categories": 0, "total_requests": 0, "total_vulnerabilities": 0}
        ...
        for category, results in self.results.items():
            ...
            category_breakdown[category] = {
                ...
                "vulnerability_rate": (len(vulnerabilities) / len(results)) * 100 if results else 0
            }
        ...
        return {
            ...
            "overall_vulnerability_rate": (total_vulnerabilities / total_requests) * 100 if total_requests > 0 else 0,
    ...
    def print_comprehensive_report(self):
        ...
        *fox presentation skills* Provides a detailed, professional
        ...
        console.print(f"  Overall Vulnerability Rate: {summary['overall_vulnerability_rate']:.1f}%")
...
def create_endpoint_orchestrator(base_url: str = "http://localhost:8000", 
                                max_concurrent: int = 10) -> EndpointOrchestrator:
    """
    ...
    *fox factory method* Creates an endpoint orchestrator with
    ...
    try:
        from ..endpoints.embedding_visualization_fuzzer import EmbeddingVisualizationFuzzer
        orchestrator.register_fuzzer("embedding_visualization", EmbeddingVisualizationFuzzer)
    except ImportError:
