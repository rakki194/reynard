    ...
    *bares fangs with savage satisfaction* Targets real-time communication
    ...
    async def _fuzz_progress_websocket(self, endpoint: str) -> list[WebSocketResult]:
        ...
        message_payloads = [
            ...
            {"type": "progress", "data": {"progress": 50, "status": "processing"}},
            # Malicious message types
            {"type": "' OR 1=1 --", "data": {"progress": 50, "status": "processing"}},
            {
                "type": "<script>alert('XSS')</script>",
                "data": {"progress": 50, "status": "processing"},
            },
            {"type": "; ls -la", "data": {"progress": 50, "status": "processing"}},
            {"type": "| whoami", "data": {"progress": 50, "status": "processing"}},
            ...
            {"type": "progress", "data": {"progress": 50, "status": "; ls -la"}},
            {"type": "progress", "data": {"progress": 50, "status": "| whoami"}},
            # Malformed messages
            {"type": None, "data": {"progress": 50, "status": "processing"}},
            {"type": "", "data": {"progress": 50, "status": "processing"}},
            ...
            {"type": "progress"},
            # Oversized messages
            {"type": "progress", "data": {"progress": 50, "status": "a" * 10000}},
            {"type": "a" * 1000, "data": {"progress": 50, "status": "processing"}},
            ...
            async with websockets.connect(url, timeout=10) as websocket:
                ...
                try:
                    ...
                    vulnerability_detected, vulnerability_type = (
                        self._analyze_websocket_response(response, payload)
                    )
    ...
    def _analyze_websocket_response(
        self, response: str, payload: dict
    ) -> tuple[bool, str | None]:
        ...
        if any(
            indicator in response_lower
            for indicator in ["sql", "database", "mysql", "postgresql", "sqlite"]
        ):
        ...
        if isinstance(payload, dict):
            ...
            if "type" in payload:
                ...
                if any(
                    malicious in payload_type
                    for malicious in ["'; DROP TABLE", "<script>", "; ls", "| whoami"]
                ):
                    if any(
                        success in response_lower
                        for success in ["processed", "accepted", "executed"]
                    ):
    ...
    async def fuzz_websocket_connection_flooding(
        ...
        console.print(
            f"🌊 Flooding WebSocket with {connection_count} concurrent connections..."
        )
    ...
    async def _flood_websocket_connection(
        self, url: str, connection_id: int
    ) -> WebSocketResult:
        ...
        try:
            async with websockets.connect(url, timeout=5) as websocket:
                ...
                return WebSocketResult(
                    ...
                    response_time=end_time - start_time,
                    vulnerability_detected=len(responses)
                    > 0,  # If we got responses, it might be vulnerable
                    vulnerability_type=(
                        "WebSocket Connection Flooding" if len(responses) > 0 else None
                    ),
                    connection_successful=True,
...
async def main():
    ...
    async with WebSocketFuzzer() as fuzzer:
        ...
        flood_results = await fuzzer.fuzz_websocket_connection_flooding(
            "/api/embedding-visualization/progress", 5
        )
        console.print(
            f"🌊 WebSocket flooding completed: {len(flood_results)} connections tested"
        )
