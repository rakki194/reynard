...
*splashes with enthusiasm* Comprehensive test runner for all fenrir security testing scripts!
...
from .test_base import SecurityTestResult
...
class FenrirTestRunner:
    """*otter enthusiasm bubbles* Comprehensive test runner for all fenrir security tests
        ...
        async with test_class(self.base_url) as tester:
            ...
            for method_name in test_methods:
                try:
                    ...
                    console.print(
                        f"[yellow]Warning: {method_name} failed: {e!s}[/yellow]",
                    )
                    completed_tests += 1
                    progress_value = int((completed_tests / total_tests) * 100)
                    progress.update(task, completed=progress_value)
    ...
    def generate_report(self) -> str:
        ...
        total_vulnerabilities = sum(
            suite.vulnerabilities_found for suite in self.suite_results
        )
        ...
        summary_table = Table(title="🦦 Fenrir Test Suite Summary")
        ...
        summary_table.add_row(
            "Failed Tests",
            str(total_failed),
            f"{(total_failed / total_tests * 100):.1f}%" if total_tests > 0 else "0%",
        )
        summary_table.add_row(
        ...
        report = f"""
🦦 FENRIR SECURITY TEST SUITE REPORT
{"=" * 50}
...
async def main():
    ...
    try:
        ...
        console.print(f"\n[red]❌ Test runner failed: {e!s}[/red]")
        sys.exit(1)
