...
class FenrirTestBase:
    ...
    def log_test_result(self, result: SecurityTestResult) -> None:
        ...
        print(  # noqa: T201
            f"{status} {result.test_name}: {vuln_status} ({result.response_time:.3f}s)",
        )
        if result.details:
            print(f"    Details: {result.details}")  # noqa: T201
        if result.error:
    ...
    def get_test_summary(self) -> dict[str, Any]:
        ...
        vulnerabilities_found = sum(
            1 for r in self.test_results if r.vulnerability_found
        )
...
class MockVulnerableServer:
    ...
    def setup_vulnerable_responses(self) -> None:
...
class MockSecureServer:
    ...
    def setup_secure_responses(self) -> None:
