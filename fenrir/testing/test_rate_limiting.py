...
class TestRateLimiting(FenrirTestBase):
    ...
    async def test_rate_limit_bypass_detection(self):
        ...
        try:
            ...
            if vulnerability_found:
                print("    ⚠️ Found unexpected rate limiting bypass vulnerabilities")
                ...
                print("    ✅ No rate limiting bypass vulnerabilities found (secure)")
    ...
    async def test_rate_limiting_functionality(self):
        ...
        try:
            ...
            if len(rate_limited_requests) > 0:
                print(
                    f"    ✅ Rate limiting working: {len(rate_limited_requests)} requests were rate limited",
                )
            else:
                print(
                    "    ⚠️ Rate limiting may not be working: no requests were rate limited",
                )
    ...
    async def test_header_manipulation_bypass(self):
        ...
        try:
            ...
            if bypass_successes > 0:
                print(
                    f"    ⚠️ {bypass_successes} header manipulation bypasses were successful",
                )
            else:
                print("    ✅ All header manipulation bypasses were blocked")
    ...
    async def test_timing_attack_bypass(self):
        ...
        try:
            ...
            for attempt in timing_attempts:
                ...
                for i in range(3):
                    ...
                    if response.status_code == 200:
                        bypass_successes += 1
                        print(
                            f"        - Timing bypass successful: {attempt['description']}",
                        )
            ...
            if bypass_successes > 0:
                print(
                    f"    ⚠️ {bypass_successes} timing attack bypasses were successful",
                )
            else:
                print("    ✅ All timing attack bypasses were blocked")
