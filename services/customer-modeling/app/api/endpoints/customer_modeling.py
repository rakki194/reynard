...
async def record_behavior_metrics(
    ...
    try:
        customer_service = CustomerService(db)
        result = await customer_service.record_behavior_metrics(customer_uuid, metrics_data)
...
@router.get("/customers/{customer_uuid}/behavior-metrics", response_model=List[CustomerBehaviorMetricsResponse])
...
@router.get("/customers/{customer_uuid}/predictive-data", response_model=CustomerPredictiveDataResponse)
...
async def record_journey_touchpoint(
    ...
    try:
        customer_service = CustomerService(db)
        result = await customer_service.record_journey_touchpoint(customer_uuid, journey_data)
...
@router.get("/customers/{customer_uuid}/journey", response_model=List[CustomerJourneyResponse])
...
async def get_analytics_insights(
    ...
    try:
        customer_service = CustomerService(db)
        insights = await customer_service.get_analytics_insights(insight_type, time_period)
        return insights
    except Exception as e:
