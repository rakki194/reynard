...
class CustomerIdentity(Base):
    ...
    demographics = relationship(
        "CustomerDemographics", back_populates="identity", uselist=False
    )
    behavior_metrics = relationship(
        "CustomerBehaviorMetrics", back_populates="identity"
    )
    predictive_data = relationship(
        "CustomerPredictiveData", back_populates="identity", uselist=False
    )
...
class CustomerBehaviorMetrics(Base):
    ...
    )  # engagement, purchase, browsing, etc.
    metric_name = Column(
        String(100), nullable=False
    )  # session_duration, page_views, etc.
