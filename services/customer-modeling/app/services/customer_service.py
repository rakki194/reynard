...
class CustomerService:
    ...
    async def get_customer_by_uuid(
        ...
        try:
            customer = (
                ...
                .filter(CustomerIdentity.customer_uuid == customer_uuid)
                .first()
            )
    ...
    async def update_customer(
        self, customer_uuid: str, customer_data: CustomerUpdate
    ) -> Optional[CustomerResponse]:
    ...
    async def record_behavior_metrics(
        self, customer_uuid: str, metrics_data: List[CustomerBehaviorMetricsCreate]
    ) -> bool:
    ...
    async def get_behavior_metrics(
        ...
        try:
            ...
            metrics = (
                query.order_by(desc(CustomerBehaviorMetrics.recorded_at))
                .limit(limit)
                .all()
            )

            return [
                CustomerBehaviorMetricsResponse.from_orm(metric) for metric in metrics
            ]
    ...
    async def get_predictive_data(
        self, customer_uuid: str
    ) -> Optional[CustomerPredictiveDataResponse]:
        ...
        try:
            ...
            predictive_data = (
                self.db.query(CustomerPredictiveData)
                .filter(CustomerPredictiveData.customer_id == customer.id)
                .first()
            )
    ...
    async def record_journey_touchpoint(
        self, customer_uuid: str, journey_data: CustomerJourneyCreate
    ) -> bool:
        ...
        try:
            ...
            self.db.rollback()
            logger.error(
                f"Failed to record journey touchpoint for {customer_uuid}: {e}"
            )
            raise
    ...
    async def get_customer_journey(
        ...
        try:
            ...
            journey = (
                query.order_by(desc(CustomerJourney.occurred_at)).limit(limit).all()
            )

            return [
                CustomerJourneyResponse.from_orm(touchpoint) for touchpoint in journey
            ]
    ...
    async def get_customer_segmentation(
        self, segment_type: str, limit: int = 100
    ) -> List[CustomerSegmentationResponse]:
            ...
            if segment_type == "behavioral":
                ...
                behavioral_segments = (
                    self.db.query(
                        ...
                        func.avg(CustomerPredictiveData.lifetime_value).label(
                            'avg_lifetime_value'
                        ),
                        func.avg(CustomerPredictiveData.engagement_score).label(
                            'avg_engagement_score'
                        ),
                        func.avg(CustomerPredictiveData.churn_probability).label(
                            'avg_churn_rate'
                        ),
                    )
                    .group_by(CustomerPredictiveData.behavioral_cluster)
                    .limit(limit)
                    .all()
                )
                    ...
                    segments.append(
                        CustomerSegmentationResponse(
                            ...
                            average_lifetime_value=float(
                                segment.avg_lifetime_value or 0
                            ),
                            average_engagement_score=float(
                                segment.avg_engagement_score or 0
                            ),
    ...
    async def update_predictions(
        self, customer_uuids: Optional[List[str]] = None, force_update: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if customer_uuids:
                customers = (
                    self.db.query(CustomerIdentity)
                    .filter(CustomerIdentity.customer_uuid.in_(customer_uuids))
                    .all()
                )
            else:
            ...
            for customer in customers:
                ...
                if not force_update:
                    ...
                    if (
                        existing_prediction
                        and existing_prediction.last_prediction_update
    ...
    async def get_analytics_insights(
        self, insight_type: str, time_period: str
    ) -> AnalyticsInsightsResponse:
        ...
        try:
            ...
            active_customers = (
                self.db.query(CustomerIdentity)
                .filter(CustomerIdentity.is_active == True)
                .count()
            )
            ...
            churn_rate = (
                (churned_customers / total_customers) if total_customers > 0 else 0
            )

            # Get average lifetime value
            avg_lifetime_value = (
                self.db.query(func.avg(CustomerPredictiveData.lifetime_value)).scalar()
                or 0
            )
            ...
            top_segments = (
                ...
                .group_by(CustomerPredictiveData.customer_segment)
                .limit(5)
                .all()
            )
            ...
            return AnalyticsInsightsResponse(
                ...
                top_segments=[
                    {"segment": seg.customer_segment, "count": seg.count}
