...
Unified BM25 search implementation with advanced features:
- Query expansion and auto-suggestions
- Smart caching with TTL
- File type and directory filtering
- Performance tracking
- Enhanced tokenization for code patterns
- Intelligent snippet generation

Based on the best features from both basic and enhanced implementations.
"""
...
class BM25SearchEngine:
    """
    BM25 (Best Matching 25) search engine with advanced features.

    This implementation combines the best features from both basic and enhanced versions:
    - Enhanced tokenization for code patterns
...
class ReynardBM25Search:
    """
    Reynard-specific BM25 search implementation with agent-friendly features.

    Combines the best features from both basic and enhanced implementations
    into a unified, clean interface.
    """
    ...
    def index_project(
        ...
        logger.info(
            f"BM25 indexing complete: {len(documents)} files in {self.indexing_stats['indexing_time']:.2f}s"
        )
    ...
    def search(
        ...
        """
        Search with all enhanced features.
    ...
    def reindex(self, project_root: str) -> None:
        ...
        self.indexed = False
        self.engine = BM25SearchEngine()  # Reset engine
...
def search_needle_in_haystack(
    ...
    if project_root is None:
        ...
        if "services/mcp-server" in current_dir:
            project_root = current_dir.replace("/services/mcp-server", "")
        else:
...
def get_query_suggestions(query: str, max_suggestions: int = 5) -> list[str]:
    ...
    return bm25_search.get_query_suggestions(query, max_suggestions)


def get_search_stats() -> dict[str, Any]:
    """Get comprehensive search statistics."""
    return bm25_search.get_search_stats()


def clear_search_cache() -> None:
    """Clear search cache for fresh results."""
    bm25_search.clear_cache()
...
def reindex_project(project_root: str | None = None) -> None:
    ...
    bm25_search.reindex(project_root)
