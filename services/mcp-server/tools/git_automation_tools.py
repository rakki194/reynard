...
async def git_branch_info(**kwargs) -> dict[str, Any]:
    ...
    try:
        # Get current branch
        branch_result = await git_tools._run_command(["git", "branch", "--show-current"])
...
async def git_commit_history(**kwargs) -> dict[str, Any]:
    ...
    try:
        result = await git_tools._run_command([
        ...
        if result["success"]:
            return {
                "content": [
                    {
                        "type": "text",
                        "text": f"✅ Files added to staging area: {files}"
                    }
                ]
            }
        else:
    ...
    if not message:
        return {
            "content": [
                {
                    "type": "text",
                    "text": "❌ Commit message is required"
                }
            ]
        }
        ...
        if result["success"]:
            ...
            return {
                "content": [
                    {
                        "type": "text",
                        "text": f"❌ Error pushing: {result['stderr']}"
                    }
                ]
            }
    except Exception as e:
        ...
        if result["success"]:
            ...
            return {
                "content": [
                    {
                        "type": "text",
                        "text": f"❌ Error pulling: {result['stderr']}"
                    }
                ]
            }
    except Exception as e:
