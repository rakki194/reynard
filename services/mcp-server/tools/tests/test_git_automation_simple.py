...
class SimpleGitAutomationTools:
    ...
    def analyze_changes(self, working_dir=None):
        ...
        try:
            return self._run_cli_command(
                ["reynard-git", "analyze-changes"], working_dir
            )
        except Exception as e:
    ...
    def generate_commit_message(self, working_dir=None):
        ...
        try:
            return self._run_cli_command(
                ["reynard-git", "generate-commit"], working_dir
            )
        except Exception as e:
    ...
    def bump_version(self, bump_type, working_dir=None):
        ...
        try:
            return self._run_cli_command(
                ["reynard-git", "bump-version", bump_type], working_dir
            )
        except Exception as e:
    ...
    def create_git_tag(self, tag, message, working_dir=None):
        ...
        try:
            return self._run_cli_command(
                ["reynard-git", "create-tag", tag, message], working_dir
            )
        except Exception as e:
...
class TestSimpleGitAutomationTools(unittest.TestCase):
    ...
    def test_update_changelog_success(self, mock_run):
        ...
        self.assertEqual(
            call_args[0][0],
            ["reynard-git", "update-changelog", "feat", "add new feature"],
        )
    ...
    def test_create_git_tag_success(self, mock_run):
        ...
        mock_result.returncode = 0
        mock_result.stdout = json.dumps(
            {"success": True, "tag": "v1.3.0", "message": "Release version 1.3.0"}
        )
        mock_run.return_value = mock_result
        ...
        self.assertEqual(
            call_args[0][0],
            ["reynard-git", "create-tag", "v1.3.0", "Release version 1.3.0"],
        )
    ...
    def test_execute_workflow_with_options(self, mock_run):
        ...
        mock_result.returncode = 0
        mock_result.stdout = json.dumps(
            {"success": True, "dryRun": True, "autoConfirm": False}
        )
        mock_run.return_value = mock_result

        options = {"dryRun": True, "autoConfirm": False, "cleanupJunk": True}
        ...
        call_args = mock_run.call_args
        self.assertEqual(
            call_args[0][0], ["reynard-git", "workflow", "--dry-run", "--cleanup-junk"]
        )
