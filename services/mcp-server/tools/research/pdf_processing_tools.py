...
class PDFProcessingService:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.backend_url}{endpoint}", json=data
                ) as response:
    ...
    async def process_single_pdf(
        ...
        data = {"pdf_path": pdf_path, "output_dir": output_dir, "use_llm": use_llm}
    ...
    async def process_paper_collection(
        ...
        data = {"papers_dir": papers_dir, "use_llm": use_llm, "max_papers": max_papers}

        result = await self._make_request(
            "/mcp/pdf-processing/process-collection", data
        )
        return result

    async def process_research_papers(
        self, use_llm: bool = False, max_papers: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.backend_url}/mcp/pdf-processing/status"
                ) as response:
                    if response.status == 200:
                        return await response.json()
                    else:
...
def process_pdf_to_markdown(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not pdf_path:
        return {"content": [{"type": "text", "text": "❌ Error: No PDF path provided"}]}
    ...
    try:
        # Run async function
        result = asyncio.run(
            pdf_service.process_single_pdf(
                pdf_path=pdf_path, output_dir=output_dir, use_llm=use_llm
            )
        )
        ...
        return {"content": [{"type": "text", "text": f"❌ Error processing PDF: {e}"}]}
...
def process_paper_collection_to_markdown(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not papers_dir:
        return {
            "content": [
                {"type": "text", "text": "❌ Error: No papers directory provided"}
            ]
        }
    ...
    try:
        # Run async function
        result = asyncio.run(
            pdf_service.process_paper_collection(
        ...
        return {
            "content": [
                {"type": "text", "text": f"❌ Error processing paper collection: {e}"}
            ]
        }
...
def process_research_papers_to_markdown(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        # Run async function
        result = asyncio.run(
            pdf_service.process_research_papers(use_llm=use_llm, max_papers=max_papers)
        )
        ...
        return {
            "content": [
                {"type": "text", "text": f"❌ Error processing research papers: {e}"}
            ]
        }
...
def get_pdf_processing_status(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "content": [
                {"type": "text", "text": f"❌ Error getting processing status: {e}"}
            ]
        }
