...
class ArxivService:
        ...
        try:
            ...
            if date_range:
                ...
                if start_date and end_date:
                    ...
                    params["search_query"] = (
                        f"({params['search_query']}) AND ({date_query})"
                    )
    ...
    def _parse_arxiv_response(self, xml_content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "total_results": len(results), "results": results}
...
def search_arxiv_papers(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if result["success"]:
        ...
        return {
            "content": [
                {"type": "text", "text": f"❌ arXiv search failed: {result['error']}"}
            ]
        }
...
def download_arxiv_paper(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not paper_id:
        return {"content": [{"type": "text", "text": "❌ Error: No paper ID provided"}]}
