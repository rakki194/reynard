...
def ingest_paper_to_rag(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Run async ingestion
    result = asyncio.run(rag_service.ingest_paper_to_rag(
    ...
    if result["success"]:
        ...
        return {
            "content": [{"type": "text", "text": f"❌ Failed to ingest paper: {result['error']}"}]
        }
...
def search_papers_in_rag(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Run async search
    result = asyncio.run(rag_service.search_papers_in_rag(
        query=query,
    ...
    if result["success"]:
        ...
        if results:
            ...
            return {
                "content": [{"type": "text", "text": f"❌ No papers found in RAG system for query: '{query}'"}]
            }
    else:
        return {
            "content": [{"type": "text", "text": f"❌ RAG search failed: {result['error']}"}]
        }
...
def get_rag_paper_stats(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if result.get("success", True):  # Assume success if no error field
        ...
        return {
            "content": [{"type": "text", "text": f"❌ Failed to get RAG stats: {result.get('error', 'Unknown error')}"}]
        }
