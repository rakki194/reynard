...
class PaperManagementService:
    ...
    def _matches_query(
        self, metadata: Dict[str, Any], query: str, category: Optional[str]
    ) -> bool:
...
def download_and_index_paper(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not paper_id or not title:
        return {
            "content": [
                {"type": "text", "text": "❌ Error: Paper ID and title are required"}
            ]
        }

    # Run async download
    result = asyncio.run(
        paper_service.download_arxiv_paper(
            paper_id=paper_id,
            title=title,
            authors=authors,
            abstract=abstract,
    ...
    if result["success"]:
        ...
        return {
            "content": [
                {
                    "type": "text",
                    "text": f"❌ Failed to download paper: {result['error']}",
                }
            ]
        }
...
def search_local_papers(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    results = paper_service.search_papers(
        query=query, source=source, category=category, max_results=max_results
    )
    ...
    if results:
        ...
        return {
            "content": [
                {"type": "text", "text": f"❌ No papers found matching '{query}'"}
            ]
        }
