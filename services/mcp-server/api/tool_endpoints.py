...
from protocol.tool_registry import get_tool_registry, tool_registry
...
async def sync_tools(request: ToolSyncRequest):
    ...
    try:
        ...
        return ToolSyncResponse(
            success=True, message=message, tools_synced=tools_synced
        )
...
async def get_tools_by_category(category: str):
    ...
    try:
        tools = []
        for tool_metadata in (
            get_tool_registry().get_tools_by_category(category).values()
        ):
            tools.append(
                ToolMetadataResponse(
                    name=tool_metadata.name,
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get tools by category: {e}"
        )
...
async def discover_tools_in_modules():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to discover tools in modules: {e}"
        )
