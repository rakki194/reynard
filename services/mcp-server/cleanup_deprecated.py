    ...
    def _check_old_string_formatting(self, file_path: Path, content: str) -> None:
        ...
        for pattern in old_format_patterns:
            ...
            if matches:
                self._add_issue(
                    file_path,
                    f"Uses old-style string formatting ({len(matches)} instances)",
                )
                break
    ...
    def _check_broad_exceptions(self, file_path: Path, content: str) -> None:
        ...
        for pattern in broad_exceptions:
            ...
            if matches:
                self._add_issue(
                    file_path,
                    f"Uses broad exception handling ({len(matches)} instances)",
                )
                break
    ...
    def _check_old_typing(self, file_path: Path, content: str) -> None:
        ...
        for pattern in old_typing_patterns:
            ...
            if matches:
                self._add_issue(
                    file_path,
                    f"Uses old-style typing annotations ({len(matches)} instances)",
                )
                break
...
def main():
    ...
    if issues:
        ...
        print("   2. Fix deprecated patterns file by file")
        print("   3. Run tests to ensure nothing breaks")
        print("   4. Re-run this scanner to verify fixes")
        return 1
    else:
