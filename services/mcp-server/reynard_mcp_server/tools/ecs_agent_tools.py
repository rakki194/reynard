...
class ECSAgentTools:
    ...
    async def create_ecs_agent(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            agent_data = await self.ecs_client.create_agent(agent_id, spirit, style, name)
            ...
            return {"content": [{"type": "text", "text": f"Failed to create ECS agent: {e}"}]}
    ...
    async def create_ecs_offspring(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            offspring_data = await self.ecs_client.create_offspring(parent1_id, parent2_id, offspring_id)
            ...
            return {"content": [{"type": "text", "text": f"Failed to create ECS offspring: {e}"}]}

    async def enable_automatic_reproduction(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "content": [
                    {
                        "type": "text",
                        "text": result.get("message", f"Automatic reproduction {'enabled' if enabled else 'disabled'}"),
                    }
                ]
            }
        except Exception as e:
            return {"content": [{"type": "text", "text": f"Failed to toggle breeding: {e}"}]}
    ...
    async def get_ecs_agent_status(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"content": [{"type": "text", "text": f"Failed to get ECS status: {e}"}]}
    ...
    def get_ecs_agent_positions(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            from reynard_ecs_world import PositionComponent, AgentComponent, LifecycleComponent

            agents = self.world.get_entities_with_components(AgentComponent, PositionComponent)
            agent_data = []
                    ...
                    agent_data.append(
                        {
                            ...
                            "target": {"x": position_comp.target_x, "y": position_comp.target_y},
                            "velocity": {
            ...
            return {"content": [{"type": "text", "text": json.dumps(agent_data, indent=2)}]}
        except Exception as e:
            return {"content": [{"type": "text", "text": f"Failed to get agent positions: {e}"}]}

    async def find_ecs_compatible_mates(self, arguments: dict[str, Any]) -> dict[str, Any]:
        """Find compatible mates for an agent via FastAPI backend."""
        agent_id = arguments.get("agent_id", "")
        max_results = arguments.get("max_results", 5)

        try:
            mates_data = await self.ecs_client.find_compatible_mates(agent_id, max_results)
            ...
            return {"content": [{"type": "text", "text": f"Failed to find compatible mates: {e}"}]}

    async def analyze_ecs_compatibility(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            compatibility = await self.ecs_client.analyze_compatibility(agent1_id, agent2_id)

            compat_text = "Compatibility Analysis:\n"
            compat_text += f"Agent 1: {agent1_id}\n"
            compat_text += f"Agent 2: {agent2_id}\n"
            compat_text += f"Compatibility: {compatibility.get('compatibility', 0):.2f}\n"
            ...
            return {"content": [{"type": "text", "text": f"Failed to analyze compatibility: {e}"}]}
    ...
    async def get_ecs_lineage(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"content": [{"type": "text", "text": f"Failed to get lineage: {e}"}]}
    ...
    def update_ecs_world(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"content": [{"type": "text", "text": f"Failed to update ECS world: {e}"}]}
    ...
    def search_agents_by_proximity(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            from reynard_ecs_world import PositionComponent, AgentComponent
            ...
            for entity in agents:
                ...
                if agent_comp and position_comp:
                    distance = math.sqrt(
                        (position_comp.x - target_x) ** 2 + (position_comp.y - target_y) ** 2
                    )
            ...
            return {
                "content": [
                    {"type": "text", "text": json.dumps(nearby_agents[:max_results], indent=2)}
                ]
            }
        except Exception as e:
            return {
                "content": [{"type": "text", "text": f"Failed to search agents by proximity: {e}"}]
            }
    ...
    def search_agents_by_region(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            for entity in agents:
                ...
                if agent_comp and position_comp:
                    if min_x <= position_comp.x <= max_x and min_y <= position_comp.y <= max_y:
            ...
            return {"content": [{"type": "text", "text": json.dumps(region_agents, indent=2)}]}
        except Exception as e:
            return {
                "content": [{"type": "text", "text": f"Failed to search agents by region: {e}"}]
            }
    ...
    def get_agent_movement_path(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if not agent_comp or not position_comp:
                return {
                    "content": [{"type": "text", "text": f"Agent {agent_id} missing position data"}]
                }
            ...
            estimated_time = (
                distance / position_comp.movement_speed if position_comp.movement_speed > 0 else 0
            )
            ...
            path_data = {
                ...
                "target_position": {"x": position_comp.target_x, "y": position_comp.target_y},
                "velocity": {"x": position_comp.velocity_x, "y": position_comp.velocity_y},
                "movement_speed": position_comp.movement_speed,
            ...
            return {"content": [{"type": "text", "text": json.dumps(path_data, indent=2)}]}
        except Exception as e:
            return {
                "content": [{"type": "text", "text": f"Failed to get agent movement path: {e}"}]
            }
    ...
    def get_spatial_analytics(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if not agents:
                return {"content": [{"type": "text", "text": "No agents with position data found"}]}
            ...
            for entity in agents:
                ...
                if position_comp:
                    ...
                    velocities.append((position_comp.velocity_x, position_comp.velocity_y))
            ...
            analytics = {
                ...
                "spatial_distribution": {
                    ...
                    "spread": {
                        "x_std": round(statistics.stdev(x_coords) if len(x_coords) > 1 else 0, 2),
                        "y_std": round(statistics.stdev(y_coords) if len(y_coords) > 1 else 0, 2),
                    },
                },
                "movement_statistics": {
                    ...
                    "speed_std": round(
                        statistics.stdev(movement_speeds) if len(movement_speeds) > 1 else 0, 2
                    ),
                },
                "velocity_analysis": {
                    "avg_velocity_magnitude": round(
                        statistics.mean([math.sqrt(vx**2 + vy**2) for vx, vy in velocities]), 2
                    ),
                    "stationary_agents": len(
                        [v for v in velocities if math.sqrt(v[0] ** 2 + v[1] ** 2) < 0.1]
                    ),
                },
            }
            ...
            return {"content": [{"type": "text", "text": f"Failed to get spatial analytics: {e}"}]}
    ...
    def start_global_breeding(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"content": [{"type": "text", "text": f"Failed to start global breeding: {e}"}]}
    ...
    def stop_global_breeding(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"content": [{"type": "text", "text": f"Failed to stop global breeding: {e}"}]}

    async def get_breeding_statistics(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"content": [{"type": "text", "text": f"Failed to get breeding statistics: {e}"}]}
    ...
    async def get_simulation_status(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"content": [{"type": "text", "text": f"ECS World Simulation Status: {json.dumps(status, indent=2)}"}]}
        except Exception as e:
            return {"content": [{"type": "text", "text": f"Failed to get simulation status: {e}"}]}
    ...
    def accelerate_time(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"content": [{"type": "text", "text": f"Time acceleration set to {factor}x"}]}
        except Exception as e:
            return {"content": [{"type": "text", "text": f"Failed to accelerate time: {e}"}]}
    ...
    def nudge_time(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"content": [{"type": "text", "text": f"Time nudged forward by {amount} units"}]}
        except Exception as e:
