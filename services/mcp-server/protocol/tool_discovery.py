...
class ToolDiscovery:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    if self._has_register_tool_decorator(node):
                        tools_found += 1
                        logger.debug(
                            f"Found @register_tool decorator on function: {node.name}"
                        )
    ...
    def import_and_register_tools(self, module_path: str) -> int:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"✅ Module {module_name} imported in {import_elapsed:.3f}s"
                )
            except Exception as import_error:
                import_elapsed = time.time() - import_start
                logger.error(
                    f"❌ Failed to import module {module_name} after {import_elapsed:.3f}s: {import_error}"
                )
                raise
            ...
            logger.debug(
                f"✅ Registered {tools_registered} tools from {module_name} in {register_elapsed:.3f}s"
            )

            total_elapsed = time.time() - start_time
            logger.debug(
                f"✅ import_and_register_tools completed for {module_path} in {total_elapsed:.3f}s"
            )
            ...
            total_elapsed = time.time() - start_time
            logger.error(
                f"❌ Failed to import module {module_path} after {total_elapsed:.3f}s: {e}"
            )
            return 0
    ...
    def discover_and_import_tools(self, directory_path: str) -> int:
        ...
        logger.debug(
            "🕐 Phase 2: Importing modules to trigger decorator registration..."
        )
        import_start = time.time()
        directory = Path(directory_path)
        if not directory.exists():
        ...
        for i, python_file in enumerate(python_files):
            ...
            try:
                ...
                if count > 0:
                    logger.info(
                        f"✅ Imported {count} tools from {python_file} in {file_elapsed:.3f}s"
                    )
                else:
                    logger.debug(
                        f"✅ Processed {python_file} in {file_elapsed:.3f}s (no tools found)"
                    )
            except Exception as e:
                file_elapsed = time.time() - file_start
                logger.error(
                    f"❌ Failed to import tools from {python_file} after {file_elapsed:.3f}s: {e}"
                )

        import_elapsed = time.time() - import_start
        total_elapsed = time.time() - start_time
        logger.info(
            f"✅ Total tools discovered and imported: {discovered_count} in {total_elapsed:.3f}s"
        )
        return discovered_count
        ...
        for tool_name, metadata in self.tool_registry.list_all_tools().items():
            ...
            if not metadata.handler_method:
                validation_results["issues"].append(
                    f"Tool {tool_name} has no handler method"
                )
