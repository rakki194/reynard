...
class CodebaseMetricsInsights:
    ...
    def analyze_code_complexity(self, file_paths: List[Path]) -> Dict[str, Any]:
                ...
                if self.radon_available:
                    try:
                        ...
                        if cc_results:
                            ...
                            max_complexity = max(max_complexity, max(r.complexity for r in cc_results))
        ...
        if python_files:
            results['summary']['average_complexity'] = total_complexity / len(python_files)
            results['summary']['max_complexity'] = max_complexity
            results['summary']['high_complexity_files'] = high_complexity_count
        
        return results

    def generate_dependency_graph(self, file_analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        most_imported = sorted(in_degrees.items(), key=lambda x: x[1], reverse=True)[:10]
        most_importing = sorted(out_degrees.items(), key=lambda x: x[1], reverse=True)[:10]
        
        # Find cycles
        try:
            cycles = list(nx.simple_cycles(G))
            metrics['cycles'] = len(cycles)
            metrics['cycle_files'] = [cycle for cycle in cycles[:5]]  # Limit to first 5 cycles
        except Exception as e:
    ...
    def _find_import_target(self, import_name: str, file_analyses: List[Dict[str, Any]]) -> Optional[str]:
        ...
        if self.bandit_available and python_files:
            try:
                ...
                results['bandit_results'] = {
                    ...
                    'high_severity': len([i for i in bandit_manager.get_issue_list() if i.severity == 'HIGH']),
                    'medium_severity': len([i for i in bandit_manager.get_issue_list() if i.severity == 'MEDIUM']),
                    'low_severity': len([i for i in bandit_manager.get_issue_list() if i.severity == 'LOW']),
                    'issues_by_file': {}
                }
    ...
    def detect_performance_bottlenecks(self, file_analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for analysis in file_analyses:
            ...
            if 'ast_node_counts' in analysis:
                ...
                # Many loops
                loop_count = sum(node_counts.get(node_type, 0) for node_type in 
                               ['For', 'While', 'AsyncFor', 'AsyncWith'])
                if loop_count > 10:
                ...
                nested_count = sum(node_counts.get(node_type, 0) for node_type in 
                                 ['If', 'For', 'While', 'With', 'Try', 'ClassDef', 'FunctionDef'])
                if nested_count > 50:
                    results['potential_issues'].append({
                        'file': file_path,
                        'issue': 'Deep nesting',
        ...
        for key in ['large_files', 'complex_functions', 'many_imports', 'potential_issues']:
            results[key].sort(key=lambda x: (x['severity'] == 'high', x.get('count', x.get('lines', x.get('complexity', 0)))), reverse=True)
    ...
    def generate_insights_report(self, 
        ...
        if 'summary' in complexity_analysis:
            high_complexity = complexity_analysis['summary'].get('high_complexity_files', 0)
