...
class ExportRequest(BaseModel):
    ...
    output_path: str = Field(
        "analysis_export", description="Output file path (without extension)"
    )
    format: str = Field(
        "json", description="Export format (json, csv, yaml, html, xml)"
    )
...
async def get_analysis_summary():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get analysis summary: {e}"
        )
...
async def start_monitoring(request: MonitoringRequest):
    ...
    try:
        ...
        if success:
            return BaseResponse(
                success=True, message="Real-time monitoring started successfully"
            )
        else:
            return BaseResponse(
                success=False,
...
async def stop_monitoring():
    ...
    try:
        ...
        return BaseResponse(
            success=True, message="Real-time monitoring stopped successfully"
        )
    except Exception as e:
...
async def get_change_history(
    limit: Optional[int] = Query(
        None, description="Maximum number of changes to return"
    ),
    since: Optional[str] = Query(
        None, description="ISO timestamp to filter changes since"
    ),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get change history: {e}"
        )
...
async def export_analysis(request: ExportRequest):
    ...
    try:
        export_results = scanner_service.export_analysis(
            output_path=request.output_path, format=request.format
        )

        if 'error' in export_results:
            return BaseResponse(
                success=False, message="Export failed", error=export_results['error']
            )

        return BaseResponse(
            success=True, message="Export completed successfully", data=export_results
        )
    except Exception as e:
...
async def list_cached_analyses():
    ...
    try:
        ...
        return {"cached_analyses": cache_keys, "count": len(cache_keys)}
    except Exception as e:
        logger.error(f"Cache list error: {e}")
        raise HTTPException(
            status_code=500, detail=f"Failed to list cached analyses: {e}"
        )
...
async def get_cached_analysis(cache_key: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get cached analysis: {e}"
        )
...
async def clear_cache():
    ...
    try:
        ...
        return BaseResponse(success=True, message="Analysis cache cleared successfully")
    except Exception as e:
...
async def get_complexity_analysis():
    ...
    try:
        ...
        if not complexity_data:
            raise HTTPException(
                status_code=404, detail="No complexity analysis data available"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get complexity analysis: {e}"
        )
...
async def get_security_analysis():
    ...
    try:
        ...
        if not security_data:
            raise HTTPException(
                status_code=404, detail="No security analysis data available"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get security analysis: {e}"
        )
...
async def get_performance_analysis():
    ...
    try:
        ...
        if not performance_data:
            raise HTTPException(
                status_code=404, detail="No performance analysis data available"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get performance analysis: {e}"
        )
...
async def get_dependency_analysis():
    ...
    try:
        ...
        if not dependency_data:
            raise HTTPException(
                status_code=404, detail="No dependency analysis data available"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get dependency analysis: {e}"
        )
