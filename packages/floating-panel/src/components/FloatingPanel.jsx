...
const FloatingPanelContent = props => {
    ...
    return (<div ref={setPanelRef} class={`floating-panel theme-${config.theme} ${isDragging() ? "dragging" : ""} ${props.class || ""}`} style={getInlineStyles()} tabIndex={config.closable ? 0 : -1} role="dialog" aria-modal="true" aria-labelledby={`panel-title-${props.id}`}>
      <div class="floating-panel-content">
        {(config.draggable || config.closable) && (<FloatingPanelHeader id={props.id} config={config} onHide={props.onHide}/>)}
