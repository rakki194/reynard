...
export const FloatingPanelAdvanced: Component<FloatingPanelProps> = (props) => {
  ...
  const panel = useDraggableResizable({ initialState, ...constraints, onDrag: handlers.handleDrag, onResize: handlers.handleResize, onShow: handlers.handleShow, onHide: handlers.handleHide });
  ...
  return (
    <div
      ...
      style={{ position: "absolute", top: `${panel.state().y}px`, left: `${panel.state().x}px`, width: `${panel.state().width}px`, height: `${panel.state().height}px`, "z-index": props.position.zIndex || 1000, "transition-delay": `${config.animationDelay}ms` }}
    >
