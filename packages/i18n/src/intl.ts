...
export class IntlNumberFormatter {
  ...
  private getFormatter(options: Intl.NumberFormatOptions): Intl.NumberFormat {
    ...
    if (!this.formatters.has(key)) {
      this.formatters.set(
        key,
        new Intl.NumberFormat(this.config.locale, options),
      );
    }
  ...
  format(
    value: number,
    preset?: keyof FormattingPresets["number"],
...
export class IntlDateFormatter {
  ...
  format(
    date: Date,
    preset?: keyof FormattingPresets["date"],
    options?: Intl.DateTimeFormatOptions,
  ): string {
    const formatOptions = preset
...
export class IntlRelativeTimeFormatter {
  ...
  private getFormatter(
    ...
    if (!this.formatters.has(key)) {
      this.formatters.set(
        key,
        new Intl.RelativeTimeFormat(this.config.locale, options),
      );
    }
    return this.formatters.get(key)!;
  }
  ...
  format(
    ...
    preset?: keyof FormattingPresets["relative"],
  ): string {
...
export class IntlPluralRules {
  ...
  private getRule(): Intl.PluralRules {
    if (!this.rules.has(this.config.locale)) {
      this.rules.set(
        this.config.locale,
        new Intl.PluralRules(this.config.locale),
      );
    }
      ...
      if (
        ...
        if (result.includes(placeholder)) {
          ...
          result = result.replace(
            placeholder,
            this.relativeTime.format(timeDiff, relativeValue.unit),
          );
        }
      }
    }
