...
export function sortTags(tags: string[], sortOrder: 'alphabetical' | 'length' | 'frequency' = 'alphabetical'): string[] {
  if (!Array.isArray(tags)) {
    return [];
  }
...
export function getTagStats(tags: string[]): {
  ...
  const longestTag = cleanedTags.reduce((longest, current) => 
    current.length > longest.length ? current : longest, '');
  
  const shortestTag = cleanedTags.reduce((shortest, current) => 
    current.length < shortest.length ? current : shortest, cleanedTags[0] || '');
