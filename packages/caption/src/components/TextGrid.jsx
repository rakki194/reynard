...
export const TextGrid = (props) => {
    const { textFiles, selectedFile, setSelectedFile, handleFileSelect, handleFileRemove, handleFileModify, addFiles, } = useTextFileManager({
    ...
    return (<div class={`text-grid ${props.class || ""}`}>
      <TextFileUpload onFileUpload={onFileUpload} isLoading={isLoading()} error={error()}/>

      <TextFilesGrid files={textFiles()} selectedFile={selectedFile()} onFileSelect={handleFileSelect} onFileRemove={handleFileRemove} onFileModify={handleFileModify} onCloseEditor={() => setSelectedFile(null)} showMetadata={props.showMetadata} editable={props.editable}/>
    </div>);
};
