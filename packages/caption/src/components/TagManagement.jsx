...
export const TagManagement = props => {
    ...
    return (<div class={`tag-management ${props.className || ""}`}>
      ...
      <Show when={props.showSelection && hasSelection()}>
        <div class="selection-controls">
          ...
          <div class="selection-actions">
            <button type="button" class="bulk-delete-button" onClick={handleBulkDelete} title="Delete selected tags (Delete key)">
              Delete Selected
            </button>
            <button type="button" class="clear-selection-button" onClick={() => setSelectedTags(new Set())} title="Clear selection (Escape key)">
              Clear Selection
            </button>
          </div>
        </div>
      ...
      {/* Tags container */}
      <div class="tags-container" classList={{
            dragging: isDragging(),
            "has-selection": hasSelection(),
        }}>
        <For each={props.tags}>
          {(tag, index) => (<div class="tag-wrapper" classList={{
                selected: selectedTags().has(tag.id),
                focused: focusedTagId() === tag.id,
                "drag-over": dragOverTagId() === tag.id,
            }} draggable={props.enableDragDrop} onDragStart={e => handleDragStart(e, tag.id)} onDragOver={e => handleDragOver(e, tag.id)} onDragLeave={handleDragLeave} onDrop={e => handleDrop(e, tag.id)} onClick={e => handleTagSelect(tag.id, e)} onFocus={() => setFocusedTagId(tag.id)} tabIndex={0}>
              <TagBubble tag={tag.text} index={index()} onEdit={newText => handleTagEdit(tag.id, newText)} onRemove={() => handleTagRemove(tag.id)} onNavigate={handleTagNavigate} editable={props.editable} removable={props.removable}/>
            </div>)}
        </For>
      </div>
