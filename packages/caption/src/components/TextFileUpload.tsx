  ...
  return (
    <div class={`text-upload-section ${props.class || ""}`}>
      ...
      <input
        ...
        type="file"
        multiple
        accept=".txt,.md,.json,.xml,.yaml,.yml,.toml,.js,.ts,.tsx,.jsx,.py,.java,.cpp,.c,.cs,.php,.rb,.go,.rs,.swift,.kt,.scala,.html,.css,.scss,.sass,.less,.sql,.sh,.bash,.zsh,.fish,.ps1,.bat,.dockerfile,.gitignore,.env,.log"
        onChange={props.onFileUpload}
        class="text-upload-input"
