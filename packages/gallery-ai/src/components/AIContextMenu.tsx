...
export const AIContextMenu: Component<AIContextMenuProps> = (props) => {
  ...
  const handleActionClick = async (action: AIContextMenuAction) => {
  ...
  const handleEditCaption = async (_action: AIContextMenuAction) => {
    // This would open a caption editor dialog
    console.log("Edit caption for:", props.item.name);
  };

  // Delete caption
  const handleDeleteCaption = async (_action: AIContextMenuAction) => {
  ...
  // Smart organize
  const handleSmartOrganize = async (_action: AIContextMenuAction) => {
    console.log("Smart organize for:", props.item.name);
    // This would implement smart organization logic
  };

  // AI search
  const handleAISearch = async (_action: AIContextMenuAction) => {
  ...
  const isActionAvailable = (action: AIContextMenuAction): boolean => {
  ...
  // Render action item
  const renderAction = (action: AIContextMenuAction, depth = 0) => {
    ...
    return (
      <div class={`ai-context-menu__item ai-context-menu__item--depth-${depth}`}>
        <button
          ...
          } ${isProcessing() ? "ai-context-menu__action--processing" : ""}`}
          ...
          <Show when={ai.aiState().batchProgress}>
            <div class="ai-context-menu__progress">
              <div class="ai-context-menu__progress-bar">
                <div 
                  class="ai-context-menu__progress-fill"
                  // eslint-disable-next-line @typescript-eslint/no-unused-expressions
