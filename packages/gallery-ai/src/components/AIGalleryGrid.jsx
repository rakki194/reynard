...
export const AIGalleryGrid = (props) => {
    ...
    const handleContextMenuAction = (action) => {
        ...
        if (action.aiActionType === "batch_annotate" &&
    ...
    const hasAIIndicators = (item) => {
        if (item.type === "folder")
            return false;
        // Check if item has captions or AI metadata
        return (!!item.metadata?.hasCaption ||
            !!item.metadata?.aiProcessed);
    };
    // Get AI indicator type
    const getAIIndicatorType = (item) => {
        if (item.type === "folder")
            return "";
        const fileItem = item;
        if (fileItem.metadata?.hasCaption)
            return "caption";
        if (fileItem.metadata?.aiProcessed)
            return "processed";
        return "";
    };
      ...
      <Show when={props.aiProps?.showBatchControls && selectedItems().length > 1}>
        <div class="ai-gallery-grid__batch-controls">
          ...
          <div class="ai-gallery-grid__batch-actions">
            <Button variant="primary" size="sm" onClick={() => setBatchDialog(true)} disabled={!ai.aiState().aiEnabled} class="ai-gallery-grid__batch-btn">
              Batch Annotate
            </Button>
      ...
      <Show when={ai.aiState().batchProgress}>
        <div class="ai-gallery-grid__progress">
          <div class="ai-gallery-grid__progress-bar">
            <div class="ai-gallery-grid__progress-fill" 
    ...
    style={`--progress-width: ${ai.aiState().batchProgress?.percentage || 0}%`}/>
      ...
      <div class="ai-gallery-grid__items">
        <For each={props.items}>
          {(item) => (<div class={`ai-gallery-grid__item ${selectedItems().some((selected) => selected.id === item.id)
                ? "ai-gallery-grid__item--selected"
                : ""}`} onClick={() => handleItemClick(item)} onDblClick={() => handleItemDoubleClick(item)} onContextMenu={(e) => {
                e.preventDefault();
      ...
      <Show when={contextMenu()}>
        <AIContextMenu visible={contextMenu().visible} position={contextMenu().position} item={contextMenu().item} selectedItems={selectedItems()} actions={contextMenu().actions} onClose={handleContextMenuClose} onActionClick={handleContextMenuAction}/>
      </Show>

      {/* Batch Processing Dialog */}
      <BatchProcessingDialog visible={batchDialog()} items={selectedItems().filter((item) => item.type !== "folder")} availableGenerators={ai.getAvailableGenerators()} onClose={handleBatchDialogClose} onComplete={handleBatchComplete} onError={handleBatchError}/>
    </div>);
};
