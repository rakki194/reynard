...
export const AIGalleryGrid = props => {
    ...
    const hasAIIndicators = (item) => {
        ...
        return !!item.metadata?.hasCaption || !!item.metadata?.aiProcessed;
    };
    ...
    style={`--progress-width: ${ai.aiState().batchProgress?.percentage || 0}%`}/>
      ...
      <div class="ai-gallery-grid__items">
        <For each={props.items}>
          {item => (<div class={`ai-gallery-grid__item ${selectedItems().some(selected => selected.id === item.id) ? "ai-gallery-grid__item--selected" : ""}`} onClick={() => handleItemClick(item)} onDblClick={() => handleItemDoubleClick(item)} onContextMenu={e => {
                e.preventDefault();
