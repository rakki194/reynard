...
export const AIImageViewer = (props) => {
    ...
    const [generationResult, setGenerationResult] = createSignal(null);
    ...
    const handleGenerateCaption = async (generator) => {
        if (!props.imageInfo || isGenerating())
            return;
    ...
    const handleSaveCaption = async () => {
        if (!currentCaption() || !props.onCaptionSave)
            return;
        try {
    ...
    return (<div class="ai-image-viewer">
      {/* Main Image Display */}
      <div class="ai-image-viewer__image-container">
        <img src={props.imageInfo?.url || props.imageInfo?.path} alt={props.imageInfo?.name || "Image"} class="ai-image-viewer__image"/>
      ...
      <Show when={props.aiProps?.showGenerationControls !== false}>
        <div class="ai-image-viewer__controls">
          <div class="ai-image-viewer__generator-selector">
            ...
            <select value={selectedGenerator()} onChange={(e) => setSelectedGenerator(e.currentTarget.value)} disabled={isGenerating()} class="ai-image-viewer__select" title="Select caption generator">
              <For each={props.aiProps?.availableGenerators ||
            ai.getAvailableGenerators()}>
                {(generator) => (<option value={generator}>
          ...
          <div class="ai-image-viewer__actions">
            <Button variant="primary" onClick={() => handleGenerateCaption(selectedGenerator())} disabled={isGenerating()} class="ai-image-viewer__generate-btn">
        ...
        <div class="ai-image-viewer__caption-section">
          ...
          <Show when={isEditing()}>
            <div class="ai-image-viewer__caption-editor">
              <textarea value={currentCaption()} onInput={(e) => setCurrentCaption(e.currentTarget.value)} placeholder="Enter caption..." class="ai-image-viewer__caption-textarea" rows={4}/>
