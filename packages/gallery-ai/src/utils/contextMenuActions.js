 ...
 */
export function createSingleItemActions(item, config) {
    const actions = [];
 ...
 */
export function createBatchActions(items, config) {
    const actions = [];
    if (!config.enableBatchOperations || !config.aiEnabled) {
        return actions;
    }
    const fileItems = items.filter((item) => item.type !== "folder");
    if (fileItems.length < 2) {
        return actions;
    }
    // Batch Annotate submenu
    const batchAnnotateAction = {
        id: "batch-annotate",
 ...
 */
export function createSmartFeatureActions(_item, config) {
    const actions = [];
 ...
 */
export function createAIContextMenuActions(item, selectedItems, config) {
    const actions = [];
 ...
 */
function getDefaultCaptionType(generator) {
    const captionTypes = {
