import { createSignal, createEffect, onCleanup, onMount, mergeProps, on, } from "solid-js";
...
export const MonacoDiffEditor = (inputProps) => {
    ...
    let monacoOnChangeSubscription;
    ...
    createEffect(on(() => props.modified, (modified) => {
    ...
    createEffect(on(() => props.original, (original) => {
    ...
    createEffect(on(() => props.options, (options) => {
        editor()?.updateOptions(options ?? {});
    }, { defer: true }));
    createEffect(on(() => props.theme, (theme) => {
        monaco()?.editor.setTheme(theme);
    }, { defer: true }));
    createEffect(on(() => props.originalLanguage, (language) => {
    ...
    createEffect(on(() => props.modifiedLanguage, (language) => {
        const model = editor()?.getModel();
        if (!language || !model) {
            return;
        }
    ...
    createEffect(on(() => [props.originalPath, props.modifiedPath], ([originalPath, modifiedPath], prevPaths) => {
        ...
        if (prevOriginalPath !== originalPath) {
            ...
            originalModel = getOrCreateModel(_monaco, props.original ?? "", props.originalLanguage, originalPath);
        }
        if (prevModifiedPath !== modifiedPath) {
            ...
            modifiedModel = getOrCreateModel(_monaco, props.modified ?? "", props.modifiedLanguage, modifiedPath);
        }
        editor()?.setModel({
    ...
    const createEditor = (monaco) => {
        const originalModel = getOrCreateModel(monaco, props.original ?? "", props.originalLanguage, props.originalPath);
        const modifiedModel = getOrCreateModel(monaco, props.modified ?? "", props.modifiedLanguage, props.modifiedPath);
        const editor = monaco.editor.createDiffEditor(containerRef, {
    ...
    return (<MonacoContainer class={props.class} width={props.width} height={props.height}>
      {!editor() && <Loader>{props.loadingState}</Loader>}
      <div class="monaco-diff-editor-container" ref={containerRef}/>
    </MonacoContainer>);
};
