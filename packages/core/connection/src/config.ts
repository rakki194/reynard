import { ConnectionConfig, ConnectionType, SecurityLevel, RecoveryStrategy } from "./types";

function env(name: string, fallback?: string): string | undefined {
  // Frontend: rely on import.meta.env when available (Vite)
  const v = (import.meta as any)?.env?.[name] ?? (globalThis as any)?.process?.env?.[name];
...
export class ConnectionConfigManager {
  ...
  private createDefault(): ConnectionConfig {
    return {
      ...
      compression: String(env("VITE_CONNECTION_COMPRESSION", "true")).toLowerCase() === "true",
      encryption: String(env("VITE_CONNECTION_ENCRYPTION", "true")).toLowerCase() === "true",
      securityLevel:
        (env("VITE_CONNECTION_SECURITY_LEVEL", "basic") as keyof typeof SecurityLevel) in SecurityLevel
          ? (SecurityLevel as any)[String(env("VITE_CONNECTION_SECURITY_LEVEL", "basic")).toUpperCase()]
          : SecurityLevel.BASIC,
      ...
      autoReconnect: String(env("VITE_CONNECTION_AUTO_RECONNECT", "true")).toLowerCase() === "true",
      autoRetry: String(env("VITE_CONNECTION_AUTO_RETRY", "true")).toLowerCase() === "true",
      circuitBreakerEnabled: String(env("VITE_CONNECTION_CIRCUIT_BREAKER", "true")).toLowerCase() === "true",
      ...
      rateLimitEnabled: String(env("VITE_CONNECTION_RATE_LIMIT", "false")).toLowerCase() === "true",
      ...
      auditLogging: String(env("VITE_CONNECTION_AUDIT_LOGGING", "true")).toLowerCase() === "true",
      monitoringEnabled: String(env("VITE_CONNECTION_MONITORING", "true")).toLowerCase() === "true",
  ...
  private loadFromEnv() {
    const prefixes = ["VITE_HTTP_", "VITE_WEBSOCKET_", "VITE_SSE_", "VITE_DATABASE_", "VITE_EXTERNAL_"];
  ...
  private createFromEnv(prefix: string, name: string): ConnectionConfig {
    ...
    return {
      ...
      circuitBreakerEnabled:
        String(env(`${prefix}CIRCUIT_BREAKER`, String(d.circuitBreakerEnabled))).toLowerCase() === "true",
