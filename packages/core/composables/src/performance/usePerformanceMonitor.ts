...
export function usePerformanceMonitor(
  ...
  let profiling = false;
  let renderingObserver: PerformanceObserver | null = null;
  let frameTimings: number[] = [];
  let domUpdateTimings: number[] = [];
  let styleApplicationTimings: number[] = [];
  ...
  const recordDOMUpdate = (updateType: string, duration: number) => {
    if (!profiling) return;
  ...
  const logPerformanceReport = () => {
    ...
    const report = {
      ...
      styleApplications: `${
        metrics.styleApplicationCount
      } applications (avg: ${metrics.averageStyleApplicationTime.toFixed(2)}ms)`,
