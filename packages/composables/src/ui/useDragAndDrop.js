...
export function useDragAndDrop(options) {
    const { onDragStateChange, onFilesDropped, onItemsDropped, maxFileSize = 100 * 1024 * 1024, // 100MB default
    allowedFileTypes, uploadFiles, moveItems, notify, } = options;
    let dragCounter = 0;
    const [isMoving, setIsMoving] = createSignal(false);
    const [movingItems, setMovingItems] = createSignal(new Set());
    ...
    const handleDragOver = (e) => {
        ...
        if (e.dataTransfer.types.includes("application/x-custom-item") ||
    ...
    const handleDrop = async (e) => {
        ...
        if (e.dataTransfer.types.includes("application/x-custom-item") ||
            ...
            try {
                ...
                const itemsToMove = items.map((item) => item.name);
                const alreadyMoving = itemsToMove.filter((name) => movingItems().has(name));
                ...
                setMovingItems((prev) => {
                    ...
                    itemsToMove.forEach((name) => newSet.add(name));
                    return newSet;
                });
                // Get target path
                const targetElement = e.target;
                const directoryItem = targetElement.closest(".item.directory");
                let targetPath = "";
                if (directoryItem) {
    ...
    const handleDragStart = (e) => {
        ...
        const itemData = {
            type: isDraggingDirectory ? "directory" : "file",
