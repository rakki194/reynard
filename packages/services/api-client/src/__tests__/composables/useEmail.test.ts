...
describe("useEmail", () => {
  ...
  describe("sendSimpleEmail", () => {
    it("should send a simple email", async () => {
      ...
      const emailService = useEmail(mockClient);
      const result = await emailService.sendSimpleEmail("test@example.com", "Test Subject", "Test Body");
  ...
  describe("getEmailStatus", () => {
    it("should get email service status", async () => {
      ...
      expect(result).toEqual(mockResponse);
      expect(global.fetch).toHaveBeenCalledWith("http://localhost:8000/api/email/status");
    });
  });
