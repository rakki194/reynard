...
export function createCanvas(
  ...
  canvas.selectionColor = config.config.selectionColor || 'rgba(100, 100, 255, 0.3)';
  canvas.selectionBorderColor = config.config.selectionBorderColor || '#6464ff';
  canvas.selectionLineWidth = config.config.selectionLineWidth || 2;
...
export function addBoundingBoxesToCanvas(
  ...
  boundingBoxes.forEach((box) => {
    ...
    const rect = new fabric.Rect({
      ...
      fill: 'transparent',
      stroke: config.config.boxStrokeColor || '#ff0000',
      strokeWidth: config.config.boxStrokeWidth || 2,
      selectable: true,
      evented: true,
      data: { boxId: box.id },
    });
    ...
    if (config.config.showLabels && box.label) {
      const label = new fabric.Text(box.label, {
        left: displayCoords.x,
        top: displayCoords.y - 20,
        fontSize: config.config.labelFontSize || 12,
        fill: config.config.labelColor || '#000000',
        selectable: false,
        evented: false,
      });
      canvas.add(label);
