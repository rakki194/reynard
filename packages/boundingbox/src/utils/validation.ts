 ...
 */
export function validateBoundingBox(box: BoundingBox, imageInfo: ImageInfo): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
 ...
 */
export function validatePolygonAnnotation(polygon: PolygonAnnotation, imageInfo: ImageInfo): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
...
export function validateTransformConstraints(constraints: TransformConstraints): {
  ...
  if (constraints.minWidth !== undefined && constraints.maxWidth !== undefined && constraints.minWidth > constraints.maxWidth) {
    errors.push('minWidth cannot be greater than maxWidth');
  }

  if (constraints.minHeight !== undefined && constraints.maxHeight !== undefined && constraints.minHeight > constraints.maxHeight) {
    errors.push('minHeight cannot be greater than maxHeight');
  }

  if (constraints.aspectRatio !== undefined && (typeof constraints.aspectRatio !== 'number' || constraints.aspectRatio <= 0)) {
...
export function checkBoundingBoxConstraints(
  ...
  if (constraints.aspectRatio !== undefined) {
    ...
    if (Math.abs(currentAspectRatio - constraints.aspectRatio) > tolerance) {
      violations.push(`Aspect ratio ${currentAspectRatio.toFixed(3)} does not match required ${constraints.aspectRatio}`);
    }
  }
