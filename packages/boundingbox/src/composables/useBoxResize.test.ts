...
describe('useBoxResize', () => {
  ...
  describe('Resize Operations', () => {
    it('should start resize operation', () => {
      ...
      resizeEngine.startResize(mockBoundingBox.id, handle, { x: 100, y: 100, width: 200, height: 150 });

      expect(mockCallbacks.onResizeStart).toHaveBeenCalledWith(mockBoundingBox.id, expect.objectContaining({
        ...handle,
  ...
  describe('Constraints and Validation', () => {
    ...
    it('should enforce maximum height constraint', () => {
      ...
      resizeEngine.startResize(mockBoundingBox.id, seHandle!, { x: 100, y: 100, width: 200, height: 150 });
  ...
  describe('Handle Types', () => {
    ...
    it('should handle edge resize handles', () => {
      ...
      const handles = resizeEngine.handles();
      const edgeHandles = handles.filter(h => 
        h.position === 'top' || h.position === 'bottom' || 
        h.position === 'left' || h.position === 'right'
      );
  ...
  describe('Error Handling', () => {
    it('should handle invalid box ID gracefully', () => {
      ...
      expect(() => {
        const handles = resizeEngine.handles();
        const seHandle = handles.find(h => h.position === 'bottom-right');
        resizeEngine.startResize('invalid-id', seHandle!, { x: 100, y: 100, width: 200, height: 150 });
      }).not.toThrow();
    });
    ...
    it('should handle invalid handle type gracefully', () => {
      ...
      expect(() => {
        const invalidHandle = { id: 'invalid', position: 'invalid' as any, cursor: 'default', constraints: {} };
        resizeEngine.startResize(mockBoundingBox.id, invalidHandle, { x: 100, y: 100, width: 200, height: 150 });
      }).not.toThrow();
    });
