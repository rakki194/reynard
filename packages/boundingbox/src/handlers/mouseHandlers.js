 ...
 */
export function setupMouseHandlers(canvas, config) {
    // Mouse down - start drawing or select box
    canvas.on("mouse:down", (event) => {
        if (!config.config.enableCreation)
            return;
        ...
        if (clickedObject && clickedObject.data?.boxId) {
            const boxId = clickedObject.data.boxId;
    ...
    canvas.on("mouse:move", (event) => {
        if (!config.isDrawing() || !config.newBox() || !config.startPoint())
            return;
        const pointer = canvas.getPointer(event.e);
    ...
    canvas.on("mouse:up", () => {
        if (!config.isDrawing() || !config.newBox())
            return;
        const newBox = config.newBox();
