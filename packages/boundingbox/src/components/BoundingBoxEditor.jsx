...
export function BoundingBoxEditor(props) {
    const { imageInfo, config, eventHandlers = {}, initialBoxes = [], className = "", containerWidth = 800, containerHeight = 600, } = props;
    const [isClient, setIsClient] = createSignal(false);
    // Initialize composables only on client side
    let boundingBoxes = null;
    let labelManagement = null;
    onMount(() => {
        setIsClient(true);
    ...
    // Handle box update
    const handleBoxUpdate = (boxId, updates) => {
        if (boundingBoxes) {
    ...
    return (<div class={`bounding-box-editor ${className}`}>
      <Show when={isClient() && boundingBoxes && labelManagement}>
        <div class="editor-container">
          <div class="editor-toolbar">
            <div class="label-selector">
              <label for="label-select">Label:</label>
              <select id="label-select" value={labelManagement.selectedLabelClass()} onChange={(e) => labelManagement.setSelectedLabelClass(e.target.value)}>
                {labelManagement.availableLabels().map((label) => (<option value={label}>{label}</option>))}
              </select>
            </div>
            <button onClick={() => {
            if (boundingBoxes) {
          ...
          <div class="editor-main">
            <div class="editor-canvas-container">
              <canvas class="bounding-box-canvas" width={containerWidth} height={containerHeight} style={{
