...
export const PerformanceDashboard = props => {
    const performanceMonitor = usePerformanceMonitor();
    const [activeTab, setActiveTab] = createSignal("overview");
    const [isRecording, setIsRecording] = createSignal(false);
    const [memoryUsage, setMemoryUsage] = createSignal(0);
    const [browserResponsiveness, setBrowserResponsiveness] = createSignal(0);
    ...
    const updateMetrics = async () => {
        try {
            ...
            const convertedWarnings = currentWarnings.map((warning, index) => ({
      ...
      <div class="performance-dashboard-overlay">
        <div class="performance-dashboard">
          ...
          <Show when={activeTab() === "overview"}>
            <PerformanceOverviewTab datasetSize={props.datasetSize || 0} selectionCount={props.selectionCount || 0} memoryUsage={memoryUsage()} browserResponsiveness={browserResponsiveness()} frameRate={frameRate()} isRecording={isRecording()} onToggleRecording={toggleRecording} onClearMetrics={clearMetrics} onExportData={exportPerformanceData}/>
          </Show>
          ...
          <Show when={activeTab() === "memory"}>
            <PerformanceMemoryTab memoryUsage={memoryUsage()} performanceHistory={performanceHistory()} refreshInterval={props.refreshInterval}/>
          </Show>
          ...
          <Show when={activeTab() === "export"}>
            <PerformanceExportTab performanceHistory={performanceHistory()} warnings={warnings()} onExport={exportPerformanceData}/>
          </Show>
