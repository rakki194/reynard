...
export const MemoryTrackingPanel = (props) => {
    ...
    const updateMemoryStats = async () => {
        ...
        try {
            ...
            const averageUsage = memoryUsages.reduce((sum, usage) => sum + usage, 0) /
                memoryUsages.length;
            // Calculate usage trend
            const recentUsages = memoryUsages.slice(-10);
            const olderUsages = memoryUsages.slice(-20, -10);
            const recentAverage = recentUsages.reduce((sum, usage) => sum + usage, 0) /
                recentUsages.length;
    ...
    return (<div class="memory-tracking-panel">
      {/* Header */}
    ...
    innerHTML={getUsageTrendIcon(memoryStats().usageTrend)?.outerHTML || ""}/>
      ...
      <div class="memory-chart">
        ...
        <Chart type="line" labels={getMemoryChartData().map((entry) => new Date(entry.timestamp).toLocaleTimeString())} datasets={[
            {
                label: "Memory Usage",
        ...
        ]} width={800} height={300} useOKLCH={true}/>
      </div>
