...
export function usePerformanceExport(performanceHistory) {
    ...
    const exportData = async () => {
        ...
        try {
            const options = state().exportOptions;
            const dataToExport = state().selectedRows.length > 0 ? state().selectedRows.map(i => filteredData()[i]) : filteredData();
    ...
    const createExportBlob = (data, options) => {
    ...
    const convertToCSV = (data, metrics) => {
        const headers = ["timestamp", ...metrics];
        const rows = data.map(point => headers.map(header => point[header] || ""));
        return [headers, ...rows].map(row => row.join(",")).join("\n");
    };
    const convertToXML = (data, metrics) => {
        const xml = [
            ...
            ...data.map(point => [
                ...
                ...metrics.map(metric => `    <${metric}>${point[metric] || ""}</${metric}>`),
                "  </data-point>",
            ].join("\n")),
            "</performance-data>",
        ].join("\n");
