...
export const PerformanceExportPanel = (props) => {
    const { state, filteredData, availableMetrics, exportFormats, updateExportOptions, selectRow, selectAllRows, clearSelection, exportData, } = usePerformanceExport(props.performanceHistory);
    ...
    return (<div class="reynard-performance-export-panel">
      <div class="reynard-performance-export-panel__header">
        ...
        <div class="reynard-performance-export-panel__actions">
          ...
          <Button variant="primary" size="sm" leftIcon="download" onClick={handleExport} loading={state().isExporting} disabled={filteredData().length === 0}>
            Export Data
          </Button>
        </div>
      </div>
        ...
        <div class="reynard-performance-export-panel__options">
          <div class="reynard-export-options">
            <div class="reynard-export-options__section">
              <h4>Export Format</h4>
              <Select value={state().exportOptions.format} onChange={(e) => updateExportOptions({ format: e.target.value })} options={exportFormats.map((f) => ({
            value: f.value,
            label: f.label,
        }))} size="sm"/>
            </div>
            ...
            <div class="reynard-export-options__section">
              ...
              <div class="reynard-metrics-selection">
                <For each={availableMetrics}>
                  {(metric) => (<label class="reynard-metric-option">
                      <Toggle checked={state().exportOptions.includeMetrics.includes(metric)} onChange={(checked) => {
