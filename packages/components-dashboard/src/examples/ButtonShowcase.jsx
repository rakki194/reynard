...
export const ButtonShowcase = () => {
    ...
    return (<div style={{ padding: "2rem", "max-width": "1200px", margin: "0 auto" }}>
      ...
      <section style={{ margin: "2rem 0" }}>
        ...
        <div style={{ display: "flex", gap: "1rem", "flex-wrap": "wrap" }}>
          ...
          <IconButton icon="heart" variant="danger" active={activeButton() === "heart"} onClick={() => setActiveButton(activeButton() === "heart" ? null : "heart")}/>
        </div>
      </section>
      ...
      <section style={{ margin: "2rem 0" }}>
        ...
        <div style={{
          ...
          <SidebarButton icon="home" label="Home" active={activeButton() === "home"} onClick={() => setActiveButton(activeButton() === "home" ? null : "home")}/>
          <SidebarButton icon="folder" label="Documents" secondaryIcon="chevron-right" showSecondaryActions secondaryActions={[
            {
                icon: "add",
          ...
          <SidebarButton icon="settings" label="Settings" layout="toggle" showContent content={<div style={{ padding: "1rem" }}>Settings content goes here</div>}/>
          <SidebarButton icon="upload" label="Upload Progress" progress={progress()} loading={progress() > 0 && progress() < 100}/>
        </div>
      </section>
      ...
      <section style={{ margin: "2rem 0" }}>
        ...
        <div style={{ display: "flex", gap: "1rem", "flex-wrap": "wrap" }}>
          <IconButton icon="download" variant="primary" progress={progress()} glow={progress() > 50} glowColor="var(--success)" tooltip={`Download progress: ${progress()}%`}>
            Download
          </IconButton>
          <IconButton icon="upload" variant="success" loading={progress() > 0 && progress() < 100} disabled={progress() > 0 && progress() < 100}>
            Upload
          </IconButton>
          <IconButton icon="star" variant="warning" active={activeButton() === "star"} onClick={() => setActiveButton(activeButton() === "star" ? null : "star")} glow={activeButton() === "star"}>
            Favorite
          </IconButton>
        </div>
      </section>
