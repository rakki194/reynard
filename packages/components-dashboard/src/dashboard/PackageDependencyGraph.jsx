...
export const PackageDependencyGraph = props => {
    ...
    const handleResolveConflict = async (package1, package2) => {
        ...
        try {
            ...
            setDependencies(prev => prev.map(dep => dep.name === package1 || dep.name === package2 ? { ...dep, status: "resolved", conflicts: [] } : dep));
            // Remove resolved conflict
            setConflicts(prev => prev.filter(conflict => !(conflict.package1 === package1 && conflict.package2 === package2)));
        }
        catch (error) {
    ...
    const handleAutoResolveAll = async () => {
        ...
        try {
            ...
            setDependencies(prev => prev.map(dep => (dep.status === "conflict" ? { ...dep, status: "resolved", conflicts: [] } : dep)));
            // Clear all conflicts
            setConflicts([]);
        }
        catch (error) {
    ...
    const filteredDependencies = () => {
        ...
        if (selectedPackage()) {
            ...
            return deps.filter(dep => dep.name === selected || dep.dependencies.includes(selected) || dep.dependents.includes(selected));
        }
        return deps;
    };
    return (<div class="package-dependency-graph">
      {/* Header */}
      ...
      <Show when={props.showConflicts && conflicts().length > 0}>
        ...
        innerHTML={fluentIconsPackage.getIcon(getConflictTypeIcon(conflict.conflictType))?.outerHTML || ""}/>
                    ...
                    <div class="conflict-status">
                      <span class="resolution-badge" classList={{
                [getResolutionColor(conflict.resolution)]: true,
                  ...
                  <Show when={props.showResolution && conflict.resolution === "manual"}>
                    <div class="conflict-actions">
                      <Button variant="primary" size="sm" onClick={() => handleResolveConflict(conflict.package1, conflict.package2)} disabled={isResolving()}>
                        Resolve Conflict
                      </Button>
                    </div>
                  </Show>
