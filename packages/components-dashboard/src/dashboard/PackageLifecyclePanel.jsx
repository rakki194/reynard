...
export const PackageLifecyclePanel = props => {
    const { state, refreshLifecycleData, loadPackage, unloadPackage, reloadPackage, setSearchQuery, setSelectedStatus } = usePackageLifecycle(props.refreshInterval);
    return (<div class="reynard-package-lifecycle-panel">
      <div class="reynard-package-lifecycle-panel__header">
        ...
        <div class="reynard-package-lifecycle-panel__actions">
          <Button variant="primary" size="sm" leftIcon="refresh" onClick={refreshLifecycleData} loading={state().isRefreshing}>
            Refresh All
          </Button>
        </div>
      </div>
      ...
      <div class="reynard-package-lifecycle-panel__content">
        <div class="reynard-package-lifecycle-panel__summary">
          <LifecycleSummaryCard summary={state().summary} onRefresh={refreshLifecycleData} isRefreshing={state().isRefreshing}/>
        </div>

        <div class="reynard-package-lifecycle-panel__list">
          <PackageLifecycleList packages={state().packages} onLoadPackage={loadPackage} onUnloadPackage={unloadPackage} onReloadPackage={reloadPackage} searchQuery={state().searchQuery} onSearchChange={setSearchQuery} selectedStatus={state().selectedStatus} onStatusChange={setSelectedStatus} isRefreshing={state().isRefreshing} onRefresh={refreshLifecycleData}/>
        </div>
      </div>
