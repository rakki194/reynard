...
export const PackageInstallationPanel = props => {
    ...
    const handleInstallPackage = async (packageName) => {
        ...
        const updatedPackages = packages().map(pkg => pkg.name === packageName ? { ...pkg, status: "installing", startTime: new Date() } : pkg);
        setPackages(updatedPackages);
        // Simulate installation progress
        let progress = 0;
        const progressInterval = setInterval(() => {
            progress += 10;
            setPackages(prev => prev.map(pkg => pkg.name === packageName && pkg.status === "installing" ? { ...pkg, progress: Math.min(progress, 100) } : pkg));
            if (progress >= 100) {
                clearInterval(progressInterval);
                setPackages(prev => prev.map(pkg => pkg.name === packageName && pkg.status === "installing"
    ...
    const handleCancelInstallation = async (packageName) => {
        ...
        const updatedPackages = packages().map(pkg => pkg.name === packageName ? { ...pkg, status: "cancelled", endTime: new Date() } : pkg);
        setPackages(updatedPackages);
    };
    const handleRetryInstallation = async (packageName) => {
        ...
        // Simulate retry
        const updatedPackages = packages().map(pkg => pkg.name === packageName
            ? {
                ...pkg,
                status: "installing",
                progress: 0,
    ...
    return (<div class="package-installation-panel">
      {/* Header */}
    ...
    innerHTML={fluentIconsPackage.getIcon("download")?.outerHTML || ""}/>
        ...
        <div class="installation-panel-actions">
          <Show when={props.showBulkOperations}>
            <Button variant="primary" onClick={handleBulkInstall} disabled={isInstalling() || summary().pendingPackages === 0}>
