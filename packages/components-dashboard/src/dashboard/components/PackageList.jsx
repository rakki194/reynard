...
export const PackageList = props => {
    const filteredPackages = () => {
        ...
        if (props.searchQuery) {
            filtered = filtered.filter(pkg => pkg.name.toLowerCase().includes(props.searchQuery.toLowerCase()) ||
    ...
    return (<div class="reynard-package-list">
      <div class="reynard-package-list__header">
        <h3>Package Configuration</h3>
        <div class="reynard-package-list__controls">
          <TextField placeholder="Search packages..." value={props.searchQuery} onInput={e => props.onSearchChange(e.currentTarget.value)} leftIcon="search" size="sm"/>
          <Select value={props.selectedCategory} onChange={e => props.onCategoryChange(e.target.value)} options={[
      ...
      <div class="reynard-package-list__content">
        <Show when={filteredPackages().length > 0} fallback={<div class="reynard-package-list__empty">
              <Icon name="package" size="lg" variant="muted"/>
              <p>No packages found</p>
            </div>}>
          <For each={filteredPackages()}>
            {pkg => (<div class={`reynard-package-item ${props.selectedPackage === pkg.name ? "reynard-package-item--selected" : ""}`} onClick={() => props.onSelectPackage(pkg.name)}>
                <div class="reynard-package-item__header">
                  ...
                  <div class="reynard-package-item__status">
                    <Icon name={pkg.isConfigured ? "checkmark-circle" : "warning"} variant={pkg.isConfigured ? "success" : "warning"} size="sm"/>
                  </div>
                </div>
