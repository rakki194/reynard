...
export const ServiceAuthStatus = props => {
    ...
    const [lastCheck, setLastCheck] = createSignal(null);
    onMount(() => {
        // Initial data load
        checkAuthServices();
    });
    ...
    if (props.compact) {
        return (<div class="service-auth-status compact">
    ...
    return (<div class="service-auth-status">
      {/* Header */}
    ...
    innerHTML={fluentIconsPackage.getIcon("shield")?.outerHTML || ""}/>
        ...
        <div class="auth-status-actions">
          <Show when={props.showRetryButton}>
            <Button variant="secondary" size="sm" onClick={checkAuthServices} disabled={isChecking()}>
    ...
    innerHTML={fluentIconsPackage.getIcon(getOverallStatusIcon())?.outerHTML || ""}/>
      ...
      <div class="auth-services-list">
        <For each={filteredServices()}>
          {service => (<div class="auth-service-item">
    ...
    innerHTML={fluentIconsPackage.getIcon("spinner")?.outerHTML || ""}/>
      ...
      {/* Last Check */}
      <Show when={lastCheck()}>
        <div class="last-check">Last checked: {lastCheck().toLocaleString()}</div>
      </Show>
    </div>);
};
