  ...
  return (
    ...
    >
      <Show when={props.showIcons && props.item.icon}>
        <span class="reynard-context-menu__item-icon">{props.item.icon}</span>
      </Show>
      <span class="reynard-context-menu__item-label">{props.item.label}</span>
      <Show when={hasSubmenu(props.item)}>
        <span class="reynard-context-menu__item-arrow" aria-hidden="true">
          ▶
        </span>
      </Show>
    </div>
