...
class ToolPermissionError(ToolError):
    ...
    def __init__(self, tool_name: str, required_permission: str, user_role: Optional[str] = None):
        message = f"Permission denied for tool '{tool_name}'. Required: {required_permission}"
        if user_role:
...
class ToolValidationError(ToolError):
    ...
    def __init__(self, tool_name: str, validation_errors: Dict[str, str]):
        errors_str = ", ".join([f"{param}: {error}" for param, error in validation_errors.items()])
        super().__init__(f"Tool '{tool_name}' parameter validation failed: {errors_str}", tool_name)
...
class ToolTimeoutError(ToolError):
    ...
    def __init__(self, tool_name: str, timeout_seconds: float):
        super().__init__(f"Tool '{tool_name}' timed out after {timeout_seconds} seconds", tool_name)
...
class ToolResourceError(ToolError):
    ...
    def __init__(self, tool_name: str, resource_type: str, message: str):
        super().__init__(f"Tool '{tool_name}' resource error ({resource_type}): {message}", tool_name)
