...
from .exceptions import ToolExecutionError, ToolTimeoutError, ToolResourceError
...
class ToolExecutor:
    ...
    async def execute_tool(
        ...
        try:
            logger.info(f"Starting execution of tool '{tool.name}' for user {context.user_id}")
            ...
            # Execute the tool
            if context.dry_run:
                result = await self._execute_dry_run(tool, execution_context, parameters)
            else:
                result = await self._execute_with_monitoring(tool, execution_context, parameters)
            ...
            logger.info(f"Tool '{tool.name}' execution completed in {execution_time:.2f}s")
            return result
    ...
    def _prepare_execution_context(self, context: ToolExecutionContext) -> ToolExecutionContext:
        """
...
def initialize_tool_executor(thread_pool: Optional[ThreadPoolExecutor] = None) -> ToolExecutor:
