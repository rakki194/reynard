...
async def nlweb_ask_tool(query: str, context: Dict[str, Any] = None, site: str = "") -> Dict[str, Any]:
    ...
    try:
        ...
        if not service_manager:
            return {
                "success": False,
                "error": "Service manager unavailable"
            }
        ...
        if not getattr(cfg, "nlweb_enabled", False):
            return {
                "success": False,
                "error": "NLWeb integration is disabled"
            }
...
async def nlweb_mcp_tool(method: str, params: Dict[str, Any] = None, id: str = "1") -> Dict[str, Any]:
...
async def nlweb_suggest_tool(query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if not nlweb_router:
            return {
                "success": False,
                "error": "NLWeb router service not available"
            }

        # Get suggestions from the router service
        suggestions = await nlweb_router.suggest_tools(query=query, context=context or {})
