...
class MockSocket {
  ...
  removeEventListener(type: string, cb: any) {
    if (type === 'open') this._openHandlers = this._openHandlers.filter(f => f !== cb);
    if (type === 'error') this._errorHandlers = this._errorHandlers.filter(f => f !== cb);
    if (type === 'message') this._messageHandlers = this._messageHandlers.filter(f => f !== cb);
  }
  send(_d: any) {}
  close() {
    this.readyState = 3;
...
describe('WebSocketConnection', () => {
  it('connects, sends and receives a message', async () => {
    if (!(globalThis as any).crypto || typeof (globalThis as any).crypto.randomUUID !== 'function') {
      (globalThis as any).crypto = { randomUUID: () => '00000000-0000-4000-8000-000000000000' } as any;
    }
    ...
    // emit one message
    (c as any).ws._messageHandlers.forEach((f: Function) => f({ data: JSON.stringify({ a: 1 }) }));
    const msg = await p;
