...
export class GitWorkflowOrchestrator {
  ...
  async executeWorkflow(options: Partial<WorkflowOptions> = {}): Promise<WorkflowResult> {
    ...
    try {
      ...
      if (!opts.skipCommitGeneration && result.steps.changeAnalysis.result) {
        result.steps.commitMessage = await this.executeCommitGeneration(result.steps.changeAnalysis.result, opts);
      ...
      if (!opts.skipVersionBump && result.steps.changeAnalysis.result) {
        result.steps.versionBump = await this.executeVersionBump(result.steps.changeAnalysis.result, opts);
      ...
      if (!opts.skipChangelogUpdate && result.steps.versionBump.result) {
        result.steps.changelogUpdate = await this.executeChangelogUpdate(result.steps.versionBump.result, opts);
   ...
   */
  private async executeJunkDetection(
    opts: WorkflowOptions
  ): Promise<{ completed: boolean; result?: JunkDetectionResult }> {
   ...
   */
  private async executeChangeAnalysis(
    opts: WorkflowOptions
  ): Promise<{ completed: boolean; result?: ChangeAnalysisResult }> {
