 ...
 */
import { BaseAIService, ServiceHealth, ServiceStatus, ServiceError, } from "reynard-ai-shared";
import { BackendAnnotationService } from "reynard-annotating-core";
import { PolygonOps, PointOps } from "reynard-algorithms";
 ...
 */
export class SegmentationService extends BaseAIService {
    constructor(config) {
        super(config);
    ...
    async initialize() {
        try {
            ...
            throw new ServiceError(`Failed to initialize segmentation service: ${this.lastBackendError}`, "INITIALIZATION_ERROR");
        }
    }
    /**
     ...
     */
    async generateSegmentation(task) {
        ...
        try {
            ...
            throw new ServiceError(`Segmentation generation failed: ${error instanceof Error ? error.message : "Unknown error"}`, "SEGMENTATION_GENERATION_ERROR");
        }
    }
    /**
     * Generate multiple segmentations
     */
    async generateBatchSegmentations(tasks, progressCallback) {
        const results = [];
     ...
     */
    async refineSegmentation(segmentation, options) {
        const startTime = Date.now();
        try {
            ...
            const processingTime = Date.now() - startTime;
            this.updateProcessingStats(processingTime, false);
            throw new ServiceError(`Segmentation refinement failed: ${error instanceof Error ? error.message : "Unknown error"}`, "SEGMENTATION_REFINEMENT_ERROR");
        }
    }
    /**
     ...
     */
    exportSegmentation(segmentation, format) {
     ...
     */
    async convertToSegmentationResult(captionResult, _originalTask, startTime) {
     ...
     */
    calculateQualityMetrics(polygon) {
        const area = PolygonOps.area(polygon);
        const perimeter = this.calculatePerimeter(polygon);
        const complexity = polygon.points.length;
    ...
    hasSelfIntersections(polygon) {
        ...
        for (let i = 0; i < points.length; i++) {
            for (let j = i + 2; j < points.length; j++) {
                if (this.linesIntersect(points[i], points[(i + 1) % points.length], points[j], points[(j + 1) % points.length])) {
                    return true;
                }
            }
        }
     ...
     */
    updateProcessingStats(processingTime, success = true) {
     ...
     */
    exportToPascalVOC(segmentation) {
        return {
            object: {
