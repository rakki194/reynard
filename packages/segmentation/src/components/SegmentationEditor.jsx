 ...
 */
import { CaptionInput } from "reynard-caption-ui";
import { FloatingPanel, FloatingPanelOverlay, useOverlayManager } from "reynard-floating-panel";
 ...
 */
export const SegmentationEditor = props => {
    ...
    const [segmentations, setSegmentations] = createSignal(props.segmentations || []);
    ...
    const isEditorActive = createMemo(() => config().enabled && overlayManager.isActive());
    ...
    const panels = createMemo(() => {
        ...
        return [
            // Main segmentation panel
            {
                ...
                content: (<SegmentationPanel segmentations={currentSegmentations} selectedSegmentation={currentState.selectedSegmentation} onSegmentationSelect={id => {
                ...
                ? [
                    {
                        ...
                        content: (<CaptionInput caption={selectedSegmentation().caption || {
                                type: "caption",
                                content: "",
                            }} state="expanded" onCaptionChange={caption => {
                                const updatedSegmentation = {
                                    ...selectedSegmentation(),
                                    caption,
                                    updatedAt: new Date(),
    ...
    return (<div class={`segmentation-editor ${props.class || ""}`}>
      {/* Main editor interface */}
      <div class="segmentation-editor-main">
        {/* Toolbar */}
        <SegmentationToolbar config={config()} state={editorState()} onToggleEditor={handleToggleEditor} onZoomChange={handleZoomChange} onPanChange={handlePanChange}/>

        {/* Canvas */}
        <SegmentationCanvas imageSrc={props.imageSrc} segmentations={segmentations()} config={config()} state={editorState()} onMouseMove={handleMouseMove} onZoomChange={handleZoomChange} onPanChange={handlePanChange} onSegmentationSelect={id => {
            setEditorState(prev => ({ ...prev, selectedSegmentation: id }));
            props.events?.onSegmentationSelect?.(id);
      ...
      <FloatingPanelOverlay isActive={isEditorActive()} transitionPhase={overlayManager.overlayState().transitionPhase}>
        <For each={panels()}>
          {panel => (<FloatingPanel id={panel.id} position={panel.position} size={panel.size} config={panel.config}>
              {panel.content}
            </FloatingPanel>)}
        </For>
