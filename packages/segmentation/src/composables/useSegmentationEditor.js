 ...
 */
export function useSegmentationEditor(options) {
    const [state, setState] = createSignal(options.state);
    const [segmentations, setSegmentations] = createSignal([]);
    ...
    const createSegmentation = (segmentation) => {
    ...
    const updateSegmentation = (segmentation) => {
        // Validate the segmentation
        if (!validateSegmentation(segmentation)) {
            console.error("Invalid segmentation data:", segmentation);
            return;
    ...
    const deleteSegmentation = (segmentationId) => {
    ...
    const selectSegmentation = (segmentationId) => {
        updateState({
    ...
    // Start editing mode
    const startEditing = (segmentationId) => {
        updateState({
            isEditing: true,
            isCreating: false,
            selectedSegmentation: segmentationId,
    ...
    const validateSegmentation = (segmentation) => {
        ...
        if (area < options.config.minPolygonArea ||
    ...
    const hasSelfIntersections = (polygon) => {
    ...
    // Utility functions
    const getSegmentation = (id) => {
        return segmentations().find((s) => s.id === id);
    };
    const getSelectedSegmentation = () => {
