...
export function useCanvasInteraction(options) {
    ...
    const [lastTouchDistance, setLastTouchDistance] = createSignal(0);
    ...
    const screenToWorld = (screenPoint) => {
    ...
    const handleMouseMove = (event) => {
        if (!options.config.enabled || !event)
            return;
    ...
    const handleMouseDown = (event) => {
        ...
        if (event.button === 0) {
            ...
            else if (options.state.isEditing &&
