...
function displayQualityGateResult(result) {
    ...
    if (failedConditions.length > 0) {
        ...
        for (const condition of failedConditions) {
            console.log(`     - ${condition.condition.metric} ${condition.condition.operator} ${condition.condition.threshold} (actual: ${condition.actualValue})`);
        }
    }
}
...
export async function handleQualityGateCommand(options) {
    try {
