...
export const ImageGrid = (props) => {
    const { t } = useI18n();
    const [imageFiles, setImageFiles] = createSignal(props.initialFiles || []);
    ...
    const handleGenerateCaption = (file) => {
        // This would be implemented based on the caption generation system
        console.log("Generate caption for:", file);
    };
    ...
    const handleFileUpload = async (event) => {
        ...
        try {
            const processedFiles = await Promise.all(filesToProcess.map(processImageFile));
    ...
    return (<div class={`image-grid ${props.class || ""}`}>
      {/* File Upload */}
      <div class="image-upload-section">
        <label for="image-upload" class="upload-label">
          Upload Images
        </label>
        <input id="image-upload" type="file" multiple accept="image/*" onChange={handleFileUpload} class="image-upload-input" disabled={isLoading()} title={t("image.selectImageFilesToUpload")}/>
        <Show when={isLoading()}>
      ...
      <div class="images-grid">
        <For each={imageFiles()}>
          {file => (<ImageFileCard file={file} isSelected={selectedFile()?.id === file.id} onSelect={() => handleFileSelect(file)} onRemove={() => handleFileRemove(file.id)} onGenerateCaption={() => handleGenerateCaption(file)} showMetadata={props.showMetadata} isGenerating={props.isGenerating}/>)}
        </For>
      </div>

      {/* Selected Image Viewer */}
      <Show when={selectedFile()}>
        <ImageViewer file={selectedFile()} onClose={() => setSelectedFile(null)} onGenerateCaption={() => selectedFile() && handleGenerateCaption(selectedFile())} isGenerating={props.isGenerating}/>
      </Show>
    </div>);
};
const ImageFileCard = (props) => {
    const { t } = useI18n();
    ...
    return (<div class="image-file-card" classList={{ selected: props.isSelected }} onClick={props.onSelect}>
      <div class="image-thumbnail">
        ...
        <div class="image-overlay">
          <div class="image-actions">
            <button class="action-button generate-button" onClick={e => {
            e.stopPropagation();
      ...
      <div class="image-info">
        ...
        <div class="image-size">{props.file.size ? (props.file.size / (1024 * 1024)).toFixed(2) + " MB" : "Unknown size"}</div>
...
const ImageViewer = (props) => {
    return (<div class="image-viewer-modal">
      <div class="image-viewer-content">
        ...
        <div class="image-viewer-header">
          ...
          <div class="image-viewer-actions">
            <button class="generate-caption-button" onClick={props.onGenerateCaption} disabled={props.isGenerating}>
            ...
            <div class="image-metadata-panel">
              ...
              <div class="metadata-grid">
                ...
                <div class="metadata-item">
                  ...
                  <span class="value">{props.file.size ? (props.file.size / (1024 * 1024)).toFixed(2) + " MB" : "Unknown"}</span>
                </div>
              </div>
            </div>
          </Show>
