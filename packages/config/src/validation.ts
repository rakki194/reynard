...
export class ConfigValidator {
    ...
    static validateValidation(config: Partial<ValidationConfig>): ValidationResult {
        ...
        if (config.password) {
            if (config.password.minLength !== undefined && (typeof config.password.minLength !== "number" || config.password.minLength < 1)) {
                errors.push("Password minLength must be a positive number");
            }
            if (config.password.maxLength !== undefined && (typeof config.password.maxLength !== "number" || config.password.maxLength < config.password.minLength || 0)) {
            ...
            if (config.password.requireSpecialChars !== undefined && typeof config.password.requireSpecialChars !== "boolean") {
                errors.push("Password requireSpecialChars must be a boolean");
            }
        }

        if (config.username) {
            if (config.username.minLength !== undefined && (typeof config.username.minLength !== "number" || config.username.minLength < 1)) {
                errors.push("Username minLength must be a positive number");
            }
            if (config.username.maxLength !== undefined && (typeof config.username.maxLength !== "number" || config.username.maxLength < config.username.minLength || 0)) {
        ...
        if (config.email) {
            if (config.email.maxLength !== undefined && (typeof config.email.maxLength !== "number" || config.email.maxLength < 1)) {
    ...
    static validatePerformance(config: Partial<PerformanceConfig>): ValidationResult {
        ...
        if (config.maxRetryAttempts !== undefined && (typeof config.maxRetryAttempts !== "number" || config.maxRetryAttempts < 0 || config.maxRetryAttempts > 10)) {
            errors.push("Performance maxRetryAttempts must be a number between 0 and 10");
        }

        if (config.healthCheckInterval !== undefined && (typeof config.healthCheckInterval !== "number" || config.healthCheckInterval <= 0)) {
            errors.push("Performance healthCheckInterval must be a positive number");
        }

        if (config.metricsCollectionInterval !== undefined && (typeof config.metricsCollectionInterval !== "number" || config.metricsCollectionInterval <= 0)) {
            errors.push("Performance metricsCollectionInterval must be a positive number");
        }

        if (config.memory) {
            if (config.memory.warningThreshold !== undefined && (typeof config.memory.warningThreshold !== "number" || config.memory.warningThreshold < 0 || config.memory.warningThreshold > 1)) {
                errors.push("Memory warningThreshold must be a number between 0 and 1");
            }
            if (config.memory.criticalThreshold !== undefined && (typeof config.memory.criticalThreshold !== "number" || config.memory.criticalThreshold < 0 || config.memory.criticalThreshold > 1)) {
                errors.push("Memory criticalThreshold must be a number between 0 and 1");
            }
            if (config.memory.warningThreshold !== undefined && config.memory.criticalThreshold !== undefined && config.memory.warningThreshold >= config.memory.criticalThreshold) {
    ...
    static validateSecurity(config: Partial<SecurityConfig>): ValidationResult {
        ...
        if (config.jwt) {
            if (config.jwt.defaultExpiry !== undefined && (typeof config.jwt.defaultExpiry !== "number" || config.jwt.defaultExpiry <= 0)) {
                errors.push("JWT defaultExpiry must be a positive number");
            }
            if (config.jwt.refreshExpiry !== undefined && (typeof config.jwt.refreshExpiry !== "number" || config.jwt.refreshExpiry <= 0)) {
        ...
        if (config.rateLimiting) {
            if (config.rateLimiting.defaultWindow !== undefined && (typeof config.rateLimiting.defaultWindow !== "number" || config.rateLimiting.defaultWindow <= 0)) {
                errors.push("Rate limiting defaultWindow must be a positive number");
            }
            if (config.rateLimiting.defaultMaxRequests !== undefined && (typeof config.rateLimiting.defaultMaxRequests !== "number" || config.rateLimiting.defaultMaxRequests <= 0)) {
    ...
    static validateUI(config: Partial<UIConfig>): ValidationResult {
        ...
        if (config.animation) {
            if (config.animation.duration !== undefined && (typeof config.animation.duration !== "number" || config.animation.duration < 0)) {
    ...
    static validateAPI(config: Partial<APIConfig>): ValidationResult {
        ...
        if (config.pagination) {
            if (config.pagination.defaultPageSize !== undefined && (typeof config.pagination.defaultPageSize !== "number" || config.pagination.defaultPageSize <= 0)) {
                errors.push("API pagination defaultPageSize must be a positive number");
            }
            if (config.pagination.maxPageSize !== undefined && (typeof config.pagination.maxPageSize !== "number" || config.pagination.maxPageSize <= 0)) {
                errors.push("API pagination maxPageSize must be a positive number");
            }
            if (config.pagination.minPageSize !== undefined && (typeof config.pagination.minPageSize !== "number" || config.pagination.minPageSize <= 0)) {
    ...
    static validateStorage(config: Partial<StorageConfig>): ValidationResult {
        ...
        if (config.indexedDB) {
            ...
            if (config.indexedDB.version !== undefined && (typeof config.indexedDB.version !== "number" || config.indexedDB.version <= 0)) {
    ...
    static validateFeatures(config: Partial<FeatureConfig>): ValidationResult {
        ...
        for (const feature of booleanFeatures) {
            if (config[feature as keyof FeatureConfig] !== undefined && typeof config[feature as keyof FeatureConfig] !== "boolean") {
