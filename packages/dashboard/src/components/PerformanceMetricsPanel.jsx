...
export const PerformanceMetricsPanel = (props) => {
    ...
    const updateMetrics = async () => {
        ...
        try {
            ...
            // Calculate metrics
            const averageFrameRate = filteredHistory.reduce((sum, entry) => sum + entry.frameRate, 0) /
                filteredHistory.length;
            const averageMemoryUsage = filteredHistory.reduce((sum, entry) => sum + entry.memoryUsage, 0) /
                filteredHistory.length;
    ...
    innerHTML={fluentIconsPackage.getIcon("chart-line")?.outerHTML || ""}/>
      ...
      <div class="performance-chart">
        ...
        <Chart type="line" labels={getChartData().map((entry) => new Date(entry.timestamp).toLocaleTimeString())} datasets={[
            {
