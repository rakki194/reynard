...
export class ImpactAnalyzer {
    constructor(codebasePath, adrPath) {
        ...
        Object.defineProperty(this, "changeHistory", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        });
        Object.defineProperty(this, "fileHashes", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
     ...
     */
    getChangesInRange(startDate, endDate) {
        return Array.from(this.changeHistory.values())
            .filter((change) => {
     ...
     */
    generateImpactReport(changes, assessments) {
    ...
    async identifyImpactAreas(change) {
        const impactAreas = [];
        const filePath = change.filePath.toLowerCase();
        const content = change.content?.toLowerCase() || "";
        // Security impact
    ...
    async checkComplianceViolations(change) {
        const violations = [];
    ...
    async generateRecommendations(change, violations) {
    ...
    calculateImpactLevel(affectedADRs, violations) {
        if (violations.some((v) => v.severity === "critical")) {
            return "critical";
        }
        if (affectedADRs.length > 3 ||
            violations.some((v) => v.severity === "high")) {
            return "high";
        }
        if (affectedADRs.length > 1 ||
            violations.some((v) => v.severity === "medium")) {
            return "medium";
        }
        return "low";
    }
    estimateEffort(change, violations) {
        let effort = 0;
    ...
    calculateRiskScore(change, violations) {
        let risk = 0;
    ...
    async findAffectedFiles(change) {
    ...
    estimateMigrationEffort(breakingChanges) {
        return breakingChanges.length * 2; // 2 hours per breaking change
    }
    isFileRelevantToADR(filePath, adr) {
    ...
    isContentRelevantToADR(content, adr) {
