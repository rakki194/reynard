...
export class CodebaseAnalyzer {
  ...
  private detectMicroservicePattern(files: string[], dependencies: DependencyAnalysis): ArchitecturePattern {
    ...
    const independentModules = Array.from(dependencies.internalDependencies.entries())
  ...
  private detectModularPattern(files: string[], dependencies: DependencyAnalysis): ArchitecturePattern {
    ...
    const avgDeps = Array.from(dependencies.internalDependencies.values())
      .reduce((sum, deps) => sum + deps.length, 0) / dependencies.internalDependencies.size;
  ...
  private detectLayeredPattern(files: string[], dependencies: DependencyAnalysis): ArchitecturePattern {
    ...
    const layerFiles = layers.map(layer => 
      files.filter(f => f.includes(layer)).length
    );
