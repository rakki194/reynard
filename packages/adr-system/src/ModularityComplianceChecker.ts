...
export class ModularityComplianceChecker {
   ...
   */
  getCriticalFiles(): Array<{ filePath: string; violations: ModularityViolation[] }> {
    const criticalFiles: Array<{ filePath: string; violations: ModularityViolation[] }> = [];
  ...
  private async detectViolations(filePath: string, content: string, metrics: ModuleAnalysis['metrics']): Promise<ModularityViolation[]> {
  ...
  private findFunctions(content: string): Array<{ name?: string; line: number; lines: number }> {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inFunction) {
        ...
        if (braceCount === 0 && line.includes('}')) {
          const functionLines = i - functionStart + 1;
          functions.push({ name: functionName, line: functionStart + 1, lines: functionLines });
          inFunction = false;
        }
      }
    }
