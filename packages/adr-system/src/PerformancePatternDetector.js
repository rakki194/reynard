...
export class PerformancePatternDetector {
    constructor(codebasePath) {
        ...
        Object.defineProperty(this, "issueCache", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
     ...
     */
    generatePerformanceSuggestions(filePath) {
        const issues = this.getFilePerformanceIssues(filePath);
        const suggestions = [];
        if (issues.length === 0) {
    ...
    detectSynchronousIO(content, filePath) {
        const issues = [];
    ...
    detectInefficientLoops(content, filePath) {
        const issues = [];
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
    ...
    detectBlockingOperations(content, filePath) {
        const issues = [];
        const lines = content.split("\n");
    ...
    detectLargeObjects(content, filePath) {
        const issues = [];
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
    ...
    detectUnnecessaryComputation(content, filePath) {
        const issues = [];
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
    ...
    detectInefficientAlgorithms(content, filePath) {
        const issues = [];
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
    ...
    detectResourceExhaustion(content, filePath) {
        const issues = [];
        const lines = content.split("\n");
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
    ...
    generatePerformanceReport(files, issues, detectedPatterns) {
        const totalIssues = issues.length;
        const criticalIssues = issues.filter((issue) => issue.severity === "critical").length;
        const overallPerformance = files.reduce((sum, file) => sum + this.getFilePerformanceScore(file), 0) /
            files.length;
    ...
    generateGlobalRecommendations(issues, detectedPatterns) {
        const immediate = [];
        const shortTerm = [];
        const longTerm = [];
        const criticalIssues = issues.filter((issue) => issue.severity === "critical");
    ...
    hasNestedLoop(content, lineIndex) {
        const lines = content.split("\n");
        let braceCount = 0;
        let foundLoop = false;
    ...
    hasArrayOperationsInLoop(content, lineIndex) {
        const lines = content.split("\n");
        const line = lines[lineIndex];
        return (line.includes("arr.length") ||
            line.includes("array.length") ||
            line.includes(".push(") ||
            line.includes(".pop("));
    }
    isLargeObjectLiteral(content, lineIndex) {
        const lines = content.split("\n");
        let braceCount = 0;
        let propertyCount = 0;
    ...
    isLargeArray(content, lineIndex) {
        const lines = content.split("\n");
        const line = lines[lineIndex];
    ...
    hasFileHandleLeak(content, lineIndex) {
        const lines = content.split("\n");
        const line = lines[lineIndex];
        return (line.includes("fs.") &&
            !line.includes("close") &&
            !line.includes("finally"));
    }
    getMemoryLeakSeverity(pattern) {
        if (pattern === "setInterval")
            return "high";
        if (pattern === "addEventListener")
            return "medium";
        if (pattern === "new Array(1000000)")
            return "critical";
        return "low";
    }
    generateIssueId() {
