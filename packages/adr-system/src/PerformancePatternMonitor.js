...
export class PerformancePatternMonitor extends EventEmitter {
    constructor(codebasePath, monitoringInterval = 30000) {
        ...
        Object.defineProperty(this, "metricHistory", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
     ...
     */
    getMetricsInRange(metricName, startDate, endDate) {
     ...
     */
    detectPerformanceRegression(metricName, currentValue) {
    ...
    async checkPerformanceRegressions(metrics) {
        for (const metric of metrics) {
    ...
    calculateTrend(metricName, currentValue) {
        ...
        if (difference > threshold)
            return "declining"; // For performance metrics, higher is often worse
        if (difference < -threshold)
            return "improving";
        return "stable";
    }
    ...
    async createInitialMetrics(performanceReport) {
        const metrics = await this.collectPerformanceMetrics();
        this.updateMetricsHistory(metrics);
    }
    // Helper methods for metric calculation
    calculateCPUScore(performanceReport) {
    ...
    simulateNetworkThroughput() {
        return Math.random() * 50 + 10; // 10-60 mbps
    }
    simulateDatabaseQueryTime() {
        return Math.random() * 100 + 20; // 20-120ms
    }
    simulateDatabaseConnectionPool() {
