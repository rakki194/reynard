...
export class DependencyMapper {
     ...
     */
    async analyzeImpactPropagation(change) {
     ...
     */
    async getDependencyImpact(changes) {
    ...
    async calculatePropagationPath(sourceNode) {
        const paths = [];
        const visited = new Set();
        const dfs = (currentNodeId, path, depth) => {
            if (depth > 10)
    ...
    getAffectedNodes(propagationPaths) {
    ...
    calculateImpactLevels(change, affectedNodes) {
    ...
    identifyBreakingChanges(change, affectedNodes) {
    ...
    calculateNodeImportance(node) {
        ...
        const importance = (dependentsCount * 0.4 +
            complexity * 0.3 +
            (1 - stability) * 0.2 +
    ...
    calculateInitialImportance(filePath, content) {
    ...
    calculateStability(filePath, lastModified) {
    ...
    calculateNodeDepth(nodeId, visited) {
        if (visited.has(nodeId))
            return 0;
        visited.add(nodeId);
        const node = this.dependencyGraph.nodes.get(nodeId);
