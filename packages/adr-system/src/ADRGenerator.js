...
export class ADRGenerator {
    constructor(adrDirectory, templateDirectory) {
     ...
     */
    async generateADRFromSuggestion(suggestion) {
     ...
     */
    async generateMultipleADRs(suggestions) {
     ...
     */
    async generateADRContent(adrId, suggestion, template) {
    ...
    async getNextADRId() {
        try {
            ...
            const maxId = Math.max(...adrFiles.map((file) => {
