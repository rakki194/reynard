...
export class PerformancePatternDetector {
   ...
   */
  getCriticalPerformanceFiles(): Array<{ filePath: string; issues: PerformanceIssue[] }> {
    const criticalFiles: Array<{ filePath: string; issues: PerformanceIssue[] }> = [];
  ...
  private detectMemoryLeaks(content: string, filePath: string): PerformanceIssue[] {
    ...
    const memoryLeakPatterns = [
      ...
      { pattern: 'addEventListener', description: 'Event listener without removal' },
      { pattern: 'new Array(1000000)', description: 'Large array allocation' },
      { pattern: 'JSON.parse', description: 'Large JSON parsing without size limits' }
    ];
  ...
  private generatePerformanceReport(
    ...
    detectedPatterns: { antiPatterns: PerformancePattern[]; optimizations: PerformancePattern[]; bestPractices: PerformancePattern[] }
  ): PerformanceReport {
  ...
  private hasArrayOperationsInLoop(content: string, lineIndex: number): boolean {
    ...
    return line.includes('arr.length') || line.includes('array.length') || line.includes('.push(') || line.includes('.pop(');
  }
