...
export class ImpactAnalyzer {
  ...
  private async identifyImpactAreas(change: CodeChange): Promise<string[]> {
    ...
    if (filePath.includes('performance') || filePath.includes('optimization') ||
        content.includes('cache') || content.includes('async') || content.includes('performance')) {
      impactAreas.push('performance');
    }
    
    // Scalability impact
    if (filePath.includes('scaling') || filePath.includes('load') ||
        content.includes('cluster') || content.includes('distributed') || content.includes('scaling')) {
      impactAreas.push('scalability');
    }
    
    // Integration impact
    if (filePath.includes('api') || filePath.includes('integration') ||
        content.includes('endpoint') || content.includes('service') || content.includes('integration')) {
      impactAreas.push('integration');
    }
    
    // Maintainability impact
    if (filePath.includes('test') || filePath.includes('util') ||
        content.includes('refactor') || content.includes('cleanup') || content.includes('maintain')) {
  ...
  private calculateImpactLevel(affectedADRs: string[], violations: ComplianceViolation[]): 'low' | 'medium' | 'high' | 'critical' {
