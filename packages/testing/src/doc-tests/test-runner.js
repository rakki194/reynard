 ...
 */
export function runDocTests(config) {
            ...
            if (testableCode) {
                it(`Example ${index + 1}: ${example.description}`, async () => {
                    try {
                        ...
                        const testFunction = new Function("expect", "render", "screen", "createSignal", "createEffect", "onCleanup", "vi", "describe", "it", "beforeEach", "afterEach", setup, testableCode);
                        // Execute the test
                        await testFunction(expect, render, screen, createSignal, createEffect, onCleanup, vi, describe, it, beforeEach, afterEach);
                    }
                    catch (error) {
 ...
 */
export function createDocTestFile(config) {
    const { docPath, packageName, setup = "" } = config;
    return `
/**
