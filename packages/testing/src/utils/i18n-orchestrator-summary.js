 ...
 */
export function generateSummary(packageResults) {
    ...
    const totalMissingTranslations = packageResults.reduce((sum, r) => sum +
        r.results.translationValidation.reduce((vSum, v) => vSum + v.missingKeys.length, 0), 0);
 ...
 */
export function generateGlobalReport(packageResults, summary, duration) {
 ...
 */
function generatePackageReportSection(result) {
