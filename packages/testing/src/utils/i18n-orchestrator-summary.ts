...
export function generateSummary(
  ...
  const totalHardcodedStrings = packageResults.reduce(
    (sum, r) => sum + r.results.hardcodedStrings.length,
    0,
  );
  ...
  const totalMissingTranslations = packageResults.reduce(
    (sum, r) =>
      sum +
      r.results.translationValidation.reduce(
        (vSum, v) => vSum + v.missingKeys.length,
        0,
      ),
    0,
  );
  ...
  const totalRTLIssues = packageResults.reduce(
    (sum, r) => sum + r.results.rtlIssues.length,
    0,
  );

  const totalCoverage = packageResults.reduce(
    (sum, r) => sum + (r.results.performanceMetrics.loadTime > 0 ? 1 : 0),
    0,
  );
  const averageCoverage =
    totalPackages > 0 ? (totalCoverage / totalPackages) * 100 : 0;
