 ...
 */
export async function runI18nCIChecks(config) {
 ...
 */
async function checkHardcodedStrings(packages) {
    try {
        ...
        const count = results.reduce((sum, file) => sum + file.messages.length, 0);
        const details = results.flatMap((file) => file.messages.map((msg) => `${file.filePath}:${msg.line}:${msg.column} - ${msg.message}`));
 ...
 */
async function validateTranslations(locales) {
    try {
        ...
        const results = JSON.parse(output);
        const missingCount = results.testResults.reduce((sum, test) => sum + (test.status === "failed" ? 1 : 0), 0);
 ...
 * Check RTL support
 */
async function checkRTLSupport(locales) {
 ...
 */
export function createGitLabCIConfig() {
    return `stages:
  - i18n-checks
