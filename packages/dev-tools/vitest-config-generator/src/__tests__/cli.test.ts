...
describe("CLI Interface", () => {
  ...
  describe("basic functionality", () => {
    ...
    it("should run with custom coverage thresholds", () => {
      expect(() => {
        execSync(`node ${cliPath} --branches 90 --functions 95 --lines 95 --statements 95 --output /tmp/test-config-thresholds.ts`, { 
          encoding: "utf8",
          cwd: process.cwd()
        });
      }).not.toThrow();
    ...
    it("should run with include/exclude options", () => {
      expect(() => {
        execSync(`node ${cliPath} --include-examples false --include-templates false --include-scripts true --output /tmp/test-config-filter.ts`, { 
          encoding: "utf8",
          cwd: process.cwd()
        });
      }).not.toThrow();
