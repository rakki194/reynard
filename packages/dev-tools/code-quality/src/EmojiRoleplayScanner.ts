...
export class EmojiRoleplayScanner {
  ...
  getExclusionInfo(): {
    ...
    const excludedFiles = [
      ...
      {
        fileName: "EmojiRoleplayScanner.test.ts",
        reason: "Test file - intentionally contains emojis and roleplay patterns as test data",
      },
      {
        fileName: "junk-detection-command.ts",
        reason: "CLI command - uses emojis for user interface feedback and better UX",
      },
      {
        fileName: "docstring-command.ts",
        reason: "CLI command - uses emojis for user interface feedback and better UX",
      },
      {
        fileName: "JunkFileDetector.ts",
        reason: "Core detector - uses emojis for status reporting and visual indicators",
      },
    ];
  ...
  private readonly roleplayPatterns = [
    ...
    {
      pattern:
        /(whiskers|snarls|splashes|prowls|gleams|twitch|eyes|claws|fur|paws|tail|ears|growls|howls|purrs|chirps)\s+(.*?)\s+with\s+(determination|cunning|enthusiasm|focus|intelligence|satisfaction|anticipation|joy|menace|authority|glee|satisfaction|pride|precision|dominance|savage|predatory)/gi,
    ...
    {
      pattern:
        /_\s*(whiskers|snarls|splashes|prowls|gleams|twitch|eyes|claws|fur|paws|tail|ears|growls|howls|purrs|chirps)\s+(.*?)\s+with\s+(.*?)_/gi,
    ...
    {
      pattern:
        /\*\s*(whiskers|snarls|splashes|prowls|gleams|twitch|eyes|claws|fur|paws|tail|ears|growls|howls|purrs|chirps)\s+(.*?)\s+with\s+(.*?)\*/gi,
      type: "ROLEPLAY_ACTION" as const,
      severity: "MAJOR" as const,
      suggestion: "Remove roleplay action and use professional language",
    },
    ...
    {
      pattern:
        /(red fur gleams|whiskers twitch|snarls with|splashes with|prowls through|alpha wolf|pack hunting|fox's den|otter's stream)/gi,
      type: "ROLEPLAY_PATTERN" as const,
      severity: "MAJOR" as const,
      suggestion: "Replace with professional technical language",
    },
