...
export function getCategoryDescription(category: DetectionCategory): string {
  const descriptions = {
    ...
    [DetectionCategory.EXCLUSIVITY_CLAIMS]: "Claims of exclusivity or uniqueness",
    [DetectionCategory.OTHER]: "Other types of boastful language",
  };
...
export function getCategorySuggestions(category: DetectionCategory): string[] {
  const suggestions = {
    ...
    [DetectionCategory.EXCLUSIVITY_CLAIMS]: [
      "Avoid claims of being the only or unique solution",
      "Acknowledge other approaches and alternatives",
      "Use more inclusive and collaborative language",
    ],
    [DetectionCategory.OTHER]: [
      "Review language for boastful or excessive claims",
      "Use more modest and factual language",
...
export function generateImprovementRecommendations(
  ...
  for (const [category, count] of sortedCategories) {
    ...
    recommendations.push(`Focus on ${category} (${count} findings): ${suggestions[0]}`);
  }
