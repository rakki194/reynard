...
import { HumilityReportSchema, HumilityAnalysisOptionsSchema } from "./schemas";
...
export class HumilityParser {
  ...
  parseFromString(jsonString: string, sourceFile?: string): ParsedHumilityReport {
    try {
      ...
      const parsedReport: ParsedHumilityReport = {
        ...
        profiles: validatedData.profiles as any,
        metadata: {
  ...
  filterReport(report: ParsedHumilityReport, options: HumilityAnalysisOptions = {}): FilteredHumilityReport {
    ...
    for (const [filePath, profile] of Object.entries(report.profiles)) {
      ...
      if (validatedOptions.filePatterns && validatedOptions.filePatterns.length > 0) {
        const matchesPattern = validatedOptions.filePatterns.some(pattern => filePath.includes(pattern));
  ...
  getSummary(report: ParsedHumilityReport | FilteredHumilityReport) {
    ...
    if (isFiltered) {
      const filteredReport = report as FilteredHumilityReport;
      return {
        totalFiles: filteredReport.filtered_summary.filtered_files,
        totalFindings: filteredReport.filtered_summary.filtered_findings,
        averageScore: filteredReport.filtered_summary.average_score,
        isFiltered: true,
        originalFiles: filteredReport.summary.total_files,
        originalFindings: filteredReport.summary.total_findings,
      };
    } else {
      return {
        totalFiles: report.summary.total_files,
        totalFindings: report.summary.total_findings,
        averageScore: report.summary.average_score,
        isFiltered: false,
      };
    }
  }
  ...
  private exportToCSV(report: ParsedHumilityReport | FilteredHumilityReport): string {
    ...
    for (const [filePath, profile] of Object.entries(report.profiles)) {
      if (profile.findings.length === 0) {
        rows.push([filePath, profile.overall_score, "0", "", "", "", "", "", ""].map(field => `"${field}"`).join(","));
      } else {
        for (const finding of profile.findings) {
          rows.push(
            [
