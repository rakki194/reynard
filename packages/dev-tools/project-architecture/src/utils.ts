...
/**
 * Get directory health status
 */
export function getDirectoryHealth(directoryName: string): "healthy" | "warning" | "error" {
  ...
  if (!directory.optional && !directoryExists(directory.path)) {
    return "error";
  }

  // Check metadata health if available
  if (directory.metadata?.health) {
    const health = directory.metadata.health;
    if (health === "unknown") {
      return "warning";
    }
    return health;
...
export function updateDirectoryMetadata(directoryName: string, updates: Partial<DirectoryMetadata>): boolean {
  ...
  Object.assign(directory.metadata, updates);
  return true;
}

/**
 * Get directories by health status
 */
export function getDirectoriesByHealth(health: "healthy" | "warning" | "error"): DirectoryDefinition[] {
  return REYNARD_ARCHITECTURE.directories.filter(dir => {
    if (dir.metadata?.health) {
      return dir.metadata.health === health;
    }
    // Default health check
    return health === "healthy" && (!dir.optional || directoryExists(dir.path));
  });
}

/**
 * Get build configuration for a directory
 */
export function getBuildConfiguration(directoryName: string): BuildConfiguration | null {
  const directory = getDirectoryDefinition(directoryName);
  return directory?.buildConfig || null;
}

/**
 * Get test configuration for a directory
 */
export function getTestConfiguration(directoryName: string): TestConfiguration | null {
  const directory = getDirectoryDefinition(directoryName);
  return directory?.testConfig || null;
}

/**
 * Get lint configuration for a directory
 */
export function getLintConfiguration(directoryName: string): LintConfiguration | null {
  const directory = getDirectoryDefinition(directoryName);
  return directory?.lintConfig || null;
}
