...
class Enemy implements Component {
    readonly __component = true;
    constructor(public type: string, public difficulty: number = 1) { }
}

class Bullet implements Component {
    readonly __component = true;
    constructor(public speed: number, public lifetime: number = 5.0) { }
}
...
class Physics implements Component {
    readonly __component = true;
    constructor(public mass: number, public friction: number, public restitution: number) { }
}

class Renderable implements Component {
    readonly __component = true;
    constructor(public mesh: string, public texture: string) { }
}
...
export class ECSBenchmarkRunner {
    ...
    async benchmarkComponentRemoval(): Promise<BenchmarkResult[]> {
        ...
        for (const entityCount of this.config.entityCounts) {
            ...
            const entities: Entity[] = [];
            for (let i = 0; i < entityCount; i++) {
                entities.push(this.world.spawn(
                    new Position(0, 0),
                    new Velocity(0, 0),
                    new Health(100, 100),
                ));
