 ...
 */
async function testMapSizeLimitFix(): Promise<void> {
    ...
    try {
        ...
        for (const entityCount of TEST_CONFIG.entityCounts) {
            ...
            try {
                ...
                if (entityCountActual >= entityCount * 0.95) { // Allow 5% tolerance
 ...
 */
async function testMemoryTracking(): Promise<void> {
 ...
 */
async function testScalingOptimizations(): Promise<void> {
    ...
    try {
        ...
        const testEntities = Array.from({ length: 5000 }, (_, i) => ({ index: i, generation: 1 }));
