...
async function runPerformanceComparison() {
    ...
    try {
        ...
        console.log(`   Query Performance (100k): ${yourECSMetrics.queryPerformance.timePerQueryUs.toFixed(2)}μs per query`);
        console.log("");
        ...
        const bestComparison = comparisons.reduce((best, current) => current.comparison.overallScore > best.comparison.overallScore
            ? current
            : best);
        const worstComparison = comparisons.reduce((worst, current) => current.comparison.overallScore < worst.comparison.overallScore
            ? current
            : worst);
        console.log("\n🎯 Performance Analysis:");
        console.log("=".repeat(70));
        console.log(`🏆 Best Match: ${bestComparison.benchmark.name} (${bestComparison.comparison.overallScore.toFixed(1)}/100)`);
        console.log(`📉 Biggest Gap: ${worstComparison.benchmark.name} (${worstComparison.comparison.overallScore.toFixed(1)}/100)`);
        // Calculate average performance
        const avgEntityRatio = comparisons.reduce((sum, c) => sum + c.comparison.entityCreationRatio, 0) / comparisons.length;
        const avgQueryRatio = comparisons.reduce((sum, c) => sum + c.comparison.queryPerformanceRatio, 0) / comparisons.length;
        const avgScore = comparisons.reduce((sum, c) => sum + c.comparison.overallScore, 0) /
            comparisons.length;
        console.log(`\n📊 Average Performance:`);
        ...
        if (jsBenchmarks.length > 0) {
            const jsAvgScore = jsBenchmarks.reduce((sum, c) => sum + c.comparison.overallScore, 0) /
                jsBenchmarks.length;
