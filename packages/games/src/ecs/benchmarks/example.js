 ...
 * @since 1.0.0
 */
import { ECSBenchmarkRunner, runECSBenchmarks, runEntityBenchmarks, runQueryBenchmarks, runStressTests, runSystemBenchmarks, } from "./ecs-benchmark.js";
/**
 * Example 1: Basic benchmark execution with default settings.
 */
export async function basicBenchmarkExample() {
 ...
 */
export async function categoryBenchmarkExample() {
    ...
    try {
        ...
        const avgTime = allResults.reduce((sum, r) => sum + r.averageTimeUs, 0) /
            allResults.length;
 ...
 */
export async function regressionTestExample() {
    console.log("\n🔥> Example 4: Performance Regression Testing");
    console.log("=".repeat(50));
    ...
    try {
        ...
        const comparisonData = {
            avgTimeUs: comparisonResults.reduce((sum, r) => sum + r.averageTimeUs, 0) /
        ...
        console.log(`  Time Change: ${timeDiff > 0 ? "+" : ""}${timeDiff.toFixed(2)}μs (${timeDiffPercent > 0 ? "+" : ""}${timeDiffPercent.toFixed(1)}%)`);
        console.log(`  Memory Change: ${memoryDiff > 0 ? "+" : ""}${memoryDiff.toFixed(2)} MB (${memoryDiffPercent > 0 ? "+" : ""}${memoryDiffPercent.toFixed(1)}%)`);
 ...
 */
export async function stressTestExample() {
    ...
    try {
        ...
        for (const result of stressResults) {
            ...
            console.log(`  ${result.entityCount.toLocaleString().padStart(8)} entities: ${result.averageTimeUs.toFixed(2).padStart(8)}μs, ${opsPerSecond.toFixed(0).padStart(8)} ops/sec, ${memoryMB.toFixed(2).padStart(6)} MB`);
        }
...
export async function analysisExample() {
    ...
    try {
        ...
        for (const [entityCount, entityResults] of byEntityCount) {
            const avgTime = entityResults.reduce((sum, r) => sum + r.averageTimeUs, 0) /
                entityResults.length;
            const avgMemory = entityResults.reduce((sum, r) => sum + (r.memoryUsageMB || 0), 0) /
                entityResults.length;
            console.log(`  ${entityCount.toLocaleString().padStart(6)} entities: ${avgTime.toFixed(2).padStart(8)}μs avg, ${avgMemory.toFixed(2).padStart(6)} MB avg`);
        }
