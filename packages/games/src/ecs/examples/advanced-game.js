...
import { CommonHooks, ComponentHookRegistry, ConditionCombinators, Conditions, createComponentHooks, createWorld, schedule, StorageType, system, systemSet, TASK_POOL, } from "../index";
...
export class AdvancedECSGame {
    constructor() {
        ...
        Object.defineProperty(this, "taskPool", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    setupComponentHooks() {
        ...
        this.hookRegistry.registerHooks(this.playerType, createComponentHooks({
            onAdd: CommonHooks.logOnAdd(this.playerType),
            onRemove: CommonHooks.logOnRemove(this.playerType),
        }));
        this.hookRegistry.registerHooks(this.enemyType, createComponentHooks({
            onAdd: CommonHooks.logOnAdd(this.enemyType),
            onRemove: CommonHooks.logOnRemove(this.enemyType),
        }));
        // Add validation hooks
        this.hookRegistry.registerHooks(this.healthType, createComponentHooks({
    ...
    setupSystems() {
        ...
        this.world.addSystem(system("playerInput", this.playerInputSystem.bind(this))
            .runIf(Conditions.resourceExists(this.inputStateType))
            .build());
        this.world.addSystem(system("shooting", this.shootingSystem.bind(this))
            .runIf(ConditionCombinators.and(Conditions.resourceExists(this.inputStateType), Conditions.anyEntityWith(this.playerType, this.positionType)))
            .build());
        this.world.addSystem(system("movement", this.movementSystem.bind(this))
            .runIf(Conditions.resourceExists(this.gameTimeType))
            .build());
        this.world.addSystem(system("enemyAI", this.enemyAISystem.bind(this))
            .runIf(ConditionCombinators.and(Conditions.anyEntityWith(this.enemyType), Conditions.timePassed(0.5)))
            .build());
        this.world.addSystem(system("collision", this.collisionSystem.bind(this))
            .runIf(Conditions.anyEntityWith(this.playerType, this.enemyType))
            .build());
        this.world.addSystem(system("rendering", this.renderingSystem.bind(this))
            .runIf(Conditions.everyNFrames(1)) // Run every frame
            .build());
        this.world.addSystem(system("gameState", this.gameStateSystem.bind(this))
            .runIf(Conditions.resourceExists(this.gameStateType))
            .build());
        // Create main schedule
        const mainSchedule = schedule("main");
        mainSchedule.addSystem(system("playerInput", this.playerInputSystem.bind(this))
            .runIf(Conditions.resourceExists(this.inputStateType))
            .build());
        mainSchedule.addSystem(system("shooting", this.shootingSystem.bind(this))
            .runIf(ConditionCombinators.and(Conditions.resourceExists(this.inputStateType), Conditions.anyEntityWith(this.playerType, this.positionType)))
            .build());
        mainSchedule.addSystem(system("movement", this.movementSystem.bind(this))
            .runIf(Conditions.resourceExists(this.gameTimeType))
            .build());
        mainSchedule.addSystem(system("enemyAI", this.enemyAISystem.bind(this))
            .runIf(ConditionCombinators.and(Conditions.anyEntityWith(this.enemyType), Conditions.timePassed(0.5)))
            .build());
        mainSchedule.addSystem(system("collision", this.collisionSystem.bind(this))
            .runIf(Conditions.anyEntityWith(this.playerType, this.enemyType))
            .build());
        mainSchedule.addSystem(system("rendering", this.renderingSystem.bind(this))
            .runIf(Conditions.everyNFrames(1))
            .build());
        mainSchedule.addSystem(system("gameState", this.gameStateSystem.bind(this))
            .runIf(Conditions.resourceExists(this.gameStateType))
            .build());
    }
    ...
    shootingSystem(world) {
        ...
        query.forEach((entity, player, position) => {
            if (inputState.keys.has("Space")) {
                // Create bullet
                const bullet = world.spawn(new Position(position.x, position.y), new Velocity(0, -bulletSpeed), new Bullet(bulletSpeed), new Color(1, 1, 0), // Yellow
                new Size(5, 10));
            }
        });
    }
 ...
 */
export function createAdvancedECSGame() {
