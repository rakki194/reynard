...
export class ComponentStorage {
  ...
  private sparseSetStorages: Map<number, SparseSetStorage<Component>> =
    new Map();
  ...
  getStorage<T extends Component>(
    ...
    if (storageType === StorageType.Table) {
      ...
      return this.sparseSetStorages.get(
        componentType.id,
      )! as SparseSetStorage<T>;
    }
  }

  getTableStorage<T extends Component>(
    componentType: ComponentType<T>,
  ): TableStorage<T> | undefined {
    return this.tableStorages.get(componentType.id) as
      | TableStorage<T>
      | undefined;
  }

  getSparseSetStorage<T extends Component>(
    componentType: ComponentType<T>,
  ): SparseSetStorage<T> | undefined {
    return this.sparseSetStorages.get(componentType.id) as
      | SparseSetStorage<T>
      | undefined;
  }
