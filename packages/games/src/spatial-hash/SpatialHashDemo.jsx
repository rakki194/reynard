...
export function SpatialHashDemo(props = {}) {
    ...
    return (<div class="spatial-hash-demo">
      ...
      <div class="spatial-canvas">
        <svg width={CANVAS_WIDTH} height={CANVAS_HEIGHT} class="spatial-svg" onClick={handleCanvasClick}>
          {/* Grid lines */}
          {Array.from({ length: Math.ceil(CANVAS_WIDTH / 50) }, (_, i) => (<line x1={i * 50} y1={0} x2={i * 50} y2={CANVAS_HEIGHT} stroke="#ddd" stroke-width="1"/>))}
          {Array.from({ length: Math.ceil(CANVAS_HEIGHT / 50) }, (_, i) => (<line x1={0} y1={i * 50} x2={CANVAS_WIDTH} y2={i * 50} stroke="#ddd" stroke-width="1"/>))}

          {/* Objects */}
          {objects().map(obj => (<rect x={obj.x} y={obj.y} width={obj.width} height={obj.height} fill={obj.color} stroke={queryResults().includes(obj.id) ? "red" : "#333"} stroke-width={queryResults().includes(obj.id) ? "3" : "1"} opacity={queryResults().includes(obj.id) ? "0.8" : "0.6"}/>))}

          {/* Query rectangle */}
          <rect x={queryRect().x} y={queryRect().y} width={queryRect().width} height={queryRect().height} fill="none" stroke="blue" stroke-width="2" stroke-dasharray="5,5"/>
        </svg>
      </div>
      ...
      {stats() && (<div class="demo-stats-detail">
