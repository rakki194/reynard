...
export function UnionFindGame(props: UnionFindGameProps = {}) {
  ...
  const initializeGame = () => {
    const newGrid = Array(GRID_SIZE).fill(null).map(() => 
      Array(GRID_SIZE).fill(0).map(() => COLORS[Math.floor(Math.random() * COLORS.length)])
    );
    setGrid(newGrid);
  ...
  return (
    <div class="union-find-game">
      ...
      <div class="game-grid">
        {grid().map((row, rowIndex) => 
          row.map((color, colIndex) => {
            ...
            const canSelect = !isEmpty && gameState() === 'playing' && 
      ...
      <div class="game-instructions">
        <p>🎯 <strong>Goal:</strong> Select connected cells of the same number to create combos of 3+ cells (orthogonal and diagonal connections allowed!)</p>
        <p>💡 <strong>Mechanics:</strong> Combos disappear, blocks fall with gravity!</p>
        <p>🔥 <strong>Scoring:</strong> Larger combos = more points! Chain combos for bonus multipliers!</p>
        <p>⌨️ <strong>Controls:</strong> Click cells to select, press <kbd>Escape</kbd> to cancel, <kbd>Enter</kbd> to clear combo!</p>
      </div>
