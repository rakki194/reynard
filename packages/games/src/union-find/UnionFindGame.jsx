...
export function UnionFindGame(props = {}) {
    ...
    const [gameState, setGameState] = createSignal("playing");
    ...
    const getCellId = (row, col) => row * GRID_SIZE + col;
    const findConnectedComponents = (selectedCells) => {
        const currentUf = uf();
        if (!currentUf)
            return [];
    ...
    const clearCombo = async (component) => {
        setIsAnimating(true);
    ...
    const canAddToSelection = (row, col, currentSelection) => {
        ...
        for (const selectedKey of Array.from(currentSelection)) {
            ...
            if (grid()[selRow][selCol] === currentColor &&
    ...
    const checkForValidMoves = () => {
        ...
        for (let row = 0; row < GRID_SIZE - 1; row++) {
            for (let col = 0; col < GRID_SIZE - 1; col++) {
                ...
                if (currentGrid[row][col + 1] === color &&
                ...
                if (currentGrid[row + 1][col] === color &&
                    currentGrid[row + 1][col + 1] === color) {
                    return true;
                }
    ...
    const clearSelectedCombo = async () => {
        ...
        if (currentSelection.size < TARGET_CONNECTIONS)
            return;
        const currentUf = uf();
        if (!currentUf)
            return;
    ...
    return (<div class="union-find-game">
      <div class="game-header">
        ...
        <Button onClick={clearSelectedCombo} variant="primary" size="sm" disabled={selectedCells().size < TARGET_CONNECTIONS}>
          Clear Combo ({selectedCells().size})
        </Button>
      </div>

      <div class="game-grid">
        {grid().map((row, rowIndex) => row.map((color, colIndex) => {
            ...
            const canSelect = !isEmpty &&
                gameState() === "playing" &&
                (selectedCells().size === 0 ||
                    canAddToSelection(rowIndex, colIndex, selectedCells()));
            return (<div class={`game-cell color-${color} ${isSelected ? "selected" : ""} ${isEmpty ? "empty" : ""} ${!canSelect && !isEmpty ? "disabled" : ""} ${selectedCells().size > 0 && !isSelected && !canSelect && !isEmpty ? "non-connectable" : ""} set-size-${setSize}`} onClick={() => canSelect && handleCellClick(rowIndex, colIndex)}>
      ...
      {gameState() === "game-over" && (<div class="game-over-message">
          <h3>🎮 Game Over!</h3>
      ...
      {stats() && (<div class="game-stats-detail">
