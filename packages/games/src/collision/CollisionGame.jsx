...
export function CollisionGame(props = {}) {
    ...
    return (<div class="collision-game">
      ...
      <div class="collision-canvas">
        <svg width={CANVAS_WIDTH} height={CANVAS_HEIGHT} class="balls-svg">
          {balls().map(ball => (<circle cx={ball.x} cy={ball.y} r={ball.radius} fill={ball.color} stroke="#333" stroke-width="2"/>))}
          {collisions().map(collision => {
            const ball1 = balls().find(b => b.id === collision.ball1);
            const ball2 = balls().find(b => b.id === collision.ball2);
            if (!ball1 || !ball2)
                return null;
            return (<line x1={ball1.x} y1={ball1.y} x2={ball2.x} y2={ball2.y} stroke="red" stroke-width="3" opacity="0.7"/>);
        })}
        </svg>
      </div>
