...
export function GeometryDemo() {
    ...
    const addShape = (type) => {
        ...
        switch (type) {
            ...
            case "line":
                ...
                break;
            case "rectangle":
                data = RectangleOps.create(Math.random() * 300 + 50, Math.random() * 200 + 50, Math.random() * 100 + 50, Math.random() * 100 + 50);
                break;
            case "circle":
                data = CircleOps.create(PointOps.create(Math.random() * 400 + 50, Math.random() * 300 + 50), Math.random() * 30 + 20);
                break;
            case "polygon":
                const points = Array.from({ length: 5 }, () => PointOps.create(Math.random() * 200 + 100, Math.random() * 150 + 100));
    ...
    return (<div class="geometry-demo">
        ...
        <svg width={500} height={400} class="geometry-svg">
          {shapes().map(shape => {
            switch (shape.type) {
                ...
                case "line":
                    return (<line x1={shape.data.start.x} y1={shape.data.start.y} x2={shape.data.end.x} y2={shape.data.end.y} stroke={shape.color} stroke-width="3"/>);
                case "rectangle":
                    return (<rect x={shape.data.x} y={shape.data.y} width={shape.data.width} height={shape.data.height} fill="none" stroke={shape.color} stroke-width="2"/>);
                case "circle":
                    return (<circle cx={shape.data.center.x} cy={shape.data.center.y} r={shape.data.radius} fill="none" stroke={shape.color} stroke-width="2"/>);
                case "polygon":
      ...
      <div class="geometry-operations">
        ...
        <div class="operation-results">
          {operations()
            .slice(-5)
            .map(op => (<div class="operation-result">
