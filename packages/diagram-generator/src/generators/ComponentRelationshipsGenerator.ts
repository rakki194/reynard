...
export class ComponentRelationshipsGenerator implements DiagramGenerator {
    ...
    for (const component of analysis.components) {
      ...
      for (const dep of component.dependencies) {
        if (dep.startsWith('reynard-') || dep.startsWith('./') || dep.startsWith('../')) {
          ...
          if (targetComponent) {
            const targetId = this.sanitizeId(`${this.getPackageName(targetComponent.filePath)}_${targetComponent.name}`);
      ...
      for (const component of components) {
        ...
        for (const rel of component.relationships) {
          const targetComponent = components.find(comp => 
            comp.name === rel.component || 
            comp.dependencies.includes(rel.component)
          );
  ...
  private groupComponentsByPackage(components: any[], packages: any[]): Record<string, any[]> {
    ...
    for (const pkg of packages) {
      grouped[pkg.name] = components.filter(comp => 
        comp.filePath.startsWith(pkg.path) || 
        comp.filePath.includes(pkg.name)
      );
    }
  ...
  private findComponentByDependency(components: any[], dependency: string): any | null {
    return components.find(comp => 
      comp.name === dependency ||
      comp.filePath.includes(dependency) ||
      comp.dependencies.includes(dependency)
    );
  }
  ...
  private calculateComplexity(analysis: CodebaseAnalysis): number {
    return analysis.components.length + 
           this.countComponentEdges(analysis);
  }
}
