...
export const retryStrategy = {
    ...
    canRecover: (_error, context) => context.category === ErrorCategory.NETWORK ||
 ...
 */
export const fallbackUIStrategy = {
    id: "fallback-ui",
 ...
 */
export function getApplicableStrategies(error, context, strategies = builtInRecoveryStrategies) {
    return strategies
 ...
 */
export async function executeRecoveryStrategy(strategy, error, context) {
    try {
        const result = await Promise.race([
            strategy.recover(error, context),
            new Promise((_, reject) => setTimeout(() => reject(new Error("Recovery timeout")), strategy.timeout || 10000)),
        ]);
        return result;
    }
    catch (recoveryError) {
 ...
 */
export function createRecoveryStrategy(id, name, description, canRecover, recover, priority = 5, timeout) {
    return {
        id,
        name,
        description,
