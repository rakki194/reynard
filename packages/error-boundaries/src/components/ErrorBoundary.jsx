...
export const ErrorBoundary = (props) => {
    ...
    const errorContext = createMemo(() => {
        ...
        if (!currentState.error || !currentState.errorInfo)
            return null;
    ...
    const handleError = (error, errorInfo) => {
        ...
        const recoveryActions = strategies.map((strategy) => ({
            id: strategy.id,
            name: strategy.name,
            description: strategy.description,
            action: strategy.id, // Type assertion for enum compatibility
    ...
    const executeRecovery = async (action) => {
        const currentState = state();
        if (!currentState.error || !errorContext())
            return;
        setState((prev) => ({
            ...prev,
            isRecovering: true,
    ...
    const reportError = async (report) => {
        if (!props.errorReporting?.endpoint)
            return;
        try {
    ...
    return (<Show when={state().error} fallback={props.children}>
      <ErrorFallback error={state().error} errorInfo={state().errorInfo} retry={retry} reset={reset} recoveryActions={state().recoveryActions} isRecovering={state().isRecovering} onRecovery={executeRecovery} fallback={props.fallback}/>
    </Show>);
};
// HOC for wrapping components with error boundary
export const withErrorBoundary = (Component, errorBoundaryProps) => {
    return (props) => (<ErrorBoundary {...errorBoundaryProps}>
      <Component {...props}/>
    </ErrorBoundary>);
};
