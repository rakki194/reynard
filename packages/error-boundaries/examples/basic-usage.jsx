...
const ErrorBoundaryDemo = () => {
    ...
    return (<div style={{ padding: "2rem", maxWidth: "800px", margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: "2rem" }}>
        <Button variant="primary" onClick={handleToggleError}>
          {shouldThrow() ? "Fix Component" : "Break Component"}
        </Button>
      </div>

      <ErrorBoundary fallback={ErrorFallback} recoveryStrategies={builtInRecoveryStrategies} onError={(error, errorInfo) => {
            console.log("Error caught:", error, errorInfo);
        }} onRecovery={action => {
            console.log("Recovery action executed:", action);
        }} reportErrors={true} errorReporting={{
            enabled: true,
            endpoint: "/api/errors",
            batchSize: 5,
            flushInterval: 30000,
