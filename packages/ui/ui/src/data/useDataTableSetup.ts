...
export const useDataTableSetup = <T = unknown>(props: DataTableProps<T>): DataTableSetup<T> => {
  const merged = mergeDataTableProps(props);
  const [local, others] = splitDataTableProps(merged);

  const handlers = useDataTableHandlers(local.data, local.onRowSelect, local.onSort);
