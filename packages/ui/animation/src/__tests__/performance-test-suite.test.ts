...
describe("Performance Test Suite", () => {
  ...
  describe("Performance Test Integration", () => {
    test("should integrate with animation system", async () => {
      ...
      const animationBenchmarks = result.benchmarks.filter(b => 
        b.name.includes('Animation') || b.name.includes('animation')
      );
      expect(animationBenchmarks.length).toBeGreaterThan(0);
      
      // Should test animation-specific bundle size
      const animationModules = Object.keys(result.bundleSize.moduleBreakdown).filter(key =>
