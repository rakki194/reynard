...
export class ImageMetadataExtractor extends BaseMetadataExtractor {
        ...
        if (mergedOptions.extractExif) {
            try {
                ...
                if (exif) {
                    ...
                    if (exif.XResolution)
                        metadata.width = exif.XResolution;
                    if (exif.YResolution)
                        metadata.height = exif.YResolution;
                    if (exif.ISOSpeedRatings)
                        metadata.bitDepth = exif.ISOSpeedRatings;
                    if (exif.ColorSpace)
                        metadata.colorSpace = exif.ColorSpace.toString();
                }
            }
            catch (error) {
     ...
     */
    async detectImageAnimation(file) {
