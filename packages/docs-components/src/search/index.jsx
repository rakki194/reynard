...
export const DocsSearch = (props) => {
    ...
    //   props.onClear?.();
    // };
    return (<div class={`docs-search ${props.className || ""} ${isFocused() ? "focused" : ""}`}>
      <div class="docs-search-input-wrapper">
        <TextField value={query()} onInput={(e) => handleSearch(e.currentTarget.value)} onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} placeholder={props.placeholder || "Search documentation..."} leftIcon="🔍" rightIcon={query() ? "✕" : undefined} class="docs-search-input"/>
      </div>
    </div>);
};
/**
 ...
 */
export const DocsSearchResults = (props) => {
    return (<div class={`docs-search-results ${props.className || ""}`}>
      <Show when={props.results.length > 0}>
        ...
        <ul class="docs-search-results-list">
          <For each={props.results}>
            {(result) => (<li class="docs-search-result">
                <button class="docs-search-result-button" onClick={() => props.onResultClick(result)}>
                      ...
                      <Show when={result.tags && result.tags.length > 0}>
                        <div class="docs-search-result-tags">
                          <For each={result.tags}>
                            {(tag) => (<span class="docs-search-result-tag">{tag}</span>)}
                          </For>
                        </div>
                      </Show>
                    </div>
                  ...
                  <Show when={result.score}>
                    <div class="docs-search-result-score">
 ...
 */
export const DocsSearchSuggestions = (props) => {
    return (<div class={`docs-search-suggestions ${props.className || ""}`}>
      <Show when={props.suggestions.length > 0}>
        <h4>Popular searches</h4>
        <ul class="docs-search-suggestions-list">
          <For each={props.suggestions}>
            {(suggestion) => (<li class="docs-search-suggestion">
 ...
 */
export const DocsAdvancedSearch = (props) => {
    const [query, setQuery] = createSignal("");
    ...
    return (<div class={`docs-advanced-search ${props.className || ""}`}>
      <div class="docs-advanced-search-form">
        <div class="docs-advanced-search-field">
          <TextField value={query()} onInput={(e) => setQuery(e.currentTarget.value)} placeholder="Search query..." label="Search"/>
        </div>

        <Show when={props.categories && props.categories.length > 0}>
          <div class="docs-advanced-search-field">
            <label class="docs-advanced-search-label">Category</label>
            <select value={category()} onChange={(e) => setCategory(e.currentTarget.value)} class="docs-advanced-search-select">
        ...
        <Show when={props.tags && props.tags.length > 0}>
          <div class="docs-advanced-search-field">
            ...
            <div class="docs-advanced-search-tags">
              <For each={props.tags}>
                {(tag) => (<button class={`docs-advanced-search-tag ${selectedTags().includes(tag) ? "selected" : ""}`} onClick={() => toggleTag(tag)}>
                    {tag}
                  </button>)}
              </For>
            </div>
        ...
        <div class="docs-advanced-search-field">
          ...
          <div class="docs-advanced-search-dates">
            <input type="date" value={dateStart()} onChange={(e) => setDateStart(e.currentTarget.value)} class="docs-advanced-search-date" placeholder="Start date"/>
            <span class="docs-advanced-search-date-separator">to</span>
            <input type="date" value={dateEnd()} onChange={(e) => setDateEnd(e.currentTarget.value)} class="docs-advanced-search-date" placeholder="End date"/>
          </div>
        </div>
 ...
 */
export const DocsSearchModal = (props) => {
    const [query, setQuery] = createSignal("");
    createEffect(() => {
        if (props.isOpen) {
            // Focus search input when modal opens
            const input = document.querySelector(".docs-search-modal-input input");
            input?.focus();
        }
    });
    const handleSearch = (value) => {
