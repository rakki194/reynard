      ...
      <div class="docs-main">
        <Show when={props.sidebar}>
          <aside class={`docs-sidebar ${sidebarOpen() ? "docs-sidebar--open" : ""}`}>
            ...
            <button class="docs-sidebar-toggle" onClick={() => setSidebarOpen(!sidebarOpen())} aria-label="Toggle sidebar">
 ...
 */
export const DocsFooter = props => {
    return (<footer class={`docs-footer ${props.className || ""}`}>
...
export const DocsPage = props => {
    return (<article class={`docs-page ${props.className || ""}`}>
...
export const DocsSection = props => {
    return (<section class={`docs-section ${props.className || ""}`}>
      <Show when={props.title || props.description}>
 ...
 */
export const DocsGrid = props => {
    const style = () => ({
        "grid-template-columns": `repeat(${props.columns || 1}, 1fr)`,
        gap: props.gap || "1rem",
    });
    return (<div class={`docs-grid ${props.className || ""}`} style={style()}>
      {props.children}
    </div>);
};
/**
...
export const DocsCardGrid = props => {
    return (<DocsGrid columns={props.columns || 3} className={props.className}>
      <For each={props.items}>
        {item => (<Card class="docs-feature-card" interactive={!!item.href} onClick={() => item.href && window.open(item.href, "_blank")}>
 ...
 */
export const DocsHero = props => {
    return (<section class={`docs-hero ${props.className || ""}`}>
