...
import { calculatePasswordStrength as basicStrength } from "../utils";
 ...
 */
function analyzeWithZxcvbn(password, userInputs = [], customDictionary = []) {
  try {
 ...
 */
function getFeedbackMessage(score, warning) {
  if (warning) {
    return warning;
  }
  const messages = [
