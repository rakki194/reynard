...
export function useEmbeddingDataLoader(): EmbeddingDataLoaderState &
  ...
  const [embeddingData, setEmbeddingData] = createSignal<unknown>(null);
  const [pcaData, setPcaData] = createSignal<unknown>(null);
  const [qualityData, setQualityData] = createSignal<unknown>(null);
