...
export const LineChart = props => {
    ...
    const getSizeClass = () => {
        ...
        if (area <= 60000)
            return "size-small"; // 300x200
        if (area <= 120000)
            return "size-medium"; // 400x300
        if (area <= 240000)
            return "size-large"; // 600x400
        return "size-xl"; // 800x500+
    };
    return (<div class={`${getContainerClasses()} ${getSizeClass()}`} role="img" aria-label={local.title || "line chart"} {...others}>
      ...
      <div class="reynard-chart-wrapper">
        ...
        <Show when={!local.loading && chartData()}>
          <div class="reynard-chart-container" style={{ position: "relative", width: "100%", height: "100%" }}>
            <canvas ref={canvasRef} width={local.width} height={local.height} style={{ "max-width": "100%", "max-height": "100%" }} data-testid="line-chart-canvas"/>
          </div>
        </Show>
      </div>
    </div>);
