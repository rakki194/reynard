...
export const EmbeddingDistributionChart = props => {
    ...
    const statisticsOverlay = () => {
        if (!props.showStatistics || !props.data.statistics)
            return null;
    ...
    return (<div class="embedding-distribution-chart">
      ...
      <Show when={!props.loading && props.data.values && props.data.values.length > 0}>
        <div class="reynard-chart-container" style={{ position: "relative", width: "100%", height: "100%" }}>
          <canvas width={props.width || 400} height={props.height || 300} style={{ "max-width": "100%", "max-height": "100%" }} data-testid="embedding-distribution-chart-canvas"/>
        </div>
        {statisticsOverlay()}
      </Show>
    </div>);
