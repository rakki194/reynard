...
export const EmbeddingQualityChart = (props) => {
    ...
    const renderQualityAssessment = () => {
        if (!local.showAssessment || !local.data.assessment)
            return null;
        ...
        return (<div class="embedding-quality-assessment">
    ...
    const renderQualityMetrics = () => {
        if (!local.data || !local.data.metrics)
            return null;
        return (<div class="quality-metrics-details">
        <div class="metrics-header">
          <h4>Quality Metrics</h4>
        </div>

        <div class="metrics-grid">
          {local.data.metrics.map((metric) => (<div class="metric-item">
    ...
    return (<div class={`reynard-embedding-quality-chart ${local.class || ""}`}>
      ...
      <Show when={!local.loading && chartData()}>
        <div class="quality-chart-container">
          <Chart type={local.type === "quality-gauge" ? "doughnut" : "bar"} labels={chartData().labels} datasets={chartData().datasets} width={others.width} height={others.height} title={others.title} xAxisLabel={others.xAxisLabel || t("qualityMetrics")} yAxisLabel={others.yAxisLabel || t("score")} showGrid={others.showGrid} showLegend={others.showLegend} useOKLCH={true} colorTheme={local.theme}/>
