...
export const PieChart = props => {
    ...
    return (<div class={getContainerClasses()} data-width={!local.responsive ? local.width : undefined} data-height={!local.responsive ? local.height : undefined} role="img" aria-label={local.title || `${local.variant} chart`} {...others}>
      ...
      <Show when={!local.loading && chartData()}>
        <div class="reynard-chart-container" style={{ position: "relative", width: "100%", height: "100%" }}>
          <canvas ref={canvasRef} width={local.width} height={local.height} style={{ "max-width": "100%", "max-height": "100%" }} data-testid={`${local.variant}-chart-canvas`}/>
        </div>
      </Show>
    </div>);
};
