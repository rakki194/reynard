...
export const StatisticalChart = props => {
    ...
    return (<div class={`reynard-statistical-chart ${local.class || ""}`}>
      ...
      <Show when={!local.loading && chartData()}>
        <Chart type={local.type === "quality-gauge" ? "doughnut" : "bar"} labels={chartData().labels} datasets={chartData().datasets} width={others.width} height={others.height} title={others.title} xAxisLabel={others.xAxisLabel || (local.type === "histogram" ? t("valueRange") : t("metric"))} yAxisLabel={others.yAxisLabel || (local.type === "histogram" ? t("frequency") : t("value"))} showGrid={others.showGrid} showLegend={others.showLegend} useOKLCH={true} colorTheme={local.theme}/>
