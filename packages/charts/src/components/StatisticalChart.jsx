...
export const StatisticalChart = (props) => {
    ...
    const getMetricColor = (metric) => {
        if (metric.color)
            return metric.color;
        if (metric.goodThreshold !== undefined &&
    ...
    const renderStatisticsOverlay = () => {
        if (!local.showStatistics || !("statistics" in local.data))
            return null;
        const stats = local.data.statistics;
        if (!stats)
            return null;
        return (<div class="statistics-overlay">
        <div class="stat-item">
    ...
    const renderQualityAssessment = () => {
        if (!local.showAssessment || !("assessment" in local.data))
            return null;
        ...
        return (<div class="quality-assessment">
        ...
        <Show when={assessment.issues.length > 0}>
          <div class="issues-section">
            <h5>Issues Found:</h5>
            <ul>
              {assessment.issues.map((issue) => (<li>{issue}</li>))}
            </ul>
          </div>
        </Show>
        ...
        <Show when={assessment.recommendations.length > 0}>
          <div class="recommendations-section">
            ...
            <ul>
              {assessment.recommendations.map((rec) => (<li>{rec}</li>))}
            </ul>
          </div>
        </Show>
      </div>);
    };
    return (<div class={`reynard-statistical-chart ${local.class || ""}`}>
      ...
      <Show when={!local.loading && chartData()}>
        <Chart type={local.type === "quality-gauge" ? "doughnut" : "bar"} labels={chartData().labels} datasets={chartData().datasets} width={others.width} height={others.height} title={others.title} xAxisLabel={others.xAxisLabel ||
            (local.type === "histogram" ? t("valueRange") : t("metric"))} yAxisLabel={others.yAxisLabel ||
            (local.type === "histogram" ? t("frequency") : t("value"))} showGrid={others.showGrid} showLegend={others.showLegend} useOKLCH={true} colorTheme={local.theme}/>
