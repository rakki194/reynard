...
const getContainerClasses = (horizontal, stacked, customClass) => {
    ...
    if (horizontal)
        classes.push("reynard-bar-chart--horizontal");
    if (stacked)
        classes.push("reynard-bar-chart--stacked");
    if (customClass)
...
const BarChartContent = props => {
    ...
    const createChart = (canvas) => {
        if (!isRegistered() || !props.chartData)
            return;
    ...
    return (<>
      ...
      <Show when={!props.loading && props.chartData}>
        <div class="reynard-chart-container" style={{ position: "relative", width: "100%", height: "100%" }}>
          <canvas ref={canvasRef} width={props.width} height={props.height} style={{ "max-width": "100%", "max-height": "100%" }} data-testid="bar-chart-canvas"/>
        </div>
      </Show>
    </>);
};
export const BarChart = props => {
    ...
    return (<div class={getContainerClasses(local.horizontal, local.stacked, local.class)} classList={{
            "reynard-bar-chart--responsive": local.responsive,
            "reynard-bar-chart--fixed-width": !local.responsive,
        }} data-width={local.width} data-height={local.height} role="img" aria-label={local.title || "bar chart"} {...others}>
      <Show when={local.title}>
        <div class="reynard-chart-title">{local.title}</div>
      </Show>
      <BarChartContent chartData={chartData()} loading={local.loading || false} emptyMessage={local.emptyMessage} width={local.width} height={local.height} options={chartOptions()}/>
    </div>);
};
