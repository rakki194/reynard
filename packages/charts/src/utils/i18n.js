 ...
 */
export function getChartTypeName(chartType) {
    return t(`chartTypes.${chartType}`, { fallback: chartType });
}
/**
 * Get axis label with fallback
 */
export function getAxisLabel(axis, customLabel) {
    if (customLabel) {
        return customLabel;
    }
    return t(`${axis}Axis`);
 ...
 */
export function getLoadingMessage(chartType) {
    switch (chartType) {
        case "statistical":
 ...
 */
export function getQualityText(quality) {
    return t(`quality.${quality}`);
}
/**
 * Get statistics label
 */
export function getStatisticsLabel(stat) {
