...
export function useTagColors() {
  ...
  const getTagColor = createMemo(
    () =>
      (tag: string, intensity: number = 1.0): string => {
        return generateTagColor(themeContext.theme, tag, intensity);
      },
  );

  const getTagStyle = createMemo(
    () =>
      (tag: string, intensity: number = 1.0) => {
        // Access theme to make this reactive to theme changes
        const currentTheme = themeContext.theme;

        console.log(
          `getTagStyle called for tag: ${tag}, theme: ${currentTheme}`,
        );
        ...
        const baseColor = tagColorGenerator.getTagColor(
          currentTheme,
          tag,
          intensity,
        );
...
export function useThemeColors() {
  ...
  const getSurfaceColor = (elevation: number = 0): string => {
    ...
    if (elevation === 1)
      return oklchColors.getColorVariant("surface", "lighter", 0.1);
    if (elevation === 2)
      return oklchColors.getColorVariant("surface", "lighter", 0.2);
  ...
  const getTextColor = (
    ...
    switch (variant) {
      case "primary":
        return oklchColors.getColor("text");
      case "secondary":
        return oklchColors.getColorVariant("text", "lighter", 0.3);
      case "tertiary":
        return oklchColors.getColorVariant("text", "lighter", 0.5);
      default:
        return oklchColors.getColor("text");
    }
  };
