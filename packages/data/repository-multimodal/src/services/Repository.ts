...
import { ModalityType, FileType, DatasetStatus, IngestionError } from "../types";
...
export class Repository extends BaseRepositoryService implements RepositoryService {
  ...
  protected async onInitialize(): Promise<void> {
    ...
    console.log("Initializing Repository with config:", this.repositoryConfig);
  ...
  async ingestFiles(request: IngestionRequest): Promise<IngestionResult> {
    ...
    for (const file of request.files) {
      try {
        ...
        results.errors.push(
          new IngestionError(error instanceof Error ? error.message : String(error), {
            file: file.path,
