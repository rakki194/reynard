 ...
 */
export function createOptimizedSpatialHash(objects, options = {}) {
 ...
 * Calculate optimal cell size based on object distribution
 */
export function calculateOptimalCellSize(objects) {
    if (objects.length === 0)
        return 100;
    const avgWidth = objects.reduce((sum, obj) => sum + (obj.width || 0), 0) / objects.length;
    const avgHeight = objects.reduce((sum, obj) => sum + (obj.height || 0), 0) / objects.length;
 ...
 */
export function estimateMemoryUsage(cellCount, objectCount, objectToCellCount) {
    let usage = 0;
