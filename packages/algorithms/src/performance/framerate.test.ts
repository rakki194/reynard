...
describe('FrameRateMonitor', () => {
  ...
  describe('monitorFrame', () => {
    it('should track frame times and increment frame count', () => {
      ...
      expect(monitor['frameTimes'][0]).toBeCloseTo(16.67, 2);
    });
    ...
    it('should detect dropped frames', () => {
      ...
      expect(monitor['droppedFrames']).toBe(2); // 50ms / 16.67ms - 1 = 2 dropped frames
    });
  ...
  describe('getMetrics', () => {
    ...
    it('should handle mixed frame times', () => {
      ...
      expect(metrics.fps).toBeCloseTo(30, 1); // Average of 22.22ms = ~45fps, but let's be more precise
