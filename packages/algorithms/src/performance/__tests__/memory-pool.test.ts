...
import { createSpatialObjectPool, createAABBPool, globalPoolManager } from '../memory-pool-utils';
...
describe('MemoryPool', () => {
  ...
  beforeEach(() => {
    pool = new MemoryPool(() => new TestObject(), { initialSize: 2, maxSize: 10 });
  });
  ...
  it('should respect max size', () => {
    const pool = new MemoryPool(() => new TestObject(), { initialSize: 0, maxSize: 2 });
