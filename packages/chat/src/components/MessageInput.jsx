...
export const MessageInput = props => {
    ...
    return (<div class={getClasses()}>
      <div class="reynard-message-input__container">
        {/* Main Input Area */}
        <div class="reynard-message-input__input-wrapper">
          <textarea ref={textareaRef} class="reynard-message-input__textarea" value={value()} placeholder={props.placeholder || "Type your message..."} disabled={props.disabled || props.isStreaming} rows={props.multiline ? 3 : 1} onInput={handleInput} onKeyDown={handleKeyDown} onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} aria-label="Message input" data-testid="message-input"/>

          {/* Character Counter */}
          <div class="reynard-message-input__meta">
            {props.showCounter && getCharacterInfo() && (<div class={`reynard-message-input__counter ${getCharacterInfo().isNearLimit ? "reynard-message-input__counter--warning" : ""} ${getCharacterInfo().isOverLimit ? "reynard-message-input__counter--error" : ""}`}>
        ...
        <div class="reynard-message-input__actions">
          {props.submitButton ? (<div class="reynard-message-input__custom-submit" onClick={handleSubmit}>
              {props.submitButton}
            </div>) : (<button type="button" class="reynard-message-input__submit" disabled={!value().trim() || props.disabled || props.isStreaming || getCharacterInfo()?.isOverLimit || false} onClick={handleSubmit} aria-label="Send message" title={props.isStreaming
                ? "Please wait..."
                : props.multiline
                    ? "Send message (Ctrl+Enter)"
                    : "Send message (Enter)"}>
              {props.isStreaming ? (<div class="reynard-message-input__spinner">
                  <div class="reynard-message-input__spinner-dot"></div>
                  <div class="reynard-message-input__spinner-dot"></div>
                  <div class="reynard-message-input__spinner-dot"></div>
                </div>) : (<svg class="reynard-message-input__send-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor">
      ...
      <div class="reynard-message-input__help">
        {props.multiline ? (<span class="reynard-message-input__shortcut">Press Enter for new line, Ctrl+Enter to send</span>) : (<span class="reynard-message-input__shortcut">Press Enter to send</span>)}
      </div>
    </div>);
};
