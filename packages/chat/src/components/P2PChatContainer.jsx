...
export const P2PChatContainer = props => {
    ...
    const filteredRooms = createMemo(() => {
        ...
        if (!query)
            return p2pChat.rooms();
        return p2pChat
            .rooms()
            .filter(room => room.name.toLowerCase().includes(query) || room.participants.some(p => p.name.toLowerCase().includes(query)));
    });
    ...
    return (<div class={`reynard-p2p-chat-container ${props.ui?.compact ? "reynard-p2p-chat-container--compact" : ""}`}>
      {/* Sidebar */}
      <Show when={sidebarOpen() && (props.ui?.showRoomList ?? true)}>
        <div class="reynard-p2p-chat-sidebar">
          ...
          <div class="reynard-p2p-chat-status">
            ...
            <button class="reynard-p2p-chat-status__toggle" onClick={() => setSidebarOpen(!sidebarOpen())} aria-label="Toggle sidebar">
              ←
            </button>
          </div>

          {/* Room List */}
          <RoomList rooms={filteredRooms()} activeRoom={p2pChat.activeRoom()} currentUser={props.currentUser} onRoomSelect={handleRoomSelect} onCreateRoom={() => {
            // TODO: Implement room creation modal
            console.log("Create room");
        }} searchQuery={searchQuery()} onSearch={setSearchQuery} compact={props.ui?.compact}/>
        </div>
      </Show>
      ...
      <div class="reynard-p2p-chat-main">
        ...
        <Show when={p2pChat.activeRoom()}>
          <div class="reynard-p2p-chat-header">
            <Show when={!sidebarOpen()}>
              <button class="reynard-p2p-chat-header__sidebar-toggle" onClick={() => setSidebarOpen(true)} aria-label="Show sidebar">
                ☰
              </button>
            </Show>
            ...
            <div class="reynard-p2p-chat-header__actions">
              <Show when={props.ui?.showUserList ?? true}>
                <button class="reynard-p2p-chat-header__user-toggle" onClick={() => setUserListOpen(!userListOpen())} aria-label="Toggle user list">
                  👥
                </button>
              </Show>
            </div>
        ...
        <div class="reynard-p2p-chat-content">
          <Show when={p2pChat.activeRoom()} fallback={<div class="reynard-p2p-chat-empty">
            ...
            <div ref={messagesContainerRef} class="reynard-p2p-chat-messages" onScroll={handleScroll}>
              <For each={p2pChat.messages()}>
                {message => (<P2PMessage message={message} currentUser={props.currentUser} showSender={true} showAvatar={!props.ui?.compact} showTimestamp={true} showReactions={props.config?.enableReactions} showReadReceipts={props.config?.enableReadReceipts} onMessageAction={(action, msg) => {
            ...
            <div class="reynard-p2p-chat-input">
              <MessageInput placeholder="Type a message..." disabled={p2pChat.p2pConnection().status !== "connected"} multiline={true} autoResize={true} maxLength={4000} onSubmit={handleMessageSubmit} onChange={handleInputChange} variant={props.ui?.compact ? "compact" : "default"}/>
            </div>
          </Show>
        </div>
      </div>
      ...
      <Show when={userListOpen() && p2pChat.activeRoom() && (props.ui?.showUserList ?? true)}>
        <div class="reynard-p2p-chat-userlist">
          <UserList users={currentRoomParticipants()} currentUser={props.currentUser} onUserSelect={user => {
            console.log("User selected:", user);
            // TODO: Handle user selection (profile, DM, etc.)
        }} showStatus={true} showActions={false} compact={props.ui?.compact}/>
        </div>
      </Show>
    </div>);
};
