...
export const MarkdownRenderer = props => {
    ...
    const processHTML = createMemo(() => {
        ...
        html = html.replace(/<pre><code class="language-(\w+)">/g, '<pre class="reynard-code-block"><code class="language-$1">');
        // Add math rendering classes
        if (props.enableMath) {
            html = html.replace(/<span class="math-inline">(.*?)<\/span>/g, '<span class="reynard-math reynard-math--inline">$1</span>');
        }
        // Process tables
        html = html
    ...
    return (<div ref={containerRef} class={`reynard-markdown-renderer ${props.streaming ? "reynard-markdown-renderer--streaming" : ""}`} onClick={handleLinkClick}>
