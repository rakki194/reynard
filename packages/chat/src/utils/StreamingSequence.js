...
import { createStreamingText, } from "./StreamingTextRenderer";
export function createStreamingSequence(sequences) {
    const [currentSequence, setCurrentSequence] = createSignal(0);
    const [isActive, setIsActive] = createSignal(false);
    const currentStream = createStreamingText(sequences[currentSequence()]?.text || "", sequences[currentSequence()]?.options || {});
    const start = () => {
        setIsActive(true);
        currentStream.start();
    };
