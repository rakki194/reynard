...
export function useChatStreaming(options = {}, messageHandlers) {
    const { fetchFn = fetch, authHeaders = {}, endpoint = "/api/chat" } = options;
    // Streaming state
    const [isStreaming, setIsStreaming] = createSignal(false);
    const [isThinking, setIsThinking] = createSignal(false);
    const [streamController, setStreamController] = createSignal(null);
    const [streamingParser, setStreamingParser] = createSignal(null);
    ...
    // Send a message
    const sendMessage = async (content, requestOptions = {}) => {
        if (isStreaming()) {
    ...
    const processStreamChunk = async (chunk, parser, messageId) => {
        switch (chunk.type) {
            case "start":
                batch(() => {
                    setIsStreaming(true);
    ...
    const handleToolCall = async (toolExecution, messageId) => {
        const toolCall = {
    ...
    const updateToolCallProgress = (toolExecution, messageId) => {
        const message = messageHandlers.messages().find((m) => m.id === messageId);
        if (!message?.toolCalls)
            return;
        const updatedToolCalls = message.toolCalls.map((tc) => tc.name === toolExecution.toolName
            ? {
                ...tc,
                progress: toolExecution.progress,
                message: toolExecution.message,
    ...
    // Handle tool result
    const handleToolResult = (toolExecution, messageId) => {
        const message = messageHandlers.messages().find((m) => m.id === messageId);
        if (!message?.toolCalls)
            return;
        const updatedToolCalls = message.toolCalls.map((tc) => tc.name === toolExecution.toolName
    ...
    // Finalize streaming
    const finalizeStreaming = async (messageId) => {
