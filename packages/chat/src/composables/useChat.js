...
export function useChat(options = {}) {
    const { endpoint = "/api/chat", authHeaders = {}, config: userConfig = {}, tools = [], initialMessages = [], autoConnect = true, fetchFn = fetch, reconnection = DEFAULT_RECONNECTION, } = options;
    // Create API client
    ...
    // Core state
    const [availableModels] = createSignal([]);
    const [selectedModel] = createSignal();
    ...
    const [connectionCheck] = createResource(() => endpoint, async (url) => {
        try {
