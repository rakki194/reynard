...
export function RAGSearch(props) {
    ...
    return (<div class={`rag-search ${props.className || ""}`}>
      {/* Advanced Search Controls */}
      <div class="rag-search-controls">
        <div class="search-input-group">
          <TextField placeholder="Search documents, images, code, or captions..." value={ragState.query()} onChange={ragState.setQuery} onKeyPress={e => {
            if (e.key === "Enter") {
                handleAdvancedSearch(ragState.query());
            }
        }} fullWidth/>

          <Button variant="primary" onClick={() => handleAdvancedSearch(ragState.query())} disabled={!ragState.query().trim()}>
            Search
          </Button>
        </div>

        <div class="search-options">
          <Select value={modality()} onChange={value => setModality(value)} options={[
          ...
          <Select value={topK()} onChange={value => setTopK(Number(value))} options={[
      ...
      <Tabs items={tabItems} activeTab={activeTab()} onTabChange={setActiveTab} variant="default" fullWidth>
        <RAGTabPanels activeTab={activeTab()} ragState={ragState} handlers={{
            ...handlers,
        ...
        <Show when={props.showSearchHistory && activeTab() === "history"}>
          <div class="rag-tab-panel">
            <RAGSearchHistory searchHistory={searchHistory()} onSearchAgain={handleSearchAgain} onClearHistory={handleClearHistory} onRemoveItem={handleRemoveHistoryItem} showFilters={true} maxItems={50}/>
          </div>
        </Show>
      </Tabs>

      {/* Modals */}
      <RAG3DVisualizationModal isOpen={threeDModalState().isOpen} onClose={handleClose3DModal} searchQuery={threeDModalState().searchQuery} searchResults={threeDModalState().searchResults} queryEmbedding={threeDModalState().queryEmbedding}/>

      <RAGFileModal isOpen={fileModalState().isOpen} onClose={handleCloseFileModal} filePath={fileModalState().filePath} fileName={fileModalState().fileName} fileContent={fileModalState().fileContent} chunkIndex={fileModalState().chunkIndex} chunkText={fileModalState().chunkText} modality={modality()}/>

      <RAGImageModal isOpen={imageModalState().isOpen} onClose={handleCloseImageModal} imagePath={imageModalState().imagePath} imageId={imageModalState().imageId} thumbnailPath={imageModalState().thumbnailPath} previewPath={imageModalState().previewPath} imageMetadata={imageModalState().imageMetadata} imageDimensions={imageModalState().imageDimensions} imageSize={imageModalState().imageSize} imageFormat={imageModalState().imageFormat} embeddingVector={imageModalState().embeddingVector} score={imageModalState().score}/>
    </div>);
}
