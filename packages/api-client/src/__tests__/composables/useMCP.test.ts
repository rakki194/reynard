...
describe('useMCP', () => {
  ...
  describe('listClients', () => {
    it('should list all MCP clients', async () => {
      ...
      expect(result).toEqual(mockResponse);
      expect(global.fetch).toHaveBeenCalledWith(
        'http://localhost:8000/mcp/clients'
      );
    });
  });
  ...
  describe('getClientInfo', () => {
    it('should get specific client information', async () => {
      ...
      expect(result).toEqual(mockResponse);
      expect(global.fetch).toHaveBeenCalledWith(
        `http://localhost:8000/mcp/client/${clientId}`
      );
    });
  });
  ...
  describe('validateToken', () => {
    it('should validate MCP token', async () => {
      ...
      expect(result).toEqual(mockResponse);
      expect(global.fetch).toHaveBeenCalledWith(
        'http://localhost:8000/mcp/validate'
      );
    });
  ...
  describe('error handling', () => {
    it('should handle HTTP errors', async () => {
      ...
      await expect(mcpService.generateToken({
        client_id: 'test-client',
