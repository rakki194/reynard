...
class AgentWorld(ECSWorld):
    ...
    def create_offspring(
        ...
        # Add components
        offspring.add_component(AgentComponent(offspring_name, agent1.spirit, agent1.style))
    ...
    def get_breeding_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_offspring_per_agent": total_offspring / len(agents) if agents else 0,
        }
    ...
    def _generate_agent_name(self, spirit: str, style: str) -> str:
        ...
        """
        # Simple name generation - in real implementation would use the naming system
        return f"{spirit.capitalize()}-{style.capitalize()}-{random.randint(1, 999)}"

    def _inherit_traits(self, traits1: TraitComponent, traits2: TraitComponent) -> Dict[str, Any]:
        """
        ...
        for trait in traits1.personality:
            if trait in traits2.personality:
                ...
                avg_value = (traits1.personality[trait] + traits2.personality[trait]) / 2
                mutation = random.uniform(-0.1, 0.1)
                inherited["personality"][trait] = max(0.0, min(1.0, avg_value + mutation))
        ...
        for ability in traits1.abilities:
            if ability in traits2.abilities:
                avg_value = (traits1.abilities[ability] + traits2.abilities[ability]) / 2
                mutation = random.uniform(-0.1, 0.1)
                inherited["abilities"][ability] = max(0.0, min(1.0, avg_value + mutation))
