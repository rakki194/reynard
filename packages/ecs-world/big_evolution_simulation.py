...
class EvolutionSimulation:
    ...
    def collect_generation_statistics(
        self, generation: int, agent_ids: List[str]
    ) -> Dict[str, Any]:
        ...
        for trait in self.trait_names:
            if trait_values[trait]:
                ...
                self.simulation_data["statistics"]["trait_evolution"][trait][
                    "mean_over_time"
                ].append(stats["trait_statistics"][trait]["mean"])
                self.simulation_data["statistics"]["trait_evolution"][trait][
                    "std_over_time"
                ].append(stats["trait_statistics"][trait]["std"])
                self.simulation_data["statistics"]["trait_evolution"][trait][
                    "min_over_time"
                ].append(stats["trait_statistics"][trait]["min"])
                self.simulation_data["statistics"]["trait_evolution"][trait][
                    "max_over_time"
                ].append(stats["trait_statistics"][trait]["max"])
        ...
        self.simulation_data["statistics"]["genetic_diversity"].append(
            stats["genetic_diversity"]
        )

        # Track population over time
        self.simulation_data["statistics"]["population_over_time"].append(
            len(agent_ids)
        )

        return stats

    def simulate_breeding_generation(
        self, current_agents: List[str], generation: int
    ) -> List[str]:
            ...
            if len(mature_agents) >= 2:
                ...
                if (
                    ...
                    if offspring:
                        ...
                        self.simulation_data["statistics"][
                            "compatibility_scores"
                        ].append(compatibility["compatibility"])

        print(
            f"   Created {len(new_agents)} offspring from {breeding_events} breeding events"
        )
        ...
        if len(all_agents) > self.population_size * 1.5:
            ...
            old_agents = [
                a for a in all_agents if a.startswith(f"gen{max(0, generation-3)}_")
            ]
            if old_agents:
    ...
    def run_simulation(self) -> Dict[str, Any]:
        ...
        for generation in range(1, self.generations + 1):
            ...
            # Simulate breeding
            current_agents = self.simulate_breeding_generation(
                current_agents, generation
            )
        ...
        print(
            f"   Total agents created: {self.simulation_data['metadata']['total_agents_created']}"
        )
        print(
            f"   Total breeding events: {len(self.simulation_data['statistics']['breeding_events'])}"
        )
    ...
    def create_visualizations(self):
        ...
        generations = list(
            range(len(self.simulation_data["statistics"]["population_over_time"]))
        )
        ...
        for i, trait in enumerate(self.trait_names):
            means = self.simulation_data["statistics"]["trait_evolution"][trait][
                "mean_over_time"
            ]
        ...
        for i, trait in enumerate(self.trait_names):
            stds = self.simulation_data["statistics"]["trait_evolution"][trait][
                "std_over_time"
            ]
            if stds and len(stds) == len(generations):
                ...
                ax4.plot(
                    generations, default_stds, color=colors[i], label=trait, linewidth=2
                )
        ...
        compatibility_scores = self.simulation_data["statistics"][
            "compatibility_scores"
        ]
        if compatibility_scores:
            ...
            ax5.set_title(
                "Compatibility Scores Distribution", fontsize=14, fontweight="bold"
            )
        ...
        if mutation_counts:
            ax6.hist(
                mutation_counts,
                bins=max(1, max(mutation_counts)),
                alpha=0.7,
                color="orange",
                edgecolor="black",
            )
        ...
        for gen in self.simulation_data["generations"]:
            current_counts = {
                spirit: 0 for spirit in self.simulation_data["metadata"]["spirits"]
            }
            for agent in gen["agents"]:
        ...
        if trait_matrix:
            ...
            ax8.set_title(
                "Trait Correlation Matrix (Final Gen)", fontsize=14, fontweight="bold"
            )
...
"""
        ...
        for trait in self.trait_names:
            initial_mean = self.simulation_data["statistics"]["trait_evolution"][trait][
                "mean_over_time"
            ][0]
            final_mean = self.simulation_data["statistics"]["trait_evolution"][trait][
                "mean_over_time"
            ][-1]
