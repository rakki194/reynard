...
> = (props) => {
  ...
  let thumbnailGenerator: AudioThumbnailGenerator | null = null;
  ...
  const handleWaveformClick = (event: MouseEvent) => {
    if (!interactionConfig().enableSeeking || !canvasRef || !duration()) return;
  ...
  const renderWaveform = () => {
    if (!canvasRef || !waveformData()) return;
  ...
  return (
    <div class={`audio-waveform-visualizer ${props.className || ""}`}>
