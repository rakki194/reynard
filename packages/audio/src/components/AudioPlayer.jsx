...
export const AudioPlayer = props => {
    ...
    const [shuffledIndices, setShuffledIndices] = createSignal([]);
    let audioRef;
    ...
    const currentTrack = getCurrentTrack();
    return (<div class={`audio-player ${props.className || ""}`}>
      {/* Audio element (hidden) */}
      <audio ref={audioRef} src={currentTrack ? (typeof currentTrack === "string" ? currentTrack : URL.createObjectURL(currentTrack)) : ""} loop={playerConfig.loopTrack} preload="metadata" onLoadedMetadata={handleLoadedMetadata} onTimeUpdate={handleTimeUpdate} onPlay={handlePlay} onPause={handlePause} onEnded={handleEnded} onError={handleError}/>

      <div class="player-content">
        {/* Waveform Visualizer */}
        <Show when={playerConfig.showWaveform && currentTrack}>
          <div class="waveform-section">
            <AudioWaveformVisualizer audioSrc={currentTrack} playbackConfig={{
            autoPlay: false,
        ...
        <div class="main-controls">
          ...
          <div class="control-buttons">
            <button type="button" class="control-button shuffle-button" classList={{ active: isShuffled() }} onClick={toggleShuffle} title="Shuffle (S)">
              🔀
            </button>

            <button type="button" class="control-button" onClick={previousTrack} disabled={currentTrackIndex() === 0 && !playerConfig.loopPlaylist} title="Previous (P)">
              ⏮️
            </button>

            <button type="button" class="control-button play-button" onClick={togglePlayback} disabled={!currentTrack} title="Play/Pause (Space)">
              <Show when={isPlaying()} fallback="▶️">
                ⏸️
              </Show>
            </button>

            <button type="button" class="control-button" onClick={nextTrack} disabled={currentTrackIndex() === playlist().length - 1 && !playerConfig.loopPlaylist} title="Next (N)">
              ⏭️
            </button>

            <div class="volume-control">
              <span class="volume-icon">🔊</span>
              <Slider min={0} max={1} step={0.1} value={volumeLevel()} onChange={e => setVolumeLevel(parseFloat(e.target.value))} class="volume-slider" title="Volume (↑/↓)"/>
            </div>
          </div>
        </div>
        ...
        <Show when={playerConfig.showPlaylist && playlist().length > 1}>
          <div class="playlist-section">
            ...
            return (<div class="playlist-item" classList={{
