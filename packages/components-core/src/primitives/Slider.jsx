...
export const Slider = props => {
    ...
    return (<div id={props.id} class={`slider ${props.class || ""} ${props.disabled ? "disabled" : ""}`} role="slider" aria-valuemin={`${min()}`} aria-valuemax={`${max()}`} aria-valuenow={`${value()}`} aria-disabled={props.disabled ? "true" : "false"} aria-label={props["aria-label"]} tabIndex={props.disabled ? -1 : 0} onKeyDown={onKeyDown} onMouseDown={onMouseDown} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
