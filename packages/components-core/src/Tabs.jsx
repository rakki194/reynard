...
export const Tabs = props => {
    ...
    return (<div class={getTabsClasses()}>
      ...
      {!local.children && local.items.length > 0 && (<div class="reynard-tabs__content">
          <For each={local.items}>{item => (<div class={`reynard-tab-panel ${item.id === local.activeTab ? "reynard-tab-panel--active" : "reynard-tab-panel--hidden"}`} role="tabpanel" aria-hidden={item.id !== local.activeTab}>
              {item.content}
            </div>)}</For>
        </div>)}
    </div>);
