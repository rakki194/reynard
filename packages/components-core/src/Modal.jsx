 ...
 */
import { splitProps, mergeProps, createEffect, Show } from "solid-js";
...
export const Modal = props => {
    ...
    // Handle escape key
    createEffect(() => {
        if (!local.open || !local.closeOnEscape)
            return;
        ...
        <div class="reynard-modal">
          <div class="reynard-modal__backdrop" onClick={handleBackdropClick}>
            <div class={getModalClasses()}>
              {(local.title || local.showCloseButton) && (<div class="reynard-modal__header">
                  {local.title && <h2 class="reynard-modal__title">{local.title}</h2>}

                  {local.showCloseButton && (<button type="button" class="reynard-modal__close" onClick={() => local.onClose?.()} aria-label="Close modal">
