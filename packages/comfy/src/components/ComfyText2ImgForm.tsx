...
export const ComfyText2ImgForm: Component<ComfyText2ImgFormProps> = (props) => {
  ...
  // Form state
  const [formData, setFormData] = createSignal<ComfyText2ImgParams>({
    caption: '',
    negative: '',
    width: 1024,
    height: 1024,
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input
            ...
            type="number"
            value={formData().seed || ''}
            onInput={(e) => handleInputChange('seed', e.currentTarget.value ? parseInt(e.currentTarget.value) : undefined)}
            min="0"
            disabled={props.disabled || isGenerating()}
          />
        </div>
