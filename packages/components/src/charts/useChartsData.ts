...
import { type RealTimeDataPoint, type StatisticalData, type QualityData } from "reynard-charts";

export const useChartsData = () => {
  // Real-time data state
  const [realTimeData, setRealTimeData] = createSignal<RealTimeDataPoint[]>([]);
  const [performanceData, setPerformanceData] = createSignal<RealTimeDataPoint[]>([]);
  ...
  const qualityData = createMemo((): QualityData => ({
    ...
    metrics: [
      ...
      { name: 'Load Time', value: 1.2, unit: 's', higherIsBetter: false, goodThreshold: 2, warningThreshold: 3 },
    ],
    assessment: {
      status: 'good',
      issues: ['SEO score could be improved', 'Load time is slightly slow'],
      recommendations: ['Optimize meta tags', 'Implement lazy loading', 'Compress images']
    }
  }));
  ...
  return {
    ...
    setAnimationId: (id: number | undefined) => { animationId = id; },
    realTimeInterval: () => realTimeInterval,
    setRealTimeInterval: (interval: NodeJS.Timeout | undefined) => { realTimeInterval = interval; },
    performanceInterval: () => performanceInterval,
    setPerformanceInterval: (interval: NodeJS.Timeout | undefined) => { performanceInterval = interval; },
    memoryInterval: () => memoryInterval,
    setMemoryInterval: (interval: NodeJS.Timeout | undefined) => { memoryInterval = interval; },
  };
};
