...
export class ClusterRenderer {
  ...
  createClusters(config: ClusterRendererConfig) {
    ...
    for (let i = 0; i < config.clusterCount; i++) {
      const cluster = this.createSingleCluster(config, i, this.colorPalette[i % this.colorPalette.length]);
      this.clusters.push(...cluster);
    }
  }

  private createSingleCluster(config: ClusterRendererConfig, index: number, color: string) {
  ...
  animate() {
    this.clusters.forEach((cluster, index) => {
      if (index % 2 === 0) { // Only rotate point clusters, not hulls
