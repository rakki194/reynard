...
export const MemoryTrackingPanel: Component<MemoryTrackingPanelProps> = (props) => {
  ...
  const [leakDetection, setLeakDetection] = createSignal<MemoryLeakDetection>({
  ...
  const [memoryHistory, setMemoryHistory] = createSignal<Array<{
    timestamp: number;
    usage: number;
    gcCount: number;
    gcTime: number;
  ...
  const updateMemoryStats = async () => {
    ...
    try {
      ...
      if (currentUsage > 500 * 1024 * 1024) { // 500MB
        memoryPressure = "critical";
      } else if (currentUsage > 200 * 1024 * 1024) { // 200MB
        memoryPressure = "high";
      } else if (currentUsage > 100 * 1024 * 1024) { // 100MB
  ...
  const performLeakDetection = (stats: MemoryStats, memoryUsages: number[]) => {
    ...
    if (leakRate > 1024 * 1024) { // 1MB per second
      leakSeverity = "critical";
      isLeakDetected = true;
    } else if (leakRate > 100 * 1024) { // 100KB per second
      leakSeverity = "high";
      isLeakDetected = true;
    } else if (leakRate > 10 * 1024) { // 10KB per second
      leakSeverity = "medium";
      isLeakDetected = true;
    } else if (leakRate > 1024) { // 1KB per second
      leakSeverity = "low";
      isLeakDetected = true;
    }
    ...
    const timeToCritical = isLeakDetected && leakRate > 0 
    ...
    const newLeakDetection: MemoryLeakDetection = {
      isLeakDetected,
      leakSeverity,
      leakRate,
      estimatedLeakSize,
