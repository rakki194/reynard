...
export const GoldenSpiralColors: Component = () => {
  ...
  const [dots, setDots] = createSignal<ColorDot[]>([]);
  ...
  const oklchToRgb = (oklch: OKLCHColor): string => {
    ...
    // Convert to RGB
    const r =
      4.0767416621 * lms1_ - 3.3077115913 * lms2_ + 0.2309699292 * lms3_;
    const g =
      -1.2684380046 * lms1_ + 2.6097574011 * lms2_ - 0.3413193965 * lms3_;
    const blue =
      -0.0041960863 * lms1_ - 0.7034186147 * lms2_ + 1.707614701 * lms3_;
  ...
  const createColorDot = (
    ...
    if (isSpiral) {
      const spiralPos = getSpiralPosition(
        index,
    ...
    return dots
      .map((dot, index) => {
        ...
        if (showSpiral()) {
          ...
          const targetRadius =
            config.spiralRadius + index * config.spiralGrowth;
          const targetX =
            config.canvasWidth / 2 + Math.cos(targetAngle) * targetRadius;
          const targetY =
            config.canvasHeight / 2 + Math.sin(targetAngle) * targetRadius;
          ...
          x = Math.max(
            dot.radius,
            Math.min(config.canvasWidth - dot.radius, x),
          );
          y = Math.max(
            dot.radius,
            Math.min(config.canvasHeight - dot.radius, y),
          );
        } else {
          // Simple free physics
  ...
  const animate = () => {
    ...
    setDots((prevDots) => {
      ...
      updatedDots.forEach((dot) => {
        ...
        // Draw border
        ctx!.strokeStyle = themeContext.isDark
  ...
  return (
    <section class="golden-spiral-colors">
      <div class="spiral-header">
        ...
        <p>
          Infinite distinct colors using the golden angle (137.507°) with
          physics-based movement that follows the spiral pattern
        </p>
      </div>
      ...
      <div class="spiral-info">
        <div class="info-grid">
          <div class="info-card">
            <h3>Golden Angle</h3>
            <p>
              137.507° - The angle that creates the most uniform distribution of
              points on a sphere
            </p>
          </div>
          <div class="info-card">
            <h3>OKLCH Colors</h3>
            <p>
              Perceptually uniform color space ensuring visually distinct colors
            </p>
          </div>
          <div class="info-card">
            <h3>Physics Simulation</h3>
            <p>
              Real-time collision detection and bouncing with gravity effects
            </p>
          </div>
          <div class="info-card">
            <h3>Infinite Generation</h3>
            <p>
              Mathematically guaranteed distinct colors for any number of dots
            </p>
          </div>
        </div>
      </div>
