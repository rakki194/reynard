...
describe("IconButton Component", () => {
    ...
    variants.forEach((variant) => {
      it(`should apply ${variant} variant class`, () => {
        const { container } = render(() => (
          <IconButton icon="save" variant={variant} />
        ));
        const button = container.querySelector("button");
  ...
  describe("Button Sizes", () => {
    ...
    sizes.forEach((size) => {
      it(`should apply ${size} size class`, () => {
        const { container } = render(() => (
          <IconButton icon="save" size={size} />
        ));
        const button = container.querySelector("button");
  ...
  describe("Progress Bar", () => {
    it("should show progress bar when progress is provided", () => {
      const { container } = render(() => (
        <IconButton icon="save" progress={50} />
      ));
      const progressBar = container.querySelector(
        ".reynard-icon-button__progress-bar",
      );
    ...
    it("should set correct progress width", () => {
      const { container } = render(() => (
        <IconButton icon="save" progress={75} />
      ));
      const progressBar = container.querySelector(
        ".reynard-icon-button__progress-bar",
      );
  ...
  describe("Glow Effect", () => {
    ...
    it("should set custom glow color", () => {
      const { container } = render(() => (
        <IconButton icon="save" glow glowColor="red" />
      ));
  ...
  describe("Tooltip and Accessibility", () => {
    it("should set title attribute from tooltip prop", () => {
      render(() => (
        <IconButton icon="save" tooltip="Save document">
          Save
        </IconButton>
      ));
    ...
    it("should set aria-label from aria-label prop", () => {
      render(() => (
        <IconButton icon="save" aria-label="Save button">
          Save
        </IconButton>
      ));
  ...
  describe("Click Events", () => {
    it("should handle click events", () => {
      ...
      render(() => (
        <IconButton icon="save" onClick={handleClick}>
          Save
        </IconButton>
      ));
    ...
    it("should not trigger click when disabled", () => {
      const handleClick = vi.fn();
      render(() => (
        <IconButton icon="save" disabled onClick={handleClick}>
          Save
        </IconButton>
      ));
