/**
 * Tests for Enhanced Icon Component
...
describe('Enhanced Icon Component', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('Basic Rendering', () => {
    ...
    it('should apply correct variant classes', () => {
      const { container } = render(() => <Icon name="settings" variant="primary" />);
      const icon = container.querySelector('.reynard-icon--primary');
      expect(icon).toBeInTheDocument();
    });

    it('should apply custom class', () => {
      const { container } = render(() => <Icon name="settings" class="custom-class" />);
  ...
  describe('Interactive Features', () => {
    it('should render as button when interactive is true', () => {
      ...
      expect(button).toHaveClass('reynard-icon--interactive');
    });

    it('should render as span when interactive is false', () => {
      const { container } = render(() => <Icon name="settings" interactive={false} />);
      const span = container.querySelector('span');
      expect(span).toBeInTheDocument();
      expect(span).not.toHaveClass('reynard-icon--interactive');
    });
  ...
  describe('Active State', () => {
    ...
    it('should not apply active class when active is false', () => {
      const { container } = render(() => <Icon name="settings" active={false} />);
      const icon = container.querySelector('.reynard-icon--active');
  ...
  describe('Progress Bar', () => {
    it('should show progress bar when progress is provided', () => {
      const { container } = render(() => <Icon name="settings" progress={50} />);
      const progressBar = container.querySelector('.reynard-icon__progress-bar');
    ...
    it('should set correct progress width', () => {
      const { container } = render(() => <Icon name="settings" progress={75} />);
      const progressBar = container.querySelector('.reynard-icon__progress-bar');
    ...
    it('should clamp progress values between 0 and 100', () => {
      ...
      const { container: container1 } = render(() => <Icon name="settings" progress={-10} />);
      const progressBar1 = container1.querySelector('.reynard-icon__progress-bar');
      expect(progressBar1).toBeNull(); // No progress bar when clamped to 0

      // Test progress over 100 - should render with 100%
      const { container: container2 } = render(() => <Icon name="settings" progress={150} />);
      const progressBar2 = container2.querySelector('.reynard-icon__progress-bar');
      expect(progressBar2).toHaveStyle({ '--progress-width': '100%' });
    });
  ...
  describe('Glow Effect', () => {
    ...
    it('should set custom glow color', () => {
      const { container } = render(() => <Icon name="settings" glow glowColor="red" />);
      const icon = container.querySelector('.reynard-icon--glow');
    ...
    it('should use default glow color when not specified', () => {
      const { container } = render(() => <Icon name="settings" glow />);
      const icon = container.querySelector('.reynard-icon--glow');
      expect(icon).toHaveStyle({ '--glow-color': 'var(--accent)' });
    });
  });
  ...
  describe('Combined Features', () => {
    it('should handle multiple features simultaneously', () => {
      ...
      const button = container.querySelector('button');
      expect(button).toHaveClass('reynard-icon--interactive');
      expect(button).toHaveClass('reynard-icon--active');
      expect(button).toHaveClass('reynard-icon--loading');
      expect(button).toHaveClass('reynard-icon--with-progress');
      expect(button).toHaveClass('reynard-icon--glow');
