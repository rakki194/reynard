...
export async function writeOutput(config: any, docConfig: DocEngineConfig): Promise<void> {
  ...
  if (docConfig.pages) {
    for (const page of docConfig.pages as DocPage[]) {
  ...
  if (docConfig.api && (docConfig.api as any[]).length > 0) {
    const apiHtml = await generateApiHtml(docConfig.api as any[], docConfig);
...
export async function generateHtmlPage(page: DocPage, config: DocEngineConfig): Promise<string> {
  ...
  renderer.code = function ({ text, lang }: { text: string; lang?: string }) {
    if (lang && (hljs as any).default.getLanguage(lang)) {
      try {
        const highlighted = (hljs as any).default.highlight(text, {
          language: lang,
        }).value;
        return `<pre><code class="hljs language-${lang}">${highlighted}</code></pre>`;
      } catch {}
    }
    const highlighted = (hljs as any).default.highlightAuto(text).value;
    return `<pre><code class="hljs">${highlighted}</code></pre>`;
  };

  (marked as any).setOptions({ renderer, breaks: true, gfm: true });
  const htmlContent = (marked as any)(page.content);
...
export async function generateApiHtml(apiDocs: any[], config: DocEngineConfig): Promise<string> {
  ...
  for (const api of apiDocs) {
    ...
    if (!apiGroups.has(packageName)) apiGroups.set(packageName, []);
  ...
  for (const [packageName, apis] of apiGroups) {
    ...
    for (const api of apis) {
      if (!typeGroups.has(api.type)) typeGroups.set(api.type, []);
    ...
    for (const [type, typeApis] of typeGroups) {
      ...
      for (const api of typeApis) htmlContent += generateApiItemHtml(api);
    }
  }
...
export async function copyAssets(outputPath: string): Promise<void> {
