...
export const GalleryDownloadManager = props => {
    const { downloads, activeDownloads, completedDownloads, failedDownloads, isLoading, error, startDownload, cancelDownload, deleteDownload, refreshDownloads, } = useGalleryDownloads({
    ...
    const handleStartDownload = async () => {
        if (!url().trim() || !validationResult()?.isValid)
            return;
        setIsStarting(true);
        try {
            const options = {
    ...
    return (<div class="p-4 border rounded-lg shadow-sm bg-gray-800 text-white">
      <h2 class="text-xl font-semibold mb-4">Gallery Download Manager</h2>

      {/* URL Input and Validation */}
      <div class="mb-6 space-y-4">
        <div class="flex space-x-2">
          <input type="text" placeholder="Enter gallery URL (e.g., https://example.com/gallery)" value={url()} onInput={e => setUrl(e.currentTarget.value)} class="flex-1 p-2 rounded bg-gray-700 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
          <button onClick={handleValidateUrl} disabled={!url().trim() || isValidating()} class="px-4 py-2 rounded bg-green-600 hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium">
        ...
        <Show when={validationResult()}>
          {result => (<div class={`p-3 rounded border ${result().isValid
                ...
                <div>
                  ...
                  <Show when={result().extractor}>
                    <p class="text-sm mt-1">
                      ...
                      {result().extractor.category && (<span class="ml-2 text-gray-300">({result().extractor.category})</span>)}
                    </p>
                  </Show>
                </div>
              </Show>
        ...
        <Show when={validationResult()?.isValid}>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-700 rounded">
            <div>
              ...
              <input type="text" value={downloadOptions().outputDirectory || ""} onInput={e => setDownloadOptions(prev => ({ ...prev, outputDirectory: e.currentTarget.value }))} placeholder="./downloads" class="w-full p-2 rounded bg-gray-600 border border-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-300 mb-1">Filename Pattern</label>
              <input type="text" value={downloadOptions().filename || ""} onInput={e => setDownloadOptions(prev => ({ ...prev, filename: e.currentTarget.value }))} placeholder="{title}_{id}" class="w-full p-2 rounded bg-gray-600 border border-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-300 mb-1">Max Concurrent</label>
              <input type="number" value={downloadOptions().maxConcurrent || ""} onInput={e => setDownloadOptions(prev => ({ ...prev, maxConcurrent: parseInt(e.currentTarget.value) }))} placeholder="5" min="1" max="10" class="w-full p-2 rounded bg-gray-600 border border-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
            <div class="flex items-end">
              <button onClick={handleStartDownload} disabled={!validationResult()?.isValid || isStarting()} class="w-full px-4 py-2 rounded bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium">
      ...
      <div class="space-y-4">
        <div class="flex justify-between items-center">
          ...
          <button onClick={refreshDownloads} disabled={isLoading()} class="px-3 py-1 rounded bg-gray-600 hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed text-sm">
          ...
          <div>
            ...
            <div class="space-y-2">
              <For each={activeDownloads()}>
                {download => (<div class="p-3 bg-gray-700 rounded-md border border-gray-600">
                    <div class="flex justify-between items-start">
                      ...
                      <div class="flex space-x-2 ml-4">
                        <button onClick={() => handleCancelDownload(download.id)} class="px-3 py-1 rounded bg-yellow-600 hover:bg-yellow-700 text-sm">
                          Cancel
                        </button>
                      </div>
                    </div>
                    <Show when={download.progress}>
                      <div class="mt-2 w-full bg-gray-600 rounded-full h-2">
                        <div class="bg-blue-500 h-2 rounded-full" style={{ width: `${download.progress.percentage || 0}%` }}/>
                      </div>
                    </Show>
                  </div>)}
              </For>
              ...
              <For each={completedDownloads()}>
                {download => (<div class="p-3 bg-gray-700 rounded-md border border-gray-600">
                    <div class="flex justify-between items-center">
                      ...
                      <div class="flex space-x-2 ml-4">
                        <button onClick={() => handleDeleteDownload(download.id)} class="px-3 py-1 rounded bg-red-600 hover:bg-red-700 text-sm">
                          Delete
                        </button>
                      </div>
                    </div>
