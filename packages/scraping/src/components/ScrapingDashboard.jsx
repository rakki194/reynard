...
export function ScrapingDashboard(props) {
    ...
    return (<div class={`scraping-dashboard ${className}`}>
      {/* Header */}
      ...
      <div class="bg-white border-b border-gray-200">
        <div class="px-6">
          <nav class="flex space-x-8">
            <For each={tabs}>
              {tab => (<button onClick={() => setActiveTab(tab.id)} class={`py-4 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab() === tab.id
      ...
      <div class="flex-1 overflow-auto">
        <div class="p-6">
          {/* Jobs Tab */}
          {activeTab() === "jobs" && (<ScrapingJobManager onJobSelect={handleJobSelect} onJobComplete={handleJobComplete}/>)}

          {/* Progress Tab */}
          {activeTab() === "progress" && (<div class="space-y-6">
              {selectedJob() ? (<>
          ...
          {/* Quality Tab */}
          {activeTab() === "quality" && (<div class="space-y-6">
              <ContentQualityDisplay content={selectedContent()} quality={selectedJob()?.results?.[0]?.quality} onQualityUpdate={quality => {
                console.log("Quality updated:", quality);
            }}/>
            </div>)}

          {/* Gallery Downloads Tab */}
          {activeTab() === "gallery" && (<div class="space-y-6">
              <GalleryDownloadManager onDownloadComplete={download => {
          ...
          {/* Configuration Tab */}
          {activeTab() === "config" && (<ConfigurationPanel onConfigUpdate={configs => {
