...
import type { ContentQuality, QualityLevel } from "../types";
...
export function ContentQualityDisplay(props: ContentQualityDisplayProps) {
  const { content, metadata, quality, onQualityUpdate, className = "" } = props;

  const [isAssessing, setIsAssessing] = createSignal(false);
  const [currentQuality, setCurrentQuality] = createSignal<ContentQuality | null>(quality || null);
  ...
  return (
    <div class={`content-quality-display ${className}`}>
      {/* Header */}
      ...
      {currentQuality() && (
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <div class="flex items-center justify-between mb-4">
            ...
            <span
              class={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getQualityColor(currentQuality()!.overall)}`}
            >
          ...
          <div class="flex items-center space-x-4">
            <div class="flex-1">
              ...
              <div class="w-full bg-gray-200 rounded-full h-3">
                <div
                  class={`h-3 rounded-full transition-all duration-500 ${getScoreBarColor(currentQuality()!.score)}`}
                  style={{ width: `${currentQuality()!.score}%` }}
                />
              </div>
            </div>
          </div>
                ...
                <div class="flex items-center justify-between">
                  <div class="flex-1">
                    ...
                    <div class="w-full bg-gray-200 rounded-full h-2">
                      <div
                        class={`h-2 rounded-full transition-all duration-500 ${getScoreBarColor(factor.score)}`}
