...
export function ScrapingJobManager(props) {
    const { autoRefresh = true, refreshInterval = 5000, onJobSelect, onJobComplete, className = "" } = props;
    const [selectedJobId, setSelectedJobId] = createSignal(null);
    const [newJobUrl, setNewJobUrl] = createSignal("");
    const [newJobType, setNewJobType] = createSignal("general");
    const { jobs, activeJobs, completedJobs, failedJobs, isLoading, error, createJob, cancelJob, retryJob, deleteJob } = useScrapingJobs({
        autoRefresh,
        refreshInterval,
        onJobComplete,
    });
    ...
    return (<div class={`scraping-job-manager ${className}`}>
      {/* Header */}
      <div class="mb-6">
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <div class="flex gap-4">
          <div class="flex-1">
            ...
            <input id="job-url" type="url" value={newJobUrl()} onInput={e => setNewJobUrl(e.currentTarget.value)} placeholder="https://example.com/article" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"/>
          </div>
          <div class="w-48">
            <label for="job-type" class="block text-sm font-medium text-gray-700 mb-2">
              Type
            </label>
            <select id="job-type" value={newJobType()} onChange={e => setNewJobType(e.currentTarget.value)} class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
          ...
          <div class="flex items-end">
            <button onClick={handleCreateJob} disabled={!newJobUrl().trim() || isLoading()} class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="divide-y divide-gray-200">
          {jobs().length === 0 ? (<div class="px-6 py-12 text-center">
              ...
              <p class="mt-1 text-sm text-gray-500">Get started by creating your first scraping job.</p>
            </div>) : (jobs().map(job => (<div class={`px-6 py-4 hover:bg-gray-50 cursor-pointer ${selectedJobId() === job.id ? "bg-blue-50 border-l-4 border-blue-500" : ""}`} onClick={() => handleJobSelect(job)}>
                <div class="flex items-center justify-between">
                  <div class="flex-1 min-w-0">
                    ...
                    {job.progress > 0 && (<div class="mt-2">
                        <div class="flex items-center">
                          <div class="flex-1 bg-gray-200 rounded-full h-2">
                            <div class="bg-blue-600 h-2 rounded-full transition-all duration-300" style={{ width: `${job.progress}%` }}/>
                          </div>
                          <span class="ml-2 text-sm text-gray-500">{job.progress}%</span>
                        </div>
                      </div>)}
                    {job.error && (<div class="mt-2">
                        <p class="text-sm text-red-600">{job.error}</p>
                      </div>)}
                  </div>
                  <div class="flex items-center space-x-2">
                    {(job.status === "pending" || job.status === "running") && (<button onClick={e => {
                    e.stopPropagation();
                    handleCancelJob(job.id);
                }} class="text-gray-400 hover:text-red-600" title="Cancel job">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                          <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/>
                        </svg>
                      </button>)}
                    {job.status === "failed" && (<button onClick={e => {
                    e.stopPropagation();
                    handleRetryJob(job.id);
                }} class="text-gray-400 hover:text-blue-600" title="Retry job">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
            ...
            }} class="text-gray-400 hover:text-red-600" title="Delete job">
                      <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
