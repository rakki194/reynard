...
class MCPGitClient:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if response.status_code != 200:
                    raise Exception(
                        f"MCP bridge returned {response.status_code}: {response.text}"
                    )

                result = response.json()
                if not result.get("success", False):
                    raise Exception(
                        f"MCP tool call failed: {result.get('error', 'Unknown error')}"
                    )
    ...
    async def diff(
        self, staged: bool = False, page: int = 1, page_size: int = 1000, **kwargs
    ) -> Dict[str, Any]:
        """Get git diff with pagination."""
        return await self._call_mcp_tool(
            "diff", {"staged": staged, "page": page, "page_size": page_size, **kwargs}
        )
    ...
    async def pull(
        self, remote: str = "origin", branch: Optional[str] = None, **kwargs
    ) -> Dict[str, Any]:
        """Pull changes."""
    ...
    async def clone(
        self, url: str, directory: Optional[str] = None, **kwargs
    ) -> Dict[str, Any]:
        """Clone a repository."""
        args = {"url": url, **kwargs}
        if directory:
