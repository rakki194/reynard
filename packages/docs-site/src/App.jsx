...
import { DocsLayout, DocsHeader, DocsSidebar, DocsFooter, DocsNav, DocsSearch, DocsSearchModal, } from "reynard-docs-components";
...
const App = () => {
    ...
    const [docEngine, setDocEngine] = createSignal(null);
    ...
    const sidebarContent = () => {
        ...
        return (<div class="docs-sidebar-content">
        <DocsNav items={sections.map(section => ({
                label: section.title,
    ...
    // Header actions
    const headerActions = () => (<div class="docs-header-actions">
      <DocsSearch onSearch={handleSearch} onClear={() => handleSearch("")} placeholder="Search documentation..." class="docs-header-search"/>
      <button class="docs-theme-toggle" onClick={() => setTheme(theme() === "light" ? "dark" : "light")} aria-label="Toggle theme">
        {theme() === "light" ? "🌙" : "☀️"}
      </button>
    </div>);
    if (isLoading()) {
    ...
    return (<Router>
      <DocsLayout header={<DocsHeader title="Reynard Documentation" logo="/logo.svg" navigation={<DocsNav items={navigationItems()}/>} actions={headerActions()}/>} sidebar={<DocsSidebar title="Documentation">{sidebarContent()}</DocsSidebar>} footer={<DocsFooter>
      ...
      <DocsSearchModal isOpen={isSearchModalOpen()} onClose={() => setIsSearchModalOpen(false)} onSearch={handleSearch} results={searchResults()} onResultClick={handleSearchResultClick} suggestions={["Getting Started", "API Reference", "Components", "Themes", "Examples"]}/>
    </Router>);
};
/**
 * Load documentation configuration
