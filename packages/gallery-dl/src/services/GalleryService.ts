...
export class GalleryService {
  ...
  private config: GalleryServiceConfig;
  private activeDownloads: Map<string, ProgressState> = new Map();
  private batchDownloads: Map<string, BatchDownload> = new Map();
  ...
  ): Promise<AsyncResult<DownloadResult, GalleryDownloadError>> {
    try {
      // Validate URL first
      ...
      return {
        success: false,
        error:
          error instanceof GalleryDownloadError
            ? error
   ...
   */
  async getExtractors(): Promise<AsyncResult<ExtractorInfo[], GalleryDownloadError>> {
    try {
      ...
      return {
        success: false,
        error: new GalleryDownloadError(
          `Failed to get extractors: ${error instanceof Error ? error.message : "Unknown error"}`,
          "EXTRACTOR_LIST_ERROR"
        ),
      };
    }
  }
  ...
  ): Promise<AsyncResult<BatchDownload, GalleryDownloadError>> {
    try {
   ...
   */
  async cancelBatchDownload(batchId: string): Promise<AsyncResult<void, GalleryDownloadError>> {
    try {
   ...
   */
  private async _performDownload(downloadId: string, url: string, options: DownloadOptions): Promise<DownloadResult> {
    const startTime = Date.now();
   ...
   */
  private async _processBatch(batchId: string): Promise<void> {
    const batch = this.batchDownloads.get(batchId);
    if (!batch) return;
   ...
   */
  private async _downloadInBatch(batchId: string, url: string, options: DownloadOptions): Promise<void> {
    const batch = this.batchDownloads.get(batchId);
    if (!batch) return;
   ...
   */
  private _updateProgress(downloadId: string, progress: Partial<ProgressState>): void {
   ...
   */
  private _updateBatchProgress(batchId: string): void {
    const batch = this.batchDownloads.get(batchId);
    if (batch) {
   ...
   */
  private _generateDownloadId(): string {
