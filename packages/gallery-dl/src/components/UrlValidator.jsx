...
export const UrlValidator = props => {
    ...
    // Service instance
    const [service] = createSignal(props.service ||
        new GalleryService({
    ...
    const getStatusMessage = () => {
        ...
        if (!result)
            return "Enter a URL to validate";
        if (result.isValid)
            return "URL is valid and supported";
    ...
    return (<div class={`reynard-url-validator ${props.class || ""}`}>
      {/* URL Input */}
      <Card variant="elevated" padding="lg" class="reynard-url-validator__input">
        <div class="reynard-url-validator__form">
          <TextField label="Gallery URL" placeholder="Enter gallery URL (e.g., https://example.com/gallery)" value={url()} onInput={e => setUrl(e.currentTarget.value)} fullWidth required error={getValidationStatus() === "invalid"} errorMessage={validationResult()?.error} rightIcon={getStatusIcon() ? <Icon name={getStatusIcon()} size="sm" variant={getStatusColor()}/> : null}/>

          <div class="reynard-url-validator__actions">
            <Button variant="secondary" size="sm" onClick={handleValidate} disabled={!url().trim() || isValidating()} loading={isValidating()} leftIcon={<Icon name="search" size="sm"/>}>
              Validate
            </Button>
            <Button variant="tertiary" size="sm" onClick={handleClear} disabled={!url().trim()} leftIcon={<Icon name="clear" size="sm"/>}>
              Clear
            </Button>
          </div>
        </div>
      ...
      <Show when={validationHistory().length > 0}>
        <Card variant="elevated" padding="md" class="reynard-url-validator__history">
          <div class="reynard-url-validator__history-header">
            ...
            <Button variant="tertiary" size="sm" onClick={() => setShowHistory(!showHistory())} rightIcon={<Icon name={showHistory() ? "chevron-up" : "chevron-down"} size="sm"/>}>
          ...
          <Show when={showHistory()}>
            <div class="reynard-url-validator__history-list">
              <For each={validationHistory()}>
                {(result, index) => (<div class="reynard-url-validator__history-item">
                    <div class="reynard-url-validator__history-status">
                      <Icon name={result.isValid ? "checkmark" : "error"} size="sm" variant={result.isValid ? "success" : "error"}/>
