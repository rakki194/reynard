...
export const DownloadManager: Component<DownloadManagerProps> = props => {
  ...
  const [downloadOptions, setDownloadOptions] = createSignal<DownloadOptions>(props.defaultOptions || {});
  const [downloads, setDownloads] = createSignal<Download[]>([]);
  const [queue, setQueue] = createSignal<DownloadQueue | null>(null);
  ...
  // Progress tracking
  const [progressTracker] = createSignal<ProgressTracker>(new ProgressTracker());
