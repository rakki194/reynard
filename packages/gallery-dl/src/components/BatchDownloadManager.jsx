...
export function BatchDownloadManager() {
    ...
    const [batches, setBatches] = createStore([]);
    const [selectedBatch, setSelectedBatch] = createSignal(null);
    const [batchItems, setBatchItems] = createStore([]);
    ...
    return (<div class="batch-download-manager space-y-6">
      ...
      <Card class="p-6">
        ...
        <div class="space-y-4">
          <div>
            ...
            <TextField value={batchName()} onInput={e => setBatchName(e.currentTarget.value)} placeholder="Enter batch name (optional)" class="w-full"/>
          </div>

          <div>
            <label class="block text-sm font-medium mb-2">URLs (one per line)</label>
            <textarea value={urls()} onInput={e => setUrls(e.currentTarget.value)} placeholder="Enter URLs, one per line..." rows={6} class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"/>
          </div>

          <div>
            <label class="block text-sm font-medium mb-2">Priority</label>
            <select value={priority()} onChange={e => setPriority(e.currentTarget.value)} class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent">
        ...
        <Card class="p-6">
          ...
          <div class="space-y-4">
            <For each={getActiveDownloads()}>
              {download => (<div class="border border-gray-200 rounded-lg p-4">
      ...
      <Card class="p-6">
        ...
        <Show when={batches.length > 0}>
          <div class="space-y-3">
            <For each={batches}>
              {batch => (<div class="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
