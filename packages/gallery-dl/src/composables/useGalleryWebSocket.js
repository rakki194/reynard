...
export function useGalleryWebSocket(baseUrl = "") {
    ...
    const [progressUpdates, setProgressUpdates] = createSignal(new Map());
    const [downloadEvents, setDownloadEvents] = createSignal(new Map());
    let ws = null;
    ...
    const handleDownloadCancelled = (data) => {
        setDownloadEvents(prev => {
            const newMap = new Map(prev);
            const events = newMap.get(data.download_id) || [];
    ...
    const getProgress = (downloadId) => {
        return progressUpdates().get(downloadId);
    };
    // Get events for a specific download
    const getEvents = (downloadId) => {
        return downloadEvents().get(downloadId) || [];
    };
    // Get all active downloads
    const getActiveDownloads = () => {
        return Array.from(progressUpdates().values()).filter(progress => progress.status === "downloading" || progress.status === "queued");
    };
    return {
        // State
