...
export class PointCloudGeometryManager {
    constructor(threeJS) {
        Object.defineProperty(this, "threeJS", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "geometryCache", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
    ...
    createPointGeometry(points, config) {
    ...
    createStandardGeometry(points, config) {
    ...
    createInstancedGeometry(points, config) {
    ...
    createThumbnailGeometry(points, config) {
        const { BufferGeometry, Float32BufferAttribute } = this.threeJS;
        const geometry = new BufferGeometry();
        const positions = new Float32Array(points.length * 3);
    ...
    createTextGeometry(points, config) {
        const { BufferGeometry, Float32BufferAttribute } = this.threeJS;
        const geometry = new BufferGeometry();
        const positions = new Float32Array(points.length * 3);
        for (let i = 0; i < points.length; i++) {
    ...
    updateGeometry(geometry, points, config) {
     ...
     */
    getOrCreatePointGeometry(points, config) {
