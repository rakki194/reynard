export class SpriteManager {
    ...
    async createThumbnailSprites(points, config, scene, onHover, onLeave) {
        const { Sprite, SpriteMaterial, TextureLoader } = this.threeJS;
        const sprites = [];
        for (const point of points) {
            if (!point.imageThumbnail && !point.thumbnailDataUrl)
                continue;
            // Create texture
            let texture;
    ...
    async createTextSprites(points, config, scene) {
