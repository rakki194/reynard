...
export class PointCloudMaterialManager {
    constructor(threeJS) {
        ...
        Object.defineProperty(this, "materialCache", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
    ...
    createThumbnailMaterial(config) {
    ...
    disposeMaterial(material) {
        if (!material || this.disposedMaterials.has(material))
            return;
     ...
     */
    getOrCreatePointMaterial(config) {
        const cacheKey = generateMaterialCacheKey(config);
