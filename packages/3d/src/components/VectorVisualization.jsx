import { Show, createSignal, createMemo, onMount, onCleanup, createEffect, } from "solid-js";
import "./VectorVisualization.css";
export const VectorVisualization = (props) => {
    const [canvasRef, setCanvasRef] = createSignal();
    const [isExpanded, setIsExpanded] = createSignal(false);
    const [selectedVector, setSelectedVector] = createSignal(-1);
    const [selectedRegion, setSelectedRegion] = createSignal(null);
    ...
    const normalizedVectors = createMemo(() => {
        ...
        if (range === 0)
            return props.vectors;
        return props.vectors.map((vector) => vector.map((val) => (val - minVal) / range));
    });
        ...
        vectors.forEach((vector, vectorIndex) => {
            vector.forEach((value, valueIndex) => {
                ...
                if (props.showValues && cellWidth > 20 && cellHeight > 20) {
                    ...
                    ctx.fillText(props.vectors[vectorIndex][valueIndex].toFixed(2), x + cellWidth / 2, y + cellHeight / 2);
                }
            });
        });
        // Draw selection
    ...
    return (<div class={`vector-visualization ${props.className || ""}`}>
      <Show when={props.title}>
        <h3 class="visualization-title">{props.title}</h3>
      </Show>

      <div class="visualization-container">
        <canvas ref={setCanvasRef} width={width()} height={height()} class="visualization-canvas"/>
      ...
      <Show when={hoveredCell() && props.enableTooltips}>
        <div class="tooltip" classList={{
          ...
          <div class="tooltip-content">
            ...
            <div>
              Value:{" "}
              {props.vectors[hoveredCell()?.vectorIndex || 0]?.[hoveredCell()?.valueIndex || 0]?.toFixed(3)}
            </div>
      ...
      <div class="visualization-controls">
        <button onClick={() => setIsExpanded(!isExpanded())} class="expand-button">
