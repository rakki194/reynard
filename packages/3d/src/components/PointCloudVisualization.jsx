...
export const PointCloudVisualization = (props) => {
    ...
    const pointCloud = usePointCloud(() => props.points, () => props.settings || {}, () => props.searchIntegration || {});
    // Scene state
    ...
    const handleSceneReady = (scene, camera, renderer, controls) => {
        ...
        const existingPointCloud = scene.children.find((child) => child.userData?.isPointCloud);
        if (existingPointCloud) {
            scene.remove(existingPointCloud);
        }
        // Create point cloud geometry
        const threeJS = pointCloud.threeJS();
        ...
        for (let i = 0; i < points.length; i++) {
            ...
            if (pointCloud.enableHighlighting()) {
                ...
                if (isSelected || isHovered) {
                    color = pointCloud.highlightColor();
     ...
     */
    const onRender = (scene, camera, renderer, controls) => {
        ...
        if (currentScene && currentCamera && currentRenderer && currentControls) {
            ...
            if (interpolatedPoints !== props.points) {
                ...
                const pointCloudMesh = currentScene.children.find((child) => child.userData?.isPointCloud);
                if (pointCloudMesh) {
    ...
    return (<div class={`point-cloud-visualization ${props.className || ""}`}>
      ...
      <Show when={!pointCloud.isLoading() && !pointCloud.error()}>
        <ThreeJSVisualization width={props.width || 800} height={props.height || 600} backgroundColor={props.backgroundColor || "#1a1a1a"} onSceneReady={handleSceneReady} onRender={onRender} className="point-cloud-canvas" enableCameraAnimations={props.animationSettings?.enableAnimations ?? true} onCameraAnimationStart={props.animationSettings?.onAnimationStart} onCameraAnimationEnd={props.animationSettings?.onAnimationEnd}/>
        ...
        <div class="point-cloud-stats">
          ...
          <Show when={animations.currentAnimation()}>
            <div class="stat-item">
              ...
              <span class="stat-value">
                {(animations.currentAnimation()?.progress || 0 * 100).toFixed(1)}
                %
              </span>
            </div>
          </Show>
        ...
        <div class="point-cloud-controls">
          <div class="control-group">
            ...
            <select value={props.settings?.colorMapping || "similarity"} onChange={(e) => {
            // This would need to be handled by parent component
            console.log("Color mapping:", e.currentTarget.value);
        }} class="control-select" title="Select color mapping method">
          ...
          <div class="control-group">
            ...
            <select value={props.settings?.sizeMapping || "uniform"} onChange={(e) => {
            console.log("Size mapping:", e.currentTarget.value);
        }} class="control-select" title="Select size mapping method">
          ...
          <label class="control-item">
            <Toggle size="sm"/> {console.log("Instancing:", e.currentTarget.checked)};
              }}
            />
          ...
          <label class="control-item">
            <Toggle size="sm"/> {console.log("LOD:", e.currentTarget.checked)};
              }}
            />
          ...
          <label class="control-item">
            <Toggle size="sm"/> {console.log("Culling:", e.currentTarget.checked)};
              }}
            />
          ...
          <label class="control-item">
            <Toggle size="sm"/> {console.log("Highlighting:", e.currentTarget.checked)};
              }}
            />
        ...
        <Show when={pointCloud.selectedPoints().length > 0}>
          <div class="selection-groups">
            ...
            <div class="selection-list">
              {pointCloud.selectedPoints().map((point, index) => (<div class="selection-item">
                  ...
                  <Show when={point.imageThumbnail || point.imageUrl}>
                    <img src={point.imageThumbnail || point.imageUrl} alt="Point thumbnail" class="selection-thumbnail"/>
                  </Show>
                  <button class="remove-selection" onClick={() => {
        ...
        <Show when={pointCloud.hoveredPoint() && pointCloud.tooltipPosition()}>
          <div class="point-tooltip point-tooltip-positioned">
            ...
            {/* Image thumbnail */}
            <Show when={pointCloud.hoveredPoint()?.imageThumbnail ||
            pointCloud.hoveredPoint()?.imageUrl}>
              <div class="tooltip-image">
                <img src={pointCloud.hoveredPoint()?.imageThumbnail ||
            pointCloud.hoveredPoint()?.imageUrl} alt="Point thumbnail" class="tooltip-thumbnail"/>
              </div>
            </Show>
            ...
            <div class="tooltip-content">
              ...
              {/* Additional metadata */}
              <Show when={pointCloud.hoveredPoint()?.metadata &&
            Object.keys(pointCloud.hoveredPoint()?.metadata || {})
                .length > 0}>
                <div class="tooltip-metadata">
                  <div class="tooltip-section-title">Metadata:</div>
                  {Object.entries(pointCloud.hoveredPoint()?.metadata || {}).map(([key, value]) => (<div class="tooltip-item">
                      <span>{key}:</span>
