...
export function usePointCloud(points, settings = () => ({}), searchIntegration = () => ({})) {
    ...
    const { processedPoints } = usePointCloudProcessing(points, settings, settingsModule.maxPoints, settingsModule.pointSize, searchIntegrationModule.processPointsWithSearchIntegration);
    // Create interaction handlers
    const { createPointSelectionHandler, createPointHoverHandler } = usePointCloudHandlers(eventsModule.raycaster, eventsModule.mouse, interactions.handlePointSelection, interactions.handlePointHover);
    return createPointCloudReturn(threeJSInit, interactions, eventsModule, searchIntegrationModule, settingsModule, stateModule, processedPoints, createPointSelectionHandler, createPointHoverHandler);
}
