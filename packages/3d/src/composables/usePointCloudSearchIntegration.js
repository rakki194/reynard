...
export function usePointCloudSearchIntegration(searchIntegration = () => ({})) {
    ...
    const [searchIntegrationData, setSearchIntegrationData] = createSignal(null);
    const [queryPointMesh, setQueryPointMesh] = createSignal(null);
    const [pathMeshes, setPathMeshes] = createSignal([]);
    const [radiusMesh, setRadiusMesh] = createSignal(null);
    /**
     * Process points with search integration highlighting
     */
    const processPointsWithSearchIntegration = (points) => {
        ...
        return points.map((point) => {
            ...
            const isHighlighted = searchData.highlighted_results?.some((result) => result.original_index === point.id);
            if (isHighlighted) {
                highlightedPoint.color = queryPointColor();
