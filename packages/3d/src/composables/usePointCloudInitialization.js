...
export function usePointCloudInitialization() {
    ...
    const createThumbnailSprites = async (scene, points, config) => {
        if (!pointCloudRenderer || !pointCloudEvents || !points)
            return;
        const sprites = await pointCloudRenderer.spriteManager().createThumbnailSprites(points, config, scene, pointId => pointCloudEvents.handlePointHover(pointId, config), () => pointCloudEvents.handlePointLeave(config));
        setThumbnailSprites(sprites);
    };
    const createTextSprites = async (scene, points, config) => {
        if (!pointCloudRenderer || !points)
            return;
        const sprites = await pointCloudRenderer.spriteManager().createTextSprites(points, config, scene);
