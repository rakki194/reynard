 ...
 */
export function dbscanClustering(points, minPoints = 5, maxDistance = 1.0) {
    if (points.length === 0)
        return [];
 ...
 */
export function hierarchicalClustering(points, maxClusters = 10) {
    if (points.length === 0)
        return [];
 ...
 */
function getNeighbors(point, allPoints, maxDistance) {
 ...
 */
function generateClusterColor(index) {
 ...
 */
export function detectClusters(points, options) {
...
export function calculateClusterStats(clusters) {
    ...
    const averageDensity = clusters.reduce((sum, cluster) => sum + cluster.density, 0) /
        clusters.length;
    const averageRadius = clusters.reduce((sum, cluster) => sum + cluster.radius, 0) /
        clusters.length;
    return {
