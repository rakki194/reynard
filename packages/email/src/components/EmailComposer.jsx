...
export function EmailComposer(props) {
    ...
    return (<div class={`email-composer ${props.className || ""}`}>
      <div class="email-composer-header">
        ...
        <div class="email-composer-actions">
          ...
          <button type="button" class="email-button email-button-secondary" onClick={props.onCancel} disabled={isSending()}>
            Cancel
          </button>
          <button type="button" class="email-button email-button-primary" onClick={handleSend} disabled={isSending() || toEmails().length === 0}>
      ...
      <div class="email-composer-content">
        {/* Template Selection */}
        {emailComposable.templates().length > 0 && (<div class="email-form-group">
        ...
        <div class="email-form-group">
          ...
          <div class="email-recipients">
            ...
            <input type="email" placeholder="Enter email address..." onKeyDown={(e) => {
        ...
        <div class="email-form-group">
          ...
          <div class="email-recipients">
            ...
            <input type="email" placeholder="Enter CC email address..." onKeyDown={(e) => {
            if (e.key === "Enter" || e.key === ",") {
                e.preventDefault();
        ...
        <div class="email-form-group">
          ...
          <div class="email-recipients">
            ...
            <input type="email" placeholder="Enter BCC email address..." onKeyDown={(e) => {
            if (e.key === "Enter" || e.key === ",") {
                e.preventDefault();
        ...
        <div class="email-form-group">
          <label>Subject</label>
          <input type="text" value={subject()} onInput={(e) => setSubject(e.currentTarget.value)} placeholder="Enter email subject..."/>
        </div>

        {/* Reply To */}
        <div class="email-form-group">
          <label>Reply To</label>
          <input type="email" value={replyTo()} onInput={(e) => setReplyTo(e.currentTarget.value)} placeholder="Enter reply-to email address..."/>
        </div>
        ...
        <div class="email-form-group">
          ...
          <div class="email-attachments">
            <For each={attachments()}>
              {(file, index) => (<div class="email-attachment">
                  <span>{file.name}</span>
                  <span class="email-attachment-size">
        ...
        <div class="email-form-group">
          <div class="email-body-header">
            ...
            <div class="email-body-actions">
              <button type="button" class={`email-button ${isHtmlMode() ? "email-button-secondary" : "email-button-primary"}`} onClick={() => setIsHtmlMode(false)}>
                Text
              </button>
              <button type="button" class={`email-button ${isHtmlMode() ? "email-button-primary" : "email-button-secondary"}`} onClick={() => setIsHtmlMode(true)}>
                HTML
              </button>
            </div>
          </div>
          
          {isHtmlMode() ? (<textarea value={htmlBody()} onInput={(e) => setHtmlBody(e.currentTarget.value)} placeholder="Enter HTML message..." rows={10}/>) : (<textarea value={body()} onInput={(e) => setBody(e.currentTarget.value)} placeholder="Enter message..." rows={10}/>)}
        </div>
      </div>

      {/* Error Display */}
      {emailComposable.error() && (<div class="email-status email-status-error">
