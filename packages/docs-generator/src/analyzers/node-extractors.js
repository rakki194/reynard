...
import { extractExamples, extractSince, extractTags, getJSDocDescription, getJSDocReturnDescription, isDeprecated, } from "./jsdoc-utils";
export const isExportedDeclaration = (node) => {
...
export const extractParameters = (node, checker) => {
    if (!node.parameters)
        return [];
    return node.parameters.map((param) => {
        const name = param.name.getText();
        const type = param.type ? param.type.getText() : "any";
        const description = getJSDocDescription(param.symbol, checker);
...
export const extractReturnType = (node) => {
    if (!node.type)
        return undefined;
    const type = node.type.getText();
    const description = getJSDocReturnDescription(node.symbol);
    return { type, description: description || "" };
};
export const extractApiInfo = (node, sourceFile, checker) => {
