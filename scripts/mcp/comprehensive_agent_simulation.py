...
from reynard_ecs_world import AgentTraits, ECSWorld, Entity, AgentComponent, TraitComponent, LifecycleComponent, ReproductionComponent
...
class SimpleTraitSimulation:
    ...
    def run_simulation(self, years: int = 20, initial_population: int = 6) -> dict[str, Any]:
        """Run a simple trait-based simulation."""
        print(f"🦦 Starting {years}-year trait simulation with {initial_population} initial agents...")
    ...
    def _create_initial_population(self, count: int) -> None:
        ...
        for i in range(count):
            ...
            print(f"Created founder {agent_id} ({spirit}, {style}) - Traits: {agent_traits._get_dominant_traits()}")
    ...
    def _simulate_year(self, year: int) -> None:
        ...
        if len(mature_agents) >= 2:
            ...
            if random.random() < breeding_chance:
                ...
                parent2_id, parent2 = random.choice([(aid, a) for aid, a in mature_agents if aid != parent1_id])
                
                # Create offspring
                offspring_id = f"offspring-{self.agent_counter + 1}"
                offspring = self._create_offspring(parent1_id, parent1, parent2_id, parent2, offspring_id, year)
    ...
    def _create_offspring(self, parent1_id: str, parent1: Entity, parent2_id: str, parent2: Entity, 
                         offspring_id: str, birth_year: int) -> Entity | None:
        ...
        # Add components
        offspring.add_component(AgentComponent(f"Agent-{offspring_id}", offspring_traits.spirit, offspring_traits.style))
    ...
    def generate_trait_diagram(self) -> str:
        ...
        founders = [aid for aid in self.simulation_data["agent_details"].keys() if aid.startswith("founder-")]
        ...
        offspring = [aid for aid in self.simulation_data["agent_details"].keys() if aid.startswith("offspring-")]
