...
from reynard_ecs_world import AgentComponent, LifecycleComponent, ReproductionComponent, TraitComponent, LineageComponent, Entity
...
class BreedingService:
    ...
    def find_compatible_mates(self, agent_id: str, max_results: int = 5) -> list[dict[str, Any]]:
    ...
    def analyze_genetic_compatibility(self, agent1_id: str, agent2_id: str) -> dict[str, Any]:
        ...
        if compatibility >= 0.8:
            level = "excellent"
            recommendation = "Excellent genetic compatibility - highly recommended for breeding"
        elif compatibility >= 0.6:
            level = "good"
            recommendation = "Good genetic compatibility - suitable for breeding"
        elif compatibility >= 0.4:
            level = "moderate"
            recommendation = "Moderate genetic compatibility - breeding possible but not ideal"
        else:
            level = "poor"
    ...
    def create_offspring(self, parent1_id: str, parent2_id: str, offspring_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if compatibility < 0.4:
                return {"success": False, "error": "Parents are not compatible for breeding"}
            
            # Create offspring using world method
            offspring = self.world.create_offspring(parent1_id, parent2_id, offspring_id)
    ...
    def get_breeding_statistics(self) -> dict[str, Any]:
        ...
        recent_events = [e for e in self.breeding_history if time.time() - e["timestamp"] < 3600]
        
        return {
            "total_breeding_events": len(self.breeding_history),
            "recent_breeding_events": len(recent_events),
            "successful_breedings": len([e for e in self.breeding_history if e["success"]]),
            "average_compatibility": sum(e["compatibility"] for e in self.breeding_history) / len(self.breeding_history) if self.breeding_history else 0,
    ...
    def _calculate_compatibility(self, agent1: Entity, agent2: Entity) -> float:
        ...
        if lineage1 and lineage2:
            if (agent2.id in lineage1.ancestors or agent2.id in lineage1.descendants or
                agent1.id in lineage2.ancestors or agent1.id in lineage2.descendants):
                return 0.0  # Inbreeding = no compatibility
        
        # Calculate compatibility based on traits
        spirit_compatibility = 0.8 if traits1.traits.spirit == traits2.traits.spirit else 0.5
        style_compatibility = 0.7 if traits1.traits.style == traits2.traits.style else 0.4
