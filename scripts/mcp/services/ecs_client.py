...
class ECSClient:
    ...
    def __init__(self, config: Optional[ECSConfig] = None, auth_client: Optional[AuthClient] = None):
        """
            ...
            try:
                ...
                async with self._session.request(
                    ...
                    if response.status == 200:
                        ...
                        raise Exception(f"ECS API error {response.status}: {error_text}")
                        
            except asyncio.TimeoutError:
                if attempt < self.config.max_retries - 1:
                    logger.warning(f"ECS request timeout, retrying in {self.config.retry_delay}s...")
                    await asyncio.sleep(self.config.retry_delay)
                    continue
                else:
                    raise Exception(f"ECS request timeout after {self.config.max_retries} attempts")
                    
            except Exception as e:
                if attempt < self.config.max_retries - 1:
                    logger.warning(f"ECS request failed: {e}, retrying in {self.config.retry_delay}s...")
                    await asyncio.sleep(self.config.retry_delay)
                    continue
                else:
                    raise Exception(f"ECS request failed after {self.config.max_retries} attempts: {e}")
    ...
    async def analyze_compatibility(
        ...
        """
        return await self._request("GET", f"/agents/{agent1_id}/compatibility/{agent2_id}")
