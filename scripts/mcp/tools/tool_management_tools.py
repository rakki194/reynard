...
from protocol.tool_registry import tool_registry, register_tool
...
def get_tool_configs(**kwargs) -> dict[str, Any]:
    ...
    try:
        configs = tool_registry.get_tool_configs()
...
def get_tool_status(**kwargs) -> dict[str, Any]:
    ...
    try:
        ...
        if tool_name:
            ...
            is_registered = tool_registry.is_tool_registered(tool_name)
            is_enabled = tool_registry.is_tool_enabled(tool_name)
            ...
            enabled_tools = [name for name in all_tools if tool_registry.is_tool_enabled(name)]
            disabled_tools = [name for name in all_tools if not tool_registry.is_tool_enabled(name)]
...
def list_tools_by_category(**kwargs) -> dict[str, Any]:
    ...
    try:
        ...
        for category, tools in categories.items():
            ...
            for tool in tools:
                enabled = "✅" if tool_registry.is_tool_enabled(tool) else "❌"
