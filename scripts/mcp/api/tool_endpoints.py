...
from protocol.tool_registry import tool_registry, ToolMetadata
...
async def sync_tools(request: ToolSyncRequest):
    ...
    try:
        ...
        if request.tool_name:
            # Sync specific tool
            metadata = tool_registry.get_tool_metadata(request.tool_name)
            if not metadata:
            ...
            tools_synced = len(tool_registry.list_all_tools())
...
async def get_tool_stats():
    ...
    try:
        ...
        stats.update({
            ...
            "total_registered": len(tool_registry.list_all_tools())
        })
...
async def toggle_tool(tool_name: str):
    ...
    try:
        success = tool_registry.toggle_tool(tool_name)
        if not success:
            raise HTTPException(status_code=404, detail=f"Tool {tool_name} not found")
