...
async def test_mcp_authorization():
    ...
    try:
        ...
        try:
            search_token = mcp_auth_service.generate_mcp_token("reynard-semantic-search")
            search_token_data = mcp_auth_service.validate_mcp_token(search_token)
            print(f"✅ Search tool token generated")
            print(f"Permissions: {search_token_data.permissions}")
            
            # Test permission for search tool
            has_rag_query = mcp_auth_service.check_permission(search_token_data, "rag:query")
            has_rag_ingest = mcp_auth_service.check_permission(search_token_data, "rag:ingest")
...
async def test_curl_commands():
    ...
    try:
        ...
        print(f"curl -X POST http://localhost:8000/api/mcp/token \\")
        print(f"  -H 'Content-Type: application/json' \\")
        print(f"  -H 'Authorization: Bearer {token}' \\")
        print(f"  -d '{{\"client_id\": \"reynard-mcp-server\"}}'")
        
        print("\n2. Test RAG Query (should work):")
        print(f"curl -X POST http://localhost:8000/api/rag/query \\")
        print(f"  -H 'Content-Type: application/json' \\")
        print(f"  -H 'Authorization: Bearer {token}' \\")
        print(f"  -d '{{\"q\": \"test query\", \"modality\": \"hybrid\", \"top_k\": 5}}'")
        
        print("\n3. Test RAG Stats (should work):")
        print(f"curl -X GET http://localhost:8000/api/rag/admin/stats \\")
        print(f"  -H 'Authorization: Bearer {token}'")
        
        print("\n4. Test RAG Ingest (should fail - no permission):")
        print(f"curl -X POST http://localhost:8000/api/rag/ingest \\")
        print(f"  -H 'Content-Type: application/json' \\")
        print(f"  -H 'Authorization: Bearer {token}' \\")
        print(f"  -d '{{\"items\": [{{\"file_path\": \"/test.txt\"}}]}}'")
