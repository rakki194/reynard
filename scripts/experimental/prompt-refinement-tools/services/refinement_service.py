...
class PromptRefinementService:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.semantic_search = SemanticSearchService(self.config.get('semantic_search', {}))
        self.nlp_processing = NLPProcessingService(self.config.get('nlp_processing', {}))
    ...
    async def refine_query(self, original_query: str) -> RefinementResult:
        ...
        try:
            ...
            analysis_results = await self._analyze_query(original_query, research_findings)
    ...
    async def _conduct_research(self, query: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'related_topics': self._identify_related_topics(web_research, semantic_research),
    ...
    async def _analyze_query(self, query: str, research_findings: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def _refine_query(
        ...
        if analysis_results['specificity_gaps']:
            refined_query, specificity_rationale = await self._apply_specificity_improvements(
    ...
    def _identify_related_topics(self, web_research: Dict[str, Any], semantic_research: Dict[str, Any]) -> List[str]:
    ...
    def _analyze_context_gaps(self, query: str, research_findings: Dict[str, Any]) -> List[str]:
    ...
    def _identify_optimization_opportunities(
        ...
        query: str, 
        research_findings: Dict[str, Any], 
        codebase_analysis: Optional[Dict[str, Any]]
    ) -> List[str]:
        ...
        if research_findings.get('clarity_issues'):
            opportunities.append("Improve clarity by addressing vague terms and unclear references")
    ...
    async def _apply_clarity_enhancements(
        ...
        for issue in clarity_issues:
            if "vague terms" in issue:
                enhanced_query = await self.nlp_processing.replace_vague_terms(enhanced_query)
                rationale.append("Replaced vague terms with specific language")
            elif "unclear pronouns" in issue:
                enhanced_query = await self.nlp_processing.clarify_pronouns(enhanced_query)
    ...
    async def _apply_specificity_improvements(
        ...
        for gap in specificity_gaps:
            if "missing details" in gap:
                improved_query = await self.nlp_processing.add_specific_details(improved_query)
                rationale.append("Added specific details and context")
            elif "missing question words" in gap:
                improved_query = await self.nlp_processing.add_question_words(improved_query)
