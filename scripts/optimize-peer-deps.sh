...
duplicates=$(find packages -name "package.json" -print0 | xargs -0 -I {} sh -c 'if grep -q "solid-js.*1.9.9" "$1" && grep -q "peerDependencies" "$1"; then echo "$1"; fi' _ {} | wc -l)
...
proper_peers=$(find packages -name "package.json" -print0 | xargs -0 grep -l "peerDependencies" | wc -l)

echo "Found ${proper_peers} packages with peer dependencies configured"

# Count packages using workspace protocol
workspace_usage=$(find packages -name "package.json" -print0 | xargs -0 grep -l "workspace:\*" | wc -l)
