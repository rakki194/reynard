...
class ChangelogFormatter {
  ...
  needsBlankLineBefore(line, prevLine) {
    // List items need blank lines after headings
    if (this.isListLine(line) && this.isHeading(prevLine)) {
      return true;
    }

    // List items need blank lines after other content (not lists or blank lines)
    if (this.isListLine(line) && prevLine.trim() !== "" && !this.isListLine(prevLine)) {
      return true;
    }
  ...
  needsBlankLineAfter(line, nextLine) {
    // Headings need blank lines before lists
    if (this.isHeading(line) && this.isListLine(nextLine)) {
      return true;
    }

    // List items need blank lines before other content (not lists or blank lines)
    if (this.isListLine(line) && nextLine.trim() !== "" && !this.isListLine(nextLine)) {
      return true;
    }
