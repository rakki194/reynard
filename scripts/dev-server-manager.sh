    ...
    for category in packages examples backend e2e; do
        ...
        if [ -n "$projects" ]; then
            while IFS= read -r project; do
                ...
                local auto_reload=$(jq -r ".$category.\"$project\".autoReload" "$config_file" 2>/dev/null)
...
stop_project() {
    ...
    for category in packages examples backend e2e; do
        local project_port=$(jq -r ".$category.\"$project_name\".port // empty" "$config_file" 2>/dev/null)
