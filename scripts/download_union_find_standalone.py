#!/usr/bin/env python3
"""
Union-Find Papers Standalone Download Script
===========================================

Standalone script to download union-find papers without MCP dependencies.
"""
...
class StandaloneUnionFindDownloader:
    """Standalone downloader for union-find papers."""
    
    def __init__(self):
        self.papers_dir = Path("/home/kade/runeset/reynard/backend/data/papers")
        ...
        self.union_find_keywords = [
            ...
            'connected component', 'kruskal', 'minimum spanning tree',
            'graph connectivity', 'find-union', 'union by rank',
            'path compression', 'disjoint-set forest'
        ]
    ...
    def _filter_relevant_papers(self, papers: List[Dict[str, Any]], search_term: str) -> List[Dict[str, Any]]:
        ...
        for paper in papers:
            ...
            is_relevant = any(keyword in text_content for keyword in self.union_find_keywords)
    ...
    async def _download_papers_batch(self, papers: List[Dict[str, Any]]) -> None:
        ...
        for i, paper in enumerate(papers, 1):
            try:
                ...
                # Download paper
                result = await self._download_single_paper(paper, arxiv_id)
                ...
                logger.error(f"  ❌ Exception downloading {paper['title'][:50]}...: {e}")
                self.failed_downloads.append({
                    'paper_id': paper['id'],
                    'title': paper['title'],
                    'error': str(e)
    ...
    async def _download_single_paper(self, paper: Dict[str, Any], arxiv_id: str) -> Dict[str, Any]:
        """Download a single paper."""
        try:
            # Create paper directory
            category = paper['categories'][0] if paper['categories'] else "misc"
            paper_dir = self.papers_dir / "arxiv" / category / arxiv_id
            ...
            metadata = {
                ...
                "source": "arxiv",
                "location": "Union-Find Papers Collection",
            ...
            logger.exception("Failed to download paper")
            return {
                "success": False,
                "error": str(e)
            }
    ...
    def _create_summary(self) -> None:
        ...
        report_path = self.papers_dir / "union_find_download_report.json"
...
async def main():
    """Main function."""
    downloader = StandaloneUnionFindDownloader()
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print("Invalid number, using default of 30 papers")
