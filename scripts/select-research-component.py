...
class ReynardComponentSelector:
    ...
    def _discover_components(self) -> dict[str, list[ComponentInfo]]:
        ...
        core_packages = ["core", "components", "themes", "i18n", "ecs-world"]
        ...
        for service in backend_services:
            ...
            if service_path.exists():
                components["backend_services"].append(
                    self._analyze_component(
                        str(service_path),
                        service.replace("/", "-"),
                        "backend_services",
        ...
        testing_components = ["e2e", "packages/testing", "packages/code-quality"]
        for test_comp in testing_components:
            test_path = self.base_path / test_comp
            if test_path.exists():
                components["testing_quality"].append(
                    self._analyze_component(
                        str(test_path),
                        test_comp.replace("/", "-"),
                        "testing_quality",
        ...
        if examples_path.exists():
            for example_dir in examples_path.iterdir():
                if example_dir.is_dir():
                    components["examples_templates"].append(
                        self._analyze_component(
                            str(example_dir),
    ...
    def _generate_description(
        ...
        return descriptions.get(
            category,
            f"Reynard ecosystem component with {file_count} files and {dir_count} subdirectories",
        )
