...
class HexacoAnalyzer:
    ...
    def _load_honesty_humility_indicators(self) -> Dict[str, List[str]]:
        ...
        return {
            'positive_indicators': [
                'acknowledges limitations', 'admits mistakes', 'shares credit',
                'asks for help', 'shows gratitude', 'recognizes others',
                'expresses uncertainty', 'shows vulnerability', 'listens actively',
                'accepts feedback', 'collaborates', 'supports others'
            ],
            'negative_indicators': [
                'takes all credit', 'never admits mistakes', 'dismisses others',
                'refuses help', 'shows entitlement', 'ignores feedback',
                'dominates conversations', 'interrupts others', 'shows superiority',
                'minimizes others', 'exaggerates achievements', 'shows arrogance'
            ]
        }
            ...
            for trait, patterns in self.hexaco_patterns.items():
                for pattern_info in patterns:
                    ...
                    for match in matches:
                        ...
                        if trait_type == 'negative' or (trait_type == 'positive' and trait == 'honesty_humility'):
                            # Determine confidence level
                            confidence_level = self._get_confidence_level(confidence_score)
                            
                            # Generate replacement suggestion
                            replacement = self._generate_replacement(original_text, trait_type)
    ...
    async def get_metrics(self, text: str) -> Dict[str, Any]:
        ...
        honesty_humility_score = max(0, min(100, (trait_scores['honesty_humility'] + 1) * 50))
