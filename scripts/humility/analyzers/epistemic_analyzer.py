...
class EpistemicHumilityAnalyzer:
            ...
            for pattern_info in self.epistemic_patterns:
                ...
                for match in matches:
                    ...
                    if pattern_type in ['overconfidence', 'certainty_claims']:
                        ...
                        replacement = self._generate_replacement(original_text, pattern_type)
    ...
    async def get_metrics(self, text: str) -> Dict[str, Any]:
        ...
        if total_lines > 0:
            ...
            epistemic_humility_score = max(0, (humility_ratio - overconfidence_ratio) * 100)
    ...
    def _calculate_epistemic_score(self, text: str) -> float:
        ...
        humility_indicators = ['uncertain', 'might', 'could', 'possibly', 'perhaps', 'maybe', 'likely']
        overconfidence_indicators = ['certain', 'definitely', 'always', 'never', 'all', 'every', 'guaranteed']
        
        humility_count = sum(1 for word in humility_indicators if word in text.lower())
        overconfidence_count = sum(1 for word in overconfidence_indicators if word in text.lower())
        
        if humility_count + overconfidence_count == 0:
            return 50.0  # Neutral score
        
        score = (humility_count - overconfidence_count) / (humility_count + overconfidence_count)
    ...
    def _generate_replacement(self, original_text: str, pattern_type: str) -> str:
        ...
        if pattern_type == 'overconfidence':
            return original_text.lower().replace('guarantee', 'aim to').replace('ensure', 'attempt to')
