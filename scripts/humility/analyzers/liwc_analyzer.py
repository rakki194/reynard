...
class LiwcAnalyzer:
    ...
    def _load_liwc_categories(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            'certainty': ['always', 'never', 'all', 'every', 'none', 'surely', 'certainly', 'definitely'],
            'tentativeness': ['maybe', 'perhaps', 'might', 'could', 'possibly', 'uncertain', 'unclear'],
            'positive_emotion': ['good', 'great', 'excellent', 'amazing', 'wonderful', 'fantastic', 'brilliant'],
            'negative_emotion': ['bad', 'terrible', 'awful', 'horrible', 'disappointing', 'frustrating'],
            'achievement': ['accomplish', 'achieve', 'success', 'win', 'victory', 'triumph', 'conquer'],
            'power': ['powerful', 'strong', 'dominant', 'control', 'authority', 'influence', 'command']
        }
    ...
    async def analyze(self, text: str, file_path: str = "") -> List[HumilityFinding]:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            first_person_count = sum(1 for word in self.liwc_categories['first_person_singular'] if word in line_lower)
            if first_person_count > 2:  # Threshold for excessive self-reference
                finding = self._create_liwc_finding(
                    file_path, line_num, line, 'excessive_first_person',
            ...
            certainty_count = sum(1 for word in self.liwc_categories['certainty'] if word in line_lower)
            if certainty_count > 1:
                finding = self._create_liwc_finding(
                    file_path, line_num, line, 'excessive_certainty',
            ...
            achievement_count = sum(1 for word in self.liwc_categories['achievement'] if word in line_lower)
            power_count = sum(1 for word in self.liwc_categories['power'] if word in line_lower)
            
            if achievement_count > 1 or power_count > 1:
                finding = self._create_liwc_finding(
                    file_path, line_num, line, 'achievement_power_language',
    ...
    def _create_liwc_finding(self, file_path: str, line_num: int, line: str, 
                           pattern_type: str, description: str, suggestion: str) -> HumilityFinding:
    ...
    def _calculate_linguistic_humility(self, category_ratios: Dict[str, float]) -> float:
        ...
        humility_score = (tentativeness * 2) - (first_person * 0.5) - (certainty * 0.5) - (achievement * 0.3) - (power * 0.3)
