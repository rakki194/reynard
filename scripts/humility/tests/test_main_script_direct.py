...
class TestMainScriptDirect:
    ...
    def test_script_with_humble_text(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as f:
            f.write("This approach aims to provide a useful solution for common problems.")
            temp_file = f.name
    ...
    def test_script_with_max_findings_limit(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as f:
            f.write("This is the best, most amazing, incredible, outstanding, remarkable solution ever!")
            temp_file = f.name
    ...
    def test_script_with_clear_cache(self):
        ...
        result = subprocess.run([
            sys.executable, "humility-detector.py", ".",
            "--clear-cache"
        ], capture_output=True, text=True, cwd=Path(__file__).parent.parent)
    ...
    def test_script_with_file_extensions_filter(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = subprocess.run([
                sys.executable, "humility-detector.py", str(temp_path),
                "--extensions", ".txt", ".py"
            ], capture_output=True, text=True, cwd=Path(__file__).parent.parent)
