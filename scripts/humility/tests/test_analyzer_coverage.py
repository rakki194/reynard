...
from analyzers import PatternAnalyzer, SentimentAnalyzer, HexacoAnalyzer, EpistemicHumilityAnalyzer, LiwcAnalyzer, TransformerAnalyzer
...
class TestSentimentAnalyzerCoverage:
    ...
    def test_sentiment_score_boundaries(self, analyzer):
        ...
        positive_score = analyzer._calculate_sentiment_score("amazing wonderful fantastic incredible")
        assert positive_score > 0
        
        # Test with very negative text
        negative_score = analyzer._calculate_sentiment_score("terrible awful horrible disgusting")
        assert negative_score < 0
        
        # Test with neutral text
        neutral_score = analyzer._calculate_sentiment_score("okay fine decent acceptable")
...
class TestHexacoAnalyzerCoverage:
    ...
    def test_hexaco_score_calculation(self, analyzer):
        ...
        arrogant_score = analyzer._calculate_hexaco_score(arrogant_text, 'honesty_humility')
        
        assert humble_score > arrogant_score
        
        # Test other traits
        for trait in ['emotionality', 'extraversion', 'agreeableness', 'conscientiousness', 'openness']:
