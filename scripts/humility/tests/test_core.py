...
from core import HumilityDetector, HumilityConfig, SeverityLevel, ConfidenceLevel, DetectionCategory
...
class TestHumilityDetector:
    ...
    async def test_analyze_text_humble(self, detector):
        ...
        assert len(profile.findings) == 0 or all(f.severity == SeverityLevel.LOW for f in profile.findings)
    ...
    async def test_analyze_text_multiple_issues(self, detector):
        ...
        boastful_words = [f.original_text.lower() for f in profile.findings]
        expected_words = ["revolutionary", "industry-leading", "award-winning", "best", "most", "innovative"]
