...
class HumilityDetector:
    ...
    def __init__(self, config: Optional[HumilityConfig] = None):
        ...
        self.hexaco_analyzer = (
            HexacoAnalyzer(self.config) if self.config.use_hexaco_assessment else None
        )
        self.epistemic_analyzer = (
            EpistemicHumilityAnalyzer(self.config)
            if self.config.use_epistemic_humility
            else None
        )
        self.liwc_analyzer = (
            LiwcAnalyzer(self.config) if self.config.use_liwc_analysis else None
        )
        self.transformer_analyzer = (
            TransformerAnalyzer(self.config)
