...
Examples:
  ...
  python3 humility-detector.py . --enable-all-features --verbose
    ...
    parser.add_argument('--cultural-context', choices=['western', 'eastern', 'nordic'],
    ...
    parser.add_argument('--cache-results', action='store_true', default=True,
    ...
    # Output report
        if args.output:
            ...
            avg_score = sum(profile.overall_score for profile in profiles.values()) / len(profiles) if profiles else 0
        ...
        # Exit with appropriate code
                if total_findings > 0:
                    logger.warning("Boastful language detected. Consider reviewing findings.")
                    return 1
            else:
                    logger.info("No boastful language detected. Great job maintaining humble communication!")
                    return 0
