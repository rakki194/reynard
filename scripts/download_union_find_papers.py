...
sys.path.append('/home/kade/runeset/reynard/services/mcp-server')
...
class UnionFindPaperDownloader:
    ...
    def __init__(self):
        ...
        self.union_find_keywords = [
            ...
            'path compression', 'disjoint-set forest', 'union find algorithm',
            'disjoint set data structure', 'connected components algorithm'
        ]
    ...
    async def download_papers(self, papers: List[Dict[str, Any]], max_papers: int = None) -> None:
        ...
        for i, paper in enumerate(papers, 1):
            try:
                ...
                logger.info(f"[{i}/{len(papers)}] Downloading: {paper['title'][:80]}...")
    ...
    def create_summary_report(self) -> None:
        ...
        report = {
            ...
            'total_papers_found': len(self.downloaded_papers) + len(self.failed_downloads),
            'successful_downloads': len(self.downloaded_papers),
            'failed_downloads': len(self.failed_downloads),
            'success_rate': len(self.downloaded_papers) / (len(self.downloaded_papers) + len(self.failed_downloads)) * 100 if (len(self.downloaded_papers) + len(self.failed_downloads)) > 0 else 0,
            'downloaded_papers': self.downloaded_papers,
            'failed_downloads': self.failed_downloads
        }
        
        # Save report
        report_path = Path('/home/kade/runeset/reynard/backend/data/papers/union_find_download_report.json')
