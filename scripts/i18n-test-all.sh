...
if [[ -f "i18n-results.json" ]]; then
    ...
    if command -v jq &> /dev/null; then
        total_packages=$(jq -r '.summary.totalPackages' i18n-results.json)
        successful_packages=$(jq -r '.summary.successfulPackages' i18n-results.json)
        failed_packages=$(jq -r '.summary.failedPackages' i18n-results.json)
        hardcoded_strings=$(jq -r '.summary.totalHardcodedStrings' i18n-results.json)
        missing_translations=$(jq -r '.summary.totalMissingTranslations' i18n-results.json)
        rtl_issues=$(jq -r '.summary.totalRTLIssues' i18n-results.json)
        duration=$(jq -r '.summary.duration' i18n-results.json)
        
        echo "   Total Packages: ${total_packages}"
        echo "   Successful: ${successful_packages}"
        echo "   Failed: ${failed_packages}"
        echo "   Hardcoded Strings: ${hardcoded_strings}"
        echo "   Missing Translations: ${missing_translations}"
        echo "   RTL Issues: ${rtl_issues}"
