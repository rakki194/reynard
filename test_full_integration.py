...
async def test_mcp_direct():
    ...
    try:
        ...
        admin_token = mcp_auth_service.generate_mcp_token(
            "test-admin-client", 
            ["mcp:admin", "mcp:tools:manage", "mcp:config:read"]
        )
        ...
        if response.get("result", {}).get("success"):
            ...
            print(f"📡 MCP server is responding correctly")
            print(f"🛠️  Found {len(tools)} tools")
            print(f"📊 Stats: {stats.get('enabled_tools', 0)}/{stats.get('total_tools', 0)} enabled")
            return True
        else:
...
async def test_backend_mcp_api():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get('http://localhost:8000/api/mcp/tool-config/') as response:
                if response.status == 200:
                    data = await response.json()
                    print("✅ Backend MCP API Test")
                    print(f"📡 Backend-MCP integration working")
                    print(f"🛠️  Retrieved {data.get('total_tools', 0)} tool configurations")
                    print(f"📊 Enabled: {data.get('enabled_tools', 0)}, Disabled: {data.get('disabled_tools', 0)}")
                    return True
                else:
...
async def test_tool_management():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get('http://localhost:8000/api/mcp/tool-config/lint_frontend') as response:
                if response.status == 200:
                    data = await response.json()
                    print("✅ Tool Management Test")
                    print(f"📡 Tool management working")
                    print(f"🔧 Tool: {data.get('name')} - {'enabled' if data.get('enabled') else 'disabled'}")
