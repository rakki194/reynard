...
function generateTestResults(results: any): string {
  ...
  return results.suites.map((suite: any) => {
    return `
### ${suite.title}
...
function generateSummary(config: any, results: any): any {
  ...
  if (results && results.suites) {
    ...
    summary.results.averageDuration = summary.results.totalTests > 0 
      ? summary.results.totalDuration / summary.results.totalTests 
      : 0;
  }
