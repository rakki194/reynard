...
test.describe("Enhanced Authentication Tests", () => {
  ...
  test.describe("Login Flow", () => {
    test("should display login form", async ({ page }) => {
      ...
      await expect(
        page.locator("#identifier, [data-testid='username-input'], [name='username']").first()
      ).toBeVisible();
    ...
    test("should validate required fields", async ({ page }) => {
      ...
      if (!isDisabled) {
        ...
        const hasValidation =
          (await page.locator(".error, .invalid, [aria-invalid='true'], [data-testid*='error']").count()) > 0;
  ...
  test.describe("Registration Flow", () => {
    test("should toggle to registration form", async ({ page }) => {
      ...
      const registerToggle = page
        .locator("button:has-text('Register'), [data-testid='show-register'], .register-toggle")
        .first();
      ...
      if (await registerToggle.isVisible()) {
        ...
        if ((await usernameField.isVisible()) || (await emailField.isVisible())) {
          await expect(
            page.locator("#password, [data-testid='password-input'], [name='password']").first()
          ).toBeVisible();
        }
      } else {
      ...
      if (await registerToggle.isVisible()) {
        ...
        if (await emailField.isVisible()) {
          ...
          if (await submitButton.isVisible()) {
            ...
            const hasEmailError =
              (await page
                .locator(".email-error, .invalid, [aria-invalid='true'], [data-testid*='email-error']")
                .count()) > 0;
    ...
    test("should be responsive on mobile", async ({ page }) => {
      ...
      if (await loginForm.isVisible()) {
        ...
        const usernameField = page
          .locator("#identifier, #username, [data-testid*='username'], [name='username']")
          .first();
  ...
  test.describe("Security Features", () => {
    ...
    test("should prevent form submission without proper data", async ({ page }) => {
      const submitButton = page
        .locator("button[type='submit'], [data-testid*='submit'], [data-testid*='login']")
        .first();
