...
async function configureServer(page, config) {
  ...
  const response = await page.evaluate(async (serverUrl, configData) => {
...
async function makeRapidApiCalls(page, count: number, delay: number = 0) {
  const results = await page.evaluate(async (serverUrl, numCalls, callDelay) => {
    const responses = [];
    ...
    return responses;
  }, MOCK_API_SERVER_URL, count, delay);
  return results;
}
