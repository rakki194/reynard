...
class I18nBundleAnalyzer {
  ...
  async analyzeBundle(page: Page, name: string): Promise<BundleAnalysis> {
      ...
      return performance
        ...
        .map((entry: any) => ({
          ...
          type: entry.name.includes(".js")
            ...
            : entry.name.includes(".css")
              ? "css"
              : entry.name.includes("i18n") || entry.name.includes("translation")
                ? "i18n"
                : "other",
        }));
    });
    ...
    const translationFiles = requests.filter(
      req => req.name.includes("translation") || req.name.includes("i18n") || req.name.includes("lang")
    ).length;
  ...
  async compareBundles(page: Page, baselineUrl: string, i18nUrl: string, approach: string): Promise<BundleComparison> {
    ...
    console.log(`\n📊 Overhead Analysis:`);
    console.log(
      `  Total Size Overhead: ${(overhead.totalSize / 1024).toFixed(2)}KB (${overhead.percentage.toFixed(1)}%)`
    );
...
test.describe("I18n Bundle Size Analysis", () => {
  ...
  test("Compare basic app vs i18n demo bundle sizes", async ({ page }) => {
    await analyzer.compareBundles(page, "/examples/basic-app", "/examples/i18n-demo", "Basic vs I18n Demo");
  });
  ...
  test("Analyze compression effectiveness", async ({ page }) => {
    ...
    console.log(
      `  Estimated uncompressed size: ${(analysis.totalSize / analysis.compressionRatio / 1024).toFixed(2)}KB`
    );
  });
