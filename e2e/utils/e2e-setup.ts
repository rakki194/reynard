...
import { setupBrowserMocks, resetBrowserMocks, mockFetch } from "reynard-testing";
...
export function setupE2ETests(config: E2ETestConfig = {}) {
  ...
  beforeAll(async () => {
    ...
    if (testConfig.debug) {
      ...
      console.log(`Backend: ${testConfig.useRealBackend ? 'Real' : 'Mock'} (port ${testConfig.backendPort})`);
...
export function captureTestArtifacts(testName: string) {
  const artifacts = {
    ...
    networkLogs: [] as Array<{ request: any; response: any; timestamp: number }>,
    consoleLogs: [] as Array<{ level: string; message: string; timestamp: number }>,
