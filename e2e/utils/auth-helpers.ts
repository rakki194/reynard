...
export class AuthTestHelpers {
  ...
  async changePassword(currentPassword: string, newPassword: string): Promise<void> {
    ...
    await this.page.fill("[data-testid='confirm-new-password-input']", newPassword);
  ...
  async updateProfile(profileData: Partial<TestUserData>): Promise<void> {
    ...
    if (profileData.fullName) {
      await this.page.fill("[data-testid='full-name-input']", profileData.fullName);
    }
   ...
   */
  async verifyErrorMessage(errorType: string, expectedMessage?: string): Promise<void> {
   ...
   */
  async verifySuccessMessage(successType: string, expectedMessage?: string): Promise<void> {
   ...
   */
  async waitForAuthStateChange(expectedState: 'authenticated' | 'unauthenticated', timeout: number = 5000): Promise<void> {
  ...
  async setAuthTokens(accessToken: string, refreshToken: string): Promise<void> {
    await this.page.evaluate(({ accessToken, refreshToken }) => {
  ...
  async makeAuthenticatedRequest(endpoint: string, options: RequestInit = {}): Promise<any> {
    return await this.page.evaluate(async ({ endpoint, options }) => {
...
export class AuthFlowScenarios {
   ...
   */
  async testPasswordChange(userData: TestUserData, newPassword: string): Promise<void> {
    await this.helpers.loginUser(userData);
   ...
   */
  async testProfileUpdate(userData: TestUserData, updates: Partial<TestUserData>): Promise<void> {
    await this.helpers.loginUser(userData);
