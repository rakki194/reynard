 ...
 */
export async function changeElementText(page: Page, selector: string, newText: string): Promise<void> {
  await page.evaluate(
    ({ sel, text }) => {
      const element = document.querySelector(sel);
 ...
 */
export async function changeElementStyle(page: Page, selector: string, property: string, value: string): Promise<void> {
  await page.evaluate(
    ({ sel, prop, val }) => {
