...
export class AuthFlowScenarios {
  private readonly coreOps: AuthCoreOperations;
  private readonly formHandlers: AuthFormHandlers;
  private readonly verification: AuthVerificationHelpers;

  constructor(coreOps: AuthCoreOperations, formHandlers: AuthFormHandlers, verification: AuthVerificationHelpers) {
   ...
   */
  async testPasswordChange(userData: ITestUserData, newPassword: string): Promise<void> {
    await this.coreOps.loginUser(userData);
   ...
   */
  async testProfileUpdate(userData: ITestUserData, updates: Partial<ITestUserData>): Promise<void> {
    await this.coreOps.loginUser(userData);
  ...
  async testConcurrentLogins(userData: ITestUserData): Promise<void> {
    const promises = Array.from({ length: 3 }, () => this.coreOps.loginUser(userData));
