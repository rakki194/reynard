...
async function main() {
  ...
  try {
    ...
    const reports = await Promise.all([
      fs
        .access(join(coverageDir, "comprehensive-report.html"))
        .then(() => true)
        .catch(() => false),
      fs
        .access(join(coverageDir, "coverage-summary.txt"))
        .then(() => true)
        .catch(() => false),
      fs
        .access(join(coverageDir, "lcov.info"))
        .then(() => true)
        .catch(() => false),
      fs
        .access(join(coverageDir, "merged-coverage.json"))
        .then(() => true)
        .catch(() => false),
    ]);
