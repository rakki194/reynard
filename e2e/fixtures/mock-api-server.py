...
def setup_logging():
    ...
    log_dir = os.path.join(os.path.dirname(__file__), '..', 'results', 'effects', 'backend-logs')
...
class MockAPIHandler(BaseHTTPRequestHandler):
    ...
    def check_rate_limit(self):
        ...
        _global_state['request_timestamps'] = [ts for ts in _global_state['request_timestamps'] if current_time - ts < _global_state['rate_limit_window']]
        cleaned_count = old_count - len(_global_state['request_timestamps'])
        
        if cleaned_count > 0:
            logger.debug(f"🧹 Cleaned {cleaned_count} old timestamps from rate limit window")
        
        # Check if we're over the limit
        if len(_global_state['request_timestamps']) >= _global_state['rate_limit_requests']:
            logger.warning(f"🚫 RATE LIMIT EXCEEDED: {len(_global_state['request_timestamps'])} requests in {_global_state['rate_limit_window']}s (limit: {_global_state['rate_limit_requests']})")
            logger.warning(f"🚫 Rate limit details: window={_global_state['rate_limit_window']}s, limit={_global_state['rate_limit_requests']}, current={len(_global_state['request_timestamps'])}")
            return False
        
        # Add current request timestamp
        _global_state['request_timestamps'].append(current_time)
        remaining = _global_state['rate_limit_requests'] - len(_global_state['request_timestamps'])
        logger.debug(f"✅ Rate limit check passed: {len(_global_state['request_timestamps'])}/{_global_state['rate_limit_requests']} requests, {remaining} remaining")
        return True
    ...
    def check_rapid_request_detection(self):
        ...
        # Clean old timestamps
        _global_state['rapid_request_timestamps'] = [ts for ts in _global_state['rapid_request_timestamps'] if current_time - ts < _global_state['rapid_request_window']]
    ...
    def check_request_pattern_detection(self, path, method):
        ...
        _global_state['request_cache'] = {k: v for k, v in _global_state['request_cache'].items() if current_time - v['last_seen'] < _global_state['cache_window']}
    ...
    def do_GET(self):
        ...
        logger.info(f"🌐 REQUEST #{_global_state['request_count']}: {self.command} {path} from {self.client_address[0]}")
        
        # Check circuit breaker first (if enabled)
        if _global_state['circuit_breaker_enabled'] and not self.check_circuit_breaker():
            logger.error(f"❌ REQUEST #{_global_state['request_count']} BLOCKED: Circuit breaker open")
        ...
        if _global_state['rate_limiting_enabled'] and not self.check_rate_limit():
            logger.error(f"❌ REQUEST #{_global_state['request_count']} BLOCKED: Rate limit exceeded")
            self.send_error(429, "Too Many Requests - Rate Limit Exceeded")
            return
        
        # Check rapid request detection (if enabled) - NEW PROTECTION
        if _global_state['rapid_request_detection_enabled'] and not self.check_rapid_request_detection():
            logger.error(f"❌ REQUEST #{_global_state['request_count']} BLOCKED: Rapid request pattern detected")
        ...
        if _global_state['request_pattern_detection_enabled'] and not self.check_request_pattern_detection(path, self.command):
            logger.error(f"❌ REQUEST #{_global_state['request_count']} BLOCKED: Identical request pattern detected")
            self.send_error(429, "Too Many Requests - Identical Request Pattern Detected")
            return
        
        logger.info(f"✅ REQUEST #{_global_state['request_count']} PASSED: All safeguards cleared")
        ...
        if path == "/api/v1/organizations":
            ...
            logger.warning(f"⚠️ REQUEST #{_global_state['request_count']} NOT FOUND: {path}")
    ...
    def handle_organizations(self):
        ...
        if _global_state['failure_simulation_enabled'] and (_global_state['request_count'] % int(1 / _global_state['failure_rate']) == 0):
            _global_state['circuit_breaker_failures'] += 1
            if _global_state['circuit_breaker_failures'] >= _global_state['circuit_breaker_threshold']:
                _global_state['circuit_breaker_reset_time'] = time.time()
                logger.warning(f"⚡ Circuit breaker triggered after {_global_state['circuit_breaker_failures']} failures")
            logger.error(f"💥 Simulated failure for request #{_global_state['request_count']} (failure rate: {_global_state['failure_rate']})")
            self.send_error(500, "Internal Server Error - Tenant Service Overloaded")
            return
        
        # Reset circuit breaker on successful request
        if _global_state['circuit_breaker_failures'] > 0:
            _global_state['circuit_breaker_failures'] = max(0, _global_state['circuit_breaker_failures'] - 1)
        ...
        response_data = {
            ...
            "timestamp": time.time(),
            "rate_limit_remaining": max(0, _global_state['rate_limit_requests'] - len(_global_state['request_timestamps'])),
            "circuit_breaker_status": "closed" if _global_state['circuit_breaker_failures'] < _global_state['circuit_breaker_threshold'] else "open"
        }
        ...
        self.send_header('X-RateLimit-Remaining', str(max(0, _global_state['rate_limit_requests'] - len(_global_state['request_timestamps']))))
        self.send_header('X-RateLimit-Reset', str(int(time.time() + _global_state['rate_limit_window'])))
        self.end_headers()
    ...
    def handle_stats(self):
        ...
        response_data = {
            ...
            "uptime": time.time() - _global_state['start_time'],
            "requests_per_second": _global_state['request_count'] / (time.time() - _global_state['start_time']) if time.time() > _global_state['start_time'] else 0
        }
    ...
    def handle_reset(self):
        ...
        response_data = {
            ...
            "reset_counters": {
                ...
                "rapid_request_timestamps": len(_global_state['rapid_request_timestamps']),
            ...
            if content_length > 0:
                ...
                try:
                    ...
                    if 'rate_limiting_enabled' in config:
                        _global_state['rate_limiting_enabled'] = config['rate_limiting_enabled']
                        logger.info(f"🔧 Rate limiting {'enabled' if _global_state['rate_limiting_enabled'] else 'disabled'}")
                    
                    if 'circuit_breaker_enabled' in config:
                        _global_state['circuit_breaker_enabled'] = config['circuit_breaker_enabled']
                        logger.info(f"🔧 Circuit breaker {'enabled' if _global_state['circuit_breaker_enabled'] else 'disabled'}")
                    
                    if 'failure_simulation_enabled' in config:
                        _global_state['failure_simulation_enabled'] = config['failure_simulation_enabled']
                        logger.info(f"🔧 Failure simulation {'enabled' if _global_state['failure_simulation_enabled'] else 'disabled'}")
                    
                    if 'failure_rate' in config:
                        _global_state['failure_rate'] = config['failure_rate']
                        logger.info(f"🔧 Failure rate set to {_global_state['failure_rate']}")
                    
                    if 'rate_limit_requests' in config:
                        _global_state['rate_limit_requests'] = config['rate_limit_requests']
                        logger.info(f"🔧 Rate limit requests set to {_global_state['rate_limit_requests']}")
                    
                    if 'circuit_breaker_threshold' in config:
                        _global_state['circuit_breaker_threshold'] = config['circuit_breaker_threshold']
                        logger.info(f"🔧 Circuit breaker threshold set to {_global_state['circuit_breaker_threshold']}")
                    
                    if 'rapid_request_detection_enabled' in config:
                        _global_state['rapid_request_detection_enabled'] = config['rapid_request_detection_enabled']
                        logger.info(f"🔧 Rapid request detection {'enabled' if _global_state['rapid_request_detection_enabled'] else 'disabled'}")
                    
                    if 'rapid_request_threshold' in config:
                        _global_state['rapid_request_threshold'] = config['rapid_request_threshold']
                        logger.info(f"🔧 Rapid request threshold set to {_global_state['rapid_request_threshold']}")
                    
                    if 'request_pattern_detection_enabled' in config:
                        _global_state['request_pattern_detection_enabled'] = config['request_pattern_detection_enabled']
                        logger.info(f"🔧 Request pattern detection {'enabled' if _global_state['request_pattern_detection_enabled'] else 'disabled'}")
                    
                    if 'identical_request_threshold' in config:
                        _global_state['identical_request_threshold'] = config['identical_request_threshold']
                        logger.info(f"🔧 Identical request threshold set to {_global_state['identical_request_threshold']}")
                    ...
                    response_data = {
                        ...
                        "configuration": {
                            ...
                            "failure_simulation_enabled": _global_state['failure_simulation_enabled'],
                            ...
                            "rapid_request_detection_enabled": _global_state['rapid_request_detection_enabled'],
                            "rapid_request_threshold": _global_state['rapid_request_threshold'],
                            "request_pattern_detection_enabled": _global_state['request_pattern_detection_enabled'],
                            "identical_request_threshold": _global_state['identical_request_threshold']
                        }
                    }
    ...
    def handle_control_status(self):
        ...
        recent_requests = [ts for ts in _global_state['request_timestamps'] if current_time - ts < _global_state['rate_limit_window']]
        rate_limit_remaining = max(0, _global_state['rate_limit_requests'] - len(recent_requests))
        
        # Calculate circuit breaker status
        circuit_breaker_open = _global_state['circuit_breaker_failures'] >= _global_state['circuit_breaker_threshold']
        circuit_breaker_reset_time_remaining = 0
        if circuit_breaker_open:
            circuit_breaker_reset_time_remaining = max(0, _global_state['circuit_breaker_timeout'] - (current_time - _global_state['circuit_breaker_reset_time']))
        ...
        response_data = {
            ...
            "rate_limiting": {
                ...
                "status": "active" if len(recent_requests) < _global_state['rate_limit_requests'] else "exceeded"
            },
            "circuit_breaker": {
            ...
            "rapid_request_detection": {
                ...
                "current_requests": len([ts for ts in _global_state['rapid_request_timestamps'] if current_time - ts < _global_state['rapid_request_window']]),
                "status": "active" if len([ts for ts in _global_state['rapid_request_timestamps'] if current_time - ts < _global_state['rapid_request_window']]) < _global_state['rapid_request_threshold'] else "triggered"
            },
...
class MockAPIServer:
    ...
    def get_stats(self):
        ...
        try:
            import requests
            response = requests.get(f"http://localhost:{self.port}/api/v1/stats", timeout=1)
