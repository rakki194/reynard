...
async def test_mcp_connection():
    ...
    try:
        ...
        print(f"📡 Connected to MCP server on localhost:8001")
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get('http://localhost:8000/docs') as response:
                if response.status == 200:
                    ...
                    print(f"📡 Connected to FastAPI backend on localhost:8000")
...
async def test_backend_mcp_integration():
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get('http://localhost:8000/mcp/tool-config/') as response:
                if response.status == 200:
                    data = await response.json()
                    print("✅ Backend-MCP Integration Test")
                    print(f"📡 Backend successfully communicated with MCP server")
                    print(f"🛠️  Retrieved {data.get('total_tools', 0)} tool configurations")
                    return True
                else:
                    print(f"❌ Backend-MCP integration failed with status {response.status}")
