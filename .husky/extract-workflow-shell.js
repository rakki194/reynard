...
class WorkflowShellExtractor {
    ...
    extractShellScripts(workflowPath) {
        this.log(`🔍 Extracting shell scripts from: ${workflowPath}`, 'blue');
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.match(/^\s*run:\s*\|/)) {
                ...
                this.log(`  📄 Found multi-line script at line ${i + 1}`, 'cyan');
                continue;
            }
            ...
            if (inScript && line.match(/^\s*[^-\s]/) && !line.match(/^\s*\|/)) {
                ...
                if (scriptContent.length > 0) {
                    ...
                    scripts.push(script);
                    this.log(`    ✅ Extracted ${scriptContent.length} lines`, 'green');
                }
                scriptContent = [];
            }
        }
...
${script.content}`;
    ...
    applyFixes(workflowPath, fixes) {
        ...
        this.log(`🔧 Applying ${fixes.length} fixes to ${workflowPath}`, 'yellow');
        ...
        for (const fix of fixes) {
            if (fix.type === 'replace') {
                ...
                if (newContent !== content) {
                    content = newContent;
                    modified = true;
                    this.log(`    ✅ Applied: ${fix.description}`, 'green');
                }
            }
        }
        ...
        if (modified) {
            ...
            this.log(`    💾 Saved fixes to ${workflowPath}`, 'green');
        }
    ...
    async processWorkflows() {
        ...
        if (workflowFiles.length === 0) {
            this.log('❌ No workflow files found', 'red');
            return { success: false, scripts: [], results: [] };
        }
        
        this.log(`📊 Found ${workflowFiles.length} workflow files`, 'blue');
    ...
    generateSummary(total, valid, invalid) {
        this.log('', 'reset');
        this.log('🎯 Summary Report', 'magenta');
        this.log('=' .repeat(30), 'magenta');
        this.log(`📊 Total shell scripts found: ${total}`, 'blue');
        this.log(`✅ Valid scripts: ${valid}`, 'green');
        this.log(`❌ Invalid scripts: ${invalid}`, invalid > 0 ? 'red' : 'green');
        ...
        if (invalid > 0) {
            ...
            this.log('', 'reset');
            this.log('🎉 All workflow shell scripts are valid!', 'green');
        }
    }
}
