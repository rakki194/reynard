...
class MarkdownLinkValidator {
  ...
  async validateAll() {
    ...
    printColored(
      `📁 Found ${markdownFiles.length} markdown files to validate`,
      Colors.BLUE,
    );
    ...
    for (const filePath of markdownFiles) {
      ...
      if (fileResult.valid) {
        ...
        allValid = false;
        printColored(
          `  ❌ ${fileResult.errors.length} broken links found`,
          Colors.RED,
        );
      ...
      if (fileResult.warnings.length > 0) {
        printColored(
          `  ⚠️  ${fileResult.warnings.length} warnings`,
          Colors.YELLOW,
        );
  ...
  printSummary() {
    ...
    if (this.stats.brokenLinks > 0) {
      ...
      printColored(
        '  - Run "node .husky/validate-markdown-links.js --fix" to auto-fix some issues',
        Colors.YELLOW,
      );
    }
  }
  ...
  getStagedMarkdownFiles() {
    try {
      ...
      printColored(
        `❌ Failed to get staged files: ${error.message}`,
        Colors.RED,
      );
      return [];
    }
  ...
  async validateStaged() {
    ...
    printColored(
      `🦊 Validating ${stagedFiles.length} staged markdown file(s):`,
      Colors.PURPLE,
    );
    ...
    for (const file of stagedFiles) {
      ...
      if (!fileResult.valid) {
        allValid = false;
        printColored(
          `❌ ${file}: ${fileResult.errors.length} broken links found`,
          Colors.RED,
        );
...
async function main() {
  ...
  if (args.includes("--help") || args.includes("-h")) {
    ...
    printColored(
      "  --staged     Validate only staged files (for pre-commit)",
      Colors.CYAN,
    );
    printColored(
...
export { MarkdownLinkValidator, LinkValidationResult, FileValidationResult };
