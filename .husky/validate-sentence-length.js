...
function findOptimalBreakPoints(line, maxLength) {
    ...
    const prepositionPattern = /\s+(in|on|at|by|for|with|from|to|of|about|under|over|through|during|before|after|above|below|between|among|within|without|against|toward|towards|into|onto|upon|beneath|beyond|beside|besides|except|including|regarding|concerning|considering|given|provided|assuming|supposing)\s+/gi;
    ...
    const relativePattern = /\s+(which|that|who|whom|whose|where|when|why|how|what|whoever|whomever|whatever|whichever|wherever|whenever|however)\s+/gi;
 ...
 */
function validateSentenceLengths(files = null, maxLength = 120) {
    ...
    for (const file of filesToCheck) {
        ...
        if (!result.valid) {
            ...
            for (const violation of result.violations) {
                ...
                printColored(`   Content: ${violation.content.substring(0, 80)}${violation.content.length > 80 ? '...' : ''}`, Colors.CYAN);
    ...
    if (!allValid) {
        ...
        printColored('   - Run "node .husky/validate-sentence-length.js --fix" to auto-fix', Colors.YELLOW);
 ...
 * @param {number} maxLength - Maximum line length
 */
function autoFixSentenceLengths(files = null, maxLength = 120) {
    const filesToCheck = files || getStagedMarkdownFiles();
...
function main() {
    ...
    if (args.includes('--fix')) {
        ...
        return autoFixSentenceLengths(files.length > 0 ? files : null, maxLength) ? 0 : 1;
    } else {
        const files = args.filter(arg => !arg.startsWith('--') && arg !== maxLength.toString());
        return validateSentenceLengths(files.length > 0 ? files : null, maxLength) ? 0 : 1;
    }
}
