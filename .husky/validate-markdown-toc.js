  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^##\s+(table\s+of\s+contents|toc|contents?)\s*$/i)) {
      ...
      for (let j = i + 1; j < lines.length; j++) {
        ...
        if (
          nextLine.match(/^#+\s+/) &&
          !nextLine.match(/^#+\s+(table\s+of\s+contents|toc|contents?)\s*$/i)
        ) {
          tocEnd = j - 1;
          break;
        }
        ...
        if (
          ...
          lines[j + 1].match(/^#+\s+/) &&
          !lines[j + 1].match(
            /^#+\s+(table\s+of\s+contents|toc|contents?)\s*$/i,
          )
        ) {
          tocEnd = j - 1;
          break;
  ...
  try {
    ...
    if (!existingToC.found) {
      return {
        ...
        suggestion:
          'Add a "## Table of Contents" section after the first H2 heading',
      };
    }
...
function getStagedMarkdownFiles() {
  try {
    ...
    const markdownFiles = allFiles.filter((file) => {
      return (
        file.endsWith(".md") && targetDirs.some((dir) => file.startsWith(dir))
      );
    });
...
function validateMarkdownToC(files = null) {
  ...
  for (const file of filesToCheck) {
    ...
    if (!result.valid) {
      ...
      printColored(
        `✅ ${file}: ToC is valid (${result.tocLineCount} lines)`,
        Colors.GREEN,
      );
    }
  }
  ...
  if (!allValid) {
    printColored("\n💡 Tips:", Colors.YELLOW);
    printColored(
      '   - Add "## Table of Contents" after your first H2 heading',
      Colors.YELLOW,
    );
    printColored(
      "   - Update ToC when adding/removing/renaming headings",
      Colors.YELLOW,
    );
    printColored(
      "   - Use consistent heading hierarchy (H2, H3, etc.)",
      Colors.YELLOW,
    );
    printColored(
      '   - Run "node .husky/validate-markdown-toc.js --fix" to auto-fix some issues',
      Colors.YELLOW,
    );
    printColored(
...
function autoFixToC(files = null) {
  ...
  printColored(
    `🦊 Auto-fixing ToC in ${filesToCheck.length} markdown file(s):`,
    Colors.PURPLE,
  );
  ...
  for (const file of filesToCheck) {
    try {
      ...
      if (!firstH2) {
        printColored(
          `⚠️  ${file}: No H2 heading found, skipping`,
          Colors.YELLOW,
        );
        continue;
      }
...
function main() {
  ...
  if (args.includes("--help") || args.includes("-h")) {
    ...
    printColored(
      "  --fix        Auto-fix ToC issues where possible",
      Colors.CYAN,
    );
