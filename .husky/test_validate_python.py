...
class TestGetTypedFiles:
    ...
    def test_get_typed_files_without_definitions(self):
        ...
        try:
            files = _get_typed_files([temp_file])
            assert temp_file not in files  # Should be excluded (no type-able content)
        finally:
            os.unlink(temp_file)
...
class TestCheckFileLengths:
    ...
    def test_check_file_lengths_test_file_under_limit(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='test_.py', delete=False) as f:
            f.write(content)
            temp_file = f.name
...
class TestValidatePythonFiles:
    ...
    def test_validate_python_files_all_passes(self, mock_lengths, mock_linting, 
                                            mock_imports, mock_formatting):
    ...
    @patch('validate_python._check_linting')
    @patch('validate_python._check_file_lengths')
    def test_validate_python_files_formatting_fails(self, mock_lengths, mock_linting,
                                                   mock_imports, mock_formatting):
    ...
    @patch('validate_python._check_linting')
    @patch('validate_python._check_file_lengths')
    def test_validate_python_files_imports_fail(self, mock_lengths, mock_linting,
                                              mock_imports, mock_formatting):
    ...
    @patch('validate_python._check_linting')
    @patch('validate_python._check_file_lengths')
    def test_validate_python_files_linting_fails(self, mock_lengths, mock_linting,
                                                mock_imports, mock_formatting):
    ...
    @patch('validate_python._check_linting')
    @patch('validate_python._check_file_lengths')
    def test_validate_python_files_lengths_fail(self, mock_lengths, mock_linting,
                                              mock_imports, mock_formatting):
...
''')
    ...
    def test_file_length_counting_accuracy(self):
        ...
        lines = []
        for i in range(125):  # 125 functions × 2 lines each = 250 lines
        ...
        try:
            ...
            result = _check_file_lengths([temp_file])
            # Should fail because we now have 252 lines of code
            assert result is False
