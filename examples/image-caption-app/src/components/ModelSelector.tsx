...
export const ModelSelector: Component<ModelSelectorProps> = (props) => {
  ...
  const loadModels = async () => {
    ...
    try {
      ...
      const models: ModelInfo[] = Object.values(generators).map((gen: any) => ({
        name: gen.name,
        description: gen.description,
        captionType: gen.caption_type,
        isAvailable: gen.is_available,
        isLoaded: gen.is_loaded,
      }));
      setAvailableModels(models);
    } catch (error) {
      notify("Failed to load models from backend", "error");
  ...
  const preloadModel = async (modelName: string) => {
    try {
      ...
      notify(`Model ${modelName} loaded successfully`, "success");
      loadModels(); // Refresh model status
    } catch (error) {
      notify(`Failed to load model ${modelName}`, "error");
      console.error("Model loading failed:", error);
    }
  };
