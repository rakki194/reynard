...
export default function LanguageSelector() {
  ...
  const availableLanguages = languages.filter((lang: { code: LanguageCode; name: string }) =>
  ...
  return (
    ...
    >
      <For each={availableLanguages}>{lang => <option value={lang.code}>🌐 {lang.name}</option>}</For>
    </select>
  );
}
