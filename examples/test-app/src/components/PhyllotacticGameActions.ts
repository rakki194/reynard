...
export function createGameActions(
  ...
  spiralLogic: { updateConfig: (config: GameConfig) => void; initializeSpiral: () => any } | undefined,
  ...
  const updateConfig = (newConfig: Partial<GameConfig>) => {
    ...
    if (spiralLogic) {
      ...
      console.log("🦊 PhyllotacticGameActions: Generated new spiral points", { count: newPoints.length });
