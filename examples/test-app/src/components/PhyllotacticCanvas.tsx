...
export const PhyllotacticCanvas: Component<PhyllotacticCanvasProps> = (props) => {
  ...
  const renderSpiral = () => {
    if (!ctx || !canvasRef) {
      console.log("🦊 PhyllotacticCanvas: renderSpiral skipped - no context or canvas", { hasCtx: !!ctx, hasCanvas: !!canvasRef });
      return;
    }

    console.log("🦊 PhyllotacticCanvas: Rendering spiral", { pointsCount: props.spiralPoints.length });
    ...
    points.forEach((point, index) => {
      ...
      if (index < 3) { // Log first few points for debugging
        console.log(`🦊 PhyllotacticCanvas: Point ${index}`, { x: point.x, y: point.y, color: point.color });
      }
    });
  ...
  onMount(() => {
    ...
    if (canvasRef) {
      ...
      console.log("🦊 PhyllotacticCanvas: Canvas setup complete", { width: canvasRef.width, height: canvasRef.height });
      
      animationEngine = createAnimationEngine(props.animationConfig || {
        frameRate: 60,
        maxFPS: 120,
        enableVSync: true,
  ...
  createEffect(() => {
    ...
    console.log("🦊 PhyllotacticCanvas: isRunning changed", { isRunning, hasAnimationEngine: !!animationEngine });
  ...
  createEffect(() => {
    const points = props.spiralPoints;
    const isRunning = props.isRunning;
    console.log("🦊 PhyllotacticCanvas: spiralPoints changed", { pointsCount: points.length, isRunning });
