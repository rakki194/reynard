        ...
        if (section.content && Array.isArray(section.content)) {
          for (const [contentIndex, content] of section.content.entries()) {
            ...
            for (const field of contentRequiredFields) {
              if (!(field in content)) {
                errors.push(`Section ${index} (${section.id}) content ${contentIndex} is missing required field "${field}"`);
              }
            }
            ...
            if (content.type && !validTypes.includes(content.type)) {
              errors.push(`Section ${index} (${section.id}) content ${contentIndex} has invalid type "${content.type}"`);
            }
          }
        }
    ...
    const validateEstimatedTimes = (tutorialData: any[]) => {
      ...
      for (const [index, section] of tutorialData.entries()) {
        if (section.estimatedTime) {
          ...
          if (!timeMatch) {
            errors.push(`Section ${index} (${section.id}) has invalid estimated time format: "${section.estimatedTime}"`);
          } else {
