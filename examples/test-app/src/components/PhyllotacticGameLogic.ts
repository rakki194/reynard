...
export function createPhyllotacticGameLogic() {
  ...
  console.log("🦊 PhyllotacticGameLogic: Signals created", { isRunning: isRunning(), showSpellCaster: showSpellCaster() });
  ...
  const initializeSpiralLogic = () => {
    ...
    animationFunctions = createAnimationFunctions(
      isRunning, setIsRunning, currentAngle, setCurrentAngle, 
      config, spiralPoints, setSpiralPoints, spiralLogic
    );
  };
  ...
  const updateAnimationConfig = (newConfig: Partial<typeof animationConfig>) => {
    console.log("🦊 PhyllotacticGameLogic: Updating animation config", { oldConfig: animationConfig(), newConfig });
  ...
  return {
    ...
    get updateSpiral() { return animationFunctions.updateSpiral; },
    get toggleAnimation() { return animationFunctions.toggleAnimation; },
    get applyPattern() { return gameActions.applyPattern; },
    get updateConfig() { return gameActions.updateConfig; },
