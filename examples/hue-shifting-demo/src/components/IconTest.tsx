    ...
    <div style="padding: 20px; border: 2px solid red; margin: 20px; background: #fff;">
      ...
      <div style="display: flex; gap: 20px; align-items: center; flex-wrap: wrap;">
        <div style="border: 2px solid blue; padding: 10px;">
          <h4>Settings Icon:</h4>
          <div style="width: 48px; height: 48px; border: 2px solid green; background: #f0f0f0;" innerHTML={allIcons.settings?.svg || 'MISSING'}></div>
          <p style="font-size: 12px; margin: 5px 0;">Length: {allIcons.settings?.svg?.length || 0}</p>
        </div>
        
        <div style="border: 2px solid blue; padding: 10px;">
          <h4>User Icon:</h4>
          <div style="width: 48px; height: 48px; border: 2px solid green; background: #f0f0f0;" innerHTML={allIcons.user?.svg || 'MISSING'}></div>
          <p style="font-size: 12px; margin: 5px 0;">Length: {allIcons.user?.svg?.length || 0}</p>
        </div>
        
        <div style="border: 2px solid blue; padding: 10px;">
          <h4>Tag Icon:</h4>
          <div style="width: 48px; height: 48px; border: 2px solid green; background: #f0f0f0;" innerHTML={allIcons.tag?.svg || 'MISSING'}></div>
          <p style="font-size: 12px; margin: 5px 0;">Length: {allIcons.tag?.svg?.length || 0}</p>
        </div>
        
        <div style="border: 2px solid blue; padding: 10px;">
          <h4>Box Icon:</h4>
          <div style="width: 48px; height: 48px; border: 2px solid green; background: #f0f0f0;" innerHTML={allIcons.box?.svg || 'MISSING'}></div>
