    ...
    def format(self, record):
        ...
        if hasattr(record, 'levelname'):
            ...
            if 'OK' in message or '[OK]' in message:
                ...
                message = message.replace('[OK]', f'{self.COLORS["OK"]}[OK]{self.COLORS["RESET"]}')
            ...
            if 'FAIL' in message or '[FAIL]' in message:
                ...
                message = message.replace('[FAIL]', f'{self.COLORS["FAIL"]}[FAIL]{self.COLORS["RESET"]}')
            ...
            if level in self.COLORS:
                message = message.replace(level, f'{self.COLORS[level]}{level}{self.COLORS["RESET"]}')
...
class LoggingConfig:
    ...
    def __init__(self):
        ...
        self.log_format = os.getenv("LOG_FORMAT", "detailed")  # default, access, detailed, json
        self.log_to_file = os.getenv("LOG_TO_FILE", "true").lower() == "true"
        self.log_file_path = os.getenv("LOG_FILE_PATH", "logs/reynard-backend.log")
        self.log_error_file_path = os.getenv("LOG_ERROR_FILE_PATH", "logs/reynard-errors.log")
        self.log_max_bytes = int(os.getenv("LOG_MAX_BYTES", "10485760"))  # 10MB
        self.log_backup_count = int(os.getenv("LOG_BACKUP_COUNT", "5"))
        self.use_yaml_config = os.getenv("USE_YAML_LOG_CONFIG", "true").lower() == "true"
    ...
    def setup_logging(self) -> None:
        ...
        try:
            if self.use_yaml_config and Path(self.yaml_config_path).exists():
                ...
                logging.config.dictConfig(config)
                print(f"[INFO] Logging configured programmatically with level: {self.log_level}")
    ...
    def _override_yaml_config(self, config: Dict[str, Any]) -> None:
        ...
        for logger_name in ["uvicorn.error", "uvicorn.access", "main", "services", "routes", "database"]:
