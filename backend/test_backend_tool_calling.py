        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    print(f"   Tool calls in text: {'✅ Yes' if tool_analysis['has_tool_calls'] else '❌ No'}")
                    ...
                    print(f"   Tool calls in response: {'✅ Yes' if tool_calls else '❌ No'}")
                    if tool_calls:
...
async def main():
    ...
    print(f"   Chat endpoint tool calls: {'✅ Working' if chat_tool_calls else '❌ Not working'}")
    print(f"   Assistant endpoint tool calls: {'✅ Working' if assistant_tool_calls else '❌ Not working'}")
    
    if chat_tool_calls or assistant_tool_calls:
        print("🎉 Tool calling is working! The model is successfully using tools.")
    else:
        print("⚠️  Tool calling may not be working properly. The model is showing thinking but not making tool calls.")
