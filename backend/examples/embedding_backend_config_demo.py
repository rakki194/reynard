...
async def demo_backend_configuration():
    ...
    print(f"Mock mode: {config.mock_mode}")
    print("In mock mode, all backends are effectively disabled and mock embeddings are returned.")
    ...
    with patch_env(test_env):
        ...
        print(f"  Sentence Transformers enabled: {env_config.backends['sentence_transformers'].enabled}")
    ...
    print(f"  Enabled backends: {stats.get('backend_config', {}).get('enabled_backends', [])}")
    ...
    print("3. Use EMBEDDING_SENTENCE_TRANSFORMERS_ENABLED=false to disable Sentence Transformers")
