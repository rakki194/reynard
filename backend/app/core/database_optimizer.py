...
class DatabasePerformanceMonitor:
    ...
    def get_slow_queries(self, limit: int = 10) -> List[QueryMetrics]:
        ...
        return sorted(
            [q for q in self.query_metrics if q.execution_time_ms > self.slow_query_threshold_ms],
    ...
    def get_query_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_hit_rate": len([q for q in successful_queries if q.cache_hit]) / len(successful_queries) * 100
        }
...
class OptimizedDatabaseConnection:
    ...
    def __init__(
        ...
        self.monitor = DatabasePerformanceMonitor(database_url) if enable_monitoring else None
    ...
    async def optimize_connection_pool(self) -> Dict[str, Any]:
        ...
        if pool_status["overflow"] > 0:
            recommendations.append("Increase max_overflow or pool_size - overflow detected")
        
        # Check for invalid connections
        if pool_status["invalid"] > 0:
            recommendations.append("Investigate invalid connections - possible connection issues")
...
class DatabaseOptimizationSuite:
    ...
    async def run_full_optimization_analysis(self) -> DatabaseOptimizationReport:
        ...
        key_tables = [
            "agents", "agent_traits", "agent_interactions", 
            "search_results", "embeddings", "documents"
        ]
        ...
        if self.connection.monitor:
            ...
            if stats.get("cache_hit_rate", 0) < 80:
                cache_recommendations.append("Implement query result caching - low cache hit rate")
            if stats.get("average_execution_time_ms", 0) > 500:
                cache_recommendations.append("Implement connection-level caching for slow queries")
        
        # Generate performance improvements
        if slow_queries:
            performance_improvements.append(f"Optimize {len(slow_queries)} slow queries identified")
        
        if connection_pool_issues:
            performance_improvements.append("Optimize connection pool configuration")
        
        if missing_indexes:
            performance_improvements.append(f"Add {len(missing_indexes)} recommended indexes")
...
async def main():
    ...
    async with DatabaseOptimizationSuite(database_url) as optimizer:
        ...
        # Save report to file
        report_file = Path(__file__).parent / f"database_optimization_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
