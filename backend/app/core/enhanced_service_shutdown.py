...
class ServiceShutdownManager:
    ...
    async def shutdown_all_services(self) -> bool:
        ...
        try:
            ...
            if not shutdown_success:
                logger.warning("⚠️ Graceful shutdown failed, initiating force shutdown...")
        ...
        try:
            ...
            for service_name in shutdown_order:
                if service_name in self.registry._services:
                    ...
                    if not success:
                        logger.warning(f"⚠️ Graceful shutdown failed for service '{service_name}'")
            
            # Use registry's shutdown method
            registry_success = await self.registry.shutdown_all(self.shutdown_timeout)
            
            self.shutdown_progress["graceful_shutdown"] = "completed" if registry_success else "failed"
    ...
    async def _shutdown_service_gracefully(self, service_name: str) -> bool:
        ...
        try:
            ...
            logger.error(f"❌ Graceful shutdown failed for service '{service_name}': {e}")
    ...
    async def _service_specific_shutdown(self, service_name: str, service_instance: Any) -> None:
        """Perform service-specific shutdown procedures."""
        try:
            if service_name == "gatekeeper":
                ...
                logger.info(f"No specific shutdown procedure for service '{service_name}'")
...
_shutdown_manager: Optional[ServiceShutdownManager] = None
