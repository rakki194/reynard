...
class EnhancedServiceRegistry(ServiceRegistry):
    ...
    def register_service(
        ...
        logger.info(f"Enhanced registration of service '{name}' with {len(dependencies or [])} dependencies")
    ...
    async def initialize_all(self, timeout: float = 30.0) -> bool:
        ...
        logger.info("Starting enhanced service initialization with dependency resolution...")
        ...
        try:
            ...
            for service_name in startup_order:
                if service_name in self._services:
                    ...
                    if not success:
                        logger.error(f"Failed to initialize service '{service_name}', stopping initialization")
                        return False
    ...
    def _resolve_startup_order(self) -> List[str]:
        ...
        remaining_deps = {name: deps.copy() for name, deps in self._dependencies.items()}
        startup_order = []
        
        while remaining_deps:
            # Find services with no remaining dependencies
            ready_services = [
                name for name, deps in remaining_deps.items()
                if not deps or all(dep.name in startup_order for dep in deps)
            ]
            
            if not ready_services:
                # Circular dependency detected
                logger.error(f"Circular dependency detected in services: {list(remaining_deps.keys())}")
                break
        ...
        for dependency in dependencies:
            if dependency.dependency_type == ServiceDependencyType.REQUIRED:
                if not await self._is_dependency_healthy(dependency.name):
                    logger.error(f"Required dependency '{dependency.name}' is not healthy for service '{service_name}'")
                    return False
    ...
    def _resolve_shutdown_order(self) -> List[str]:
        ...
        while remaining_services:
            ...
            ready_services = [
                name for name in remaining_services
                if not self._dependents[name] or all(dep in shutdown_order for dep in self._dependents[name])
            ]
    ...
    async def _monitor_service_health(self, service_name: str) -> None:
        ...
        while self._health_check_running and service_info.status == ServiceStatus.RUNNING:
            try:
                ...
                if health_info.consecutive_failures > 0:
                    logger.warning(f"Service '{service_name}' health check failed {health_info.consecutive_failures} times")
    ...
    def discover_services(self) -> Dict[str, Dict[str, Any]]:
        ...
        for name in self._services:
            services[name] = {
                ...
                "health": self._health_info.get(name, {}).get("status", ServiceHealthStatus.UNKNOWN).value,
    ...
    def get_registry_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "healthy_services": len([h for h in self._health_info.values() if h.status == ServiceHealthStatus.HEALTHY]),
