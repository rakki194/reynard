...
class HealthCheckRouter(BaseServiceRouter):
    ...
    def _setup_health_endpoints(self) -> None:
        ...
        @self.router.get("/dependencies/{service_name}", summary="Service Dependencies Health")
    ...
    async def _get_health_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_metrics": {
                ...
                "cache_hit_rate": len(self.health_manager.health_cache) / len(all_results) if all_results else 0
            },
            "service_metrics": {
                name: {
                    "response_time": result.response_time,
    ...
    async def _get_predictive_alerts(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_alerts": sum(len(service_alerts) for service_alerts in alerts.values()),
            "services_with_alerts": len([s for s in alerts.values() if s]),
            "alert_summary": {
                "degrading_trends": len([a for service_alerts in alerts.values() for a in service_alerts if "degrading trend" in a]),
                "performance_issues": len([a for service_alerts in alerts.values() for a in service_alerts if "response time" in a])
            }
        }
    ...
    async def _get_dependencies_health(self, service_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "healthy_dependencies": len([d for d in dependencies_info if d["status"] == "healthy"]),
            "degraded_dependencies": len([d for d in dependencies_info if d["status"] == "degraded"]),
            "unhealthy_dependencies": len([d for d in dependencies_info if d["status"] == "unhealthy"])
        }
    ...
    async def _get_health_system_status(self) -> Dict[str, Any]:
        ...
        return {
            "system_status": "running" if self.health_manager.health_check_tasks else "stopped",
            "configuration": {
            ...
            "history_status": {
                ...
                "total_history_entries": sum(len(history) for history in self.health_manager.health_history.values())
            }
        }


# Global health check router instance
_health_check_router: Optional[HealthCheckRouter] = None
...
def create_health_check_router(prefix: str = "/health", tags: list[str] | None = None) -> HealthCheckRouter:
