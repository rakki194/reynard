...
class PenetrationTestingCoordinator:
    ...
    def activate_penetration_testing(self, session_id: str, timeout_minutes: int = 30) -> bool:
        """
    ...
    def _write_state_file(self):
        ...
        try:
            ...
            with open(self.state_file, 'w') as f:
                ...
                f.write(f"START_TIME={self.start_time.isoformat() if self.start_time else ''}\n")
...
_penetration_coordinator: Optional[PenetrationTestingCoordinator] = None
...
class PenetrationTestingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        elif request.url.path == "/api/penetration-testing/deactivate":
            return await self._handle_deactivate(request)
        elif request.url.path == "/api/penetration-testing/status":
        ...
        if self.coordinator.is_penetration_testing_active():
            ...
            response.headers["X-PT-Remaining"] = str(session_info.get("remaining_seconds", 0))
    ...
    async def _handle_activate(self, request: Request) -> Response:
        ...
        try:
            ...
            success = self.coordinator.activate_penetration_testing(session_id, timeout_minutes)
            ...
            return Response(f"Error activating penetration testing: {str(e)}", status_code=500)
    ...
    async def _handle_deactivate(self, request: Request) -> Response:
        ...
        try:
            ...
            if success:
                ...
                return Response("Penetration testing mode not active or invalid session", status_code=404)
        
        except Exception as e:
            return Response(f"Error deactivating penetration testing: {str(e)}", status_code=500)
