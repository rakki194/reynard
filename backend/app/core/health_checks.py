...
class HealthCheckManager:
    ...
    async def perform_health_check(self, service_name: str) -> HealthCheckResult:
        ...
        try:
            ...
            basic_result = await self._perform_basic_health_check(service_name, service_info)
            
            # Perform dependency health check
            dependency_result = await self._perform_dependency_health_check(service_name)
            
            # Perform performance health check
            performance_result = await self._perform_performance_health_check(service_name)
            ...
            return HealthCheckResult(
                ...
                errors=self._collect_errors([basic_result, dependency_result, performance_result]),
                warnings=self._collect_warnings([basic_result, dependency_result, performance_result]),
    ...
    async def _perform_basic_health_check(self, service_name: str, service_info: Any) -> Dict[str, Any]:
        """Perform basic health check."""
    ...
    async def _perform_dependency_health_check(self, service_name: str) -> dict[str, Any]:
        """Perform dependency health check."""
        ...
        try:
            ...
            unhealthy_deps, degraded_deps = self._analyze_dependency_health(dependencies)
            
            # Build dependency results
            self._build_dependency_results(result, dependencies, unhealthy_deps, degraded_deps)
            
            # Determine overall dependency status
            result["status"] = self._determine_dependency_status(dependencies, unhealthy_deps, degraded_deps)
    ...
    def _analyze_dependency_health(self, dependencies: list) -> tuple[list[str], list[str]]:
    ...
    def _build_dependency_results(self, result: dict, dependencies: list, unhealthy_deps: list[str], degraded_deps: list[str]) -> None:
    ...
    def _determine_dependency_status(self, dependencies: list, unhealthy_deps: list[str], degraded_deps: list[str]) -> HealthStatus:
        """Determine overall dependency health status."""
        if unhealthy_deps:
            if any(dep.dependency_type.value == "required" for dep in dependencies if dep.name in unhealthy_deps):
    ...
    async def _perform_performance_health_check(self, service_name: str) -> Dict[str, Any]:
        """Perform performance health check."""
        ...
        try:
            ...
            if metrics:
                ...
                if metrics.average_response_time > 5.0:
                    result["status"] = HealthStatus.DEGRADED
                    result["details"]["slow_response_time"] = metrics.average_response_time
    ...
    async def _aggregate_health_results(self) -> None:
        ...
        healthy_services = len([r for r in self.health_results.values() if r.status == HealthStatus.HEALTHY])
        degraded_services = len([r for r in self.health_results.values() if r.status == HealthStatus.DEGRADED])
        unhealthy_services = len([r for r in self.health_results.values() if r.status == HealthStatus.UNHEALTHY])
        
        # Calculate average response time
        avg_response_time = sum(r.response_time for r in self.health_results.values()) / total_services if total_services > 0 else 0
        ...
        logger.debug(f"Health aggregation completed: {system_status.value} ({healthy_services}/{total_services} healthy)")
