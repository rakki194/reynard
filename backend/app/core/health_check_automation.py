...
class HealthCheckAutomation:
    ...
    def __init__(self, health_manager: Optional[HealthCheckManager] = None):
    ...
    def _setup_default_rules(self) -> None:
        ...
        self.add_rule(AutomationRule(
            ...
            actions=[AutomationAction.RESTART_SERVICE, AutomationAction.NOTIFY_ADMIN],
            cooldown=300.0
        ))
        ...
        self.add_rule(AutomationRule(
            ...
            conditions=["cpu_usage < 0.2", "memory_usage < 0.3", "response_time < 1.0"],
    ...
    def _setup_action_handlers(self) -> None:
        ...
        self.action_handlers[AutomationAction.RESTART_SERVICE] = self._handle_restart_service
        ...
        self.action_handlers[AutomationAction.SWITCH_TO_BACKUP] = self._handle_switch_to_backup
        self.action_handlers[AutomationAction.ISOLATE_SERVICE] = self._handle_isolate_service
    ...
    async def start_automation(self) -> None:
        ...
        self.automation_tasks["alert_monitor"] = asyncio.create_task(self._alert_monitoring_loop())
        
        # Start rule evaluation
        self.automation_tasks["rule_evaluator"] = asyncio.create_task(self._rule_evaluation_loop())
        
        # Start trend analysis
        self.automation_tasks["trend_analyzer"] = asyncio.create_task(self._trend_analysis_loop())
    ...
    async def stop_automation(self) -> None:
        ...
        if self.automation_tasks:
            await asyncio.gather(*self.automation_tasks.values(), return_exceptions=True)
    ...
    async def _rule_evaluation_loop(self) -> None:
        ...
        while True:
            try:
                ...
                await asyncio.sleep(self.check_interval * 2)  # Check rules less frequently
            except asyncio.CancelledError:
                break
            except Exception as e:
    ...
    async def _trend_analysis_loop(self) -> None:
        ...
        while True:
            try:
                ...
                await asyncio.sleep(self.check_interval * 5)  # Analyze trends less frequently
            except asyncio.CancelledError:
                break
            except Exception as e:
        ...
        for alert_id, alert in self.alerts.items():
            ...
            for service_name, health_result in all_health.items():
                ...
                if self._evaluate_alert_condition(alert, health_result):
                    await self._trigger_alert(alert_id, alert, service_name, health_result)
    ...
    async def _trigger_alert(self, alert_id: str, alert: HealthAlert, service_name: str, health_result: Any) -> None:
    ...
    def _evaluate_rule_conditions(self, rule: AutomationRule, all_health: Dict[str, Any]) -> bool:
    ...
    def _evaluate_condition(self, condition: str, all_health: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if "status == 'unhealthy'" in condition:
                return any(h.status == HealthStatus.UNHEALTHY for h in all_health.values())
            elif "error_count > 5" in condition:
                return any(len(h.errors) > 5 for h in all_health.values())
            elif "response_time > 3.0" in condition:
                return any(h.response_time > 3.0 for h in all_health.values())
            elif "memory_usage > 0.7" in condition:
                return any(h.metrics.get("memory_usage", 0) > 0.7 for h in all_health.values())
            elif "cpu_usage < 0.2" in condition:
                return all(h.metrics.get("cpu_usage", 0) < 0.2 for h in all_health.values())
            elif "memory_usage < 0.3" in condition:
                return all(h.metrics.get("memory_usage", 0) < 0.3 for h in all_health.values())
    ...
    async def _execute_rule(self, rule_id: str, rule: AutomationRule, all_health: Dict[str, Any]) -> None:
        ...
        logger.info(f"🤖 Rule executed: {rule.name} (execution #{rule.execution_count})")
        
        # Execute all actions
        for action in rule.actions:
            await self._execute_action(action, "*", None)  # Global action

    async def _execute_action(self, action: AutomationAction, service_name: str, health_result: Any) -> None:
    ...
    async def _handle_restart_service(self, service_name: str, health_result: Any) -> None:
    ...
    async def _handle_switch_to_backup(self, service_name: str, health_result: Any) -> None:
        """Handle switch to backup action."""
        logger.info(f"🔄 Switching to backup for service: {service_name}")

    async def _handle_isolate_service(self, service_name: str, health_result: Any) -> None:
    ...
    async def _analyze_health_trends(self) -> None:
        ...
        try:
            ...
            for service_name, history in health_history.items():
                ...
                if degraded_count >= 5:
                    logger.warning(f"📊 Trend analysis: Service {service_name} showing degrading trend")
                ...
                if len(recent_response_times) >= 5:
                    ...
                    if avg_late > avg_early * 1.5:
                        logger.warning(f"📊 Trend analysis: Service {service_name} response time increasing")
    ...
    def get_automation_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "alerts": {
                ...
                "triggered": len([a for a in self.alerts.values() if a.trigger_count > 0])
            },
            "rules": {
                "total": len(self.rules),
                "enabled": len([r for r in self.rules.values() if r.enabled]),
                "executed": len([r for r in self.rules.values() if r.execution_count > 0])
            },
            "history": {
...
_automation: Optional[HealthCheckAutomation] = None
