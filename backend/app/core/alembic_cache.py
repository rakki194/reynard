...
class AlembicCacheManager:
    ...
    async def cache_migration_metadata(self, revision: str, metadata: Dict[str, Any]) -> None:
        ...
        try:
            if self.cache_manager:
                ...
                self.fallback_cache[cache_key] = {
                    'data': metadata,
                    'expires': datetime.now() + timedelta(seconds=self.migration_metadata_ttl)
                }
        except Exception as e:
            logger.error(f"Failed to cache migration metadata for {revision}: {e}")
    
    async def get_cached_migration_metadata(self, revision: str) -> Optional[Dict[str, Any]]:
    ...
    async def cache_migration_data(self, revision: str, data: List[Dict[str, Any]]) -> None:
        ...
        try:
            if self.cache_manager:
                ...
                self.fallback_cache[cache_key] = {
                    'data': data,
                    'expires': datetime.now() + timedelta(seconds=self.migration_data_ttl)
                }
        except Exception as e:
    ...
    async def cache_schema_info(self, table_name: str, schema_info: Dict[str, Any]) -> None:
        ...
        try:
            if self.cache_manager:
                ...
                self.fallback_cache[cache_key] = {
                    'data': schema_info,
                    'expires': datetime.now() + timedelta(seconds=self.schema_cache_ttl)
                }
        except Exception as e:
