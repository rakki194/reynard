...
class ServiceLoadBalancer:
    ...
    def __init__(self, config: Optional[LoadBalancerConfig] = None):
        ...
        logger.info(f"ServiceLoadBalancer initialized with strategy: {self.config.strategy.value}")
    ...
    def add_service_instance(
        ...
        logger.info(f"Added instance {instance_id} for service {service_name} at {endpoint}")
    ...
    def remove_service_instance(self, service_name: str, instance_id: str) -> None:
        ...
        for i, instance in enumerate(instances):
            if instance.id == instance_id:
                ...
                logger.info(f"Removed instance {instance_id} for service {service_name}")
                break

    def get_service_instance(self, service_name: str, session_id: Optional[str] = None) -> Optional[ServiceInstance]:
        ...
        healthy_instances = [
            ...
            if instance.status in [ServiceInstanceStatus.HEALTHY, ServiceInstanceStatus.DEGRADED]
        ]
        ...
        selected_instance = self._select_instance(service_name, healthy_instances, session_id)
    ...
    def _health_based_selection(self, instances: List[ServiceInstance]) -> ServiceInstance:
        ...
        healthy_instances = [i for i in instances if i.status == ServiceInstanceStatus.HEALTHY]
    ...
    def _ip_hash_selection(self, instances: List[ServiceInstance], session_id: Optional[str] = None) -> ServiceInstance:
            ...
            if instance.id == instance_id:
                ...
                if total_requests > 0:
                    instance.response_time_avg = (
                        (instance.response_time_avg * (total_requests - 1) + response_time) / total_requests
                    )
    ...
    def get_service_metrics(self, service_name: str) -> Optional[Dict[str, Any]]:
        ...
        if service_name in self.service_instances:
            ...
            metrics["total_active_connections"] = sum(i.active_connections for i in instances)
...
_load_balancer: Optional[ServiceLoadBalancer] = None
