...
class IntelligentCacheManager:
    ...
    def _update_cache_key_metadata(self, full_key: str, namespace: str, ttl: Optional[int] = None):
    ...
    async def get(
        ...
        try:
            ...
            self.metrics.average_get_time_ms = sum(self.operation_times[-100:]) / min(len(self.operation_times), 100)
    ...
    async def set(
        ...
        try:
            ...
            operation_time = (time.time() - start_time) * 1000
            self.operation_times.append(operation_time)
            self.metrics.average_set_time_ms = sum(self.operation_times[-100:]) / min(len(self.operation_times), 100)
    ...
    async def clear_namespace(self, namespace: str) -> int:
        ...
        try:
            ...
            if keys:
                ...
                # Remove from metadata
                keys_to_remove = [k for k in self.cache_keys.keys() if k.startswith(f"{namespace}:")]
    ...
    async def get_metrics(self) -> CacheMetrics:
        ...
        if self.metrics.total_requests > 0:
            ...
            self.metrics.miss_rate = (self.metrics.misses / self.metrics.total_requests) * 100
        ...
        while True:
            try:
                ...
                for key, metadata in self.cache_keys.items():
                    if metadata.ttl and (current_time - metadata.created_at).seconds > metadata.ttl:
                ...
                if expired_keys:
                    logger.info(f"Cleaned up {len(expired_keys)} expired cache keys from metadata")
...
class CacheOptimizationAnalyzer:
    ...
    async def analyze_cache_performance(self) -> Dict[str, Any]:
        ...
        if metrics.hit_rate < 80:
            analysis["recommendations"].append("Improve cache hit rate - consider longer TTL or better key strategies")
        
        if metrics.average_get_time_ms > 5:
            analysis["recommendations"].append("Optimize cache response times - check Redis configuration")
        
        if metrics.memory_usage_bytes > 1024 * 1024 * 1024:  # > 1GB
            analysis["recommendations"].append("Reduce memory usage - implement cache eviction policies")
        
        if metrics.key_count > 100000:
            analysis["recommendations"].append("Too many cache keys - consider namespace cleanup")
