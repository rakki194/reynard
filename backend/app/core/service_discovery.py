...
class ServiceDiscovery:
    ...
    def __init__(self, service_directories: Optional[List[str]] = None):
        ...
        logger.info(f"ServiceDiscovery initialized with directories: {self.service_directories}")
    ...
    async def _discover_services_in_directory(self, directory: str) -> Dict[str, Dict[str, Any]]:
    ...
    async def _analyze_service_file(self, file_path: Path) -> Optional[Dict[str, Any]]:
    ...
    def _extract_service_name(self, file_path: Path, module: Any) -> str:
        ...
        elif file_name.endswith("_router"):
            return file_name[:-7]  # Remove "_router"
        elif file_name.endswith("_endpoints"):
    ...
    def _extract_endpoints(self, module: Any) -> List[Dict[str, Any]]:
        ...
        for name, obj in inspect.getmembers(module):
            if inspect.isfunction(obj) and hasattr(obj, "__annotations__"):
                ...
                if any(param.annotation.__name__ == "Request" for param in inspect.signature(obj).parameters.values()):
                    endpoint_info = {
                        "name": name,
                        "function": obj,
            ...
            for dep_name, dep_config in deps_config.items():
                if isinstance(dep_config, dict):
                    dependency = ServiceDependency(
                        name=dep_name,
                        dependency_type=ServiceDependencyType(dep_config.get("type", "required")),
    ...
    def _extract_health_check(self, module: Any) -> Optional[callable]:
    ...
    async def _register_discovered_services(self) -> None:
        ...
        for service_name, service_info in self.discovered_services.items():
            try:
                ...
                logger.error(f"Error registering discovered service {service_name}: {e}")
