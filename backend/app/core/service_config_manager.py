...
class ServiceConfigManager:
    ...
    def __init__(self, config_dir: Optional[str] = None):
        ...
        logger.info(f"ServiceConfigManager initialized for environment: {self.environment}")
    ...
    def _load_default_configurations(self) -> None:
        ...
        default_configs = {
            ...
            "search": {
                "enabled": True,
                "rag_backend_url": os.getenv("RAG_BACKEND_URL", "http://localhost:8002"),
                "cache_enabled": True,
                "cache_ttl": 300,
                "max_results": 100,
            },
    ...
    def _apply_environment_overrides(self, service_name: str, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _validate_configuration(self, service_name: str, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _validate_with_base_model(self, service_name: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.config_validation_level == ConfigValidationLevel.STRICT:
                raise
            elif self.config_validation_level == ConfigValidationLevel.WARN:
                logger.warning("Configuration validation warning for %s: %s", service_name, e)
            return config

    def _validate_with_schema(self, service_name: str, config: Dict[str, Any]) -> Dict[str, Any]:
        """Validate configuration using service schema."""
        schema = self._schemas[service_name]
        validated_config = {}
        
        # Validate required fields
        validated_config.update(self._validate_required_fields(service_name, config, schema))
        
        # Validate optional fields
        validated_config.update(self._validate_optional_fields(service_name, config, schema))
    ...
    def _validate_required_fields(
        ...
        for field in schema.required_fields:
            if field not in config:
                error_msg = f"Required field '{field}' missing for service '{service_name}'"
                if self.config_validation_level == ConfigValidationLevel.STRICT:
                ...
                validated[field] = self._validate_field(service_name, field, config[field], schema)
        return validated
    ...
    def _validate_field(
        ...
        for rule_name, rule_value in rules.items():
            if not self._apply_validation_rule(field_name, value, rule_name, rule_value):
    ...
    def _apply_validation_rule(self, field_name: str, value: Any, rule_name: str, rule_value: Any) -> bool:
        """Apply a validation rule to a field value."""
        if rule_name == "min" and isinstance(value, (int, float)):
            return value >= rule_value
        elif rule_name == "max" and isinstance(value, (int, float)):
            return value <= rule_value
        elif rule_name == "min_length" and isinstance(value, str):
    ...
    def update_service_config(
        ...
        try:
            ...
            if validate:
                current_config = self._validate_configuration(service_name, current_config)
            
            # Update configuration
            self._configurations[service_name] = current_config
            self._metadata[service_name].last_updated = 0.0  # Will be set to current time
    ...
    def save_service_config(self, service_name: str, config_file: Optional[str] = None) -> bool:
        ...
        try:
            if not config_file:
                config_file = self.config_dir / f"{service_name}_{self.environment}.json"
            
            with open(config_file, 'w') as f:
                json.dump(self._configurations[service_name], f, indent=2)
            
            logger.info(f"Saved configuration for service '{service_name}' to {config_file}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to save configuration for service '{service_name}': {e}")
            return False
    ...
    def _notify_config_watchers(self, service_name: str, config: Dict[str, Any]) -> None:
        ...
        if service_name in self._watchers:
            for callback in self._watchers[service_name]:
                try:
                    ...
                    logger.error(f"Error in config watcher for service '{service_name}': {e}")

    def get_all_configurations(self) -> Dict[str, Dict[str, Any]]:
        """Get all service configurations."""
        return self._configurations.copy()

    def get_config_metadata(self, service_name: str) -> Optional[ConfigMetadata]:
    ...
    def reload_service_config(self, service_name: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to reload configuration for service '{service_name}': {e}")
            return False
        ...
        try:
            ...
            for service_name, config in import_data["configurations"].items():
                ...
                self._metadata[service_name] = ConfigMetadata(
                    ...
                    version=import_data.get("metadata", {}).get(service_name, {}).get("version", "1.0.0"),
                )
