...
class TTSService:
    ...
    def __init__(self):
        ...
        self._kokoro_unload_timer: Optional[float] = None
        self._kokoro_unload_delay: int = 300  # 5 minutes in powersave mode
        self._kokoro_prewarm_threshold: int = 10  # Prewarm after 10 requests in normal mode
        self._kokoro_request_count: int = 0
        self._kokoro_last_used: Optional[float] = None
    ...
    async def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self._audio_dir = Path(config.get("tts_audio_dir", "generated/audio")).resolve()
            ...
            self._backend_health_check_interval = config.get("tts_backend_health_check_interval", 300)
            self._backend_timeout_seconds = config.get("tts_backend_timeout_seconds", 30)
            self._enable_voice_compatibility_checks = config.get("tts_enable_voice_compatibility_checks", True)
            self._enable_language_validation = config.get("tts_enable_language_validation", True)
    ...
    async def synthesize_text(
        ...
        try:
            logger.debug(f"Starting TTS synthesis: text_length={len(text)}, backend={backend}, voice={voice}")
            ...
            # Update metrics
            self._update_metrics(backend or self._backend, voice, processing_time, False)
    ...
    async def synthesize_batch(
        ...
        try:
            ...
            return {
                ...
                "error": None if successful_count == len(texts) else f"{len(texts) - successful_count} failed"
            }
    ...
    async def _handle_kokoro_usage(self):
        ...
        if self._kokoro_mode == "normal" and self._kokoro_request_count >= self._kokoro_prewarm_threshold:
    ...
    async def _check_backend_health(self, backend: str) -> bool:
        ...
        if current_time - self._last_backend_health_check < self._backend_health_check_interval:
        ...
        try:
            ...
            if backend_service and hasattr(backend_service, 'health_check'):
                ...
                self._backend_health_status[backend] = True  # Assume healthy if no health check
    ...
    def _update_metrics(self, backend: str, voice: str, processing_time: float, success: bool):
        ...
        self._metrics["backend_usage"][backend] = self._metrics["backend_usage"].get(backend, 0) + 1
        
        # Update voice usage
        self._metrics["voice_usage"][voice] = self._metrics["voice_usage"].get(voice, 0) + 1
    ...
    async def get_available_backends(self) -> List[Dict[str, Any]]:
        ...
        for name, service in self._backend_services.items():
            try:
                ...
                voices = await service.get_available_voices() if hasattr(service, 'get_available_voices') else []
                languages = await service.get_supported_languages() if hasattr(service, 'get_supported_languages') else ["en"]
                formats = await service.get_supported_formats() if hasattr(service, 'get_supported_formats') else ["wav"]
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        if self._metrics["total_requests"] > 0:
            error_rate = (self._metrics["failed_requests"] / self._metrics["total_requests"]) * 100
