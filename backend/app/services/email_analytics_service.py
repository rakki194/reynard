...
class EmailAnalyticsService:
    ...
    async def get_email_metrics(
        ...
        try:
            # Get email data
            emails_data = await self._get_emails_data(period_start, period_end, agent_id)
            
            if not emails_data:
                return self._create_empty_metrics()
            
            # Calculate metrics
            metrics = await self._calculate_metrics(emails_data, period_start, period_end)
    ...
    async def generate_report(
        ...
        try:
            ...
            charts_data = await self._generate_charts_data(period_start, period_end, agent_id)
    ...
    async def get_agent_performance(
        ...
        try:
            ...
            agent_emails = [email for email in emails_data if 
                          email.get('from_agent') == agent_id or email.get('to_agent') == agent_id]
            ...
            return {
                ...
                'sent_emails': len([e for e in agent_emails if e.get('from_agent') == agent_id]),
                'received_emails': len([e for e in agent_emails if e.get('to_agent') == agent_id]),
                'avg_response_time_hours': avg_response_time,
                'most_active_hour': most_active_hour,
                'most_active_day': most_active_day,
                'activity_by_hour': dict(activity_by_hour),
                'activity_by_day': dict(activity_by_day),
                'avg_email_length': statistics.mean([len(body) for body in bodies if body]) if bodies else 0,
        ...
        try:
            ...
            for date_str in sorted(daily_data.keys()):
                ...
                if metric == "volume":
                    ...
                    response_times = [1.0 for email in day_emails if email.get('status') == 'replied']
                    value = statistics.mean(response_times) if response_times else 0
                elif metric == "agent_activity":
                    value = len([e for e in day_emails if e.get('is_agent_email', False)])
                else:
                    value = 0
    ...
    async def _calculate_metrics(
        ...
        processed_emails = len([e for e in emails_data if e.get('status') == 'processed'])
        ...
        top_senders = [{'email': email, 'count': count} for email, count in sender_counts.most_common(10)]
        top_recipients = [{'email': email, 'count': count} for email, count in recipient_counts.most_common(10)]
        ...
        content_analysis = {
            'common_subjects': self._get_common_phrases(subjects),
            'common_content': self._get_common_phrases(bodies),
            'avg_subject_length': statistics.mean([len(s) for s in subjects if s]) if subjects else 0,
        ...
        performance_metrics = {
            ...
            'agent_engagement': (agent_emails / total_emails * 100) if total_emails > 0 else 0,
        }
    ...
    def _get_common_phrases(self, texts: List[str], min_length: int = 3) -> List[Dict[str, Any]]:
        ...
        return [{'phrase': word, 'count': count} for word, count in word_counts.most_common(20)]
    ...
    async def _analyze_volume_trends(self, emails_data: List[Dict[str, Any]]) -> List[EmailInsight]:
        ...
        if len(volumes) >= 3:
            ...
            if recent_avg > overall_avg * 1.2:
                insights.append(EmailInsight(
                    ...
                    actionable=True,
                    suggested_actions=["Review email processing capacity", "Consider automation for high-volume periods"]
                ))
        
        return insights
    
    async def _analyze_response_times(self, emails_data: List[Dict[str, Any]]) -> List[EmailInsight]:
        ...
        if response_times:
            ...
            if avg_response_time > 24:  # More than 24 hours
                insights.append(EmailInsight(
                    ...
                    timestamp=datetime.now(),
                    actionable=True,
                    suggested_actions=["Implement auto-reply system", "Set up email alerts", "Review response processes"]
                ))
        
        return insights
    
    async def _analyze_content_patterns(self, emails_data: List[Dict[str, Any]]) -> List[EmailInsight]:
        ...
        if subjects:
            ...
            if common_words:
                ...
                if top_word['count'] > len(subjects) * 0.3:  # Appears in >30% of subjects
                    insights.append(EmailInsight(
    ...
    async def _analyze_agent_activity(self, emails_data: List[Dict[str, Any]]) -> List[EmailInsight]:
        ...
        if total_emails > 0:
            ...
            if agent_percentage > 50:
                insights.append(EmailInsight(
                    ...
                    severity="low",
                    confidence=0.8,
                    data={"agent_percentage": agent_percentage, "agent_emails": len(agent_emails)},
                    timestamp=datetime.now(),
                    actionable=False,
                    suggested_actions=[]
                ))
    ...
    async def _detect_anomalies(self, emails_data: List[Dict[str, Any]]) -> List[EmailInsight]:
            ...
            for date, volume in daily_volumes.items():
                if std_volume > 0 and abs(volume - mean_volume) > 2 * std_volume:
                    insights.append(EmailInsight(
                        ...
                        severity="medium",
                        confidence=0.8,
                        data={"date": date, "volume": volume, "mean_volume": mean_volume},
                        timestamp=datetime.now(),
                        actionable=True,
                        suggested_actions=["Investigate cause of volume spike", "Check for system issues"]
                    ))
    ...
    async def _generate_recommendations(
        ...
        if metrics.avg_response_time_hours > 24:
            recommendations.append("Consider implementing auto-reply system to improve response times")
        
        if metrics.unread_emails > metrics.total_emails * 0.3:
            recommendations.append("High number of unread emails - consider email prioritization system")
        
        if metrics.performance_metrics.get('response_rate', 0) < 50:
            recommendations.append("Low response rate - review email handling processes")
