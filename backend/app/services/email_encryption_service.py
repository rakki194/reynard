...
class EmailEncryptionService:
    ...
    def __init__(self, config: Optional[EncryptionConfig] = None, data_dir: str = "data/email_encryption"):
        ...
        if PGP_AVAILABLE and self.config.pgp_enabled:
            ...
            logger.info(f"PGP encryption available - using gpg with homedir: {self.gpg_homedir}")
        else:
        ...
        if not SMIME_AVAILABLE and self.config.smime_enabled:
            logger.warning("SMIME encryption not available - cryptography not installed")
...
"""
            ...
            result = self._run_gpg_command(['--gen-key', '--batch'], input_data=key_input.encode())
            ...
            private_result = self._run_gpg_command(['--armor', '--export-secret-keys', fingerprint])
    ...
    async def import_pgp_key(self, key_data: str) -> EncryptionKey:
        ...
        try:
            ...
            if keys:
                user_id = keys[0]['uids'][0] if keys[0]['uids'] else f"Unknown <{fingerprint}>"
    ...
    async def sign_email(
        ...
        try:
            ...
            if signing_key.key_type == "pgp":
                ...
                raise ValueError(f"Unsupported key type for signing: {signing_key.key_type}")
    ...
    async def get_public_key(self, email: str, encryption_method: str = "pgp") -> Optional[str]:
        """
    ...
    def _run_gpg_command(self, args: List[str], input_data: Optional[bytes] = None, 
                        passphrase: Optional[str] = None) -> subprocess.CompletedProcess:
        ...
        try:
            ...
            logger.error(f"GPG command failed: {e.stderr.decode() if e.stderr else 'Unknown error'}")
            raise
        ...
        try:
            ...
            for line in lines:
                if line.startswith('pub:') or line.startswith('sec:'):
                    ...
                    current_key = {'type': 'secret' if line.startswith('sec:') else 'public'}
    ...
    async def _find_recipient_key(self, email: str, encryption_method: str) -> Optional[EncryptionKey]:
        """Find encryption key for recipient email."""
        for key in self.keys.values():
            if (key.email == email and 
                key.key_type == encryption_method and 
                not key.is_revoked):
                return key
        return None
    ...
    async def _decrypt_with_pgp(self, encrypted_content: str, passphrase: Optional[str] = None) -> str:
    ...
    async def _decrypt_with_smime(self, encrypted_content: str, passphrase: Optional[str] = None) -> str:
        ...
        try:
            ...
            # For now, return a placeholder
            if encrypted_content.startswith("[SMIME-ENCRYPTED]") and encrypted_content.endswith("[/SMIME-ENCRYPTED]"):
    ...
    async def _sign_with_pgp(self, content: str, signing_key: EncryptionKey, passphrase: Optional[str] = None) -> str:
        """Sign content with PGP."""
        if not PGP_AVAILABLE or self.gpg is None:
            raise ValueError("PGP encryption not available")
        
        try:
            signed_data = self.gpg.sign(content, keyid=signing_key.fingerprint, passphrase=passphrase)
    ...
    async def _sign_with_smime(self, content: str, signing_key: EncryptionKey, passphrase: Optional[str] = None) -> str:
    ...
    async def _verify_smime_signature(self, signed_content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # For now, return a placeholder
            if signed_content.startswith("[SMIME-SIGNED]") and signed_content.endswith("[/SMIME-SIGNED]"):
                return {
                    "valid": True,
