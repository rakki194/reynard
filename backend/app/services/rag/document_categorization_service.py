...
class DocumentCategorizationService:
    ...
    async def batch_categorize_papers(
        self, papers_directory: Path, output_file: Optional[Path] = None
    ...
    async def _save_categorization_results(
        self, results: Dict[str, DocumentCategory], output_file: Path
    ) -> None:
    ...
    def get_domain_statistics(
        self, results: Dict[str, DocumentCategory]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_average_confidence": (
                sum(avg_confidence.values()) / len(avg_confidence)
                if avg_confidence
                else 0
            ),
        }
