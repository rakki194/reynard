...
class RAGService:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"RAG service initialized successfully in {self.startup_time:.2f}s")
            return True
    ...
    async def _initialize_core_services(self) -> None:
        ...
        self.search_engine = SearchEngine(self.embedding_service, self.vector_store_service)
    ...
    async def _initialize_advanced_services(self) -> None:
        ...
        if self.config.get("rag_model_evaluation_enabled", True):
            self.model_evaluator = ModelEvaluator(self.embedding_service, self.vector_store_service)
        ...
        try:
            ...
            if self.performance_monitor:
                ...
                await self.performance_monitor.record_metric(
                    ...
                    latency_ms,
                    tags={"model": model, "batch_size": len(texts), "status": "success"}
                )
            else:
                embeddings = await self.embedding_service.embed_batch(texts, model)
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check core services
            health_status["services"]["embedding_service"] = await self.embedding_service.health_check()
            health_status["services"]["vector_store"] = await self.vector_store_service.health_check()
            health_status["services"]["document_indexer"] = await self.document_indexer.health_check()
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.embedding_service:
                stats["core_services"]["embedding_service"] = await self.embedding_service.get_stats()
            
            if self.vector_store_service:
                stats["core_services"]["vector_store"] = await self.vector_store_service.get_stats()
            
            if self.document_indexer:
                stats["core_services"]["document_indexer"] = await self.document_indexer.get_stats()
            
            if self.search_engine:
                stats["core_services"]["search_engine"] = self.search_engine.get_search_stats()
            
            # Get advanced service stats
            if self.performance_monitor:
                stats["advanced_services"]["performance_monitor"] = self.performance_monitor.get_monitor_stats()
            
            if self.security_service:
                stats["advanced_services"]["security_service"] = self.security_service.get_security_stats()
            
            if self.continuous_improvement:
                stats["advanced_services"]["continuous_improvement"] = self.continuous_improvement.get_continuous_improvement_stats()
            
            if self.documentation_service:
                stats["advanced_services"]["documentation_service"] = self.documentation_service.get_documentation_stats()
            
            if self.model_evaluator:
                stats["advanced_services"]["model_evaluator"] = self.model_evaluator.get_evaluation_stats()
