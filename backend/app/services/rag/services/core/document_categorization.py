...
class DocumentCategorizationService(BaseService):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.papers_directory = Path(
            self.config.get("papers_directory", str(get_papers_directory()))
        )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            self.update_status(
                ...
                "Initializing document categorization service",
            )
            ...
            self.update_status(
                ServiceStatus.HEALTHY, "Document categorization service initialized"
            )
            return True

        except Exception as e:
            self.logger.error(
                f"Failed to initialize document categorization service: {e}"
            )
        ...
        try:
            ...
            for category in arxiv_categories:
                if category in self.arxiv_category_mapping:
                    ...
                    arxiv_domain_scores[domain] = (
                        arxiv_domain_scores.get(domain, 0) + 2
                    )  # Higher weight for arXiv categories

            # Combine scores
            combined_scores = {}
            for domain in ScientificDomain:
                combined_scores[domain] = domain_scores.get(
                    domain, 0
            ...
            domain_tags = self._generate_domain_tags(
            ...
            self.metrics["domain_distribution"][primary_domain.value] = (
                self.metrics["domain_distribution"].get(primary_domain.value, 0) + 1
            )
            ...
            self.metrics["average_confidence"] = (
                current_avg * (total_docs - 1) + confidence
            ) / total_docs
    ...
    def _generate_domain_tags(
        ...
        tags.extend(
            matched_keywords.get(primary_domain, [])[:5]
    ...
    async def batch_categorize_papers(
        self, papers_directory: Path, max_papers: Optional[int] = None
        ...
        for metadata_file in metadata_files:
            try:
                ...
                if category:
                    ...
                    self.logger.info(
                        f"Categorized {paper_id}: {category.primary_domain.value} (confidence: {category.confidence:.2f})"
                    )
                else:
                ...
                with open(cache_file, 'r') as f:
                    ...
                    for key, data in cache_data.items():
                        self._cache[key] = DocumentCategory(
                            ...
                            secondary_domains=[
                                ScientificDomain(d) for d in data["secondary_domains"]
                            ],
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            "status": (
                "healthy" if self.status == ServiceStatus.HEALTHY else "unhealthy"
            ),
