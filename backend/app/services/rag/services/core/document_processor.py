...
from ...interfaces.document import (
    ChunkMetadata,
    Document,
    DocumentChunk,
    DocumentProcessor,
...
class ASTDocumentProcessor(IDocumentIndexer):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.max_chunk_size = self.config.get(
            "max_chunk_size", 1000
        )  # Reduced for embedding models
    ...
    async def initialize(self) -> bool:
        ...
        try:
            self.update_status(
                ServiceStatus.INITIALIZING, "Initializing AST document processor"
            )
            ...
            self.update_status(
                ServiceStatus.HEALTHY, "AST document processor initialized"
            )
            return True
    ...
    async def shutdown(self) -> None:
        ...
        try:
            self.update_status(
                ServiceStatus.SHUTTING_DOWN, "Shutting down AST document processor"
            )
            ...
            self.update_status(
                ServiceStatus.SHUTDOWN, "AST document processor shutdown complete"
            )
    ...
    async def process_document(
        ...
        try:
            ...
            if language in self.supported_languages:
                chunks = await self._process_structured_document(
    ...
    async def extract_metadata(self, document: Document) -> Dict[str, Any]:
        ...
        if language in self.language_patterns:
            ...
            function_count = len(
                re.findall(patterns["function"], document.content, re.MULTILINE)
            )
            class_count = len(
                re.findall(patterns["class"], document.content, re.MULTILINE)
            )
            import_count = len(
                re.findall(patterns["import"], document.content, re.MULTILINE)
            )
    ...
    async def _process_structured_document(
        self, document: Document, language: str, metadata: Dict[str, Any]
    ) -> List[DocumentChunk]:
        ...
        for i, line in enumerate(lines):
            ...
            if len('\n'.join(current_chunk_lines)) > self.max_chunk_size:
                ...
                overlap_lines = (
                    current_chunk_lines[-self.overlap_size :]
        ...
        for line in lines:
            ...
            if len('\n'.join(current_lines)) > self.max_chunk_size:
                ...
                if len(current_lines) > 1:
                    ...
                    # Start new chunk with overlap
                    overlap_lines = (
                        current_lines[-self.overlap_size :]
                        if self.overlap_size > 0
                        else []
                    )
                    current_lines = overlap_lines + [
                        line
                    ]  # Include the line that caused overflow
                else:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "average_chunks_per_document": stats.get(
    ...
    async def process_document(
        ...
        try:
            ...
            logger.debug(
                f"Processed document {document.id} into {len(chunks)} chunks in {processing_time:.2f}ms"
            )
            return chunks
