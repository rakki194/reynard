...
class PaperIndexingIntegration(BaseService):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            self.update_status(
                ServiceStatus.INITIALIZING, "Initializing paper indexing integration"
            )
            ...
            self.update_status(
                ServiceStatus.HEALTHY, "Paper indexing integration initialized"
            )
            return True
    ...
    async def process_paper_for_rag(
        ...
        if rag_metadata_path.exists() and not force_reprocess:
            try:
                ...
                self.logger.warning(
                    f"Failed to load existing RAG metadata for {paper_id}: {e}"
                )
        ...
        try:
            ...
            if self.auto_categorize and self.categorization_service:
                try:
                    categorization = await self.categorization_service.categorize_paper_from_metadata(
                        metadata
                    )
                    if categorization:
            ...
            if categorization:
                rag_metadata["categorization"] = {
                    ...
                    "secondary_domains": [
                        d.value for d in categorization.secondary_domains
                    ],
            ...
            domain = (
                categorization.primary_domain.value
                if categorization
                else 'uncategorized'
            )
            confidence = categorization.confidence if categorization else 0.0
            self.logger.info(
                f"Processed paper {paper_id} for RAG: {domain} (confidence: {confidence:.2f})"
            )
    ...
    def _generate_search_keywords(
        self, metadata: Dict[str, Any], categorization: DocumentCategory
    ) -> List[str]:
    ...
    async def batch_process_papers(
        ...
        self.logger.info(
            f"Processing {len(metadata_files)} papers for RAG indexing with categorization"
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if self.categorization_service:
            stats["categorization_service"] = (
                self.categorization_service.get_statistics()
            )
