...
class PostgreSQLVectorStore(BaseService, VectorStore):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            self.update_status(
                ServiceStatus.INITIALIZING, "Initializing PostgreSQL vector store"
            )
            ...
            if not await self._test_connection():
                self.update_status(
                    ServiceStatus.ERROR, "Failed to connect to PostgreSQL"
                )
                return False
            ...
            if not await self._initialize_schema():
                self.update_status(
                    ServiceStatus.ERROR, "Failed to initialize database schema"
                )
                return False

            self.update_status(
                ServiceStatus.HEALTHY, "PostgreSQL vector store initialized"
            )
            return True
    ...
    async def shutdown(self) -> None:
        ...
        try:
            self.update_status(
                ServiceStatus.SHUTTING_DOWN, "Shutting down PostgreSQL vector store"
            )
            ...
            self.update_status(
                ServiceStatus.SHUTDOWN, "PostgreSQL vector store shutdown complete"
            )
    ...
    async def similarity_search(
        ...
        **kwargs,
        ...
        if DEBUG_RAG_OPERATIONS:
            logger.info(
                f"🦊 [VECTOR_STORE] Starting similarity search with limit: {limit}, threshold: {similarity_threshold}"
            )
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                if DEBUG_RAG_OPERATIONS:
                    logger.info(
                        f"🦊 [VECTOR_STORE] Executing query with {len(params)} parameters"
                    )
                ...
                if DEBUG_RAG_OPERATIONS:
                    logger.info(
                        f"🦊 [VECTOR_STORE] Search completed in {search_time:.2f}ms, found {len(results)} results"
                    )
    ...
    async def search_by_metadata(
        self, filters: Dict[str, Any], limit: int = 10, **kwargs
    ) -> List[VectorDocument]:
    ...
    async def store_documents(self, documents: List[VectorDocument]) -> List[str]:
        ...
        for doc in documents:
            ...
            try:
                ...
                logger.error(
                    f"🚨 [VECTOR_STORE] Failed to store document {doc.id}: {e}"
                )
                raise
        ...
        try:
            async with self.pool.acquire() as conn:
                for doc_id in document_ids:
                    await conn.execute(
                        "DELETE FROM document_embeddings WHERE file_id = $1", doc_id
                    )
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            async with self.pool.acquire() as conn:
                # Get total documents (count distinct metadata->>'source' or id)
                total_docs = await conn.fetchval(
                    f"SELECT COUNT(DISTINCT COALESCE(metadata->>'source', id::text)) FROM {self.table_name}"
                )
