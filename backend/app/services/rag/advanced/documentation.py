...
""",
    ...
    async def generate_user_documentation(self) -> str:
        ...
        doc.append("## Overview")
        doc.append("")
        doc.append("The RAG (Retrieval-Augmented Generation) system provides advanced semantic search and embedding capabilities for code and documents.")
        doc.append("")
    ...
    async def generate_api_reference(self) -> str:
        ...
        methods = [
            ...
            ("embed_batch(texts, model)", "Generate embeddings for multiple texts", "List[List[float]]"),
            ("search(query, search_type, limit, filters)", "Perform search", "List[Dict]"),
    ...
    async def generate_developer_guide(self) -> str:
        ...
        doc.append("")
        doc.append("The RAG system follows a modular architecture with the following components:")
        doc.append("")
        ...
        doc.append("- **Document Indexer**: Intelligent document processing and chunking")
        doc.append("- **Search Engine**: Advanced search with semantic and keyword matching")
        ...
        issues = [
            {
                ...
                "causes": ["Ollama service overload", "Network issues", "Model loading"],
                "solutions": ["Check Ollama service health", "Reduce concurrent requests", "Use caching"]
            },
            {
                "issue": "Low search recall",
                "symptoms": "Search results are not relevant",
                "causes": ["Poor chunking", "Inadequate embeddings", "Wrong similarity threshold"],
                "solutions": ["Improve chunking strategy", "Use hybrid search", "Tune similarity threshold"]
            },
            {
                "issue": "Memory usage high",
                "symptoms": "System running out of memory",
                "causes": ["Large embedding cache", "Too many concurrent requests", "Memory leaks"],
                "solutions": ["Reduce cache size", "Limit concurrency", "Check for memory leaks"]
            }
        ]
    ...
    def get_documentation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "api_examples_count": sum(len(examples) for examples in self.api_examples.values()),
