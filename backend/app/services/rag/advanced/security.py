...
class SecurityService:
    ...
    def _initialize_default_policies(self) -> None:
        ...
        default_policies = [
            ...
            SecurityPolicy(
                ...
                audit_required=True,
                retention_days=90,
                allowed_operations=[OperationType.READ, OperationType.SEARCH, OperationType.EMBED],
    ...
    async def check_access_permission(self, 
        ...
        try:
            ...
            if operation not in policy.allowed_operations:
                logger.warning(f"Operation {operation} not allowed for access level {access_level}")
                return False
            
            # Check user access
            if not self._check_user_access(user_id, policy):
                logger.warning(f"User {user_id} not authorized for access level {access_level}")
                return False
    ...
    async def _check_suspicious_activity(self, audit_log: AuditLog) -> None:
        ...
        try:
            ...
            if len(recent_failures) >= 5:
                logger.warning(f"Suspicious activity detected: {len(recent_failures)} failed access attempts by {audit_log.user_id}")
            ...
            if len(recent_access) >= 100:  # More than 100 operations in an hour
                logger.warning(f"High activity detected: {len(recent_access)} operations by {audit_log.user_id} in the last hour")
    ...
    async def _handle_suspicious_activity(self, user_id: str, failed_attempts: List[AuditLog]) -> None:
        ...
        logger.warning(f"Handling suspicious activity for user {user_id}: {len(failed_attempts)} failed attempts")
    ...
    def get_security_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_logs_24h": len([log for log in self.audit_logs 
