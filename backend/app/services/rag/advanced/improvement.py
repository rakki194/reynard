...
class ContinuousImprovement:
    ...
    async def analyze_experiment(self, experiment_id: str) -> Dict[str, Any]:
        ...
        if len(control_data) < experiment.minimum_sample_size or len(treatment_data) < experiment.minimum_sample_size:
        ...
        significant_improvements = sum(1 for r in analysis_results.values() if r["is_significant"])
    ...
    async def _record_improvement(self, experiment: Experiment, analysis: Dict[str, Any]) -> None:
        """Record successful improvement."""
        improvement_record = {
            "experiment_id": experiment.experiment_id,
            "improvement_type": experiment.improvement_type.value,
            "improvement_percent": analysis.get("significant_improvements", 0) / analysis.get("total_metrics", 1) * 100,
            "date": datetime.now(),
    ...
    async def _generate_optimization_recommendations(self, experiment: Experiment, analysis: Dict[str, Any]) -> None:
        ...
        recommendation = {
            ...
            "estimated_impact": analysis.get("significant_improvements", 0) / analysis.get("total_metrics", 1) * 100,
    ...
    async def get_improvement_progress(self) -> Dict[str, Any]:
        ...
        total_improvement = sum(imp["improvement_percent"] for imp in monthly_improvements)
        target_improvement = self.monthly_improvement_target * 100  # Convert to percentage
        ...
        progress = {
            ...
            "progress_percent": (total_improvement / target_improvement) * 100 if target_improvement > 0 else 0,
    ...
    async def get_optimization_recommendations(self) -> List[Dict[str, Any]]:
        ...
        pending_recommendations.sort(
            key=lambda x: (x["priority"] == "high", x["estimated_impact"]),
            reverse=True
        )
