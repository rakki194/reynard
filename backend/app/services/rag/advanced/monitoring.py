...
class PerformanceMonitor:
    ...
    async def _trigger_alert(self, alert: PerformanceAlert, metric: PerformanceMetric) -> None:
        ...
        try:
            ...
            recent_alerts = [
                ...
                if (a["alert_id"] == alert.alert_id and 
                    datetime.now() - a["timestamp"] < timedelta(seconds=alert.cooldown_seconds))
            ]
            ...
            logger.warning(f"Performance alert triggered: {alert.alert_id} - {alert.description}")
    ...
    async def get_system_health(self) -> SystemHealth:
        ...
        try:
            ...
            if PSUTIL_AVAILABLE:
                ...
                await self.record_metric("cpu_usage_percent", self.system_metrics["cpu_usage"])
                await self.record_metric("memory_usage_percent", self.system_metrics["memory_usage"])
                await self.record_metric("disk_usage_percent", self.system_metrics["disk_usage"])
            
            # Determine overall health status
            critical_alerts = [a for a in self.alert_history 
                             if a["severity"] == "critical" and 
                             datetime.now() - a["timestamp"] < timedelta(minutes=5)]
            
            warning_alerts = [a for a in self.alert_history
    ...
    def _get_recent_metrics(self, minutes: int = 5) -> Dict[str, List[PerformanceMetric]]:
    ...
    async def get_performance_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            summary["performance_score"] = self._calculate_performance_score(summary["metrics"])
    ...
    async def generate_performance_report(self, hours: int = 24) -> str:
        ...
        try:
            ...
            report.append(f"- **Last Updated**: {health.last_updated.strftime('%Y-%m-%d %H:%M:%S')}")
            report.append("")
            ...
            if summary["alerts"]:
                ...
                for alert in summary["alerts"][-10:]:  # Last 10 alerts
                    ...
                    report.append(f"  - Value: {alert['actual_value']:.2f}, Threshold: {alert['threshold_value']:.2f}")
                    report.append(f"  - Time: {alert['timestamp'].strftime('%Y-%m-%d %H:%M:%S')}")
                report.append("")
