...
class CalendarIntegrationService:
    ...
    def __init__(self, config: Optional[CalendarConfig] = None, data_dir: str = "data/calendar"):
    ...
    async def extract_meeting_requests_from_email(
        ...
        try:
            ...
            meeting_keywords = [
                ...
                'book', 'reserve', 'arrange', 'plan', 'coordinate'
            ]
    ...
    async def schedule_meeting(
        ...
        try:
            ...
            # Calculate end time
            end_time = selected_time + timedelta(minutes=meeting_request.duration_minutes)
            ...
            event = CalendarEvent(
                ...
                organizer=meeting_request.attendees[0] if meeting_request.attendees else None,
    ...
    async def get_available_times(
        ...
        try:
            ...
            existing_events = await self._get_events_in_range(start_date, end_date, calendar_id)
            ...
            while current_time + timedelta(minutes=duration_minutes) <= end_date:
                ...
                if self._is_time_slot_available(current_time, duration_minutes, existing_events):
    ...
    async def get_upcoming_meetings(
        ...
        try:
            ...
            for event in self.events.values():
                ...
                if (user_email in event.attendees or 
                    user_email == event.organizer or
                    user_email in [event.organizer]):
    ...
    async def reschedule_meeting(
        ...
        try:
            ...
            if new_duration_minutes:
                event.end_time = new_start_time + timedelta(minutes=new_duration_minutes)
            else:
    ...
    def _extract_meeting_location(self, text: str) -> Optional[str]:
        ...
        for keyword in location_keywords:
            pattern = rf'(?:in|at|held in|held at)\s+([A-Z][^.!?]*(?:{keyword}[^.!?]*)?)'
            match = re.search(pattern, text, re.IGNORECASE)
            if match:
                return match.group(1).strip()
    ...
    def _extract_meeting_type(self, text: str) -> str:
        ...
        if any(word in text_lower for word in ['video', 'zoom', 'teams', 'webinar', 'google meet']):
            return 'video_call'
    ...
    async def _get_events_in_range(
        ...
        for event in self.events.values():
            if (event.calendar_id == calendar_id and
                event.start_time < end_date and
                event.end_time > start_date and
                event.status != "cancelled"):
                events.append(event)
