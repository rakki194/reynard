...
class AgentEmailService:
        ...
        try:
            ...
            if self.templates_file.exists():
                with open(self.templates_file, 'r') as f:
                    data = json.load(f)
                    self._templates = {
                        agent_id: [AgentEmailTemplate(**template_data) for template_data in templates_data]
            ...
            if self.interactions_file.exists():
                with open(self.interactions_file, 'r') as f:
                    data = json.load(f)
                    self._interactions = [AgentEmailInteraction(**interaction_data) for interaction_data in data]
            
            # Load messages
            if self.messages_file.exists():
                with open(self.messages_file, 'r') as f:
                    data = json.load(f)
                    self._messages = [AgentEmailMessage(**message_data) for message_data in data]
            
            # Load notifications
            if self.notifications_file.exists():
                with open(self.notifications_file, 'r') as f:
                    data = json.load(f)
                    self._notifications = [AgentEmailNotification(**notification_data) for notification_data in data]
            ...
            logger.info(f"Loaded agent email data: {len(self._configs)} configs, {len(self._stats)} stats")
    ...
    def _save_data(self) -> None:
        ...
        try:
            ...
            # Save interactions
            interactions_data = [interaction.dict() for interaction in self._interactions]
            ...
            # Save notifications
            notifications_data = [notification.dict() for notification in self._notifications]
            with open(self.notifications_file, 'w') as f:
                json.dump(notifications_data, f, indent=2, default=str)
            
            # Save settings
            settings_data = {
                agent_id: settings.dict() for agent_id, settings in self._settings.items()
            }
    ...
    async def update_agent_config(self, agent_id: str, config: AgentEmailConfig) -> AgentEmailConfig:
    ...
    async def update_agent_stats(self, agent_id: str, action: str, count: int = 1) -> None:
    ...
    async def create_agent_template(self, agent_id: str, template: AgentEmailTemplate) -> AgentEmailTemplate:
    ...
    async def delete_agent_template(self, agent_id: str, template_id: str) -> bool:
        ...
        self._templates[agent_id] = [
            template for template in self._templates[agent_id]
    ...
    async def get_agent_messages(self, agent_id: str, limit: int = 50, offset: int = 0) -> List[AgentEmailMessage]:
    ...
    async def get_agent_interactions(self, agent_id: str, limit: int = 50, offset: int = 0) -> List[AgentEmailInteraction]:
        """Get agent email interactions."""
        agent_interactions = [
            interaction for interaction in self._interactions
            if interaction.sender_agent_id == agent_id or interaction.receiver_agent_id == agent_id
        ]
    ...
    async def process_automated_email(
        ...
        try:
            ...
            if not matching_template:
                logger.info(f"No matching template found for agent {agent_id} and event {event_type}")
                return False
            
            # Generate email content
            subject = self._process_template_variables(matching_template.subject, context)
            body = self._process_template_variables(matching_template.body, context)
            html_body = None
            if matching_template.html_body:
                html_body = self._process_template_variables(matching_template.html_body, context)
            
            # Determine recipients
            recipients = self._determine_recipients(agent_id, event_type, context)
            
            if not recipients:
                logger.info(f"No recipients found for agent {agent_id} and event {event_type}")
                return False
            ...
            if result["success"]:
                ...
                logger.info(f"Automated email sent for agent {agent_id} and event {event_type}")
                return True
            else:
                logger.error(f"Failed to send automated email for agent {agent_id}: {result.get('error')}")
                return False
    ...
    async def update_agent_settings(self, agent_id: str, settings: AgentEmailSettings) -> AgentEmailSettings:
