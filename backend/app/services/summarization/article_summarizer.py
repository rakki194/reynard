...
class ArticleSummarizer(BaseSummarizer):
    ...
    async def summarize(
        ...
        try:
            ...
            if options.include_highlights:
                result.highlights = await self._extract_article_highlights(processed_text)

            # Calculate quality score
            result.quality_score = await self._calculate_article_quality(text, summary_text)
    ...
    async def summarize_stream(
        ...
        try:
            yield {"event": "start", "data": {"message": "Starting article summarization"}}
            ...
            summary_text = ""
            async for chunk in self._generate_article_summary_stream(processed_text, options):
    ...
    async def _generate_article_summary(self, text: str, options: SummarizationOptions) -> str:
    ...
    def _get_article_prompts(self, text: str, options: SummarizationOptions) -> tuple[str, str]:
...
- Ensure the summary is self-contained and informative"""
    ...
    def _extract_abstract(self, summary: str) -> Optional[str]:
    ...
    def _detect_article_type(self, text: str) -> str:
        ...
        if any(keyword in text_lower for keyword in ['breaking', 'news', 'reports', 'according to']):
            return 'news'
        elif any(keyword in text_lower for keyword in ['tutorial', 'how to', 'guide', 'step by step']):
            return 'tutorial'
        elif any(keyword in text_lower for keyword in ['opinion', 'viewpoint', 'analysis', 'commentary']):
            return 'opinion'
        elif any(keyword in text_lower for keyword in ['review', 'evaluation', 'assessment', 'rating']):
            return 'review'
        else:
            return 'general'
    ...
    async def _extract_article_outline(self, summary: str) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if len(sentence) > 30 and any(keyword in sentence.lower() for keyword in 
                ['main', 'key', 'important', 'primary', 'major', 'central', 'core']):
    ...
    async def _extract_article_highlights(self, text: str) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if len(sentence) > 40 and any(indicator in sentence.lower() for indicator in 
                ['important', 'significant', 'notable', 'key finding', 'study shows', 'research indicates']):
                highlights.append(sentence)
        
        return highlights[:4]  # Limit to 4 highlights

    async def _calculate_article_quality(self, original_text: str, summary: str) -> float:
        ...
        if any(keyword in summary.lower() for keyword in ['introduction', 'conclusion', 'main point']):
            structure_score += 0.3
