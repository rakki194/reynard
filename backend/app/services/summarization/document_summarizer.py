...
class DocumentSummarizer(BaseSummarizer):
    ...
    async def summarize(
        ...
        try:
            ...
            # Generate summary
            summary_text = await self._generate_document_summary(processed_text, options)
            ...
            if options.include_outline:
                result.outline = await self._extract_document_outline(summary_text, sections)
            
            if options.include_highlights:
                result.highlights = await self._extract_document_highlights(processed_text)

            # Calculate quality score
            result.quality_score = await self._calculate_document_quality(text, summary_text)
    ...
    async def summarize_stream(
        ...
        try:
            yield {"event": "start", "data": {"message": "Starting document summarization"}}
            ...
            summary_text = ""
            async for chunk in self._generate_document_summary_stream(processed_text, options):
    ...
    def _detect_document_type(self, text: str) -> str:
        ...
        if any(keyword in text_lower for keyword in ['report', 'analysis', 'findings', 'conclusions']):
            return 'report'
        elif any(keyword in text_lower for keyword in ['proposal', 'recommendation', 'suggest']):
            return 'proposal'
        elif any(keyword in text_lower for keyword in ['policy', 'procedure', 'guideline', 'standard']):
            return 'policy'
        elif any(keyword in text_lower for keyword in ['manual', 'guide', 'instruction', 'tutorial']):
            return 'manual'
        else:
            return 'general'

    async def _generate_document_summary(self, text: str, options: SummarizationOptions) -> str:
    ...
    def _get_document_prompts(self, text: str, options: SummarizationOptions) -> tuple[str, str]:
...
- Ensure the summary is comprehensive yet concise"""
    ...
    async def _extract_document_outline(self, summary: str, sections: List[str]) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if len(sentence) > 30 and any(keyword in sentence.lower() for keyword in 
                ['main', 'key', 'important', 'primary', 'finding', 'conclusion', 'recommendation']):
    ...
    async def _extract_document_highlights(self, text: str) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if len(sentence) > 40 and any(indicator in sentence.lower() for indicator in 
                ['important', 'significant', 'notable', 'key finding', 'conclusion', 'recommendation']):
                highlights.append(sentence)
        
        return highlights[:4]  # Limit to 4 highlights

    async def _calculate_document_quality(self, original_text: str, summary: str) -> float:
        ...
        if any(keyword in summary.lower() for keyword in ['introduction', 'conclusion', 'finding', 'recommendation']):
            structure_score += 0.3
