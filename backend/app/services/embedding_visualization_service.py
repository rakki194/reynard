...
class EmbeddingVisualizationService(BaseService):
    ...
    async def get_embedding_stats(self) -> EmbeddingStats:
        ...
        try:
            ...
            logger.info(f"Retrieved embedding stats: {stats.total_embeddings} embeddings")
            return stats
    ...
    async def perform_reduction(
        ...
        try:
            # Generate cache key
            cache_key = self._generate_cache_key(method, filters, parameters, max_samples, random_seed)
            ...
            embeddings, original_indices = await self._get_embeddings(filters, max_samples)
            ...
            result = EmbeddingReductionResult(
                ...
                metadata={
                    ...
                    'compression_ratio': embeddings_array.shape[1] / transformed_data.shape[1],
            ...
            logger.info(f"Successfully performed {method} reduction: {len(embeddings)} embeddings -> {transformed_data.shape}")
            return result
    ...
    async def analyze_embedding_quality(self, embeddings: List[List[float]]) -> EmbeddingQualityMetrics:
        """
        ...
        try:
            ...
            if coherence_score < 0.5:
                issues.append("Low coherence - embeddings may not capture semantic relationships well")
                recommendations.append("Consider using a different embedding model or fine-tuning")
            
            if separation_score < 0.4:
                issues.append("Poor separation - embeddings may be too similar")
                recommendations.append("Increase embedding dimensionality or use contrastive learning")
            ...
            if distribution_score < 0.6:
                ...
                recommendations.append("Normalize embeddings or use balanced training data")
            
            if overall_score >= 0.8:
                recommendations.append("Excellent embedding quality - no immediate improvements needed")
            elif overall_score >= 0.6:
                recommendations.append("Good embedding quality - minor optimizations possible")
            else:
                recommendations.append("Consider significant improvements to embedding quality")
    ...
    def _is_cache_valid(self, cached_entry: Dict[str, Any], custom_ttl: Optional[int] = None) -> bool:
