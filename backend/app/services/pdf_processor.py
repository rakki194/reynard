...
MARKER_LLM_ENHANCEMENT_ENABLED = os.getenv("MARKER_LLM_ENHANCEMENT_ENABLED", "false").lower() == "true"
...
class PDFProcessorService:
    ...
    def _check_service_enabled(self):
        ...
        if not PDF_PROCESSING_ENABLED:
            raise RuntimeError("PDF processing is disabled via PDF_PROCESSING_ENABLED environment variable")
        if not MARKER_PACKAGE_ENABLED:
            raise RuntimeError("Marker package is disabled via MARKER_PACKAGE_ENABLED environment variable")
    ...
    async def _ensure_models_initialized(self):
        ...
        if not self._models_initialized:
            ...
            try:
                ...
                self.models = await loop.run_in_executor(None, marker_imports['create_model_dict'])
    ...
    async def _create_converter(self, use_llm: bool = False):
        ...
        if use_llm and not MARKER_LLM_ENHANCEMENT_ENABLED:
            logger.warning("LLM enhancement requested but disabled via MARKER_LLM_ENHANCEMENT_ENABLED")
            use_llm = False
    ...
    async def process_paper_collection(
        ...
        for pdf_file in pdf_files:
            logger.info(f"Processing paper {successful + failed + 1}/{len(pdf_files)}: {pdf_file.name}")
            
            result = await self.process_pdf_to_markdown(
                str(pdf_file),
                use_llm=use_llm
            )
    ...
    def get_processing_stats(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_compression_ratio": total_markdown_size / total_pdf_size if total_pdf_size > 0 else 0
        }
