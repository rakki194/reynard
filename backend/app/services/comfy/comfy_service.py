...
class ComfyService:
    ...
    def __init__(self):
        ...
        self._object_info_etag: Optional[str] = None
    ...
    async def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self._image_dir = Path(config.get("comfy_image_dir", "generated/comfy")).resolve()
            
            # Apply reconnection settings
            self._reconnect_max_attempts = config.get("comfy_reconnect_max_attempts", 5)
            self._reconnect_base_delay_s = config.get("comfy_reconnect_base_delay_s", 0.5)
            self._reconnect_max_delay_s = config.get("comfy_reconnect_max_delay_s", 30.0)
    ...
    async def _reconnection_loop(self) -> None:
        ...
        while self._connection_attempts < self._reconnect_max_attempts:
            ...
            try:
                ...
                async with session.get(f"{self._api_url}/") as resp:
                    if resp.status == 200:
                        logger.info(f"ComfyUI reconnected after {self._connection_attempts} attempts")
                        self._connection_state = ConnectionState.CONNECTED
                        self._last_ok_ts = time.time()
                        self._connection_attempts = 0
                        return
            ...
            effective_delay = min(delay * (1.0 + (self._connection_attempts - 1) * 0.5), self._reconnect_max_delay_s)
            logger.info(f"ComfyUI reconnection attempt {self._connection_attempts}, retrying in {effective_delay:.1f}s")
    ...
    async def queue_prompt(self, workflow: Dict[str, Any], client_id: Optional[str] = None) -> QueueResult:
        ...
        try:
            ...
            try:
                async with session.get(f"{self._api_url}/history/{prompt_id}") as resp:
                    ...
                    if resp.status != 200:
                        return {"status": "error", "progress": 0.0, "images": [], "error": f"HTTP {resp.status}"}
                    ...
                    if isinstance(data, dict) and prompt_id in data:
                        ...
                        progress = float(item.get("progress", 0.0)) if isinstance(item, dict) else 0.0
    ...
    async def get_object_info(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if (not force_refresh and 
            self._object_info_cache is not None and 
            (now - self._object_info_cache_ts) < self._object_info_ttl_seconds):
    ...
    def get_object_info_etag(self) -> Optional[str]:
        """Get current ETag for object info cache."""
        return self._object_info_etag
    
    async def stream_status(self, prompt_id: str) -> AsyncGenerator[Dict[str, Any], None]:
    ...
    def _extract_checkpoints_from_info(self, info: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if isinstance(ckpt_info, list) and ckpt_info:
                checkpoints = ckpt_info[0] if isinstance(ckpt_info[0], list) else ckpt_info
            
            # Fallback: search all nodes for checkpoint parameters
            if not checkpoints:
                for node_type, node_info in info.items():
                    if isinstance(node_info, dict):
                        for param_name, param_info in node_info.get("input", {}).get("required", {}).items():
                            if str(param_name).lower() in ("ckpt_name", "checkpoint", "model_name"):
                                if isinstance(param_info, list) and param_info:
                                    checkpoints = param_info[0] if isinstance(param_info[0], list) else param_info
                                    break
                    if checkpoints:
                        break
        except Exception:
    ...
    def _extract_loras_from_info(self, info: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            lora_info = info.get("LoraLoader", {}).get("input", {}).get("required", {}).get("lora_name")
                    ...
                    if ("Lora" in str(node_type)) or ("LoRA" in str(node_type)):
                        for param_name, param_info in node_info.get("input", {}).get("required", {}).items():
                            if str(param_name).lower() in ("lora_name", "lora"):
                                if isinstance(param_info, list) and param_info:
                                    loras = param_info[0] if isinstance(param_info[0], list) else param_info
                                    break
                    if loras:
                        break
        except Exception:
    ...
    def _extract_samplers_from_info(self, info: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            sampler_info = info.get("KSampler", {}).get("input", {}).get("required", {}).get("sampler_name")
            if isinstance(sampler_info, list) and sampler_info:
                samplers = sampler_info[0] if isinstance(sampler_info[0], list) else sampler_info
        except Exception:
            pass
        return samplers
    ...
    def _extract_schedulers_from_info(self, info: Dict[str, Any]) -> List[str]:
        ...
        try:
            # Try standard KSampler path
            scheduler_info = info.get("KSampler", {}).get("input", {}).get("required", {}).get("scheduler")
            if isinstance(scheduler_info, list) and scheduler_info:
                schedulers = scheduler_info[0] if isinstance(scheduler_info[0], list) else scheduler_info
        except Exception:
            pass
        return schedulers
