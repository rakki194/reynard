...
class DiffusionPipeService:
    ...
    async def start_training(self, request: TrainingRequest) -> str:
        ...
        try:
            ...
            if len(self.active_trainings) >= self.config.get("max_concurrent_trainings", 2):
    ...
    async def list_trainings(self, status_filter: Optional[TrainingStatus] = None) -> List[TrainingStatus]:
    ...
    def _validate_configuration(self) -> bool:
        ...
        try:
            ...
            if not diffusion_pipe_path.exists():
                logger.error(f"Diffusion-pipe path does not exist: {diffusion_pipe_path}")
                return False
    ...
    def _validate_training_paths(self, request: TrainingRequest) -> bool:
        ...
        try:
            ...
            if not output_dir.parent.exists():
                logger.error(f"Output directory parent does not exist: {output_dir.parent}")
                return False
    ...
    async def _execute_training(self, request: TrainingRequest, training_status: TrainingStatus):
        ...
        try:
            ...
            logger.info(f"Training {request.request_id} completed with status: {training_status.status}")
