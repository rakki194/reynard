...
class MetricsCollector:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.config = config
        self.metrics_dir = Path(config.get("default_output_dir", "/tmp/diffusion_pipe_output")) / "metrics"
    ...
    async def get_metrics(self, request_id: Optional[str] = None) -> List[TrainingMetrics]:
    ...
    async def get_metrics_summary(self, request_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            throughputs = [m.steps_per_second for m in metrics if m.steps_per_second is not None]
            gpu_utilizations = [m.gpu_utilization for m in metrics if m.gpu_utilization is not None]
            gpu_memories = [m.gpu_memory_usage for m in metrics if m.gpu_memory_usage is not None]
            ...
            summary = {
                ...
                "throughput": {
                    ...
                    "average": sum(throughputs) / len(throughputs) if throughputs else None,
                    "max": max(throughputs) if throughputs else None,
                },
                "gpu_utilization": {
                    "current": gpu_utilizations[-1] if gpu_utilizations else None,
                    "average": sum(gpu_utilizations) / len(gpu_utilizations) if gpu_utilizations else None,
                    "max": max(gpu_utilizations) if gpu_utilizations else None,
                },
                "gpu_memory": {
                    "current": gpu_memories[-1] if gpu_memories else None,
                    "average": sum(gpu_memories) / len(gpu_memories) if gpu_memories else None,
    ...
    async def _monitor_training_session(self, request_id: str, training_status: TrainingStatus):
        """Monitor a training session and collect metrics."""
        try:
            while request_id in self.monitoring_tasks:
                # Collect training metrics
                metrics = await self._collect_training_metrics(request_id, training_status)
    ...
    async def _system_monitoring_loop(self):
        ...
        try:
            while self.is_initialized:
                ...
                # Log system status
                if system_metrics.get("gpu_memory_used", 0) > self.gpu_memory_threshold * 24:  # 24GB threshold
                    logger.warning(f"High GPU memory usage: {system_metrics.get('gpu_memory_used', 0):.1f}GB")
    ...
    async def _collect_training_metrics(
        ...
        try:
            ...
            if training_status.total_epochs > 0:
                progress_percentage = (training_status.current_epoch / training_status.total_epochs) * 100.0
            ...
            metrics = TrainingMetrics(
                ...
                samples_per_second=steps_per_second * (training_status.metadata.get("batch_size", 1)),
    ...
    async def _check_metrics_alerts(self, metrics: TrainingMetrics):
        ...
        try:
            ...
            if metrics.gpu_memory_usage > self.gpu_memory_threshold * 24:  # 24GB threshold
                logger.warning(f"High GPU memory usage for {metrics.request_id}: {metrics.gpu_memory_usage:.1f}GB")
            
            # Check GPU temperature
            if metrics.temperature_gpu and metrics.temperature_gpu > 80:
                logger.warning(f"High GPU temperature for {metrics.request_id}: {metrics.temperature_gpu}°C")
            
            # Check loss trends
            if metrics.loss_trend == "increasing":
                logger.warning(f"Loss increasing for {metrics.request_id}: {metrics.current_loss}")
            
            # Check throughput
            if metrics.steps_per_second < 0.1:
                logger.warning(f"Low throughput for {metrics.request_id}: {metrics.steps_per_second:.3f} steps/sec")
