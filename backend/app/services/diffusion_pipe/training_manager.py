...
class TrainingManager:
    ...
    async def _create_output_directories(self):
        ...
        try:
            output_dir = Path(
                self.config.get("default_output_dir", "/tmp/diffusion_pipe_output")
            )
    ...
    async def _prepare_training_command(
        self, request: TrainingRequest, config_file: str
    ) -> List[str]:
        """Prepare training command."""
        try:
            # Base command
    ...
    async def _start_process(
        self, command: List[str], request_id: str
    ) -> subprocess.Popen:
    ...
    async def _monitor_training_process(self, request_id: str):
        ...
        try:
            ...
            while process.poll() is None:
                ...
                if runtime > timeout:
                    logger.warning(
                        f"Training {request_id} timed out after {timeout} seconds"
                    )
                    process.terminate()
            ...
            logger.info(
                f"Training process {request_id} completed with status: {training_status.status}"
            )
