...
class NLWebRouter:
    ...
    def __init__(self, tool_registry: NLWebToolRegistry):
        ...
        self.cache: OrderedDict[str, Tuple[NLWebSuggestionResponse, float]] = OrderedDict()
    ...
    async def suggest_tools(self, request: NLWebSuggestionRequest) -> NLWebSuggestionResponse:
        ...
        try:
            ...
            filtered_tools = [
                (tool, score) for tool, score in scored_tools
        ...
        if pattern_matches:
            ...
            for tool in candidates:
                ...
                for pattern_type in pattern_matches:
                    if pattern_type in tool.category.lower() or any(pattern_type in tag.lower() for tag in tool.tags):
                        score += 1
                scored_candidates.append((tool, score))
            
            # Sort by pattern match score and take top candidates
            scored_candidates.sort(key=lambda x: x[1], reverse=True)
            candidates = [tool for tool, score in scored_candidates if score > 0 or len(candidates) < 10]
        
        return candidates
    
    def _filter_by_context(self, tools: List[NLWebTool], context: NLWebContext) -> List[NLWebTool]:
    ...
    def _is_tool_relevant_to_context(self, tool: NLWebTool, context: NLWebContext) -> bool:
    ...
    async def _score_tools(self, request: NLWebSuggestionRequest, tools: List[NLWebTool]) -> List[Tuple[NLWebTool, float]]:
        ...
        for tool in tools:
            ...
            for pattern_type in pattern_matches:
                if pattern_type in tool.category.lower() or any(pattern_type in tag.lower() for tag in tool.tags):
                    weight = self.priority_weights.get(pattern_type, self.priority_weights["default"])
                    score += 25.0 * weight
    ...
    def _extract_parameters(self, request: NLWebSuggestionRequest, tool: NLWebTool) -> Dict[str, Any]:
    ...
    def _extract_parameter_value(self, query: str, param: NLWebToolParameter) -> Optional[Any]:
    ...
    def _generate_reasoning(self, request: NLWebSuggestionRequest, tool: NLWebTool, score: float) -> str:
        ...
        if query_lower in tool.description.lower():
            reasons.append(f"Tool description matches query")
        ...
        if request.context:
            if request.context.git_status and request.context.git_status.get("isRepository", False):
                if "git" in tool.category.lower() or "git" in tool.tags:
        ...
        if not reasons:
            reasons.append(f"Tool has high priority ({tool.priority}) and general relevance")
        
        return "; ".join(reasons)
    
    def _generate_parameter_hints(self, request: NLWebSuggestionRequest, tool: NLWebTool) -> Dict[str, Any]:
    ...
    def _suggest_parameter_value(self, request: NLWebSuggestionRequest, param: NLWebToolParameter) -> Optional[Any]:
        ...
        if request.context:
            ...
            if "file" in param.name.lower() and request.context.selected_items:
                return request.context.selected_items[0] if request.context.selected_items else None
    ...
    def _generate_cache_key(self, request: NLWebSuggestionRequest) -> str:
        ...
        if request.context:
            ...
            if request.context.git_status and request.context.git_status.get("isRepository"):
    ...
    def _update_performance_stats(self, processing_time_ms: float):
        ...
        self.latency_samples = {
            timestamp: samples for timestamp, samples in self.latency_samples.items()
        ...
        if all_latencies:
            all_latencies.sort()
            self.performance_stats.avg_processing_time_ms = sum(all_latencies) / len(all_latencies)
            self.performance_stats.p95_processing_time_ms = all_latencies[int(len(all_latencies) * 0.95)]
            self.performance_stats.p99_processing_time_ms = all_latencies[int(len(all_latencies) * 0.99)]
        
        # Update cache statistics
        total_cache_requests = self.performance_stats.cache_hits + self.performance_stats.cache_misses
        if total_cache_requests > 0:
            self.performance_stats.cache_hit_rate = (self.performance_stats.cache_hits / total_cache_requests) * 100
