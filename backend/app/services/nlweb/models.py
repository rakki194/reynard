...
class NLWebTool(BaseModel):
    ...
    parameters: List[NLWebToolParameter] = Field(default_factory=list, description="Required parameters")
    examples: List[str] = Field(default_factory=list, description="Example usage prompts")
    enabled: bool = Field(True, description="Whether the tool is currently enabled")
    priority: int = Field(50, description="Priority for tool selection (higher = more likely to be selected)")
...
class NLWebContext(BaseModel):
    ...
    current_path: Optional[str] = Field(None, description="Current working directory or path")
    selected_items: Optional[List[str]] = Field(None, description="Selected files or items")
    git_status: Optional[Dict[str, Any]] = Field(None, description="Git repository status")
    user_preferences: Optional[Dict[str, Any]] = Field(None, description="User preferences and settings")
    application_state: Optional[Dict[str, Any]] = Field(None, description="Current application state")
    user_id: Optional[str] = Field(None, description="User identifier for personalization")
    session_id: Optional[str] = Field(None, description="Session information")
...
class NLWebSuggestion(BaseModel):
    ...
    parameters: Dict[str, Any] = Field(default_factory=dict, description="Suggested parameters for the tool")
    reasoning: str = Field(..., description="Reasoning for why this tool was suggested")
    parameter_hints: Dict[str, Any] = Field(default_factory=dict, description="Parameter hints for better execution")
...
class NLWebSuggestionRequest(BaseModel):
    ...
    context: Optional[NLWebContext] = Field(None, description="Additional context for the query")
    max_suggestions: Optional[int] = Field(5, description="Maximum number of suggestions to return", ge=1, le=20)
    min_score: Optional[float] = Field(0.0, description="Minimum confidence score for suggestions", ge=0, le=100)
    include_reasoning: Optional[bool] = Field(True, description="Whether to include reasoning in suggestions")


class NLWebSuggestionResponse(BaseModel):
    """Response containing tool suggestions."""
    suggestions: List[NLWebSuggestion] = Field(..., description="List of tool suggestions")
    query: str = Field(..., description="Original query")
    processing_time_ms: float = Field(..., description="Processing time in milliseconds")
    cache_hit: bool = Field(False, description="Whether this was served from cache")
    total_tools_considered: int = Field(..., description="Total number of tools considered")
...
class NLWebHealthStatus(BaseModel):
    ...
    connection_state: str = Field(..., description="Connection state to external services")
    connection_attempts: int = Field(0, description="Number of connection attempts")
    last_ok_timestamp: Optional[datetime] = Field(None, description="Last successful operation timestamp")
    base_url: Optional[str] = Field(None, description="Base URL for external NLWeb service")
    ...
    rollback_enabled: bool = Field(False, description="Whether emergency rollback is enabled")
...
class NLWebPerformanceStats(BaseModel):
    ...
    avg_processing_time_ms: float = Field(0.0, description="Average processing time in milliseconds")
    p95_processing_time_ms: float = Field(0.0, description="95th percentile processing time")
    p99_processing_time_ms: float = Field(0.0, description="99th percentile processing time")
...
class NLWebConfiguration(BaseModel):
    ...
    base_url: Optional[str] = Field(None, description="Base URL for external NLWeb service")
    suggest_timeout_s: float = Field(1.5, description="Timeout for suggestion requests in seconds")
    ...
    allow_stale_on_error: bool = Field(True, description="Allow returning stale cache on errors")
    warm_timeout_s: float = Field(2.0, description="Timeout for warmup requests in seconds")
    ...
    rollback_enabled: bool = Field(False, description="Whether emergency rollback is enabled")
    performance_monitoring_enabled: bool = Field(True, description="Whether performance monitoring is enabled")
    proxy_max_retries: int = Field(2, description="Maximum retries for proxy requests")
    proxy_backoff_ms: int = Field(200, description="Backoff delay for proxy requests in milliseconds")
    proxy_connect_timeout_ms: int = Field(2000, description="Connect timeout for proxy requests in milliseconds")
    proxy_read_timeout_ms: int = Field(10000, description="Read timeout for proxy requests in milliseconds")
    proxy_sse_idle_timeout_ms: int = Field(15000, description="SSE idle timeout in milliseconds")
...
class NLWebVerificationCheck(BaseModel):
    ...
    value: Union[str, int, float, bool] = Field(..., description="Check value")
...
class NLWebVerificationResponse(BaseModel):
    ...
    service_available: bool = Field(..., description="Whether NLWeb service is available")
    config_loaded: bool = Field(..., description="Whether configuration is loaded")
    checks: List[NLWebVerificationCheck] = Field(default_factory=list, description="List of verification checks")
...
class NLWebAskRequest(BaseModel):
    ...
    context: Optional[NLWebContext] = Field(None, description="Additional context")
...
class NLWebMCPRequest(BaseModel):
    ...
    params: Optional[Dict[str, Any]] = Field(None, description="Parameters for the MCP method")
