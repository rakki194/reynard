...
class NLWebContext(BaseModel):
    ...
    current_path: str | None = Field(
        None,
        description="Current working directory or path",
    )
    selected_items: list[str] | None = Field(
        None,
        description="Selected files or items",
    )
...
class NLWebSuggestion(BaseModel):
    ...
    parameter_hints: dict[str, Any] = Field(
        default_factory=dict,
        description="Parameter hints for better execution",
    )
...
class NLWebSuggestionRequest(BaseModel):
    ...
    include_reasoning: bool | None = Field(
        True,
        description="Whether to include reasoning in suggestions",
    )
...
class NLWebHealthStatus(BaseModel):
    ...
    base_url: str | None = Field(
        None,
        description="Base URL for external NLWeb service",
    )
    ...
    rollback_enabled: bool = Field(
        False,
        description="Whether emergency rollback is enabled",
    )
...
class NLWebConfiguration(BaseModel):
    ...
    suggest_timeout_s: float = Field(
        1.5,
        description="Timeout for suggestion requests in seconds",
    )
    ...
    allow_stale_on_error: bool = Field(
        True,
        description="Allow returning stale cache on errors",
    )
    warm_timeout_s: float = Field(
        2.0,
        description="Timeout for warmup requests in seconds",
    )
    ...
    proxy_backoff_ms: int = Field(
        200,
        description="Backoff delay for proxy requests in milliseconds",
    )
    proxy_connect_timeout_ms: int = Field(
        2000,
        description="Connect timeout for proxy requests in milliseconds",
    )
    proxy_read_timeout_ms: int = Field(
        10000,
        description="Read timeout for proxy requests in milliseconds",
    )
    proxy_sse_idle_timeout_ms: int = Field(
        15000,
        description="SSE idle timeout in milliseconds",
    )
