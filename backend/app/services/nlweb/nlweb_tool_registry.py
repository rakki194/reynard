...
from .models import NLWebTool, NLWebToolParameter
...
class NLWebToolRegistry:
    ...
    def __init__(self):
        ...
        self._tool_usage_stats: Dict[str, Dict[str, any]] = defaultdict(lambda: {
            "usage_count": 0,
            "last_used": None,
            "success_count": 0,
            "failure_count": 0,
    ...
    def register_tool(self, tool: NLWebTool) -> bool:
        ...
        try:
            ...
            logger.info(f"Registered NLWeb tool: {tool.name} in category {tool.category}")
            return True
    ...
    def get_tool(self, tool_name: str) -> Optional[NLWebTool]:
        """Get a tool by name."""
    ...
    def search_tools(self, query: str, category: Optional[str] = None, tags: Optional[List[str]] = None) -> List[NLWebTool]:
        ...
        if tags:
            candidates = [tool for tool in candidates if any(tag in tool.tags for tag in tags)]
    ...
    def update_tool_usage(self, tool_name: str, success: bool, execution_time: float):
        ...
        total_time = stats["avg_execution_time"] * (stats["usage_count"] - 1) + execution_time
    ...
    def _validate_tool(self, tool: NLWebTool) -> bool:
        ...
        if tool.priority < 0 or tool.priority > 100:
            logger.error(f"Tool priority must be between 0 and 100, got: {tool.priority}")
            return False
