...
def initialize_nlweb_service(config: Dict[str, Any]) -> NLWebService:
    ...
    try:
        ...
        configuration = NLWebConfiguration(
            ...
            performance_monitoring_enabled=nlweb_config.get("performance_monitoring_enabled", True),
            ...
            proxy_sse_idle_timeout_ms=nlweb_config.get("proxy_sse_idle_timeout_ms", 15000)
        )
...
def get_nlweb_service() -> Optional[NLWebService]:
    """
...
async def ensure_nlweb_service_initialized() -> bool:
    ...
    if _nlweb_service is None:
        logger.warning("NLWeb service not initialized, attempting to initialize with defaults")
        try:
