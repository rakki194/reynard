...
class EmbeddingBackendsConfig:
    ...
    # Global settings
    enabled: bool = field(
        default_factory=lambda: os.getenv("EMBEDDING_BACKENDS_ENABLED", "true").lower() == "true"
    )
    
    # Fallback behavior
    allow_fallback: bool = field(
        default_factory=lambda: os.getenv("EMBEDDING_ALLOW_FALLBACK", "true").lower() == "true"
    )
    ...
    mock_mode: bool = field(
        default_factory=lambda: os.getenv("EMBEDDING_MOCK_MODE", "false").lower() == "true"
    )
    ...
    def _initialize_default_backends(self) -> None:
        ...
        ollama_config = EmbeddingBackendConfig(
            ...
            default_model=os.getenv("EMBEDDING_OLLAMA_DEFAULT_MODEL", "embeddinggemma:latest"),
            supported_models=[
            ...
            max_concurrent_requests=int(os.getenv("EMBEDDING_OLLAMA_MAX_CONCURRENT", "8")),
        ...
        sentence_transformers_config = EmbeddingBackendConfig(
            ...
            enabled=os.getenv("EMBEDDING_SENTENCE_TRANSFORMERS_ENABLED", "true").lower() == "true",
            timeout_seconds=int(os.getenv("EMBEDDING_SENTENCE_TRANSFORMERS_TIMEOUT", "30")),
            max_retries=int(os.getenv("EMBEDDING_SENTENCE_TRANSFORMERS_MAX_RETRIES", "3")),
            retry_delay=float(os.getenv("EMBEDDING_SENTENCE_TRANSFORMERS_RETRY_DELAY", "1.0")),
            default_model=os.getenv("EMBEDDING_SENTENCE_TRANSFORMERS_DEFAULT_MODEL", "sentence-transformers/all-MiniLM-L6-v2"),
            supported_models=[
                "sentence-transformers/all-MiniLM-L6-v2",
                "sentence-transformers/all-mpnet-base-v2",
                "sentence-transformers/all-distilroberta-v1"
            ],
            max_concurrent_requests=int(os.getenv("EMBEDDING_SENTENCE_TRANSFORMERS_MAX_CONCURRENT", "4")),
            batch_size=int(os.getenv("EMBEDDING_SENTENCE_TRANSFORMERS_BATCH_SIZE", "8")),
            priority=2,
        ...
        openai_config = EmbeddingBackendConfig(
            ...
            default_model=os.getenv("EMBEDDING_OPENAI_DEFAULT_MODEL", "text-embedding-3-small"),
            supported_models=[
                "text-embedding-3-small",
                "text-embedding-3-large",
                "text-embedding-ada-002"
            ],
            max_concurrent_requests=int(os.getenv("EMBEDDING_OPENAI_MAX_CONCURRENT", "5")),
        ...
        huggingface_config = EmbeddingBackendConfig(
            ...
            enabled=os.getenv("EMBEDDING_HUGGINGFACE_ENABLED", "false").lower() == "true",
            ...
            default_model=os.getenv("EMBEDDING_HUGGINGFACE_DEFAULT_MODEL", "sentence-transformers/all-MiniLM-L6-v2"),
            supported_models=[
                "sentence-transformers/all-MiniLM-L6-v2",
                "sentence-transformers/all-mpnet-base-v2",
                "BAAI/bge-large-en-v1.5"
            ],
            max_concurrent_requests=int(os.getenv("EMBEDDING_HUGGINGFACE_MAX_CONCURRENT", "4")),
    ...
    def _validate_backend(self, backend: EmbeddingBackendConfig) -> None:
        ...
        if backend.provider == "ollama" and backend.enabled:
            if not backend.base_url:
                raise ValueError(f"Backend '{backend.name}': base_url is required for Ollama")
        
        if backend.provider in ["openai", "huggingface"] and backend.enabled:
            if not backend.api_key:
                raise ValueError(f"Backend '{backend.name}': api_key is required for {backend.provider}")
        
        if backend.timeout_seconds <= 0:
            raise ValueError(f"Backend '{backend.name}': timeout_seconds must be positive")
        
        if backend.max_retries < 0:
            raise ValueError(f"Backend '{backend.name}': max_retries must be non-negative")
        
        if backend.retry_delay < 0:
            raise ValueError(f"Backend '{backend.name}': retry_delay must be non-negative")
