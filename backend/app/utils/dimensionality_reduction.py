    ...
    def _reduce_sync(self, data: np.ndarray, parameters: PCAParameters) -> np.ndarray:
        ...
        try:
            ...
            if parameters.variance_threshold < 1.0:
                ...
                n_components = np.argmax(cumsum_variance >= parameters.variance_threshold) + 1
            ...
            logger.info(f"Cumulative explained variance: {np.cumsum(pca.explained_variance_ratio_)}")
...
def validate_parameters(method: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if method == 'pca':
        return {
            ...
            'variance_threshold': min(max(float(parameters.get('variance_threshold', 0.95)), 0.0), 1.0),
        ...
        return {
            ...
            'perplexity': min(max(float(parameters.get('perplexity', 30.0)), 5.0), 100.0),
            'learning_rate': min(max(float(parameters.get('learning_rate', 200.0)), 10.0), 1000.0),
            'early_exaggeration': min(max(float(parameters.get('early_exaggeration', 12.0)), 1.0), 50.0),
        ...
        return {
            ...
            'learning_rate': min(max(float(parameters.get('learning_rate', 1.0)), 0.1), 10.0),
            'spread': min(max(float(parameters.get('spread', 1.0)), 0.1), 10.0),
            'metric': str(parameters.get('metric', 'euclidean')),
            'local_connectivity': min(max(int(parameters.get('local_connectivity', 1)), 1), 10)
        }
