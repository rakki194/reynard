...
class GitTagTracker:
    ...
    async def get_all_tags(self) -> List[GitTag]:
        ...
        try:
            ...
            result = await self._run_git_command([
                "tag", "-l", "--format=%(refname:short)|%(objectname)|%(creatordate:iso)|%(contents:subject)|%(creator)|%(type)"
            ])
    ...
    async def get_success_advisor_8_releases(self) -> List[SuccessAdvisor8Release]:
        ...
        for tag in tags:
            ...
            if activities:
                ...
                releases.append(SuccessAdvisor8Release(
                    tag=tag,
    ...
    async def _run_git_command(self, command: List[str]) -> str:
        ...
        try:
            ...
            if process.returncode != 0:
                logger.warning(f"Git command failed: {' '.join(command)}, stderr: {stderr.decode()}")
                return ""
    ...
    async def get_release_statistics(self) -> Dict[str, any]:
        ...
        for release in releases:
            release_types[release.release_type] = release_types.get(release.release_type, 0) + 1
        ...
        most_active_period = max(monthly_counts.items(), key=lambda x: x[1]) if monthly_counts else None
