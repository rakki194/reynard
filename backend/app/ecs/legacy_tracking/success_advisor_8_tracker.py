...
class SuccessAdvisor8LegacyTracker:
    ...
    async def parse_changelog_entries(self) -> list[SuccessAdvisor8Activity]:
        ...
        logger.info(f"Found {len(activities)} Success-Advisor-8 activities in CHANGELOG")
        return activities
    ...
    async def scan_codebase_movements(self) -> list[CodeMovement]:
        ...
        logger.info(f"Found {len(movements)} Success-Advisor-8 code movements")
        return movements
    ...
    async def generate_legacy_report(self) -> LegacyReport:
        ...
        logger.info(f"Generated legacy report with {report.total_activities} activities and {report.total_code_movements} code movements")
        return report
    ...
    async def _extract_activity_from_line(
        ...
        try:
            ...
            activity_id = f"sa8-{version or 'unreleased'}-{line_number}-{hash(line) % 10000}"
    ...
    def _classify_activity_type(self, line: str) -> str:
        ...
        if any(keyword in line_lower for keyword in ['release', 'version', 'bump']):
            ...
            return 'fix'
        elif any(keyword in line_lower for keyword in ['refactor', 'restructure', 'reorganize']):
            return 'refactor'
        elif any(keyword in line_lower for keyword in ['doc', 'documentation', 'guide']):
    ...
    async def _analyze_python_file(self, file_path: Path) -> list[CodeMovement]:
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                if self._contains_success_advisor_8_reference(line):
                    movement_type = self._classify_python_movement(line, i, lines)
    ...
    async def _analyze_js_file(self, file_path: Path) -> list[CodeMovement]:
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                if self._contains_success_advisor_8_reference(line):
                    movement_type = self._classify_js_movement(line, i, lines)
            ...
            logger.error(f"Failed to analyze JS file {file_path}: {e}")
    ...
    async def _analyze_markdown_file(self, file_path: Path) -> list[CodeMovement]:
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                if self._contains_success_advisor_8_reference(line):
                    movement_type = self._classify_markdown_movement(line, i, lines)
            ...
            logger.error(f"Failed to analyze Markdown file {file_path}: {e}")
        
        return movements
    
    def _classify_python_movement(self, line: str, line_number: int, lines: list[str]) -> str:
        """Classify Python code movement type."""
        line_lower = line.lower()
        
        if 'import' in line_lower or 'from' in line_lower:
            return 'import'
        elif 'class' in line_lower and 'success' in line_lower:
            return 'definition'
        elif 'def' in line_lower and 'success' in line_lower:
            return 'definition'
    ...
    def _classify_js_movement(self, line: str, line_number: int, lines: list[str]) -> str:
        ...
        if 'import' in line_lower or 'require' in line_lower:
            ...
            return 'definition'
        elif 'function' in line_lower and 'success' in line_lower:
            return 'definition'
        elif 'success' in line_lower:
            return 'usage'
        else:
    ...
    def _classify_markdown_movement(self, line: str, line_number: int, lines: list[str]) -> str:
    ...
    def _get_js_function_context(self, lines: list[str], line_number: int) -> str | None:
    ...
    def _generate_summary(
        ...
        for movement in movements:
            movement_types[movement.movement_type] = movement_types.get(movement.movement_type, 0) + 1
        ...
        return {
            ...
            'versions': versions,
            'total_files_scanned': len(set(movement.file_path for movement in movements)),
            'date_range': {
                'earliest': min((activity.timestamp for activity in activities), default=None),
                'latest': max((activity.timestamp for activity in activities), default=None)
            }
        }
