...
class UnifiedChangelogParser:
    ...
    def _parse_with_existing_parser(self) -> List[SuccessAdvisor8Activity]:
        ...
        try:
            ...
            logger.info(f"Found {len(activities)} Success-Advisor-8 activities using existing parser")
            
            # If existing parser didn't find any Success-Advisor-8 activities, fall back
            if len(activities) == 0:
                logger.info("No Success-Advisor-8 activities found in existing parser, falling back to fallback parser")
                return self._parse_with_fallback()
    ...
    def _parse_with_fallback(self) -> List[SuccessAdvisor8Activity]:
        ...
        logger.info(f"Found {len(activities)} Success-Advisor-8 activities using fallback parser")
        return activities
    
    def _convert_contribution_to_activity(self, contribution: AgentContribution) -> SuccessAdvisor8Activity:
    ...
    def get_changelog_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            success_advisor_lines = [line for line in lines if self.success_advisor_pattern.search(line)]
            ...
            return {
                ...
                "last_modified": datetime.fromtimestamp(self.changelog_path.stat().st_mtime).isoformat()
            }
        except Exception as e:
    ...
    def validate_changelog_format(self) -> Dict[str, Any]:
        ...
        try:
            ...
            section_pattern = re.compile(r'^###\s+(Added|Changed|Deprecated|Removed|Fixed|Security)', re.MULTILINE)
            ...
            if not success_advisor_lines:
                validation_results["warnings"].append("No Success-Advisor-8 references found in CHANGELOG")
            ...
            if not date_matches:
                validation_results["warnings"].append("No properly formatted dates found in version headers")
    ...
    def _convert_to_success_advisor_activity(self, contribution) -> SuccessAdvisor8Activity:
...
class SuccessAdvisor8ChangelogAnalyzer:
    ...
    def analyze_activity_trends(self) -> Dict[str, Any]:
        ...
        # Time analysis
        timestamps = [activity.timestamp for activity in activities if activity.timestamp]
        time_range = {
    ...
    def get_parser_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "changelog_path": str(self.changelog_path),
            "changelog_exists": self.changelog_path.exists(),
            "base_parser_type": type(self.existing_parser).__name__ if self.existing_parser else None,
            "success_advisor_pattern": self.success_advisor_pattern.pattern
        }
