...
from app.ecs.database import Agent, AgentInteraction, PersonalityTrait, PhysicalTrait, AbilityTrait
from app.ecs.legacy_tracking import SuccessAdvisor8LegacyTracker, SuccessAdvisor8Activity
...
class UnifiedAgentStateManager:
    ...
    def __init__(self, ecs_service: PostgresECSWorldService, codebase_path: str = "."):
        ...
        self.legacy_tracker = SuccessAdvisor8LegacyTracker(codebase_path, "CHANGELOG.md")
    ...
    async def track_agent_activity(self, agent_id: str, activity: str, context: Dict[str, Any]) -> None:
        """
    ...
    async def _update_agent_traits(self, agent_id: str, traits: Dict[str, float]) -> None:
        """Update agent traits in ECS database."""
        try:
            # Update personality traits
            personality_traits = {k: v for k, v in traits.items() if k.startswith('personality_')}
            ...
            physical_traits = {k: v for k, v in traits.items() if k.startswith('physical_')}
            ...
            ability_traits = {k: v for k, v in traits.items() if k.startswith('ability_')}
    ...
    async def _update_agent_specializations(self, agent_id: str, specializations: List[str]) -> None:
        """Update agent specializations in ECS database."""
        try:
            await self.ecs_service.update_agent_specializations(agent_id, specializations)
        except Exception as e:
            logger.error(f"Error updating specializations for agent {agent_id}: {e}")
    
    async def _update_agent_achievements(self, agent_id: str, achievements: List[Dict]) -> None:
