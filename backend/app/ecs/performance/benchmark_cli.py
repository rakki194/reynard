...
class LoadTester:
    ...
    async def run_load_test(self) -> List[LoadTestResult]:
        ...
        print(f"⏱️  Duration: {self.config.duration_seconds}s, Ramp-up: {self.config.ramp_up_seconds}s")
        ...
        for endpoint_config in self.config.endpoints:
            task = asyncio.create_task(
                self._test_endpoint(endpoint_config, semaphore)
            )
            tasks.append(task)
    ...
    async def _test_endpoint(self, endpoint_config: Dict[str, Any], 
                           semaphore: asyncio.Semaphore) -> LoadTestResult:
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=self.config.timeout_seconds)) as session:
            ...
            if self.config.ramp_up_seconds > 0:
                await self._ramp_up(semaphore, session, url, method, payload, headers, endpoint_config)
    ...
    async def _ramp_up(self, semaphore: asyncio.Semaphore, session: aiohttp.ClientSession,
                      url: str, method: str, payload: Any, headers: Dict[str, str],
        ...
        while time.time() - ramp_up_start < ramp_up_duration:
            ...
            current_concurrency = min(
                ...
                int((time.time() - ramp_up_start) / ramp_up_duration * self.config.concurrent_users)
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='FastAPI ECS Performance Benchmarking Tool')
    parser.add_argument('--base-url', default='http://localhost:8000', help='Base URL of the API')
    parser.add_argument('--mode', choices=['load-test', 'profile', 'monitor'], required=True, help='Benchmarking mode')
    
    # Load test arguments
    parser.add_argument('--endpoints', nargs='+', help='Endpoints to test (e.g., /api/health /api/users)')
    parser.add_argument('--concurrent-users', type=int, default=10, help='Number of concurrent users')
    parser.add_argument('--duration', type=int, default=60, help='Test duration in seconds')
    parser.add_argument('--ramp-up', type=int, default=10, help='Ramp-up duration in seconds')
    ...
    if args.mode == 'load-test':
        ...
        result = await profiler.profile_endpoint(endpoint, args.method, payload, args.iterations)
