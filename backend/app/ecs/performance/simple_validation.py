...
async def test_performance_tracker():
    ...
    success = (
        ...
        avg_duration > 10 and  # Should be > 10ms due to our delays
        error_rate == 20.0 and  # 2 out of 10 requests failed
        len(tracker.db_queries) == 10 and
        len(tracker.async_tasks) == 10
    )
...
async def test_memory_profiler():
    ...
    success = (
        summary['snapshots_count'] > 0 and
        summary['current_memory_mb'] > 0 and
...
async def test_bottleneck_analysis():
    ...
    error_bottlenecks = [b for b in bottlenecks if b.bottleneck_type == 'high_error_rate']
...
async def test_performance_trends():
    ...
    for i in range(20):
        ...
        await asyncio.sleep(0.01 + (i * 0.02))  # 10ms + 20ms per request (more dramatic increase)
