...
class PerformanceAnalyzer:
    ...
    def _analyze_endpoint_bottlenecks(self) -> List[BottleneckAnalysis]:
        ...
        for endpoint, stats in self.performance_tracker.endpoint_stats.items():
            ...
            if stats['avg_duration'] > 0.1:  # > 100ms average (more realistic threshold)
    ...
    def _analyze_database_bottlenecks(self) -> List[BottleneckAnalysis]:
        ...
        slow_queries = [q for q in self.performance_tracker.db_queries if q.duration > 0.05]  # 50ms threshold
        if slow_queries:
            avg_slow_duration = sum(q.duration for q in slow_queries) / len(slow_queries)
        ...
        frequent_queries = [(query, count) for query, count in query_counts.items() if count > 10]
    ...
    def _analyze_async_bottlenecks(self) -> List[BottleneckAnalysis]:
        ...
        slow_tasks = [t for t in self.performance_tracker.async_tasks if t.duration > 0.1]  # 100ms threshold
        if slow_tasks:
        ...
        failed_tasks = [t for t in self.performance_tracker.async_tasks if not t.success]
        if failed_tasks:
            failure_rate = (len(failed_tasks) / len(self.performance_tracker.async_tasks)) * 100
            if failure_rate > 5:  # > 5% failure rate
                ...
                bottlenecks.append(BottleneckAnalysis(
                    ...
                    metrics={
                        ...
                        'total_task_count': len(self.performance_tracker.async_tasks)
                    }
                ))
        
        return bottlenecks
    
    def analyze_performance_trends(self, time_period_hours: int = 24) -> List[PerformanceTrend]:
        ...
        for i in range(0, len(recent_metrics), 10):  # Sample every 10 requests
            ...
            error_rate = sum(1 for m in batch if m.status_code >= 400) / len(batch) * 100
    ...
    def generate_optimization_report(self) -> Dict[str, Any]:
        ...
        for bottleneck in critical_bottlenecks + high_bottlenecks:
            priority_recommendations.extend(bottleneck.recommendations[:2])  # Top 2 recommendations
        ...
        return {
            ...
            "priority_recommendations": list(set(priority_recommendations)),  # Remove duplicates
            "optimization_score": self._calculate_optimization_score(bottlenecks, trends)
        }
    
    def _calculate_optimization_score(self, bottlenecks: List[BottleneckAnalysis], 
                                    trends: List[PerformanceTrend]) -> Dict[str, Any]:
        """Calculate overall optimization score."""
        if not bottlenecks:
            return {"score": 100, "grade": "A", "description": "No performance issues detected"}
