...
class ECSBottleneckAnalyzer:
    ...
    async def _test_database_intensive_endpoints(self):
        ...
        for i in range(15):
            ...
            self.tracker.start_request(request_id, "/agents/agent1/compatibility/agent2", "GET")
    ...
    async def _test_memory_intensive_endpoints(self):
        ...
        for i in range(3):
            ...
            personality_traits = [{"trait": f"trait_{j}", "value": j} for j in range(50000)]
            physical_traits = [{"trait": f"physical_{j}", "value": j} for j in range(30000)]
            ability_traits = [{"trait": f"ability_{j}", "value": j} for j in range(40000)]
    ...
    def _generate_ecs_analysis(self, bottlenecks: List, trends: List, report: Dict) -> Dict[str, Any]:
        ...
        for bottleneck in bottlenecks:
            endpoint = bottleneck.affected_endpoints[0] if bottleneck.affected_endpoints else ""
            
            if '/agents' in endpoint and 'compatibility' in endpoint:
                ecs_bottlenecks['genetic_calculations'].append(bottleneck)
            elif '/agents' in endpoint and ('mates' in endpoint or 'lineage' in endpoint):
        ...
        agent_requests = len([m for m in self.tracker.metrics_history if '/agents' in m.endpoint])
        database_requests = len([m for m in self.tracker.metrics_history if any(db in m.endpoint for db in ['naming', 'traits', 'breeding'])])
        
        ecs_metrics = {
            'total_requests': total_requests,
            'agent_management_requests': agent_requests,
            'database_intensive_requests': database_requests,
            'avg_agent_request_time': sum(m.duration for m in self.tracker.metrics_history if '/agents' in m.endpoint) / max(agent_requests, 1),
            'avg_database_request_time': sum(m.duration for m in self.tracker.metrics_history if any(db in m.endpoint for db in ['naming', 'traits', 'breeding'])) / max(database_requests, 1),
        ...
        return {
            ...
            'summary': {
                ...
                'ecs_components_affected': len([k for k, v in ecs_bottlenecks.items() if v]),
                'optimization_score': report.get('optimization_score', {}).get('score', 0)
            }
        }
...
async def main():
    ...
    try:
        ...
        print(f"  Database-Intensive Requests: {metrics['database_intensive_requests']}")
        print(f"  Avg Agent Request Time: {metrics['avg_agent_request_time']*1000:.1f}ms")
        print(f"  Avg Database Request Time: {metrics['avg_database_request_time']*1000:.1f}ms")
