...
class SpiritValidator:
    ...
    VALID_SPIRITS = {
        ...
        'yeti', 'loch_ness', 'chupacabra', 'wendigo', 'skinwalker'
    }
...
class StyleValidator:
    ...
    VALID_STYLES = {
        'foundation', 'exo', 'hybrid', 'cyberpunk', 
...
class CoordinateValidator:
    ...
    @classmethod
    def validate(cls, value: Union[int, float], field_name: str = "coordinate") -> float:
...
class InteractionTypeValidator:
    ...
    VALID_TYPES = {
        'communication', 'social', 'collaboration', 'conflict',
        'breeding', 'mentoring', 'learning', 'playing'
    }
    ...
    @classmethod
    def validate_and_raise(cls, interaction_type: str, field_name: str = "interaction_type") -> str:
...
class ValidatedAgentCreateRequest(BaseModel):
    ...
    agent_id: str = Field(..., min_length=3, max_length=50, description="Unique agent identifier")
...
def validate_agent_exists(agent_id: str, agent_data: Optional[Dict[str, Any]] = None) -> str:
...
def validate_endpoint_inputs(endpoint: str, **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        if 'agent_id' in kwargs:
            validated['agent_id'] = AgentIDValidator.validate_and_raise(kwargs['agent_id'])
        
        if 'agent1_id' in kwargs:
            validated['agent1_id'] = AgentIDValidator.validate_and_raise(kwargs['agent1_id'], 'agent1_id')
        
        if 'agent2_id' in kwargs:
            validated['agent2_id'] = AgentIDValidator.validate_and_raise(kwargs['agent2_id'], 'agent2_id')
        ...
        if 'interaction_type' in kwargs:
            validated['interaction_type'] = InteractionTypeValidator.validate_and_raise(kwargs['interaction_type'])
