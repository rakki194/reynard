...
from .database import ecs_db, Agent, PersonalityTrait, PhysicalTrait, AbilityTrait, AgentPosition, AgentAchievement, AgentSpecialization, AgentDomainExpertise, AgentWorkflowPreference, KnowledgeBaseEntry, PerformanceMetric
...
class SuccessAdvisorMigration:
    ...
    async def migrate_success_advisor_8(self, json_file_path: str) -> bool:
        ...
        try:
            ...
            with self.db.get_session() as session:
                ...
                existing_agent = session.query(Agent).filter(Agent.agent_id == data.get('id')).first()
                if existing_agent:
                    ...
                    # Create the agent
                    agent = Agent(
                        agent_id=data.get('id', 'permanent-release-manager-success-advisor-8'),
                ...
                session.query(PersonalityTrait).filter(PersonalityTrait.agent_id == agent.id).delete()
                session.query(PhysicalTrait).filter(PhysicalTrait.agent_id == agent.id).delete()
                session.query(AbilityTrait).filter(AbilityTrait.agent_id == agent.id).delete()
                session.query(AgentSpecialization).filter(AgentSpecialization.agent_id == agent.id).delete()
                session.query(AgentDomainExpertise).filter(AgentDomainExpertise.agent_id == agent.id).delete()
                session.query(AgentAchievement).filter(AgentAchievement.agent_id == agent.id).delete()
                session.query(AgentWorkflowPreference).filter(AgentWorkflowPreference.agent_id == agent.id).delete()
                session.query(KnowledgeBaseEntry).filter(KnowledgeBaseEntry.agent_id == agent.id).delete()
                session.query(PerformanceMetric).filter(PerformanceMetric.agent_id == agent.id).delete()
                ...
                # Add specializations
                specializations = data.get('knowledge_base', {}).get('specializations', [])
                ...
                for domain, expertise_data in knowledge_base.items():
                    if isinstance(expertise_data, dict):
                        ...
                        if expertise_data:
                            avg_expertise = sum(expertise_data.values()) / len(expertise_data)
                ...
                workflow_prefs = data.get('knowledge_base', {}).get('workflow_preferences', {})
                ...
                for perf_data in performance_history:
                    metric = PerformanceMetric(
                        ...
                        timestamp=datetime.fromisoformat(perf_data.get('timestamp', datetime.now(timezone.utc).isoformat())),
                        metadata={
                ...
                # Add position data
                position = session.query(AgentPosition).filter(AgentPosition.agent_id == agent.id).first()
                if not position:
    ...
    async def migrate_all_phoenix_data(self) -> Dict[str, bool]:
        ...
        if Path(success_advisor_file).exists():
            results["success_advisor_8"] = await self.migrate_success_advisor_8(success_advisor_file)
        ...
        if Path(new_agent_file).exists():
            results["new_agent_candidate"] = await self.migrate_phoenix_agent_data(new_agent_file)
        
        # Phoenix test generations
        phoenix_test_dir = Path("/home/kade/runeset/reynard/experimental/phoenix/data/phoenix_test")
        if phoenix_test_dir.exists():
            for gen_file in phoenix_test_dir.glob("generation_*.json"):
                results[f"phoenix_test_{gen_file.stem}"] = await self.migrate_phoenix_agent_data(str(gen_file))
    ...
    async def migrate_phoenix_agent_data(self, json_file_path: str) -> bool:
        ...
        try:
            ...
            with self.db.get_session() as session:
                # Create agent
                agent = Agent(
                    agent_id=data.get('id', f'phoenix-agent-{Path(json_file_path).stem}'),
