...
"""
    ...
    @pytest.mark.asyncio
    async def test_complete_success_advisor_8_workflow(self, agent_manager, legacy_service, mock_ecs_service):
        ...
        with patch.object(legacy_service.changelog_parser, 'parse_success_advisor_8_activities') as mock_activities:
        ...
        with patch.object(legacy_service, 'get_success_advisor_8_activities') as mock_activities, \
             ...
             patch.object(legacy_service.changelog_parser, 'analyze_activity_trends') as mock_analysis, \
             patch.object(legacy_service.changelog_parser, 'generate_activity_summary') as mock_summary:
        ...
        with patch.object(legacy_service.changelog_parser, 'analyze_activity_trends') as mock_trends:
    ...
    async def test_agent_state_update_workflow(self, agent_manager, mock_ecs_service):
        ...
        updated_state = AgentState(
            ...
            specializations=["release_management", "quality_assurance", "legacy_tracking"],
            achievements=[
                {
        ...
        # Update agent state
        update_result = await agent_manager.update_agent_state("success-advisor-8", updated_state)
    ...
    @pytest.mark.asyncio
    async def test_legacy_tracking_activity_workflow(self, legacy_service, mock_ecs_service):
    ...
    async def test_performance_characteristics(self, agent_manager, legacy_service):
        ...
        with patch.object(legacy_service, 'get_codebase_movements') as mock_movements, \
             patch.object(legacy_service, 'get_success_advisor_8_activities') as mock_activities, \
             patch.object(legacy_service, '_get_ecs_agent_data') as mock_ecs_data, \
             patch.object(legacy_service.changelog_parser, 'analyze_activity_trends') as mock_analysis, \
             patch.object(legacy_service.changelog_parser, 'generate_activity_summary') as mock_summary:
