...
class TestUnifiedAgentStateManager:
    ...
    @pytest.mark.asyncio
    async def test_track_success_advisor_8_activity(self, agent_manager, mock_ecs_service):
    ...
    @pytest.mark.asyncio
    async def test_get_success_advisor_8_legacy_report(self, agent_manager, mock_ecs_service):
        """Test Success-Advisor-8 legacy report generation."""
        # Mock legacy tracker
        mock_report = {"total_activities": 5, "summary": "Test report"}
        with patch.object(agent_manager.legacy_tracker, 'generate_legacy_report', new_callable=AsyncMock) as mock_tracker:
    ...
    async def test_update_agent_state_success(self, agent_manager, mock_ecs_service):
        ...
        state = AgentState(
            ...
            achievements=[{"name": "Update Test", "description": "Successfully updated"}]
        )
