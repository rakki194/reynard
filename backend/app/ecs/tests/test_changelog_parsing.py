...
class TestChangelogParsing:
    ...
    def test_parse_real_changelog(self):
        ...
        assert len(activities) > 0, "Should find Success-Advisor-8 activities in CHANGELOG"
        
        # Check that all activities contain Success-Advisor-8 references
        for activity in activities:
            assert "Success-Advisor-8" in activity.description or "SUCCESS-ADVISOR-8" in activity.description
...
"""
        ...
        try:
            ...
            assert len(activities) >= 3, "Should find multiple Success-Advisor-8 activities"
    ...
    def test_activity_classification(self):
        ...
        for description, expected_type in test_cases:
            ...
            assert actual_type == expected_type, f"Expected {expected_type} for '{description}', got {actual_type}"
    ...
    def test_pattern_matching(self):
        ...
        for text, should_match in test_cases:
            ...
            assert (matches is not None) == should_match, f"Pattern matching failed for '{text}'"
    ...
    def test_changelog_stats(self):
        ...
        assert "success_advisor_references" in stats, "Stats should include Success-Advisor-8 references"
    ...
    def test_parser_info(self):
        ...
        assert "existing_parser_available" in info, "Info should include existing parser status"
        assert "changelog_path" in info, "Info should include changelog path"
        assert info["parser_type"] in ["existing", "fallback", "unified"], "Parser type should be valid"
