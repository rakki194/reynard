...
class GenderComponent(Component):
    ...
    def __init__(self, profile: GenderProfile) -> None:
        """Initialize the gender component."""
        super().__init__()
        self.profile = profile
        self.gender_energy: float = 1.0  # Energy for gender expression (0.0 to 1.0)
        self.expression_confidence: float = (
            1.0  # Confidence in gender expression (0.0 to 1.0)
        )
        self.dysphoria_level: float = 0.0  # Gender dysphoria level (0.0 to 1.0)
        self.euphoria_level: float = 0.0  # Gender euphoria level (0.0 to 1.0)
    ...
    def get_expression_readiness(self) -> float:
        ...
        return (
            self.gender_energy + self.expression_confidence + self.social_comfort
        ) / 3.0

    def needs_support(self) -> bool:
        """Check if the agent needs additional support."""
        return (
            self.dysphoria_level > 0.5
            or self.expression_confidence < 0.3
            or self.social_comfort < 0.3
            or len(self.support_needs) > 0
        )

    def is_transitioning(self) -> bool:
        """Check if the agent is currently transitioning."""
        return self.transition_stage is not None or len(self.transition_goals) > 0
