...
from backend.app.ecs.customer_modeling.components.behavior_metrics import (
    BehaviorMetric,
    BehaviorMetrics,
    MetricType,
)
from backend.app.ecs.customer_modeling.components.customer_identity import (
    CustomerIdentity,
)
...
class BehaviorAnalysisSystem:
    ...
    async def analyze_customer_behavior(
        ...
        predictive_data: Optional[PredictiveData] = None,
    ) -> Dict[str, Any]:
    ...
    async def _detect_engagement_trend(
        self, behavior_metrics: BehaviorMetrics
        ...
        return BehaviorPattern(
            ...
            duration=(
                engagement_metrics[-1].recorded_at - engagement_metrics[0].recorded_at
            ).total_seconds()
            / 3600,
    ...
    async def _detect_purchase_pattern(
        self, behavior_metrics: BehaviorMetrics
    ) -> Optional[BehaviorPattern]:
        ...
        avg_purchase_value = sum(m.metric_value for m in purchase_metrics) / len(
            purchase_metrics
        )
        ...
        return BehaviorPattern(
            ...
            duration=(
                purchase_metrics[-1].recorded_at - purchase_metrics[0].recorded_at
            ).total_seconds()
            / 3600,
    ...
    async def _detect_session_behavior(
        self, behavior_metrics: BehaviorMetrics
    ) -> Optional[BehaviorPattern]:
        """Detect session behavior pattern."""

        session_metrics = [
            m for m in behavior_metrics.metrics if m.metric_name == "session_duration"
        ]
        if not session_metrics:
            return None

        avg_session_duration = sum(m.metric_value for m in session_metrics) / len(
            session_metrics
        )
    ...
    async def _detect_device_preference(
        self, behavior_metrics: BehaviorMetrics
    ) -> Optional[BehaviorPattern]:
        ...
        for metric in behavior_metrics.metrics:
            if metric.device_type:
                device_metrics[metric.device_type] = (
                    device_metrics.get(metric.device_type, 0) + 1
                )
    ...
    async def _detect_time_pattern(
        ...
        peak_day = (
            max(day_distribution, key=day_distribution.get)
            if day_distribution
            else None
        )
    ...
    async def _detect_churn_indicators(
        self, behavior_metrics: BehaviorMetrics
    ) -> Optional[BehaviorPattern]:
            ...
            return BehaviorPattern(
                ...
                characteristics={
                    ...
                    "days_since_last_activity": (
                        (datetime.utcnow() - behavior_metrics.last_activity).days
                        if behavior_metrics.last_activity
                        else 999
                    ),
                },
                first_observed=behavior_metrics.first_activity or datetime.utcnow(),
    ...
    async def _generate_behavior_insights(
        ...
        predictive_data: Optional[PredictiveData],
        ...
        # Engagement insight
        engagement_pattern = next(
            (p for p in patterns if p.pattern_type == "engagement_trend"), None
        )
            ...
            if engagement_pattern.trend == "increasing":
                insights.append(
                    BehaviorInsight(
                        ...
                        supporting_evidence={
                            "trend": engagement_pattern.trend,
                            "confidence": engagement_pattern.confidence,
                        },
        ...
        purchase_pattern = next(
            (p for p in patterns if p.pattern_type == "purchase_pattern"), None
        )
        if purchase_pattern:
            insights.append(
                BehaviorInsight(
        ...
        # Churn insight
        churn_pattern = next(
            (p for p in patterns if p.pattern_type == "churn_indicators"), None
        )
        if churn_pattern and churn_pattern.characteristics["risk_level"] in [
            "high",
            "medium",
        ]:
            insights.append(
    ...
    async def _calculate_behavior_scores(
        self, behavior_metrics: BehaviorMetrics, patterns: List[BehaviorPattern]
    ) -> Dict[str, float]:
        """Calculate behavior scores."""
        ...
        if behavior_metrics.session_duration > 0:
            scores["session_quality"] = min(
                1.0, behavior_metrics.session_duration / 1800
