...
class BehaviorMetrics:
    ...
    def _update_aggregated_metrics(self):
        ...
        if engagement_metrics:
            self.engagement_score = sum(m.metric_value for m in engagement_metrics) / len(engagement_metrics)
        
        # Calculate activity frequency
        if self.first_activity and self.last_activity:
            time_diff = (self.last_activity - self.first_activity).total_seconds()
            if time_diff > 0:
                self.activity_frequency = len(self.metrics) / (time_diff / 3600)  # metrics per hour
        ...
        session_metrics = [m for m in self.metrics if m.metric_name == "session_duration"]
        if session_metrics:
