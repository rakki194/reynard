...
class Demographics:
    ...
    def is_highly_educated(self) -> bool:
        ...
        return self.education_level in [EducationLevel.MASTER, EducationLevel.PHD, EducationLevel.PROFESSIONAL]
    
    def is_working_professional(self) -> bool:
        """Check if customer is a working professional."""
        return (self.occupation is not None and 
                self.industry is not None and 
                self.work_experience_years is not None and 
                self.work_experience_years > 0)
    ...
    def get_lifestyle_indicators(self) -> Dict[str, Any]:
        ...
        indicators = {
            ...
            "professional_status": "working" if self.is_working_professional() else "non_working",
    ...
    def get_completeness_score(self) -> float:
        ...
        required_fields = [
            'age', 'gender', 'location_country', 'income_bracket', 
            'education_level', 'family_status', 'occupation', 'industry'
        ]
        
        completed_fields = sum(1 for field in required_fields if getattr(self, field) is not None)
