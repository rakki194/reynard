...
class ECSCacheManager:
    ...
    def __init__(self, redis_enabled: bool = None):
        ...
        """
        self.redis_enabled = redis_enabled if redis_enabled is not None else CacheConfig.ECS_CACHE_ENABLED
...
def cache_ecs_query(namespace: str = "ecs", ttl: int = 1800, cache_key_prefix: str = None):
...
def cache_naming_spirits(ttl: int = 1800):
    ...
    Returns:
        Decorator function
    """
    return cache_ecs_query(namespace="naming_spirits", ttl=ttl, cache_key_prefix="spirits")
...
def cache_naming_components(ttl: int = 1800):
    ...
    Returns:
        Decorator function
    """
    return cache_ecs_query(namespace="naming_components", ttl=ttl, cache_key_prefix="components")
...
def cache_naming_config(ttl: int = 3600):
    ...
    Returns:
        Decorator function
    """
    return cache_ecs_query(namespace="naming_config", ttl=ttl, cache_key_prefix="config")
            ...
            try:
                ...
                if pattern:
                    ...
                    if cache_manager.cache_manager:
                        keys = await cache_manager.cache_manager.get_keys(f"{cache_manager.namespace}:{pattern}")
                        for key in keys:
                            await cache_manager.delete(key)
                    else:
                        # Remove from fallback cache
                        keys_to_remove = [k for k in cache_manager.fallback_cache.keys() if pattern in k]
