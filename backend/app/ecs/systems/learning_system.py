...
class LearningSystem(System):
        ...
        for i, entity1 in enumerate(entities):
            ...
            for entity2 in entities[i+1:]:
                ...
                if self._are_agents_in_proximity(entity1, entity2):
                    self._attempt_knowledge_sharing(entity1, entity2, knowledge_comp1, knowledge_comp2)
    ...
    def _attempt_knowledge_sharing(
        ...
        shareable_knowledge = self._find_shareable_knowledge(knowledge_comp1, knowledge_comp2)
    ...
    def _target_has_knowledge(self, target_comp: KnowledgeComponent, knowledge: Knowledge) -> bool:
        """Check if target agent already has similar knowledge."""
        for existing_knowledge in target_comp.knowledge.values():
            if (existing_knowledge.title.lower() == knowledge.title.lower() or
                existing_knowledge.knowledge_type == knowledge.knowledge_type):
                return True
        return False

    def _target_meets_prerequisites(self, target_comp: KnowledgeComponent, knowledge: Knowledge) -> bool:
    ...
    def _transfer_knowledge(
        ...
        max_proficiency = min(0.8, knowledge.proficiency * 0.9)  # Can't exceed teacher's level
        final_proficiency = base_proficiency + (max_proficiency - base_proficiency) * teaching_effectiveness * learning_effectiveness
    ...
    def _should_agent_teach(
        ...
        teachable_knowledge = [
            k for k in knowledge_comp.knowledge.values()
            if k.can_teach()
        ]
    ...
    def _initiate_teaching_session(
        ...
        for target_entity in nearby_entities:
            ...
            teachable_knowledge = self._find_shareable_knowledge(knowledge_comp, target_knowledge_comp)
            if teachable_knowledge:
                success = self._transfer_knowledge(
                    entity, target_entity, teachable_knowledge, knowledge_comp, target_knowledge_comp
                )
    ...
    def _seek_learning_opportunities(
        ...
        for source_entity in nearby_entities:
            ...
            learnable_knowledge = self._find_shareable_knowledge(source_knowledge_comp, knowledge_comp)
            if learnable_knowledge:
                success = self._transfer_knowledge(
                    source_entity, entity, learnable_knowledge, source_knowledge_comp, knowledge_comp
                )
    ...
    def force_knowledge_transfer(
        ...
        success = self._transfer_knowledge(
            source_entity, target_entity, knowledge, source_knowledge_comp, target_knowledge_comp
        )
