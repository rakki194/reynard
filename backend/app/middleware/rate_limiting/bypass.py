...
class RateLimitBypassMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if self._should_bypass_rate_limit(request):
            ...
            self.logger.debug(f"Rate limit bypass applied for {request.method} {request.url.path}")
...
def setup_rate_limit_bypass(
    ...
    **config_kwargs
    ...
    logger.info(f"Rate limit bypass middleware setup complete for {environment} environment")