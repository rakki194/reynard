...
class AdaptiveRateLimiter(BaseMiddleware):
    ...
    def _get_adaptive_limit(self, client_id: str, request: Request) -> Tuple[int, int]:
        ...
        adaptive_max_requests = max(
            1,
            min(adaptive_max_requests, int(max_requests * self._max_limit_multiplier)),
        )
    ...
    def _check_adaptive_rate_limit(
        self, client_id: str, limit: Tuple[int, int]
    ) -> bool:
    ...
    def _update_adaptive_rate_limit(
        self, client_id: str, limit: Tuple[int, int]
    ) -> None:
    ...
    def _calculate_system_load(self) -> float:
        ...
        recent_loads = [
            load
            for timestamp, load in self._system_load_history
    ...
    def _reduce_limits_globally(self) -> None:
        ...
        for client_id in self._adaptive_limits:
            ...
            self._adaptive_limits[client_id] = max(
                self._min_limit_multiplier, self._adaptive_limits[client_id]
            )
