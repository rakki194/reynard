...
class InputValidationMiddleware(BaseMiddleware):
    ...
    async def _validate_request(self, request: Request) -> Dict[str, Any]:
        ...
        if self.config.validate_request_body and request.method in ['POST', 'PUT', 'PATCH']:
    ...
    async def _validate_request_body(self, request: Request) -> List[str]:
        ...
        try:
            ...
            if body:
                ...
                security_errors = self._check_security_patterns(body_str, "request body")
    ...
    def _validate_username(self, username: str) -> List[str]:
        ...
        if len(username) < self.config.username_min_length:
            errors.append(f"Username too short (minimum {self.config.username_min_length} characters)")
        
        if len(username) > self.config.username_max_length:
            errors.append(f"Username too long (maximum {self.config.username_max_length} characters)")
    ...
    def _validate_password(self, password: str) -> List[str]:
        ...
        if len(password) < self.config.password_min_length:
            errors.append(f"Password too short (minimum {self.config.password_min_length} characters)")
        
        if len(password) > self.config.password_max_length:
            errors.append(f"Password too long (maximum {self.config.password_max_length} characters)")
    ...
    def _validate_email(self, email: str) -> List[str]:
        ...
        if len(email) > self.config.email_max_length:
            errors.append(f"Email too long (maximum {self.config.email_max_length} characters)")
