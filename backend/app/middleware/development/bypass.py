...
class DevelopmentBypassMiddleware(BaseMiddleware):
    ...
    def _is_development_environment(self) -> bool:
        ...
        return (
            ...
            or self.config.environment == "testing"
        )
    ...
    def get_bypass_info(self, request: Request) -> dict:
        ...
        return {
            ...
            "should_apply_bypass": (
                ...
                and self._is_localhost(client_ip)
                and self._is_fenrir_request(user_agent)
            ),
        }
...
def setup_development_bypass_middleware(
    ...
    Args:
        ...
        **config_kwargs: Additional configuration parameters
    """
    bypass_middleware = create_development_bypass_middleware(
        app, environment, **config_kwargs
    )

    # Add the middleware to the FastAPI app
    app.add_middleware(
        DevelopmentBypassMiddleware, config=bypass_middleware.config, **config_kwargs
    )

    logger.info(
        f"Development bypass middleware setup complete for {environment} environment"
    )
