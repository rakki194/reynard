...
class TestingSupportMiddleware(BaseMiddleware):
    ...
    def _add_performance_info(self, response: Response, request: Request) -> None:
        ...
        if hasattr(request.state, "testing_start_time"):
            ...
            if path in self._performance_metrics:
                recent_metrics = self._performance_metrics[path][-10:]  # Last 10 requests
    ...
    def add_test_scenario(self, scenario_name: str, scenario_config: Dict[str, Any]) -> None:
...
def setup_testing_support_middleware(
    ...
    **config_kwargs
    ...
    Args:
        ...
        **config_kwargs: Additional configuration parameters
    """
    testing_middleware = create_testing_support_middleware(app, environment, **config_kwargs)
    
    # Add the middleware to the FastAPI app
    app.add_middleware(
        TestingSupportMiddleware,
        config=testing_middleware.config,
        **config_kwargs
    )
    
    logger.info(f"Testing support middleware setup complete for {environment} environment")