...
class AgentEmailConfig(BaseModel):
    ...
    auto_reply_template: Optional[str] = Field(None, description="Auto-reply template ID")
...
class AgentEmailBulkRequest(BaseModel):
    ...
    target_agent_ids: List[str] = Field(..., min_length=1, description="Target agent IDs")
    message: AgentEmailMessage = Field(..., description="Email message")
    batch_size: Optional[int] = Field(10, ge=1, le=100, description="Batch size")
    delay_between_batches: Optional[float] = Field(1.0, ge=0.0, le=60.0, description="Delay between batches")
