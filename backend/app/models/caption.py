...
class CaptionRequest(BaseModel):
    ...
    generator_name: str = Field(..., description="Name of the caption generator (jtp2, florence2, wdv3, joycaption)")
...
class CaptionResponse(BaseModel):
    ...
    caption: Optional[str] = Field(None, description="Generated caption text")
    ...
    processing_time: Optional[float] = Field(None, description="Time taken to generate caption in seconds")
    caption_type: Optional[str] = Field(None, description="Type of caption generated (tags, caption, etc.)")
...
class BatchCaptionRequest(BaseModel):
    ...
    progress_callback: Optional[str] = Field(None, description="WebSocket endpoint for progress updates")
...
class BatchCaptionResponse(BaseModel):
    ...
    results: List[CaptionResponse] = Field(..., description="Results for each caption task")
...
class ModelStatus(BaseModel):
    ...
    error_message: Optional[str] = Field(None, description="Error message if model failed to load")
...
class ModelManagementRequest(BaseModel):
    ...
    config: Optional[Dict[str, Any]] = Field(None, description="Model configuration for loading")
...
class ModelManagementResponse(BaseModel):
    ...
    model_status: Optional[ModelStatus] = Field(None, description="Updated model status")
