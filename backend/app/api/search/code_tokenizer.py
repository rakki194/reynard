...
class CodeTokenizer:
    ...
    def _initialize_parsers(self) -> None:
        ...
        language_configs = {
            ...
            'typescript': {
                ...
                'keywords': ['function', 'class', 'interface', 'type', 'import', 'export', 'const', 'let', 'var']
            },
            'javascript': {
                'library': 'tree-sitter-javascript',
                'file_extensions': ['.js', '.jsx'],
                'keywords': ['function', 'class', 'import', 'export', 'const', 'let', 'var', 'async', 'await']
            },
            'java': {
                'library': 'tree-sitter-java',
                'file_extensions': ['.java'],
                'keywords': ['public', 'private', 'protected', 'class', 'interface', 'method', 'static', 'final']
            },
            'cpp': {
        ...
        for lang, config in language_configs.items():
            try:
                ...
                if language:
                    ...
                    logger.info(f"Tree-sitter parser for {lang} initialized successfully")
                else:
    ...
    def tokenize_with_ast(self, code: str, language: str) -> List[str]:
        ...
        try:
            ...
            logger.warning(f"AST tokenization failed for {language}: {e}, using fallback")
            return self._fallback_tokenize(code, language)
    
    def _extract_tokens_from_ast(self, node, code: str, tokens: List[str], language: str) -> None:
