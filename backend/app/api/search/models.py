...
class SemanticSearchRequest(SearchRequest):
    ...
    search_type: str = Field(
        default="hybrid",
        description="Search type: hybrid, vector, text, code",
    )
...
class SyntaxSearchRequest(SearchRequest):
    ...
    pattern_type: str | None = Field(
        default=None,
        description="Code pattern type to search for",
    )
...
class HybridSearchRequest(SearchRequest):
    ...
    syntax_weight: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Weight for syntax results",
    )
    similarity_threshold: float = Field(
        default=0.7,
        ge=0.0,
...
class IndexRequest(BaseModel):
    ...
    force_reindex: bool = Field(
        default=False,
        description="Force reindexing of existing files",
    )
...
class SearchResponse(BaseModel):
    ...
    search_strategies: list[str] = Field(
        default=[],
        description="Search strategies used",
    )
...
class SuggestionsResponse(BaseModel):
    ...
    error: str | None = Field(
        default=None,
        description="Error message if suggestions failed",
    )
