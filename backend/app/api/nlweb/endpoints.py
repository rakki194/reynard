...
async def get_health(current_user: User = Depends(require_active_user)):
...
async def get_performance_stats(current_user: User = Depends(require_active_user)):
...
async def enable_tool(
    ...
    try:
        ...
        return {"success": True, "message": f"Tool '{tool_name}' enabled successfully"}
...
async def disable_tool(
    ...
    try:
        ...
        return {"success": True, "message": f"Tool '{tool_name}' disabled successfully"}
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        timeout=aiohttp.ClientTimeout(
                            connect=service.configuration.proxy_connect_timeout_ms
                            / 1000,
                        ...
                        async for line in response.content:
                            if line:
                                try:
                                    ...
                                    yield f"data: {json.dumps({'type': 'error', 'error': f'Parsing error: {e!s}'})}\n\n"
...
async def clear_cache(current_user: User = Depends(require_active_user)):
...
def _map_nlweb_event_to_reynard(event_data: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"type": "content", "content": event.get("content") or json.dumps(event)}

    except json.JSONDecodeError:
        # If we can't parse the JSON, return as error
        return {"type": "error", "error": f"Invalid JSON in event: {event_data}"}
    except Exception as e:
        # Any other error
        return {"type": "error", "error": f"Event mapping error: {e!s}"}
