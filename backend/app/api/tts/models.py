...
class TTSSynthesisRequest(BaseModel):
    ...
    backend: str | None = Field(
        None,
        description="TTS backend to use (kokoro, coqui, xtts)",
    )
...
class TTSSynthesisResponse(BaseModel):
    ...
    quality_metrics: dict[str, Any] = Field(
        ...
        description="Audio quality metrics and analysis",
    )
...
class TTSBatchResponse(BaseModel):
    ...
    error: str | None = Field(
        None,
        description="Error message if batch synthesis failed",
    )
...
class TTSConfigRequest(BaseModel):
    ...
    chunk_size: int | None = Field(
        None,
        description="Text chunk size for long texts",
        ge=100,
        le=5000,
    )
...
class AudioQualityMetrics(BaseModel):
    ...
    frequency_response_score: float = Field(
        0.0,
        description="Frequency response quality",
    )
...
class AudioValidationRequest(BaseModel):
    ...
    validate_content: bool = Field(
        True,
        description="Whether to perform content validation",
    )
...
class QualityTrendResponse(BaseModel):
    ...
    trends: dict[str, Any] = Field(
        default_factory=dict,
        description="Quality trends by category",
    )
...
class QualitySummaryResponse(BaseModel):
    ...
    backend_performance: dict[str, Any] = Field(
        default_factory=dict,
        description="Backend performance metrics",
    )
