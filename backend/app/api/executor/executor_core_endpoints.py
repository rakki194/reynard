...
from app.executor import ExecutorConfig, ExecutorState, get_global_executor
...
async def get_executor_state():
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Failed to get executor state: {e!s}",
        )
...
async def get_executor_stats():
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get executor stats: {e!s}",
        )
...
async def get_active_tasks():
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get active tasks: {e!s}",
        )


@router.post("/initialize")
async def initialize_executor(config: ExecutorConfigRequest | None = None):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to initialize executor: {e!s}",
        )
