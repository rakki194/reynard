...
class MCPBootstrapRequest(BaseModel):
    ...
    client_secret: str = Field(
        ..., description="Client secret for bootstrap authentication"
    )
...
def _validate_bootstrap_credentials(client_id: str, client_secret: str) -> bool:
    ...
    if not expected_secret:
        logger.warning(
            f"Environment variable {env_var_name} not set for client: {client_id}"
        )
        return False
...
async def bootstrap_mcp_authentication(
    request: MCPBootstrapRequest,
    ...
    try:
        ...
        if not _validate_bootstrap_credentials(
            request.client_id, request.client_secret
        ):
            logger.warning(
                f"Invalid bootstrap credentials for client: {request.client_id}"
            )
            raise HTTPException(
        ...
        if not client:
            logger.warning(
                f"Bootstrap authentication failed: client '{request.client_id}' not found"
            )
            raise HTTPException(
        ...
        if not client.is_active:
            logger.warning(
                f"Bootstrap authentication failed: client '{request.client_id}' is inactive"
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        logger.info(
            f"Bootstrap authentication successful for client: {request.client_id}"
        )
