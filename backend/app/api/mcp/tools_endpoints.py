...
class MCPToolCall(BaseModel):
    ...
    params: Dict[str, Any] = Field(default_factory=dict, description="Arguments for the MCP tool")

class MCPToolResponse(BaseModel):
    """Response model for an MCP tool call."""
    success: bool = Field(..., description="True if the tool call was successful")
    result: Optional[Dict[str, Any]] = Field(None, description="Result of the tool call")
...
async def _send_mcp_request(method: str, params: Dict[str, Any]) -> Any:
    ...
    try:
        ...
        reader, writer = await asyncio.wait_for(
            asyncio.open_connection(MCP_HOST, MCP_PORT),
            timeout=MCP_TIMEOUT
        )
...
async def call_mcp_tool(tool_call: MCPToolCall) -> MCPToolResponse:
    ...
    """
    try:
        logger.info(f"Calling MCP tool: {tool_call.method} with params {tool_call.params}")
        
        # Map the tool call to the appropriate MCP method
        if tool_call.method in ["get_simulation_status", "get_ecs_agent_positions", "create_ecs_agent"]:
