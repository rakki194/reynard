...
class CaptionConfigModel(BaseModel):
    ...
    max_batch_size: int = Field(
        ...
        ge=1,
        le=100,
        description="Maximum batch size for caption generation",
    )
    ...
    enable_auto_loading: bool = Field(
        True,
        description="Enable automatic model loading",
    )
    ...
    model_unload_timeout: int = Field(
        ...
        ge=300,
        le=7200,
        description="Model unload timeout in seconds",
    )

    # Quality settings
    enable_quality_validation: bool = Field(
        True,
        description="Enable caption quality validation",
    )
    ...
    enable_post_processing: bool = Field(
        True,
        description="Enable post-processing of captions",
    )
    ...
    generation_rate_limit: int = Field(
        ...
        ge=1,
        le=100,
        description="Generation requests per minute",
    )
    ...
    model_management_rate_limit: int = Field(
        ...
        ge=1,
        le=100,
        description="Model management requests per minute",
    )
    ...
    enable_content_filtering: bool = Field(
        True,
        description="Enable content filtering for generated captions",
    )
...
class CaptionGenerationServiceRouter(
    ...
    def _setup_caption_endpoints(self):
        ...
        async def get_available_generators():
            return await self._standard_async_operation(
                ...
                self._get_available_generators_impl,
            )
    ...
    async def _generate_caption_impl(
        ...
        if not image_path or not generator_name:
            raise HTTPException(
                status_code=400,
                detail="image_path and generator_name are required",
            )
