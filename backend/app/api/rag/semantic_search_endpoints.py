...
async def analyze_code_structure(
    ...
    analysis_type: str = Query(
        "functions",
        description="Analysis type: functions, classes, imports, or symbols",
    ),
...
async def search_documentation(
    ...
    try:
        ...
        for result in results:
            ...
            if doc_type == "all" or (
                (doc_type == "markdown" and chunk_type in ["markdown", "documentation"])
                or (
                    doc_type == "comments"
                    and chunk_type in ["comment", "inline_comment"]
                )
                or (
                    doc_type == "docstrings"
                    and chunk_type in ["docstring", "function_doc"]
                )
            ):
                doc_results.append(
                    {
