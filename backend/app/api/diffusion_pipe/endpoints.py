    ...
    try:
        ...
        active_count = len(
            [
                t
                for t in trainings
                if t.status in [TrainingStatus.PENDING, TrainingStatus.RUNNING]
            ]
        )
        completed_count = len(
...
async def resume_training(
    ...
    try:
        ...
        if training_status.status not in [
...
async def get_metrics(
    ...
    try:
        ...
        # Get metrics summary
        summary = (
            await service.metrics_collector.get_metrics_summary(request_id)
...
async def get_training_logs(
    ...
    try:
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename={training_id}_logs.txt"
            },
        )
...
async def get_chroma_recommendations(
    ...
    try:
        logger.info(
            f"Getting Chroma recommendations for {dataset_size} images, {gpu_memory_gb}GB GPU"
        )
...
async def import_training_profile(
    ...
    try:
        logger.info(
            f"Importing training profile: {profile_data.get('name', 'unknown')}"
        )
...
async def websocket_training_updates(
    ...
    try:
        ...
        logger.info(
            f"WebSocket connection established for training {training_id}: {connection_id}"
        )
        ...
        logger.info(
            f"WebSocket disconnected for training {training_id}: {connection_id}"
        )
    except Exception as e:
