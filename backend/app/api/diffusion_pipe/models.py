...
class DatasetConfig(BaseModel):
    ...
    enable_ar_bucket: bool = Field(default=True, description="Enable aspect ratio bucketing")
    ...
    frame_buckets: List[int] = Field(default=[1], description="Frame buckets for video models")
    shuffle_tags: bool = Field(default=True, description="Shuffle tags during training")
    
    # Directory configurations
    directories: List[Dict[str, Any]] = Field(
        default_factory=list, 
        description="Dataset directory configurations"
    )
...
class ChromaTrainingConfig(BaseModel):
    ...
    transformer_dtype: DataType = Field(default=DataType.FLOAT8, description="Transformer data type")
    ...
    adapter_dtype: DataType = Field(default=DataType.BFLOAT16, description="Adapter data type")
    
    # Optimizer configuration
    optimizer_type: OptimizerType = Field(default=OptimizerType.ADAMW_OPTIMI, description="Optimizer type")
    ...
    micro_batch_size_per_gpu: int = Field(default=4, description="Micro batch size per GPU")
    ...
    wandb_tracker_name: str = Field(default="chroma-lora", description="Wandb tracker name")
...
class DiffusionPipeConfig(BaseModel):
    ...
    training_model_config: ChromaTrainingConfig = Field(..., description="Model configuration")
    ...
    enable_gradient_checkpointing: bool = Field(default=True, description="Enable gradient checkpointing")
    ...
    def validate_configuration_consistency(self) -> "DiffusionPipeConfig":
        ...
        if self.training_model_config.optimizer_type in [OptimizerType.ADAMW, OptimizerType.ADAMW_OPTIMI]:
...
class TrainingRequest(BaseModel):
    ...
    # Resource management
    max_gpu_memory_gb: Optional[int] = Field(default=None, description="Maximum GPU memory usage")
...
class TrainingStatus(BaseModel):
    ...
    progress_percentage: float = Field(default=0.0, description="Training progress percentage")
    
    # Timing information
    start_time: Optional[datetime] = Field(default=None, description="Training start time")
    ...
    gpu_memory_usage: Optional[float] = Field(default=None, description="GPU memory usage in GB")
    ...
    # Output information
    output_path: Optional[str] = Field(default=None, description="Output path for results")
    checkpoint_path: Optional[str] = Field(default=None, description="Latest checkpoint path")
...
class TrainingMetrics(BaseModel):
    ...
    loss_trend: str = Field(default="stable", description="Loss trend (increasing/decreasing/stable)")
    ...
    estimated_time_remaining: Optional[int] = Field(default=None, description="Estimated time remaining in seconds")
    ...
    # System metrics
    temperature_gpu: Optional[float] = Field(default=None, description="GPU temperature in Celsius")
    power_usage: Optional[float] = Field(default=None, description="Power usage in watts")
...
class CheckpointInfo(BaseModel):
    ...
    checksum: Optional[str] = Field(default=None, description="Checkpoint file checksum")
    
    # Training state
    loss: float = Field(..., description="Loss value at checkpoint")
    learning_rate: float = Field(..., description="Learning rate at checkpoint")
    optimizer_state: Optional[Dict[str, Any]] = Field(default=None, description="Optimizer state")
    
    # Model state
    model_state: Optional[Dict[str, Any]] = Field(default=None, description="Model state")
    adapter_state: Optional[Dict[str, Any]] = Field(default=None, description="Adapter state")
...
class TrainingListResponse(BaseModel):
    ...
    trainings: List[TrainingStatus] = Field(..., description="List of training statuses")
...
class HealthResponse(BaseModel):
    ...
    gpu_memory_free: Optional[float] = Field(default=None, description="Free GPU memory in GB")
