...
router = APIRouter(
    prefix="/api/embedding-visualization", tags=["embedding-visualization"]
)
...
async def get_embedding_stats(current_user: User = Depends(require_active_user)):
    """
...
async def analyze_embedding_quality(
    request: EmbeddingQualityRequest, current_user: User = Depends(require_active_user)
):
    """
    ...
    try:
        ...
        # Analyze quality
        quality_metrics = await embedding_viz_service.analyze_embedding_quality(
            request.embeddings
        )
...
async def get_cache_stats(current_user: User = Depends(require_active_user)):
    """
