...
async def get_mcp_bridge_status():
    ...
    try:
        ...
        return MCPBridgeStatus(
            ...
            message=(
                "MCP bridge service is operational"
                if bridge._initialized
                else "MCP bridge service not initialized"
            ),
        )
...
async def initialize_mcp_bridge():
    ...
    try:
        ...
        if success:
            return {
                "success": True,
                "message": "MCP bridge service initialized successfully",
            }
        else:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def list_mcp_tools():
    ...
    try:
        ...
        if hasattr(bridge, '_tool_registry') and bridge._tool_registry:
            ...
            tools = (
                list(bridge._tool_registry.keys())
                if isinstance(bridge._tool_registry, dict)
                else []
            )
