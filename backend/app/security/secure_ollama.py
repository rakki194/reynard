...
from ..security.input_validator import (
    SecureChatRequest, validate_command_input, sanitize_input
)
...
class SecureOllamaService:
    ...
    def _validate_chat_security(self, request: SecureChatRequest) -> bool:
        ...
        try:
            ...
            if not validate_command_input(request.message):
                logger.warning(f"Command injection attempt in chat message: {request.message[:100]}...")
                return False
            
            # Check for suspicious patterns in message
            if self._contains_suspicious_patterns(request.message):
                logger.warning(f"Suspicious pattern in chat message: {request.message[:100]}...")
                return False
            
            # Validate system prompt if provided
            if request.system_prompt:
                if not validate_command_input(request.system_prompt):
                    logger.warning(f"Command injection attempt in system prompt: {request.system_prompt[:100]}...")
                    return False
                
                if self._contains_suspicious_patterns(request.system_prompt):
                    logger.warning(f"Suspicious pattern in system prompt: {request.system_prompt[:100]}...")
                    return False
    ...
    def _sanitize_chat_response(self, response: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            sensitive_fields = [
                'internal_state', 'debug_info', 'error_details', 'stack_trace',
                'file_paths', 'system_info', 'environment_variables',
                'database_queries', 'api_keys', 'secrets', 'tokens'
            ]
            ...
            for field in text_fields:
                if field in response_dict and isinstance(response_dict[field], str):
                    response_dict[field] = self._sanitize_text_content(response_dict[field])
            
            # Sanitize metadata
            if 'metadata' in response_dict:
                response_dict['metadata'] = self._sanitize_metadata(response_dict['metadata'])
    ...
    def _sanitize_assistant_response(self, response: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            sensitive_fields = [
                ...
                'file_paths', 'system_info', 'environment_variables',
                'database_queries', 'api_keys', 'secrets', 'tokens',
                'tool_calls', 'function_calls', 'execution_details'
            ]
    ...
    def _sanitize_text_content(self, text: str) -> str:
        ...
        try:
            ...
            system_patterns = [
                r'Linux\s+[^\s]+', r'Ubuntu\s+[^\s]+', r'CentOS\s+[^\s]+',
                r'Debian\s+[^\s]+', r'Red\s+Hat\s+[^\s]+', r'Fedora\s+[^\s]+',
                r'Arch\s+Linux', r'Gentoo\s+[^\s]+', r'Slackware\s+[^\s]+',
                r'FreeBSD\s+[^\s]+', r'OpenBSD\s+[^\s]+', r'NetBSD\s+[^\s]+',
                r'Darwin\s+[^\s]+', r'macOS\s+[^\s]+', r'Windows\s+[^\s]+',
            ]
    ...
    def _sanitize_metadata(self, metadata: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            sensitive_fields = [
                'file_paths', 'system_info', 'environment_variables',
                'database_queries', 'api_keys', 'secrets', 'tokens',
                'internal_state', 'debug_info', 'error_details'
            ]
