...
from ..security.input_validator import (
    SecureUserCreate, SecureUserLogin,
    validate_sql_input, validate_command_input, sanitize_input
)
...
class SecureAuthManager:
    ...
    def _validate_user_creation_security(self, user_data: SecureUserCreate) -> bool:
        ...
        try:
            ...
            if not validate_sql_input(user_data.username):
                logger.warning(f"SQL injection attempt in username: {user_data.username}")
                return False
            ...
            if not validate_sql_input(user_data.full_name):
                logger.warning(f"SQL injection attempt in full_name: {user_data.full_name}")
                return False
            
            # Check for command injection patterns
            if not validate_command_input(user_data.username):
                logger.warning(f"Command injection attempt in username: {user_data.username}")
                return False
            ...
            if not validate_command_input(user_data.full_name):
                logger.warning(f"Command injection attempt in full_name: {user_data.full_name}")
                return False
    ...
    def _validate_login_security(self, login_data: SecureUserLogin) -> bool:
        ...
        try:
            ...
            if not validate_sql_input(login_data.username):
                logger.warning(f"SQL injection attempt in login username: {login_data.username}")
                return False
            
            # Check for command injection patterns
            if not validate_command_input(login_data.username):
                logger.warning(f"Command injection attempt in login username: {login_data.username}")
                return False
            
            # Check for suspicious patterns
            if self._contains_suspicious_patterns(login_data.username):
                logger.warning(f"Suspicious login username pattern: {login_data.username}")
                return False
    ...
    def _contains_suspicious_patterns(self, text: str) -> bool:
        ...
        suspicious_patterns = [
            ...
            r'<iframe', r'<object', r'<embed', r'<applet', r'<form',
            r'onload=', r'onerror=', r'onclick=', r'onmouseover=',
            r'alert\(', r'confirm\(', r'prompt\(', r'document\.write',
            r'innerHTML', r'outerHTML', r'document\.cookie',
            r'window\.open', r'window\.location', r'window\.history'
        ]
