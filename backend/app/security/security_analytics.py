...
class SecurityAnalytics:
    ...
    def __init__(self, max_events: int = 10000, retention_hours: int = 24):
        ...
        self.hourly_stats: dict[int, dict[str, int]] = defaultdict(
    ...
    def get_analytics_summary(self, hours: int = 24) -> dict[str, Any]:
        ...
        return {
            ...
            "top_user_agents": dict(
                sorted(user_agents.items(), key=lambda x: x[1], reverse=True)[:5],
            ),
    ...
    def get_ip_analysis(self, ip: str, hours: int = 24) -> dict[str, Any]:
        ...
        cutoff_time = time.time() - (hours * 3600)
        ip_events = [
            e for e in self.events if e.client_ip == ip and e.timestamp >= cutoff_time
        ]
    ...
    def _generate_recommendations(
        ...
        if "sql_injection" in event_types:
            recommendations.append(
                "Review database query patterns and implement parameterized queries",
            )

        if "xss_attempt" in event_types:
            recommendations.append(
                "Implement additional input sanitization and output encoding",
            )

        if "path_traversal" in event_types:
            recommendations.append(
                "Review file access patterns and implement path validation",
            )
    ...
    def _get_ip_recommendations(
        ...
        if len(set(event_types)) > 3:
            recommendations.append(
                "Multiple attack vectors detected - sophisticated attacker",
            )

        if len(events) > 20:
            recommendations.append(
                "High frequency of events - potential automated attack",
            )
    ...
    def _cleanup_analytics_data(self, cutoff_time: float) -> None:
        ...
        for path in list(self.path_activity.keys()):
            self.path_activity[path] = [
                t for t in self.path_activity[path] if t >= cutoff_time
            ]
        ...
        for ua in list(self.user_agent_activity.keys()):
            self.user_agent_activity[ua] = [
                t for t in self.user_agent_activity[ua] if t >= cutoff_time
            ]
