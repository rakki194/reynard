...
async def secure_register(
    ...
    """
    try:
        logger.info(f"Secure user registration attempt for username: {user_data.username}")
...
async def secure_get_current_user(
    ...
    """
    try:
        logger.info(f"Secure user info request for username: {current_user.get('username', 'unknown')}")
...
async def secure_logout(
    ...
    """
    try:
        logger.info(f"Secure logout request for username: {current_user.get('username', 'unknown')}")
