...
class QualityMetricsAnalyzer:
    ...
    def analyze_quality(self) -> RefactoringQualityReport:
        ...
        refactoring_completeness = self._calculate_refactoring_completeness(
            service_metrics,
        )
    ...
    def _find_all_python_files(self) -> list[Path]:
        ...
        for root, dirs, files in os.walk(self.backend_root):
            ...
            dirs[:] = [
                ...
                for d in dirs
                if not d.startswith(".")
                and d not in ["__pycache__", "node_modules", "venv", "tests"]
            ]
    ...
    def _analyze_file_quality(self, file_path: Path) -> CodeQualityMetrics | None:
    ...
    def _analyze_service_quality(
        ...
        avg_complexity = statistics.mean(
            [fm.cyclomatic_complexity for fm in service_files],
        )
        ...
        error_handling_standardized = self._check_error_handling_standardization(
            service_files,
        )
        configuration_centralized = self._check_configuration_centralization(
            service_files,
        )
    ...
    def _check_logging_standardization(
        ...
        avg_docstring_ratio = statistics.mean(
            [fm.docstring_ratio for fm in service_files],
        )
    ...
    def _calculate_refactoring_score(
        self,
        health_check: bool,
        error_handling: bool,
        configuration: bool,
    ...
    def _calculate_overall_quality(
        ...
        if service_metrics:
            avg_refactoring_score = statistics.mean(
                [sm.refactoring_score for sm in service_metrics],
            )
        else:
    ...
    def _calculate_refactoring_completeness(
        ...
        well_refactored = len(
            [sm for sm in service_metrics if sm.refactoring_score >= 0.7],
        )
    ...
    def _calculate_code_consistency(
        ...
        maintainability_variance = (
            statistics.variance(maintainabilities) if len(maintainabilities) > 1 else 0
        )
        ...
        for service in service_metrics:
            if service.refactoring_score < 0.5:
                recommendations.append(
                    f"Service '{service.service_name}' needs significant refactoring (score: {service.refactoring_score:.2f})",
                )

            if not service.health_check_implemented:
                recommendations.append(
                    f"Service '{service.service_name}' should implement standardized health checks",
                )

            if not service.error_handling_standardized:
                recommendations.append(
                    f"Service '{service.service_name}' should standardize error handling",
                )

            if service.test_coverage < 0.5:
                recommendations.append(
                    f"Service '{service.service_name}' needs better test coverage (current: {service.test_coverage:.1%})",
                )

        # File-level recommendations
        high_complexity_files = [
            fm for fm in file_metrics if fm.cyclomatic_complexity > 15
        ]
        if high_complexity_files:
            recommendations.append(
                f"Refactor {len(high_complexity_files)} files with high complexity (>15)",
            )

        low_maintainability_files = [
            fm for fm in file_metrics if fm.maintainability_index < 0.4
        ]
        if low_maintainability_files:
            recommendations.append(
                f"Improve maintainability of {len(low_maintainability_files)} files",
            )

        # Overall recommendations
        avg_test_coverage = (
            statistics.mean([fm.test_coverage for fm in file_metrics])
            if file_metrics
            else 0
        )
        if avg_test_coverage < 0.7:
            recommendations.append(
                f"Increase overall test coverage (current: {avg_test_coverage:.1%})",
            )
    ...
    def print_summary(self) -> None:
        ...
        for service in report.service_metrics:
            status = (
                "✅"
                if service.refactoring_score >= 0.7
                else "⚠️" if service.refactoring_score >= 0.4 else "🚨"
            )
