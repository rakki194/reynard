...
def upgrade():
    ...
    # Create enum types
    op.execute("CREATE TYPE environmenttype AS ENUM ('development', 'staging', 'production', 'all')")
    op.execute("CREATE TYPE operatortype AS ENUM ('GT', 'LT', 'EQ', 'NE', 'GTE', 'LTE')")
    op.execute("CREATE TYPE gatestatus AS ENUM ('PASSED', 'FAILED', 'WARN')")
