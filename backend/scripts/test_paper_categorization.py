...
from app.services.rag.services.core.document_categorization import DocumentCategorizationService
...
async def test_categorization_service():
    ...
    category = await categorization_service.categorize_paper_from_metadata(test_metadata)
...
async def test_paper_indexing_integration():
    ...
    result = await integration_service.process_paper_for_rag(
        paper_id=test_metadata["paper_id"],
...
async def test_with_real_papers():
    ...
    papers_dir = Path("/home/kade/runeset/reynard/backend/data/papers/arxiv")
    ...
    for i, metadata_file in enumerate(metadata_files[:test_count]):
        try:
            ...
            if category:
                logger.info(f"   ✅ Categorized: {category.primary_domain.value} (confidence: {category.confidence:.2f})")
                successful_tests += 1
            else:
    ...
    logger.info(f"Real paper test completed: {successful_tests}/{test_count} successful")
