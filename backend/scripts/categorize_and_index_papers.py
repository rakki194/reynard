...
from app.services.rag.document_categorization_service import DocumentCategorizationService
...
async def full_rag_processing(
    ...
    for domain, count in sorted(domain_stats["domain_distribution"].items(), key=lambda x: x[1], reverse=True):
    ...
    print(f"  Papers with processed content: {rag_summary['content_availability']['with_content']}")
    print(f"  Papers without content: {rag_summary['content_availability']['without_content']}")
...
Examples:
  ...
  python categorize_and_index_papers.py --full-processing --force-reprocess
    ...
    parser.add_argument(
        "--papers-dir",
        type=Path,
        default=Path("/home/kade/runeset/reynard/backend/data/papers/arxiv"),
    ...
    parser.add_argument(
        "--output-file",
        type=Path,
        help="Output file for categorization results"
    )
    ...
    parser.add_argument(
        "--domain",
        type=str,
        help="Filter by scientific domain (for --get-ready)"
    )
    ...
    parser.add_argument(
        "--limit",
        type=int,
        help="Limit number of results (for --get-ready)"
    )
    ...
    if not any([args.categorize_only, args.full_processing, args.get_ready]):
        logger.error("Must specify one of: --categorize-only, --full-processing, or --get-ready")
        return
