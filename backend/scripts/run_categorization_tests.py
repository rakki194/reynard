...
class CategorizationTestSuite:
    ...
    async def test_algorithm_paper_categorization(self, service: DocumentCategorizationService):
        ...
        try:
            ...
            if (category and 
                category.primary_domain == ScientificDomain.ALGORITHMS and 
                category.confidence > 0.5 and 
                "union-find" in category.keywords):
    ...
    async def test_ai_paper_categorization(self, service: DocumentCategorizationService):
        ...
        try:
            ...
            if (category and 
                category.primary_domain in [ScientificDomain.ARTIFICIAL_INTELLIGENCE, ScientificDomain.MACHINE_LEARNING] and 
    ...
    async def test_math_paper_categorization(self, service: DocumentCategorizationService):
        ...
        try:
            ...
            if (category and 
                category.primary_domain in [ScientificDomain.PURE_MATH, ScientificDomain.MATHEMATICS] and 
    ...
    async def test_physics_paper_categorization(self, service: DocumentCategorizationService):
        ...
        try:
            ...
            if (category and 
                category.primary_domain in [ScientificDomain.QUANTUM_PHYSICS, ScientificDomain.PHYSICS] and 
                category.confidence > 0.3):
    ...
    async def test_domain_keywords_mapping(self, service: DocumentCategorizationService):
        ...
        try:
            ...
            # Test AI domain
            ai_keywords = service.domain_keywords[ScientificDomain.ARTIFICIAL_INTELLIGENCE]
            ...
            if (has_algorithm and has_union_find and has_data_structure and 
                has_ai and has_neural_network and has_ml):
    ...
    async def test_arxiv_category_mapping(self, service: DocumentCategorizationService):
        ...
        try:
            ...
            cs_ds_mapping = service.arxiv_category_mapping.get("cs.DS") == ScientificDomain.ALGORITHMS
            cs_ai_mapping = service.arxiv_category_mapping.get("cs.AI") == ScientificDomain.ARTIFICIAL_INTELLIGENCE
            cs_lg_mapping = service.arxiv_category_mapping.get("cs.LG") == ScientificDomain.MACHINE_LEARNING
            
            # Test mathematics categories
            math_nt_mapping = service.arxiv_category_mapping.get("math.NT") == ScientificDomain.PURE_MATH
            math_st_mapping = service.arxiv_category_mapping.get("math.ST") == ScientificDomain.STATISTICS
            
            # Test physics categories
            quant_ph_mapping = service.arxiv_category_mapping.get("quant-ph") == ScientificDomain.QUANTUM_PHYSICS
            physics_gen_mapping = service.arxiv_category_mapping.get("physics.gen-ph") == ScientificDomain.PHYSICS
            
            if (cs_ds_mapping and cs_ai_mapping and cs_lg_mapping and 
                math_nt_mapping and math_st_mapping and 
                quant_ph_mapping and physics_gen_mapping):
                ...
                self.add_result(TestResult(
                    ...
                    "All expected arXiv categories mapped correctly",
                    {
                        ...
                        "cs_mappings": len([k for k in service.arxiv_category_mapping.keys() if k.startswith("cs.")]),
                        "math_mappings": len([k for k in service.arxiv_category_mapping.keys() if k.startswith("math.")]),
                        "physics_mappings": len([k for k in service.arxiv_category_mapping.keys() if k.startswith("physics.")])
                    }
                ))
            else:
                self.add_result(TestResult(
    ...
    async def test_statistics_tracking(self, service: DocumentCategorizationService):
        ...
        try:
            ...
            if new_count == initial_count + 1 and "algorithms" in updated_stats["metrics"]["domain_distribution"]:
                self.add_result(TestResult(
                    "Statistics Tracking",
                    True,
                    "Statistics properly tracked and updated",
                    {
                        ...
                        "domain_distribution": updated_stats["metrics"]["domain_distribution"]
                    }
                ))
            else:
                self.add_result(TestResult(
    ...
    async def test_paper_indexing_integration_initialization(self):
        ...
        try:
            ...
            if (success and 
                service.status.value == "healthy" and 
                service.categorization_service is not None):
                ...
                self.add_result(TestResult(
                    ...
                    "Integration service initialized successfully",
                    {
                        ...
                        "categorization_service_available": service.categorization_service is not None
                    }
                ))
                await service.shutdown()
                return service
    ...
    async def test_paper_processing_for_rag(self, integration_service: PaperIndexingIntegration):
        ...
        try:
            ...
            if ("error" not in result and 
                result["paper_id"] == "test-rag-001" and 
                result["rag_ready"]["searchable"] is True and
                result["rag_ready"]["auto_categorized"] is True and
                ...
                self.add_result(TestResult(
                    ...
                    "Paper successfully processed for RAG",
                    {
                        ...
                        "primary_domain": result["categorization"]["primary_domain"],
    ...
    async def test_batch_processing(self, integration_service: PaperIndexingIntegration):
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if (results["processed"] == 2 and 
                    results["failed"] == 0 and 
                    results["processing_summary"]["success_rate"] == 100.0 and
                    "algorithms" in results["domain_statistics"]["domain_distribution"] and
                    "artificial_intelligence" in results["domain_statistics"]["domain_distribution"]):
                    ...
                    self.add_result(TestResult(
                        ...
                        "Batch processing completed successfully",
                        {
                            ...
                            "domains_found": list(results["domain_statistics"]["domain_distribution"].keys())
                        }
                    ))
                else:
                    self.add_result(TestResult(
    ...
    async def test_health_checks(self, categorization_service: DocumentCategorizationService, 
        ...
        try:
            ...
            cat_healthy = (cat_health["status"] == "healthy" and 
                          cat_health["service"] == "document-categorization" and
                          cat_health["enabled"] is True)
            
            # Test integration service health check
            int_health = await integration_service.health_check()
            int_healthy = (int_health["status"] == "healthy" and 
                          int_health["service"] == "paper-indexing-integration" and
                          int_health["enabled"] is True)
            ...
            self.add_result(TestResult(
                "Health Checks",
                False,
                f"Exception during health checks: {str(e)}"
            ))
    ...
    async def run_all_tests(self):
        ...
        integration_service = await self.test_paper_indexing_integration_initialization()
        if not integration_service:
            logger.error("Cannot continue integration tests without integration service")
    ...
    def print_summary(self):
        ...
        if passed == total:
            logger.info("🎉 All tests passed! The categorization system is working correctly.")
        else:
