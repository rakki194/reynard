...
class TestDocumentCategorizationService:
    ...
    async def test_categorize_ai_paper(self, categorization_service):
        ...
        assert category is not None
        assert category.primary_domain in [
            ScientificDomain.ARTIFICIAL_INTELLIGENCE,
            ScientificDomain.MACHINE_LEARNING,
        ]
        assert category.confidence > 0.4
        assert (
            "neural network" in category.keywords
            or "deep learning" in category.keywords
        )
    ...
    async def test_domain_keywords_mapping(self, categorization_service):
        ...
        algo_keywords = categorization_service.domain_keywords[
            ScientificDomain.ALGORITHMS
        ]
        ...
        # Test AI domain
        ai_keywords = categorization_service.domain_keywords[
            ScientificDomain.ARTIFICIAL_INTELLIGENCE
        ]
    ...
    async def test_arxiv_category_mapping(self, categorization_service):
        ...
        assert (
            categorization_service.arxiv_category_mapping["cs.DS"]
            == ScientificDomain.ALGORITHMS
        )
        assert (
            categorization_service.arxiv_category_mapping["cs.AI"]
            == ScientificDomain.ARTIFICIAL_INTELLIGENCE
        )
        assert (
            categorization_service.arxiv_category_mapping["cs.LG"]
            == ScientificDomain.MACHINE_LEARNING
        )

        # Test mathematics categories
        assert (
            categorization_service.arxiv_category_mapping["math.NT"]
            == ScientificDomain.PURE_MATH
        )
        assert (
            categorization_service.arxiv_category_mapping["math.ST"]
            == ScientificDomain.STATISTICS
        )

        # Test physics categories
        assert (
            categorization_service.arxiv_category_mapping["quant-ph"]
            == ScientificDomain.QUANTUM_PHYSICS
        )
        assert (
            categorization_service.arxiv_category_mapping["physics.gen-ph"]
            == ScientificDomain.PHYSICS
        )
    ...
    async def test_confidence_scoring(self, categorization_service):
        ...
        category_high = await categorization_service.categorize_paper_from_metadata(
            metadata_high
        )
        ...
        category_low = await categorization_service.categorize_paper_from_metadata(
            metadata_low
        )
...
class TestPaperIndexingIntegration:
    ...
    async def test_get_rag_ready_papers(self, integration_service):
        ...
        ready_papers = await integration_service.get_rag_ready_papers(
            domain_filter=ScientificDomain.ALGORITHMS, min_confidence=0.3, limit=10
        )
