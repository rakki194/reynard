...
class TestCalendarIntegrationService:
    ...
    @pytest.mark.asyncio
    async def test_extract_meeting_requests_from_email_success(self, calendar_service, sample_email_content):
    ...
    async def test_schedule_meeting_success(self, calendar_service):
        ...
        assert event.end_time == selected_time + timedelta(minutes=meeting_request.duration_minutes)
    ...
    async def test_get_available_times_with_existing_events(self, calendar_service):
        ...
        for slot in time_slots:
            if slot['available']:
                ...
                assert not (slot_start < existing_event.end_time and slot_end > existing_event.start_time)
    ...
    async def test_cancel_meeting_success(self, calendar_service):
        ...
        result = await calendar_service.cancel_meeting(event.event_id, "Meeting cancelled")
    ...
    async def test_reschedule_meeting_success(self, calendar_service):
        ...
        assert updated_event.end_time == new_start_time + timedelta(minutes=new_duration_minutes)
    ...
    def test_extract_attendees(self, calendar_service):
        ...
        attendees = calendar_service._extract_attendees(text_with_emails, "sender@example.com")
    ...
    def test_is_within_working_hours(self, calendar_service):
        ...
        working_time = datetime.now().replace(hour=14, minute=0, second=0, microsecond=0)
        # Set to Tuesday (weekday 1)
        working_time = working_time.replace(day=working_time.day + (1 - working_time.weekday()) % 7)
        ...
        weekend_time = working_time.replace(day=working_time.day + (5 - working_time.weekday()) % 7)
    ...
    def test_save_and_load_events(self, calendar_service):
        ...
        new_service = CalendarIntegrationService(
            ...
            data_dir=calendar_service.data_dir
        )
    ...
    async def test_error_handling(self, calendar_service):
        ...
        with patch.object(calendar_service, '_save_events', side_effect=Exception("Save error")):
            # Should not raise exception
            calendar_service._save_events()
        
        with patch.object(calendar_service, '_save_meeting_requests', side_effect=Exception("Save error")):
            # Should not raise exception
            calendar_service._save_meeting_requests()
        
        with patch.object(calendar_service, '_load_events', side_effect=Exception("Load error")):
            # Should handle gracefully
            calendar_service._load_events()
            assert calendar_service.events == {}
        
        with patch.object(calendar_service, '_load_meeting_requests', side_effect=Exception("Load error")):
            # Should handle gracefully
...
class TestMeetingRequest:
    ...
    def test_meeting_request_creation(self):
        ...
        proposed_times = [datetime.now() + timedelta(hours=1), datetime.now() + timedelta(hours=2)]
