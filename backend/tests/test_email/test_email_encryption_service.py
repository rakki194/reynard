...
class TestEmailEncryptionService:
    ...
    @pytest.mark.asyncio
    async def test_generate_pgp_key_success(self, encryption_service, mock_gpg, sample_key_data):
    ...
    @pytest.mark.asyncio
    async def test_import_pgp_key_success(self, encryption_service, mock_gpg, sample_key_data):
        ...
        key = await encryption_service.import_pgp_key("-----BEGIN PGP PUBLIC KEY BLOCK-----\nTest Key Data\n-----END PGP PUBLIC KEY BLOCK-----")
    ...
    async def test_encrypt_email_pgp_success(self, encryption_service, mock_gpg):
        ...
        mock_encrypted.__str__ = Mock(return_value="-----BEGIN PGP MESSAGE-----\nEncrypted Content\n-----END PGP MESSAGE-----")
    ...
    async def test_encrypt_email_unsupported_method(self, encryption_service):
        ...
        with pytest.raises(ValueError, match="No encryption key found for test@example.com"):
            await encryption_service.encrypt_email(
                content="Test email content",
                recipient_email="test@example.com",
    ...
    async def test_decrypt_email_pgp_failure(self, encryption_service, mock_gpg):
        ...
        with pytest.raises(ValueError, match="PGP decryption failed"):
            await encryption_service.decrypt_email(
                encrypted_content="Invalid encrypted content",
                encryption_method="pgp"
            )
    ...
    async def test_decrypt_email_smime_invalid_content(self, encryption_service):
        ...
        with pytest.raises(ValueError, match="Invalid SMIME encrypted content"):
            await encryption_service.decrypt_email(
                encrypted_content="Invalid SMIME content",
                encryption_method="smime"
            )
    ...
    async def test_sign_email_pgp_success(self, encryption_service, mock_gpg):
        ...
        mock_signed.__str__ = Mock(return_value="-----BEGIN PGP SIGNED MESSAGE-----\nSigned Content\n-----END PGP SIGNATURE-----")
        ...
        assert signed_content == "-----BEGIN PGP SIGNED MESSAGE-----\nSigned Content\n-----END PGP SIGNATURE-----"
    ...
    async def test_verify_signature_pgp_failure(self, encryption_service, mock_gpg):
        ...
        verification_result = await encryption_service.verify_signature(
            signed_content="Invalid signed content",
            encryption_method="pgp"
        )
    ...
    async def test_verify_signature_smime_invalid_content(self, encryption_service):
        ...
        verification_result = await encryption_service.verify_signature(
            signed_content="Invalid SMIME content",
            encryption_method="smime"
        )
    ...
    async def test_get_public_key_not_found(self, encryption_service):
        ...
        public_key = await encryption_service.get_public_key("nonexistent@example.com", "pgp")
    ...
    async def test_find_recipient_key_found(self, encryption_service):
        ...
        found_key = await encryption_service._find_recipient_key("test@example.com", "pgp")
    ...
    async def test_find_recipient_key_not_found(self, encryption_service):
        ...
        found_key = await encryption_service._find_recipient_key("nonexistent@example.com", "pgp")
    ...
    def test_save_and_load_keys(self, encryption_service):
        ...
        new_service = EmailEncryptionService(
            config=encryption_service.config,
            data_dir=encryption_service.data_dir
        )
    ...
    async def test_error_handling(self, encryption_service):
        ...
        with patch.object(encryption_service, '_save_keys', side_effect=Exception("Save error")):
        ...
        with patch.object(encryption_service, '_load_keys', side_effect=Exception("Load error")):
