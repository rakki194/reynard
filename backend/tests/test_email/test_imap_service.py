...
class TestIMAPService:
    ...
    @pytest.mark.asyncio
    async def test_detect_agent_email_with_agent_service(self, imap_service, sample_email):
        ...
        with patch('app.services.imap_service.agent_email_service') as mock_service:
            mock_service.get_agent_config_by_email = AsyncMock(return_value=mock_agent_config)
    ...
    @pytest.mark.asyncio
    async def test_detect_agent_email_without_agent_service(self, imap_service, sample_email):
        """Test agent email detection without agent service."""
        with patch('app.services.imap_service.agent_email_service', side_effect=ImportError):
            result = await imap_service._detect_agent_email(sample_email)
    ...
    async def test_process_agent_email(self, imap_service, sample_email):
        ...
        with patch('app.services.imap_service.agent_email_service') as mock_service:
            ...
            mock_service.get_agent_config = AsyncMock(return_value=Mock(auto_reply_enabled=False))
            
            await imap_service._process_agent_email(sample_email)
            
            # Verify agent stats were updated
            mock_service.update_agent_stats.assert_called_once_with("agent-123", "received")
    ...
    async def test_mark_email_as_processed(self, imap_service, sample_email):
        ...
        assert imap_service.received_emails[sample_email.message_id].status == "processed"
    ...
    async def test_start_email_monitoring(self, imap_service):
        ...
        with patch.object(imap_service, 'get_unread_emails', new_callable=AsyncMock) as mock_get_emails:
