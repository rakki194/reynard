...
class TestSetupReynardMiddleware:
    ...
    def test_setup_reynard_middleware_logging(self, mock_logger):
        ...
        assert any("Setting up Reynard middleware" in str(call) for call in mock_logger.info.call_args_list)
        assert any("Reynard middleware setup complete" in str(call) for call in mock_logger.info.call_args_list)
...
class TestCreateMiddlewareStack:
    ...
    def test_create_middleware_stack_logging(self, mock_logger):
        ...
        # Check that logging was called
        assert mock_logger.info.called
        assert any("Creating custom middleware stack" in str(call) for call in mock_logger.info.call_args_list)
        assert any("Custom middleware stack creation complete" in str(call) for call in mock_logger.info.call_args_list)
...
class TestMiddlewareFactoryIntegration:
    ...
    def test_middleware_factory_development_bypass_integration(self):
        ...
        response = client.get(
            "/test",
            headers={"User-Agent": "Fenrir Exploit Suite"}
        )
