...
class TestSecurityHeaders:
    ...
    def test_content_length_validation(self, client: TestClient):
        ...
        response = client.post("/api/auth/register", 
                             json={"username": "test", "email": "test@example.com", 
                                   "password": "test123", "full_name": large_data})
    ...
    def test_security_headers_consistency(self, client: TestClient):
        ...
        for response in responses[1:]:
            ...
            assert headers.get("x-frame-options") == first_headers.get("x-frame-options")
            assert headers.get("x-content-type-options") == first_headers.get("x-content-type-options")
            assert headers.get("x-xss-protection") == first_headers.get("x-xss-protection")
            assert headers.get("strict-transport-security") == first_headers.get("strict-transport-security")
            assert headers.get("referrer-policy") == first_headers.get("referrer-policy")
