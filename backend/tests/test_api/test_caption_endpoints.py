...
class TestCaptionGenerators:
    ...
    def test_get_available_generators_success(self, client: TestClient, mock_caption_service):
        """Test successful retrieval of available generators."""
        with patch('app.api.caption.upload.get_caption_api_service', return_value=mock_caption_service):
    ...
    def test_get_available_generators_service_error(self, client: TestClient):
        ...
        with patch('app.api.caption.endpoints.get_caption_api_service') as mock_service:
            mock_service.return_value.get_available_generators.side_effect = Exception("Service error")
            response = client.get("/api/caption/generators")
    ...
    def test_get_generator_info_success(self, client: TestClient, mock_caption_service):
        ...
        with patch('app.api.caption.upload.get_caption_api_service', return_value=mock_caption_service):
            ...
            assert response.status_code == 200
            data = response.json()
            assert data["description"] == "Microsoft Florence2 - General purpose image captioning model"
            assert data["is_available"] is True
            assert data["caption_type"] == "caption"
            assert "config_schema" in data

    def test_get_generator_info_not_found(self, client: TestClient, mock_caption_service):
        """Test retrieval of nonexistent generator info."""
        mock_caption_service.get_generator_info.side_effect = ValueError("Generator not found")
...
class TestSingleCaptionGeneration:
    ...
    def test_generate_single_caption_success(self, client: TestClient, mock_caption_service, test_image_data):
    ...
    def test_generate_single_caption_invalid_generator(self, client: TestClient, test_image_data):
    ...
    def test_generate_single_caption_service_error(self, client: TestClient, mock_caption_service, test_image_data):
        """Test error handling during caption generation."""
        mock_caption_service.upload_and_generate_caption.side_effect = Exception("Generation failed")
    ...
    def test_generate_single_caption_generation_failed(self, client: TestClient, mock_caption_service, test_image_data):
...
class TestBatchCaptionGeneration:
    ...
    def test_generate_batch_captions_success(self, client: TestClient, mock_caption_service, test_image_data):
    ...
    def test_generate_batch_captions_mixed_results(self, client: TestClient, mock_caption_service, test_image_data):
