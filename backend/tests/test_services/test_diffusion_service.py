...
class TestDiffusionLLMServiceSimple:
    ...
    async def test_service_initialization_success(self, service, mock_config):
        ...
        with patch('app.services.diffusion.diffusion_service.DiffusionModelManager') as mock_model_manager, \
             patch('app.services.diffusion.diffusion_service.DeviceManager') as mock_device_manager:
    ...
    async def test_generate_stream_success(self, service, mock_config):
        ...
        with patch('app.services.diffusion.diffusion_service.DiffusionModelManager') as mock_model_manager, \
            ...
            async def mock_stream(params):
                yield DiffusionStreamEvent(type="token", data="Hello", timestamp=1234567890)
                yield DiffusionStreamEvent(type="token", data=" world", timestamp=1234567891)
                yield DiffusionStreamEvent(type="complete", data="", timestamp=1234567892)
    ...
    async def test_infill_stream_success(self, service, mock_config):
        ...
        with patch('app.services.diffusion.diffusion_service.DiffusionModelManager') as mock_model_manager, \
            ...
            # Mock streaming response
            async def mock_stream(params):
                yield DiffusionStreamEvent(type="token", data="infilled", timestamp=1234567890)
                yield DiffusionStreamEvent(type="complete", data="", timestamp=1234567891)
    ...
    async def test_get_stats(self, service, mock_config):
        ...
        with patch('app.services.diffusion.diffusion_service.DiffusionModelManager') as mock_model_manager, \
            ...
            assert stats.average_processing_time == 0.11052631578947368  # 10.5 / 95 (successful requests)
    ...
    async def test_health_check(self, service, mock_config):
        ...
        with patch('app.services.diffusion.diffusion_service.DiffusionModelManager') as mock_model_manager, \
            ...
            mock_model_mgr.get_available_models.return_value = [DiffusionModelInfo(
                model_id="test-model",
                name="Test Model",
                max_length=512,
                is_available=True,
