...
""")
    ...
    @pytest.mark.asyncio
    async def test_file_discovery_with_file_indexing(self, initial_indexing_service, temp_directory_with_files):
        """Test file discovery using file indexing service."""
        # Update config to use temp directory
        initial_indexing_service.config['rag_continuous_indexing_watch_root'] = str(temp_directory_with_files)
    ...
    @pytest.mark.asyncio
    async def test_file_discovery_performance(self, initial_indexing_service, temp_directory_with_files):
    ...
    @pytest.mark.asyncio
    async def test_file_discovery_fallback(self, initial_indexing_config, temp_directory_with_files):
        ...
        # Update config to use temp directory
        service.config['rag_continuous_indexing_watch_root'] = str(temp_directory_with_files)
    ...
    @pytest.mark.asyncio
    async def test_file_discovery_with_nonexistent_directory(self, initial_indexing_service):
        """Test file discovery with non-existent directory."""
        # Set non-existent directory
        initial_indexing_service.config['rag_continuous_indexing_watch_root'] = '/non/existent/directory'
    ...
    async def test_file_discovery_with_empty_directory(self, initial_indexing_service):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            # Set empty directory
            initial_indexing_service.config['rag_continuous_indexing_watch_root'] = temp_dir
    ...
    @pytest.mark.asyncio
    async def test_file_discovery_file_type_filtering(self, initial_indexing_service, temp_directory_with_files):
    ...
    @pytest.mark.asyncio
    async def test_file_discovery_recursive(self, initial_indexing_service, temp_directory_with_files):
    ...
    @pytest.mark.asyncio
    async def test_file_indexing_service_integration(self, initial_indexing_service, temp_directory_with_files):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_file_discovery(self, initial_indexing_service, temp_directory_with_files):
        ...
        tasks = [
            initial_indexing_service.discover_files()
            for _ in range(5)
        ]
        ...
        for result in results:
            assert len(result) > 0
            assert len(result) == len(results[0])  # All should have same number of files
    ...
    async def test_file_discovery_with_large_directory(self, initial_indexing_service):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i in range(10):
                ...
                for j in range(5):
                    (subdir / f"file_{i}_{j}.py").write_text(f"def test_{i}_{j}(): pass")
            
            # Update config to use temp directory
            initial_indexing_service.config['rag_continuous_indexing_watch_root'] = str(temp_path)
...
class TestInitialIndexingServiceIntegration:
    ...
    @pytest.mark.asyncio
    async def test_initial_indexing_service_with_continuous_indexing(self, initial_indexing_config):
