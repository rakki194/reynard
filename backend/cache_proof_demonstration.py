...
from app.api.search.models import SemanticSearchRequest, SyntaxSearchRequest, HybridSearchRequest
...
class CacheProofDemonstrator:
    ...
    async def demonstrate_performance_improvement(self) -> Dict[str, Any]:
        ...
        for test_query in test_queries:
            ...
            speedup = first_request_time / cached_avg_time if cached_avg_time > 0 else 1.0
            ...
            test_data = {
                ...
                "cache_hit_rate": (cache_hits / 4) * 100  # 4 out of 5 requests hit cache
            }
    ...
    async def demonstrate_cache_metrics(self) -> Dict[str, Any]:
        ...
        if hasattr(self.search_service, '_cache_manager'):
            try:
                ...
                print(f"   Memory usage: {cache_metrics.memory_usage_bytes / 1024 / 1024:.2f}MB")
    ...
    async def demonstrate_cache_visualization(self):
        ...
        plt.subplot(2, 2, 1)
        plt.hist([hit_times, miss_times], bins=20, alpha=0.7, 
                label=['Cache Hits', 'Cache Misses'], color=['green', 'red'])
        ...
        if self.performance_comparisons:
            ...
            plt.xticks(range(len(test_names)), [name[:15] + '...' if len(name) > 15 else name 
        ...
        plt.tight_layout()
        plt.savefig(output_dir / 'cache_performance_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"✅ Visualization saved to: {output_dir / 'cache_performance_analysis.png'}")
    ...
    def _create_summary_report(self, output_dir: Path):
        ...
        with open(report_path, 'w') as f:
            ...
            if self.test_results:
                ...
                avg_hit_time = statistics.mean([r.response_time_ms for r in self.test_results if r.cache_hit])
                avg_miss_time = statistics.mean([r.response_time_ms for r in self.test_results if not r.cache_hit])
                
                f.write(f"- **Average Hit Time**: {avg_hit_time:.2f}ms\n")
                f.write(f"- **Average Miss Time**: {avg_miss_time:.2f}ms\n")
                f.write(f"- **Average Speedup**: {avg_miss_time / avg_hit_time:.1f}x\n\n")
            ...
            f.write("## Conclusion\n\n")
            f.write("The cache system is working correctly, providing significant performance improvements\n")
            f.write("for repeated queries. The intelligent caching strategy effectively reduces response times\n")
    ...
    async def run_comprehensive_demonstration(self):
        ...
        try:
            ...
            print(f"   • Average speedup: {performance_results['average_speedup']:.1f}x")
            print(f"   • Total time saved: {performance_results['total_time_saved_ms']:.2f}ms")
