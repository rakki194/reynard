...
class ReynardAPIToolIntegration:
        ...
        comfy_tool = NLWebTool(
            ...
            parameters=[
                NLWebToolParameter(
                    ...
                    required=True,
                    constraints={"enum": ["text2img", "img2img", "upscale", "controlnet", "custom"]}
                ),
                NLWebToolParameter(
                    name="prompt",
                    type="string",
        ...
        tts_tool = NLWebTool(
            ...
            parameters=[
                ...
                NLWebToolParameter(
                    ...
                    required=False,
                    default="default",
                    constraints={"enum": ["default", "male", "female", "child", "elderly"]}
                ),
                NLWebToolParameter(
                    name="backend",
                    type="string",
        ...
        image_utils_tool = NLWebTool(
            ...
            parameters=[
                NLWebToolParameter(
                    ...
                    required=True,
                    constraints={"enum": ["resize", "convert", "enhance", "analyze", "validate"]}
                ),
                NLWebToolParameter(
                    name="input_format",
                    type="string",
                    ...
                    required=True,
                    constraints={"enum": ["jpg", "png", "webp", "jxl", "avif", "bmp", "tiff"]}
                ),
                NLWebToolParameter(
                    name="output_format",
                    type="string",
        ...
        ollama_tool = NLWebTool(
            ...
            parameters=[
                ...
                NLWebToolParameter(
                    ...
                    constraints={"enum": ["qwen3:latest", "llama3:latest", "mistral:latest", "codellama:latest"]}
                ),
                NLWebToolParameter(
                    name="chat_options",
                    type="object",
                ...
                "steps": [
                    {
                        ...
                        "parameters": {
                            ...
                            "input_format": "jpg",
                            "output_format": "webp",
                            "processing_options": {"quality": 85, "optimize_size": True}
                        }
                    },
                    {
                        "service": "caption_generation",
    ...
    async def _mock_reynard_suggestions(self, request: NLWebSuggestionRequest) -> List[Dict[str, Any]]:
        """Mock Reynard-specific tool suggestions for demo purposes."""
        suggestions = []
        query_lower = request.query.lower()
        
        # ComfyUI suggestions
        if any(word in query_lower for word in ["image", "generate", "comfy", "visual", "picture"]):
            suggestions.append({
        ...
        # RAG suggestions
        if any(word in query_lower for word in ["search", "document", "find", "information", "knowledge"]):
            suggestions.append({
        ...
        # TTS suggestions
        if any(word in query_lower for word in ["speech", "audio", "voice", "sound", "tts"]):
            suggestions.append({
        ...
        # Ollama suggestions
        if any(word in query_lower for word in ["chat", "explain", "help", "understand", "analyze"]):
            suggestions.append({
