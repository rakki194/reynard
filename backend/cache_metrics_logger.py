...
class CacheMetricsLogger:
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        # File handler
        log_file = (
            self.log_dir / f"cache_metrics_{datetime.now().strftime('%Y%m%d')}.log"
        )
    ...
    def export_metrics_to_csv(self, hours: int = 24) -> str:
        ...
        recent_entries = [
            entry for entry in self.recent_metrics if entry.timestamp >= cutoff_time
        ]
        ...
        csv_file = (
            self.log_dir
            / f"cache_metrics_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
    ...
    def export_performance_tests_to_csv(self) -> str:
        ...
        csv_file = (
            self.log_dir
            / f"performance_tests_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
    ...
    def generate_metrics_report(self, hours: int = 24) -> dict[str, Any]:
        ...
        report = {
            ...
            "cache_performance": {
                ...
                "hit_rate_trend": (
                    hit_rates[-1] - hit_rates[0] if len(hit_rates) > 1 else 0
                ),
            },
            "search_performance": {
                "average_search_time_ms": (
                ...
                "total_searches": (
                    max(total_searches) - min(total_searches)
                    if len(total_searches) > 1
                    else 0
                ),
            },
            "system_status": {
                ...
                "current_memory_usage_mb": recent_entries[-1].memory_usage_bytes
                / 1024
                / 1024,
        ...
        if self.performance_tests:
            recent_tests = [
                test for test in self.performance_tests if test.timestamp >= cutoff_time
            ]
            ...
            if recent_tests:
                ...
                report["performance_tests"] = {
                    ...
                    "cache_hit_rate": sum(1 for test in recent_tests if test.cache_hit)
                    / len(recent_tests)
                    * 100,
                }
