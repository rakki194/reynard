...
async def test_cache_initialization():
    ...
    # Test Alembic cache
    try:
        alembic_cache = await initialize_alembic_cache(redis_enabled=CacheConfig.REDIS_ENABLED)
        print(f"✅ Alembic cache initialized: Redis enabled = {CacheConfig.REDIS_ENABLED}")
    except Exception as e:
...
async def test_alembic_cache():
    ...
    try:
        ...
        cached_metadata = await alembic_cache.get_cached_migration_metadata(test_revision)
