...
class FastAPIBenchmarkDB:
    ...
    def analyze_results(self, results: list[BenchmarkResult]) -> BenchmarkSuite:
        ...
        successful_results = [
            r for r in results if r.status_code == 200 and r.error_message is None
        ]
        failed_results = [
            r for r in results if r.status_code != 200 or r.error_message is not None
        ]
        ...
        p99_response_time = statistics.quantiles(durations, n=100)[
            98
        ]  # 99th percentile

        # Calculate throughput
        total_time = max(r.timestamp for r in results) - min(
            r.timestamp for r in results
        )
        requests_per_second = (
            len(successful_results) / total_time.total_seconds()
    ...
    async def benchmark_search_endpoints(self) -> BenchmarkSuite:
        ...
        test_cases = [
            ...
            {"method": "GET", "endpoint": "/api/search/health"},
        ]
    ...
    async def benchmark_ecs_endpoints(self) -> BenchmarkSuite:
        ...
        test_cases = [
            {"method": "GET", "endpoint": "/api/ecs/status"},
            {
                "method": "POST",
            ...
            {"method": "GET", "endpoint": "/api/ecs/agents"},
            {
                "method": "POST",
    ...
    async def benchmark_rag_endpoints(self) -> BenchmarkSuite:
        ...
        test_cases = [
            ...
            {"method": "GET", "endpoint": "/api/rag/health"},
        ]
    ...
    async def generate_and_store_report(self, suites: list[BenchmarkSuite]) -> str:
        ...
        for suite in suites:
            ...
            failed_results = [
                r for r in suite.results if r.status_code != 200 or r.error_message
            ]
            if failed_results:
