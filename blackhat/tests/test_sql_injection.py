...
class TestSQLInjection(BlackhatTestBase):
    """
    *otter curiosity* Test suite for SQL injection exploits
    ...
    async def test_blind_sql_injection(self):
        ...
        try:
            ...
            with patch.object(exploit, '_test_boolean_based_blind', return_value=False):
                with patch.object(exploit, '_test_time_based_blind', return_value=False):
            ...
            if vulnerability_found:
                print(f"    ⚠️ Found unexpected blind SQL injection vulnerabilities")
                for result in results:
                    if result.success:
                        print(f"        - {result.vulnerability_type}: {result.description}")
            else:
                print(f"    ✅ No blind SQL injection vulnerabilities found (secure)")
    ...
    async def test_obfuscated_sql_payloads(self):
        ...
        try:
            ...
            if vulnerability_found:
                print(f"    ⚠️ Found unexpected obfuscated SQL vulnerabilities")
                for result in results:
                    if result.success:
                        print(f"        - {result.vulnerability_type}: {result.description}")
            else:
                print(f"    ✅ No obfuscated SQL vulnerabilities found (secure)")
    ...
    async def test_regex_bypass_exploit(self):
        ...
        try:
            ...
            if vulnerability_found:
                print(f"    ⚠️ Found unexpected regex bypass vulnerabilities")
                for result in results:
                    if result.success:
                        print(f"        - {result.vulnerability_type}: {result.description}")
            else:
                print(f"    ✅ No regex bypass vulnerabilities found (secure)")
    ...
    async def test_union_based_sql_injection(self):
        ...
        try:
            ...
            if vulnerability_found:
                print(f"    ⚠️ Found unexpected union-based SQL injection vulnerabilities")
                for result in results:
                    if result.success:
                        print(f"        - {result.vulnerability_type}: {result.description}")
            else:
                print(f"    ✅ No union-based SQL injection vulnerabilities found (secure)")
