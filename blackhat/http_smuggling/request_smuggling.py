...
class HTTPRequestSmuggling:
    ...
    async def send_raw_request(self, raw_request: bytes) -> Tuple[int, str, Dict[str, str]]:
        """
        ...
        try:
            ...
            logger.error(f"Raw request failed: {str(e)}")
            return 0, "", {}
        ...
        try:
            ...
            if 'content-length' in headers:
                try:
                    ...
                    smuggling_indicators['content_length_mismatch'] = abs(declared_length - actual_length) > 10
                except ValueError:
                    smuggling_indicators['malformed_response'] = True
            
            # Check for admin/sensitive content leakage
            admin_keywords = ['admin', 'debug', 'internal', 'config', 'sensitive', 'private']
            smuggling_indicators['admin_content'] = any(keyword in body.lower() for keyword in admin_keywords)
            
            # Check for debug information
            debug_keywords = ['traceback', 'exception', 'error', 'stack', 'dump']
            smuggling_indicators['debug_info'] = any(keyword in body.lower() for keyword in debug_keywords)
            ...
            if result['potentially_successful']:
                logger.warning(f"🐺 POTENTIAL SMUGGLING SUCCESS: {name} (score: {success_score})")
    ...
    async def run_comprehensive_smuggling_test(self) -> Dict[str, Any]:
        ...
        logger.info(f"🐺 Smuggling test complete: {results['attack_summary']['total_successful']} potentially successful attacks")
        return results
...
async def main():
    ...
    print(f"Potentially successful attacks: {results['attack_summary']['total_successful']}")
    ...
    if summary['total_successful'] > 0:
        ...
        for attack in results['successful_smuggling']:
            print(f"  {attack['name']}: Status {attack['status_code']}, Score {attack['success_score']}")
            print(f"    Indicators: {[k for k, v in attack['indicators'].items() if v]}")
