...
"""
Correct PEP 734: Multiple Interpreters Test
Using the actual API available in Python 3.15.0a0
"""
...
def test_basic_interpreter_creation():
    ...
    print("=" * 60)
    print("🔄 PEP 734: MULTIPLE INTERPRETERS (CORRECT API)")
    print("=" * 60)
    ...
    # Run simple code using exec
    interp.exec("result = 'Hello from interpreter!'")
    result = interp.call("eval", "result")
    print(f"✅ Interpreter result: {result}")

    # Check if running
    print(f"✅ Interpreter running: {interp.is_running()}")
...
def test_parallel_execution():
    ...
    for i, interp in enumerate(interp_list):
        future = interp.call_in_thread(cpu_bound_task, 1000000)
...
def test_shared_data():
    ...
    future1 = interp1.call_in_thread(worker_task, shared_data, 1)
    future2 = interp2.call_in_thread(worker_task, shared_data, 2)
...
def test_error_handling():
    ...
    try:
        result = interp.call_in_thread(error_function).result()
        print(f"❌ Unexpected success: {result}")
    except Exception as e:
        print(f"✅ Caught expected error: {e}")

    # Interpreter should still be functional
    result = interp.call("eval", "'Interpreter still working'")
...
def test_performance_comparison():
    ...
    futures = [
        interp.call_in_thread(cpu_bound_task, task_size) for interp in interp_list
    ]
...
def test_interpreter_listing():
    """Test listing all interpreters"""
    print("📋 TESTING INTERPRETER LISTING")
    print("-" * 40)
