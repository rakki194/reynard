...
class ReynardIntegration:
    ...
    def _check_text_processor_availability(self) -> bool:
        ...
        try:
            ...
            sys.path.append('/home/kade/runeset/reynard/third_party/yipyap/app/data_access')
...
async def demonstrate_reynard_integration():
    ...
    print(f"   • PawPrint Scraper: {'✅' if reynard_integration.pawprint_available else '❌'}")
    print(f"   • RAG Backend: {'✅' if reynard_integration.rag_available else '❌'}")
    print(f"   • TextProcessor: {'✅' if reynard_integration.text_processor_available else '❌'}")
...
async def demonstrate_hybrid_approach():
    ...
    try:
        ...
        print(f"   ✅ New tools refined query with score: {result.improvement_score:.2f}")
    ...
    pawprint_result = await reynard_integration.use_pawprint_scraper(test_query)
    if pawprint_result.get('success'):
        print(f"   ✅ PawPrint provided {len(pawprint_result['results'])} high-quality results")
