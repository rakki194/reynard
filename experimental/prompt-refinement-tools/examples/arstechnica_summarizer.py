...
class ArsTechnicaSummarizer:
    ...
    async def scrape_arstechnica_articles(self, count: int = 10) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(f"Successfully scraped {len(self.articles)} articles from Ars Technica")
    ...
    async def analyze_articles(self) -> List[Dict[str, Any]]:
        ...
        for article in self.articles:
            try:
                ...
                clarity_issues = await self.nlp_processor.assess_clarity_issues(article['title'])
                specificity_gaps = await self.nlp_processor.evaluate_specificity_gaps(article['title'])
                
                # Predict effectiveness
                effectiveness_score = await self.nlp_processor.predict_effectiveness(article['title'])
    ...
    async def create_semantic_index(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Successfully indexed {len(documents)} articles in semantic search")
            return True
    ...
    async def generate_summaries(self) -> List[Dict[str, Any]]:
        ...
        for article in self.articles:
            try:
                ...
                refinement_result = await self.refinement_service.refine_query(summarization_query)
                ...
                summary = {
                    ...
                    'nlp_insights': {
                        'sentiment': getattr(article.get('nlp_analysis', {}), 'sentiment', {}),
                        'language': getattr(article.get('nlp_analysis', {}), 'language', 'en'),
                        'processing_time': getattr(article.get('nlp_analysis', {}), 'processing_time', 0)
                    },
                    'analysis_quality': {
    ...
    async def perform_semantic_search(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            search_results = await self.semantic_search.semantic_search(query, limit=top_k)
    ...
    async def generate_insights_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            effectiveness_scores = [article.get('effectiveness_score', 0.5) for article in self.articles]
            ...
            report = {
                'collection_summary': {
                    ...
                    'date_range': {
                        'earliest': min(article.get('date', '') for article in self.articles),
                        'latest': max(article.get('date', '') for article in self.articles)
                    },
                ...
                'tool_performance': {
                    'nlp_processing_success_rate': len([a for a in self.articles if 'nlp_analysis' in a]) / len(self.articles),
                    'semantic_indexing_success': len(self.articles) > 0,
                    'summarization_success_rate': len(self.summaries) / len(self.articles) if self.articles else 0
                },
                'generation_timestamp': datetime.now().isoformat()
            }
...
async def main():
    ...
    print("=" * 60)
    print("Demonstrating experimental prompt refinement tools with real-world content analysis")
    print("=" * 60)
    ...
    try:
        ...
        for i, summary in enumerate(summaries, 1):
            ...
            print(f"   Effectiveness Score: {summary.get('effectiveness_score', 0):.2f}")
        ...
        for query in search_queries:
            ...
            for j, result in enumerate(results, 1):
                print(f"   {j}. {result['title']} (similarity: {result['similarity_score']:.2f})")
        ...
        print(f"Top Concepts: {[concept for concept, count in insights['collection_summary']['top_concepts'][:5]]}")
        print(f"Sentiment Distribution: {insights['content_analysis']['sentiment_distribution']}")
        print(f"Average Effectiveness: {insights['content_analysis']['average_effectiveness_score']:.2f}")
