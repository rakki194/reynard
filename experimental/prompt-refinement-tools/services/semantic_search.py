...
class SemanticSearchService:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if FAISS_AVAILABLE and self.embedding_model_instance:
                embedding_dim = self.embedding_model_instance.get_sentence_embedding_dimension()
    ...
    async def research_related_concepts(
        ...
        return {
            ...
            'concept_relationships': self._analyze_concept_relationships(ranked_results)
        }
    ...
    def _generate_semantic_queries(self, query: str, key_concepts: List[str]) -> List[str]:
    ...
    async def _add_document_batch(self, documents: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            if self.embedding_model_instance:
                ...
                embeddings = [self._generate_fallback_embedding(content) for content in contents]
    ...
    def _analyze_concept_relationships(self, results: List[SearchResult]) -> Dict[str, List[str]]:
        ...
        for result in results:
            content_words = result.content.lower().split()
            concepts = [word for word in content_words if len(word) > 4 and word.isalpha()]
    ...
    async def _load_existing_data(self) -> None:
        ...
        try:
            ...
            if fallback_file.exists():
                ...
                logger.info(f"Loaded {len(self.vector_store)} documents from fallback storage")
    ...
    async def research_related_concepts(self, query: str, key_concepts: List[str] = None) -> Dict[str, Any]:
        """
        Research related concepts using semantic search.
        
        Enhanced version that accepts key concepts for better research.
        """
        logger.debug(f"Researching related concepts for: {query} with concepts: {key_concepts}")
        ...
        return {
            ...
            'related_concepts': list(set(related_concepts))[:10],  # Remove duplicates and limit
