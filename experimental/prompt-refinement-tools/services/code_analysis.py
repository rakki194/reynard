...
class CodeAnalysisService:
            ...
            if TREE_SITTER_AVAILABLE:
                ...
                for lang_name in self.supported_languages.keys():
                    try:
                        ...
                        logger.info(f"Tree-sitter parser for {lang_name} would be initialized")
                    except Exception as e:
                        logger.warning(f"Failed to load {lang_name} parser: {e}")
                        self.parsers[lang_name] = None
            else:
                logger.warning("Tree-sitter not available, using fallback code analysis")
    ...
    async def analyze_codebase_relevance(
        ...
        try:
            ...
            architectural_context = self._identify_architectural_context(query, code_concepts)
            
            # Assess dependency awareness
            dependency_awareness = self._assess_dependency_awareness(query, code_concepts)
    ...
    def _extract_code_concepts(self, query: str) -> List[str]:
        ...
        code_indicators = [
            ...
            'python', 'javascript', 'typescript', 'java', 'cpp', 'react',
            'vue', 'angular', 'node', 'express', 'django', 'flask'
        ]
    ...
    def _identify_terminology_gaps(self, query: str, research_findings: Dict[str, Any]) -> List[str]:
    ...
    def _identify_architectural_context(self, query: str, code_concepts: List[str]) -> str:
        """Identify architectural context needs."""
        query_lower = query.lower()
        ...
        architectural_terms = [
            ...
            'layered', 'microservice', 'monolith', 'distributed', 'scalable'
        ]
    ...
    def _assess_dependency_awareness(self, query: str, code_concepts: List[str]) -> str:
        ...
        dependency_terms = [
            'dependency', 'import', 'require', 'package', 'library', 'framework',
            'npm', 'pip', 'maven', 'gradle', 'yarn', 'pnpm'
        ]
    ...
    def _evaluate_coding_standard_alignment(self, query: str) -> str:
        ...
        standard_terms = [
            'style', 'format', 'lint', 'eslint', 'prettier', 'black', 'flake8',
            'convention', 'best practice', 'guideline', 'standard'
        ]
    ...
    def _calculate_complexity_score(self, content: str, functions: List[Dict[str, Any]], classes: List[Dict[str, Any]]) -> float:
