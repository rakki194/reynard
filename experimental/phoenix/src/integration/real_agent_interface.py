...
from ..utils.data_structures import AgentState, AgentGeneticMaterial, StructuredKnowledge
...
class RealAgentInterface:
    ...
    async def collect_agent_data(self, agent: AgentState, task_prompt: str) -> Dict[str, Any]:
        """Collect real data from an agent by having it perform a task."""
        self.logger.info(f"Collecting data from agent {agent.name} for task: {task_prompt[:50]}...")
    ...
    def _create_system_prompt(self, agent: AgentState) -> str:
        ...
        spirit_desc = spirit_descriptions.get(agent.spirit.value, "You are a unique being with special characteristics.")
        ...
        trait_desc = ", ".join(trait_characteristics) if trait_characteristics else "well-rounded"
    ...
    async def _analyze_agent_response(self, agent: AgentState, response: str, task_prompt: str) -> Dict[str, Any]:
    ...
    def _count_technical_terms(self, text: str) -> int:
        ...
        technical_terms = [
            "algorithm", "architecture", "system", "framework", "implementation", "optimization",
            "distributed", "scalable", "efficient", "performance", "database", "API", "interface",
            "component", "module", "service", "protocol", "security", "authentication", "authorization"
        ]
    ...
    def _count_creativity_indicators(self, text: str) -> int:
        ...
        creativity_indicators = [
            "innovative", "creative", "unique", "novel", "imagine", "design", "invent",
            "breakthrough", "revolutionary", "cutting-edge", "groundbreaking", "pioneering"
        ]
    ...
    def _count_leadership_indicators(self, text: str) -> int:
        ...
        leadership_indicators = [
            "lead", "guide", "direct", "manage", "coordinate", "oversee", "supervise",
            "mentor", "inspire", "command", "authority", "strategy", "vision", "team"
        ]
    ...
    def _count_analytical_indicators(self, text: str) -> int:
        ...
        analytical_indicators = [
            "analyze", "examine", "evaluate", "assess", "consider", "review", "scrutinize",
            "investigate", "logical", "rational", "systematic", "methodical", "data", "evidence"
        ]
    ...
    def _assess_spirit_alignment(self, agent: AgentState, response: str) -> float:
        ...
        spirit_keywords = {
            ...
            "alien": ["innovative", "unconventional", "advanced", "otherworldly", "fresh"],
    ...
    def _assess_trait_manifestation(self, agent: AgentState, response: str) -> Dict[str, float]:
        """Assess how well the response manifests the agent's traits."""
        manifestations = {}

        # Check personality traits
        if agent.personality_traits.get("creativity", 0) > 0.7:
            manifestations["creativity"] = self._count_creativity_indicators(response) / 10.0

        if agent.personality_traits.get("leadership", 0) > 0.7:
            manifestations["leadership"] = self._count_leadership_indicators(response) / 10.0

        # Check ability traits
        if agent.ability_traits.get("strategist", 0) > 0.7:
            strategic_terms = ["strategy", "strategic", "plan", "approach", "method"]
            manifestations["strategy"] = sum(1 for term in strategic_terms if term.lower() in response.lower()) / 5.0

        if agent.ability_traits.get("analyzer", 0) > 0.7:
            manifestations["analysis"] = self._count_analytical_indicators(response) / 10.0
    ...
    def _calculate_quality_score(self, analysis: Dict[str, Any]) -> float:
        ...
        word_count_score = min(1.0, analysis["word_count"] / 200.0)  # Optimal around 200 words
    ...
    async def run_comparison_experiment(self, agents: List[AgentState], num_trials: int = 10) -> Dict[str, Any]:
        """Run a comparison experiment with and without genome data."""
        self.logger.info(f"Running comparison experiment with {len(agents)} agents, {num_trials} trials each")
        ...
        for trial in range(num_trials):
            ...
            without_genome_data = await self.collect_agent_data_without_genome(agent, task)
        ...
        # Save results
        results_file = self.data_dir / f"genome_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(results_file, 'w') as f:
            json.dump(results, f, indent=2)

        self.logger.info(f"Comparison experiment completed. Results saved to {results_file}")
        return results

    async def collect_agent_data_without_genome(self, agent: AgentState, task_prompt: str) -> Dict[str, Any]:
    ...
    async def _analyze_comparison_results(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with_genome_avg = {
            "quality_score": sum(r["analysis"]["quality_score"] for r in with_genome) / len(with_genome),
            "spirit_alignment": sum(r["analysis"]["spirit_alignment"] for r in with_genome) / len(with_genome),
            "technical_terms": sum(r["analysis"]["technical_terms"] for r in with_genome) / len(with_genome),
            "creativity_indicators": sum(r["analysis"]["creativity_indicators"] for r in with_genome) / len(with_genome),
            "leadership_indicators": sum(r["analysis"]["leadership_indicators"] for r in with_genome) / len(with_genome),
            "analytical_indicators": sum(r["analysis"]["analytical_indicators"] for r in with_genome) / len(with_genome),
            "response_length": sum(r["response_length"] for r in with_genome) / len(with_genome)
        }

        without_genome_avg = {
            "quality_score": sum(r["analysis"]["quality_score"] for r in without_genome) / len(without_genome),
            "spirit_alignment": sum(r["analysis"]["spirit_alignment"] for r in without_genome) / len(without_genome),
            "technical_terms": sum(r["analysis"]["technical_terms"] for r in without_genome) / len(without_genome),
            "creativity_indicators": sum(r["analysis"]["creativity_indicators"] for r in without_genome) / len(without_genome),
            "leadership_indicators": sum(r["analysis"]["leadership_indicators"] for r in without_genome) / len(without_genome),
            "analytical_indicators": sum(r["analysis"]["analytical_indicators"] for r in without_genome) / len(without_genome),
            "response_length": sum(r["response_length"] for r in without_genome) / len(without_genome)
        }
        ...
        for metric in with_genome_avg:
            if without_genome_avg[metric] > 0:
                improvements[metric] = ((with_genome_avg[metric] - without_genome_avg[metric]) / without_genome_avg[metric]) * 100
            else:
                improvements[metric] = float('inf') if with_genome_avg[metric] > 0 else 0
