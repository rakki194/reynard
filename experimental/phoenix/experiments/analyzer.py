...
class StatisticalAnalyzer:
    ...
    async def analyze_results(self, trial_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        analysis['descriptive_statistics'] = self._calculate_descriptive_statistics(method_metrics)

        # Perform comparative analysis
        analysis['comparative_analysis'] = self._perform_comparative_analysis(method_metrics)

        # Perform significance testing
        analysis['significance_testing'] = self._perform_significance_testing(method_metrics)

        # Calculate effect sizes
        analysis['effect_sizes'] = self._calculate_effect_sizes(method_metrics)

        # Calculate confidence intervals
        analysis['confidence_intervals'] = self._calculate_confidence_intervals(method_metrics)
    ...
    def _extract_method_metrics(self, trial_results: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, float]]]:
    ...
    def _calculate_descriptive_statistics(self, method_metrics: Dict[str, List[Dict[str, float]]]) -> Dict[str, Any]:
    ...
    def _perform_comparative_analysis(self, method_metrics: Dict[str, List[Dict[str, float]]]) -> Dict[str, Any]:
        """Perform comparative analysis between methods."""

        comparative_analysis = {}

        # Compare baseline vs PHOENIX methods
        baseline_methods = [k for k in method_metrics.keys() if k.startswith('baseline_')]
    ...
    def _find_best_method(self, method_metrics: Dict[str, List[Dict[str, float]]], method_names: List[str]) -> str:
        ...
        for method_name in method_names:
            if method_name in method_metrics and method_metrics[method_name]:
                ...
                overall_success_values = [metrics.get('overall_success', 0) for metrics in method_metrics[method_name]]
    ...
    def _compare_methods(self, method1_metrics: List[Dict[str, float]],
        ...
        if method1_metrics and method2_metrics:
            common_metrics = set(method1_metrics[0].keys()) & set(method2_metrics[0].keys())
            ...
            for metric_name in common_metrics:
                ...
                if values1 and values2:
                    comparison['metric_comparisons'][metric_name] = {
                        ...
                        'percent_improvement': ((np.mean(values2) - np.mean(values1)) / np.mean(values1)) * 100 if np.mean(values1) != 0 else 0
                    }

        return comparison

    def _perform_significance_testing(self, method_metrics: Dict[str, List[Dict[str, float]]]) -> Dict[str, Any]:
    ...
    def _test_significance(self, method1_metrics: List[Dict[str, float]],
                          method2_metrics: List[Dict[str, float]],
                          method1_name: str, method2_name: str) -> Dict[str, Any]:
            ...
            for metric_name in common_metrics:
                ...
                if len(values1) >= 2 and len(values2) >= 2:
                    ...
                    significance_results['metric_tests'][metric_name] = {
                        ...
                        'effect_direction': 'method2_better' if t_stat > 0 else 'method1_better'
                    }

        return significance_results

    def _calculate_effect_sizes(self, method_metrics: Dict[str, List[Dict[str, float]]]) -> Dict[str, Any]:
    ...
    def _calculate_cohens_d(self, method1_metrics: List[Dict[str, float]],
                           method2_metrics: List[Dict[str, float]]) -> Dict[str, float]:
    ...
    def _calculate_confidence_intervals(self, method_metrics: Dict[str, List[Dict[str, float]]]) -> Dict[str, Any]:
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if 'baseline_vs_phoenix' in significance_results:
            ...
            for metric_name, test_result in metric_tests.items():
                if test_result.get('is_significant', False) and test_result.get('effect_direction') == 'method2_better':
                    significant_improvements.append(metric_name)

            if significant_improvements:
                recommendations.append(f"PHOENIX methods show significant improvements over baseline in: {', '.join(significant_improvements)}")
            else:
                recommendations.append("No significant improvements found between PHOENIX and baseline methods")
        ...
        if 'baseline_vs_phoenix' in effect_sizes:
            ...
            if large_effects:
                recommendations.append(f"Large effect sizes observed for: {', '.join(large_effects)}")

        # Check descriptive statistics
        descriptive_stats = analysis.get('descriptive_statistics', {})
        if descriptive_stats:
            best_method = max(descriptive_stats.keys(),
                            key=lambda k: descriptive_stats[k].get('overall_success', {}).get('mean', 0))
            best_score = descriptive_stats[best_method].get('overall_success', {}).get('mean', 0)
            recommendations.append(f"Best performing method: {best_method} (overall success: {best_score:.3f})")
